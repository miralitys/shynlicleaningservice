...
function createAccountRenderers(deps = {}) {
  ...
  function getAccountNextAssignmentAction(orderStatus, checklistSummary = {}) {
    ...
    if (orderStatus === "checklist" && Number(checklistSummary.completedCount) > 0) {
      return {
    ...
    if (uiState.orderStatus === "checklist") {
      ...
      if (canShowNextAction) {
        forms.push(`<form method="post" action="${ACCOUNT_ROOT_PATH}">
          ...
          <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
          <button class="${primaryClass}" type="submit">${escapeHtml(uiState.nextAction.label)}</button>
        </form>`);
      }
      ...
      if (
        canShowNextAction &&
        (Number(uiState.photoSummary && uiState.photoSummary.beforeCount) > 0 ||
          Number(uiState.photoSummary && uiState.photoSummary.afterCount) > 0)
      ) {
        forms.push(`<form method="post" action="${ACCOUNT_ROOT_PATH}">
          <input type="hidden" name="action" value="${escapeHtmlAttribute(uiState.nextAction.action)}">
          <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
  ...
  function renderAccountMobileDetailChecklist(item = {}, uiState = {}) {
    ...
    const nextActionForm =
      entryId && uiState.nextAction && uiState.nextAction.action === "mark-assignment-photos"
        ? `<form method="post" action="${ACCOUNT_ROOT_PATH}" data-account-no-card-open>
            <input type="hidden" name="action" value="${escapeHtmlAttribute(uiState.nextAction.action)}">
            <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
            <button class="account-mobile-detail-soft-button account-mobile-detail-save-button" type="submit">${escapeHtml(
              uiState.nextAction.label
            )}</button>
          </form>`
        : "";
  ...
  function renderAccountMobileDetailTopAction(item = {}, uiState = {}) {
    ...
    if (
      ...
      Number(uiState.checklistSummary && uiState.checklistSummary.completedCount) <= 0
    ) {
      return "";
    }
    if (
