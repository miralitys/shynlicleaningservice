...
function createAccountRenderers(deps = {}) {
  ...
  function sortAccountDashboardOrders(orderItems = [], staffId = "") {
    return (Array.isArray(orderItems) ? orderItems.slice() : []).sort((left, right) => {
  ...
  function splitAccountDashboardOrders(orderItems = [], staffId = "") {
    const sortedOrders = sortAccountDashboardOrders(orderItems, staffId);
    const activeOrders = [];
  ...
  function renderAccountAssignmentActionForms(item = {}, uiState = {}, options = {}) {
    ...
    function renderPhotoEditor() {
      ...
      const renderPhotoUploadField = (options = {}) => {
        const count = Number(options.count) || 0;
        const type = normalizeString(options.type, 20);
        const inputName = normalizeString(options.inputName, 80);
        const title = normalizeString(options.title, 80);
        const hint = normalizeString(options.hint, 140);
        const statusText = count > 0 ? `${count} уже загружено` : "Нужно добавить";
        const metaText = count > 0 ? `Загружено фото: ${count}` : "Фото еще не выбраны";
        const helpText =
          count > 0
            ? "Если выбрать новые файлы, они заменят текущий набор."
            : "Можно выбрать одну или несколько фотографий.";

        return `<label class="account-photo-upload-field">
          <span class="account-photo-upload-heading">
            <span>
          ...
          <span
            ...
            data-account-photo-shell="${escapeHtmlAttribute(type)}"
            data-has-files="${count > 0 ? "true" : "false"}"
            data-disabled="false"
          >
            <input
              ...
              accept="image/*"
              multiple
              data-account-photo-input="${escapeHtmlAttribute(type)}"
              data-account-photo-stored-count="${escapeHtmlAttribute(String(count))}"
            >
            <span class="account-photo-upload-glyph" aria-hidden="true">
              <svg viewBox="0 0 24 24"><path d="M4.5 7.8c0-1.2 1-2.3 2.3-2.3h2.1l1.2-1.5h3.8l1.2 1.5h2.1c1.2 0 2.3 1 2.3 2.3v8.9c0 1.2-1 2.3-2.3 2.3H6.8c-1.2 0-2.3-1-2.3-2.3V7.8Z"/><circle cx="12" cy="12.4" r="3.2"/></svg>
            </span>
            <span class="account-photo-upload-main">
              <span class="account-photo-upload-button">${count > 0 ? "Заменить фото" : "Выбрать фото"}</span>
              <span class="account-photo-upload-meta" data-account-photo-meta="${escapeHtmlAttribute(type)}">${escapeHtml(metaText)}</span>
            </span>
          </span>
          <small data-account-photo-help="${escapeHtmlAttribute(type)}">${escapeHtml(helpText)}</small>
        </label>`;
      };
      const summaryLabel =
        uiState.photoSummary && uiState.photoSummary.skipped
          ...
          : "Фото до / после";
      return `<details class="account-stage-editor account-photo-stage-editor" data-account-photo-editor>
        <summary class="account-stage-editor-summary">${escapeHtml(summaryLabel)}</summary>
        <div class="account-stage-editor-body">
          <div class="account-photo-editor-note">
            <strong>Закройте этап фото одним из двух способов.</strong>
            <span>Добавьте фото до и после уборки или отметьте, что клиент запретил съемку.</span>
          </div>
          <form class="account-stage-editor-form" method="post" action="${ACCOUNT_ROOT_PATH}" enctype="multipart/form-data" data-account-async-form="true" data-account-photo-complete-form="true">
            ...
            <div class="account-photo-upload-grid">
              ${renderPhotoUploadField({
                type: "before",
                inputName: "beforePhotos",
                title: "До уборки",
                hint: "Состояние объекта перед началом.",
                count: beforeCount,
              })}
              ${renderPhotoUploadField({
                type: "after",
                inputName: "afterPhotos",
                title: "После уборки",
                hint: "Результат после завершения.",
                count: afterCount,
              })}
              <label class="account-photo-skip-field">
                ...
                <span>
                  <strong>Фото нельзя сделать</strong>
                  <small>Клиент не разрешил съемку. Этот этап можно закрыть без загрузки.</small>
                </span>
              </label>
            </div>
            <div class="account-stage-editor-actions">
  ...
  function renderAccountMobileDashboardStyles() {
    return `<style>
      ...
      .account-photo-editor-note strong {
        color: #0f514b;
        font-weight: 800;
      }
      .account-photo-editor-note span {
        line-height: 1.45;
      }
      ...
      .account-photo-upload-heading > span {
        display: grid;
        gap: 3px;
        min-width: 0;
      }
      .account-photo-upload-heading strong {
        font-weight: 800;
        color: #3c2230;
      }
      .account-photo-upload-heading small {
        color: #7b7079;
        line-height: 1.35;
      }
      .account-photo-upload-heading em {
        flex-shrink: 0;
        padding: 6px 10px;
        border-radius: 999px;
        background: #f3e4e9;
      ...
      .account-photo-upload-glyph svg {
        width: 25px;
        height: 25px;
        fill: none;
        stroke: currentColor;
      ...
      .account-photo-skip-field span {
        display: grid;
        gap: 4px;
      }
      .account-photo-skip-field strong {
        font-weight: 800;
      }
      .account-photo-skip-field small {
        color: #736570;
        line-height: 1.35;
      }
      ...
      @media (max-width: 720px) {
        ...
        .account-mobile-detail-view .account-photo-editor-note {
          padding: 18px 20px;
          border-radius: 22px;
          font-size: 18px;
        }
        ...
        .account-mobile-detail-view .account-photo-upload-heading {
          align-items: flex-start;
        }
        .account-mobile-detail-view .account-photo-upload-heading strong {
          font-size: 18px;
        }
        .account-mobile-detail-view .account-photo-upload-heading small,
        .account-mobile-detail-view .account-photo-upload-heading em {
          font-size: 14px;
        }
        ...
        .account-mobile-detail-view .account-photo-upload-glyph {
          width: 58px;
          height: 58px;
          border-radius: 20px;
        }
        ...
        .account-mobile-detail-view .account-photo-skip-field strong {
          font-size: 18px;
        }
        .account-mobile-detail-view .account-photo-skip-field small {
          font-size: 15px;
          line-height: 1.35;
        }
        ...
        function updatePhotoCompletionState(editor) {
          ...
          [beforeInput, afterInput].forEach((input) => {
            if (input instanceof HTMLInputElement) {
              ...
              if (meta) {
                if (selectedCount > 1) {
                  meta.textContent = "Выбрано фото: " + String(selectedCount);
                } else if (selectedCount === 1 && input.files && input.files[0]) {
                  meta.textContent = input.files[0].name;
                } else if (storedCount > 0) {
                  meta.textContent = "Загружено фото: " + String(storedCount);
                } else {
                  meta.textContent = "Фото еще не выбраны";
                }
              }
              if (help) {
                if (selectedCount > 0) {
  ...
  function renderAccountMobileDashboard(orderItems = [], options = {}) {
    ...
    const { activeOrders, completedOrders, sortedOrders } = splitAccountDashboardOrders(orderItems, options.staffId || "");
    const primaryAssignment = getAccountPrimaryMobileAssignment(activeOrders, options.staffId || "");
  ...
  function renderDashboardPage(userContext, options = {}) {
    ...
    } = splitAccountDashboardOrders(assignedOrders, staffId);
