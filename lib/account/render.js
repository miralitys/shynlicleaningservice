...
function createAccountRenderers(deps = {}) {
  ...
  function renderAccountMobileDetailChecklist(item = {}, uiState = {}) {
    ...
    const entryId = item && item.entry ? item.entry.id : "";
    const canEditChecklist = Boolean(entryId && uiState.canAccessAssignment && uiState.orderStatus === "cleaning-started");
    ...
    return `<section class="account-mobile-detail-card account-mobile-detail-checklist">
      ...
      ${
        checklistItems.length
          ? canEditChecklist
            ? `<form class="account-mobile-detail-checklist-form" method="post" action="${ACCOUNT_ROOT_PATH}" data-account-checklist-editor>
                <input type="hidden" name="action" value="save-assignment-checklist">
                <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
                <button class="account-mobile-detail-soft-button" type="button" data-account-checklist-select-all="true" data-account-no-card-open>
                  <span aria-hidden="true">✓</span> Отметить всё
                </button>
                <div class="account-mobile-detail-checklist-list">${checklistItemsMarkup}</div>
                <button class="account-mobile-detail-soft-button account-mobile-detail-save-button" type="submit" data-account-no-card-open>Сохранить чеклист</button>
              </form>`
            : `<div class="account-mobile-detail-checklist-list">${checklistItemsMarkup}</div>`
  ...
  function renderAccountMobileDetailNotes(item = {}) {
    ...
    return `<section class="account-mobile-detail-card account-mobile-detail-notes">
      <h3>Заметки</h3>
      <div class="account-mobile-detail-note-box">${escapeHtml(notesLabel || "Добавить заметку")}</div>
      <button class="account-mobile-detail-soft-button" type="button" data-account-no-card-open>Добавить</button>
    </section>`;
  }
  ...
  function renderAccountMobileDetailContactActions() {
    return `<div class="account-mobile-detail-contact-actions">
      ...
      <button class="account-mobile-detail-contact-button account-mobile-detail-contact-chat" type="button" data-account-no-card-open>
        <span aria-hidden="true">○</span> Чат с менеджером
      </button>
    </div>`;
  }
  ...
  function renderAccountMobileDetailBottomNav() {
    return `<nav class="account-mobile-detail-bottom-nav" aria-label="Навигация кабинета">
      <a href="${ACCOUNT_ROOT_PATH}" aria-current="page"><span aria-hidden="true">▣</span>Заказы</a>
      <a href="${ACCOUNT_ROOT_PATH}"><span aria-hidden="true">▦</span>Календарь</a>
      <a href="${ACCOUNT_ROOT_PATH}"><span aria-hidden="true">▱</span>Заработок</a>
    </nav>`;
  }
  ...
  function renderAccountMobileAssignmentDetailView(item = {}, options = {}) {
    ...
    const instructions = item.assignment && item.assignment.notes ? item.assignment.notes : "Инструкции не добавлены.";
    const mapsHref = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(addressParts.fullAddress)}`;
    const shouldRenderStageActions = uiState.orderStatus !== "cleaning-started";
    ...
    >
      ...
      ${renderAccountMobileDetailTimeline(uiState)}
      ${renderAccountMobileDetailChecklist(item, uiState)}
      ${renderAccountMobileDetailNotes(item)}
      ${renderAccountMobileDetailIssueHistory()}
      ${renderAccountMobileDetailContactActions()}
      ${uiState.helperCopy ? `<p class="account-mobile-detail-helper">${escapeHtml(uiState.helperCopy)}</p>` : ""}
      ${shouldRenderStageActions ? renderAccountAssignmentActionForms(item, uiState, { mobile: true }) : ""}
  ...
  function renderAccountMobileDashboardStyles() {
    return `<style>
      ...
      @media (max-width: 720px) {
        ...
        .account-mobile-detail-view > .account-mobile-action-stack {
          position: sticky;
          bottom: 82px;
          z-index: 3;
        }
        .account-mobile-detail-view > .account-mobile-action-stack .account-mobile-action-button {
          min-height: 72px;
          border-radius: 20px;
          background: #a54d63;
          border-color: #a54d63;
        ...
        .account-mobile-detail-view > .account-mobile-action-stack .admin-button-secondary.account-mobile-action-button {
          background: #ececf2;
          color: #5f5c65;
          border-color: #ececf2;
        }
  ...
  function renderAccountStageEditorsScript() {
    return `<script>
      (() => {
        ...
        const dashboardMain = document.querySelector("[data-account-mobile-dashboard-main]");
        const detailViews = Array.from(document.querySelectorAll("[data-account-mobile-detail-view]"));
        ...
        document.querySelectorAll("[data-account-mobile-detail-open]").forEach((card) => {
        ...
        document.querySelectorAll("[data-account-mobile-detail-back]").forEach((button) => {
          button.addEventListener("click", closeAccountOrderDetail);
        });

        document.querySelectorAll("[data-account-copy-text]").forEach((button) => {
  ...
  function renderAccountMobileDashboard(orderItems = [], options = {}) {
    ...
    return `<section class="account-mobile-dashboard" data-account-mobile-dashboard>
      ...
      ${renderAccountMobileAssignmentDetailViews(orderItems, { staffId: options.staffId || "" })}
    </section>`;
  }
