...
function createAccountRenderers(deps = {}) {
  ...
  function renderAccountMobileCalendarMonthGrid(orderItems = [], selectedDate = "") {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      ...
      cells.push(`<a class="${className}" href="${escapeHtmlAttribute(buildAccountSectionHref("calendar", { date: dayValue }))}">
  ...
  function renderAccountMobileCalendar(orderItems = [], options = {}) {
    ...
    const selectedDayItems = orderItems.filter((item) => getAccountCalendarOrderDate(item) === selectedDate);
    const selectedMonthItems = orderItems.filter((item) => getAccountCalendarOrderDate(item).startsWith(monthKey));
    const visibleItems = view === "month" ? selectedMonthItems : selectedDayItems;
    const todayHref = buildAccountSectionHref("calendar", { date: selectedDate });
    ...
    return `<section class="account-mobile-calendar" data-account-mobile-calendar>
      <div data-account-mobile-dashboard-main>
        ...
        ${view === "month" ? renderAccountMobileCalendarMonthGrid(orderItems, selectedDate) : ""}
        ${
          visibleItems.length
