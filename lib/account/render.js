...
function createAccountRenderers(deps = {}) {
  ...
  function renderAccountAssignmentActions(item = {}, staffId = "") {
    if (!staffId || !item || !item.assignment || !Array.isArray(item.assignment.staffIds)) {
      return "";
    }
    if (!item.assignment.staffIds.includes(staffId)) {
      return "";
    }
    ...
    if (orderStatus === "cleaning-complete") {
      ...
      helperCopy = "Подтвердите, что сможете выйти на этот заказ, отметьте, что не подтверждаете его, или сразу укажите, что уже выехали.";
    }

    return `<div class="admin-table-cell-stack">
      <div class="admin-inline-badge-row">
        ${renderAccountOrderStageBadge(orderStatus)}
        ${renderAccountCleanerConfirmationBadge(cleanerConfirmationStatus)}
      </div>
      <span class="admin-table-muted">${escapeHtml(helperCopy)}</span>
      <div class="admin-inline-actions">
        ${orderStatus === "scheduled" && cleanerConfirmationStatus !== "confirmed"
          ? `<form method="post" action="${ACCOUNT_ROOT_PATH}">
              <input type="hidden" name="action" value="confirm-assignment">
              <input type="hidden" name="entryId" value="${escapeHtmlAttribute(item.entry.id)}">
              <button class="admin-button" type="submit">Подтвердить</button>
            </form>`
          : ""}
        ${getAccountNextAssignmentAction(orderStatus)
          ? `<form method="post" action="${ACCOUNT_ROOT_PATH}">
              <input type="hidden" name="action" value="${escapeHtmlAttribute(
                getAccountNextAssignmentAction(orderStatus).action
              )}">
              <input type="hidden" name="entryId" value="${escapeHtmlAttribute(item.entry.id)}">
              <button class="admin-button admin-button-secondary" type="submit">${escapeHtml(
                getAccountNextAssignmentAction(orderStatus).label
              )}</button>
            </form>`
          : ""}
        ${orderStatus === "scheduled" && cleanerConfirmationStatus !== "declined"
          ? `<form method="post" action="${ACCOUNT_ROOT_PATH}">
              <input type="hidden" name="action" value="decline-assignment">
              <input type="hidden" name="entryId" value="${escapeHtmlAttribute(item.entry.id)}">
              <button class="admin-button admin-button-secondary" type="submit">Не подтверждаю</button>
            </form>`
          : ""}
      </div>
    </div>`;
      ...
      <div class="admin-section-grid">
        ${renderAdminCard(
          ...
          renderAccountAssignmentsTable(assignedOrders, {
            staffId: staffRecord && staffRecord.id ? staffRecord.id : "",
          }),
          { eyebrow: "Работа" }
        )}
      </div>
