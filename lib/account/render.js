...
function createAccountRenderers(deps = {}) {
  ...
  function renderAccountSystemNotice(message, tone = "info", timeoutMs = 5000) {
    ...
    return `<div class="${alertClass} account-system-notice" data-account-system-notice="true" data-account-notice-timeout="${escapeHtmlAttribute(
      String(timeout)
    )}" role="status" aria-live="${escapeHtmlAttribute(liveMode)}">${escapeHtml(normalizedMessage)}</div>`;
  }
  ...
  function renderAccountSystemNoticeChrome() {
    return `<style>
      ...
      .account-system-notice {
        pointer-events: auto;
        box-shadow: 0 18px 36px rgba(32, 20, 30, 0.12);
        border-radius: 18px;
    ...
    <script>
      (() => {
        const notices = Array.from(document.querySelectorAll('[data-account-system-notice="true"]'));
  ...
  function renderAccountNotice(notice) {
    ...
    if (notice === "assignment-checklist") {
      return renderAccountSystemNotice(
        "Чеклист сохранён. Карточка перенесена в этап «Чеклист»."
      );
    }
    ...
    if (orderStatus === "cleaning-complete") {
      ...
      helperCopy =
        photoSummary.beforeCount > 0 || photoSummary.afterCount > 0
          ? `Фото сохранены: до — ${photoSummary.beforeCount}, после — ${photoSummary.afterCount}. Когда всё готово, завершите уборку последней кнопкой.`
          : "Загрузите фото до и после. После этого можно завершать уборку.";
    } else if (orderStatus === "checklist") {
      helperCopy =
        checklistSummary.totalCount > 0
          ? `Чеклист сохранён: ${checklistSummary.completedCount} из ${checklistSummary.totalCount}. Следующий шаг — загрузить фото до и после.`
          : "Следующий шаг — загрузить фото до и после.";
    } else if (orderStatus === "cleaning-started") {
      helperCopy = "Вы отметили, что начали уборку. Откройте чеклист, отметьте выполненные пункты и сохраните результат.";
  ...
  function renderAccountAssignmentActionForms(item = {}, uiState = {}, options = {}) {
    ...
    function renderChecklistEditor() {
      ...
      return `<details class="account-stage-editor" data-account-checklist-editor>
        ...
        <div class="account-stage-editor-body">
          ...
          <form class="account-stage-editor-form" method="post" action="${ACCOUNT_ROOT_PATH}">
            <input type="hidden" name="action" value="save-assignment-checklist">
            <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
            <div class="account-stage-editor-toolbar">
              <button class="admin-button admin-button-secondary" type="button" data-account-checklist-select-all="true">Выбрать все</button>
            ...
            <div class="account-stage-editor-actions">
              <button class="${primaryClass}" type="submit">Сохранить чеклист</button>
            </div>
          </form>
        </div>
      </details>`;
    ...
    function renderPhotoEditor() {
      ...
      return `<details class="account-stage-editor" data-account-photo-editor>
        <summary class="account-stage-editor-summary">${escapeHtml(summaryLabel)}</summary>
        <div class="account-stage-editor-body">
          <p class="account-stage-editor-copy">Загрузите фото до и после по той же логике, что и в Cleaning app. Сначала сохраняются материалы, и только потом открывается завершение уборки.</p>
          <form class="account-stage-editor-form" method="post" action="${ACCOUNT_ROOT_PATH}" enctype="multipart/form-data">
            <input type="hidden" name="action" value="save-assignment-photos">
            <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entryId)}">
            <div class="account-photo-upload-grid">
              <label class="account-photo-upload-field">
                <span>Фото до</span>
                <input class="admin-input" type="file" name="beforePhotos" accept="image/*" multiple>
                <small>${escapeHtml(beforeCount > 0 ? `Сейчас загружено: ${beforeCount}` : "Можно выбрать несколько фотографий.")}</small>
              </label>
              <label class="account-photo-upload-field">
                <span>Фото после</span>
                <input class="admin-input" type="file" name="afterPhotos" accept="image/*" multiple>
                <small>${escapeHtml(afterCount > 0 ? `Сейчас загружено: ${afterCount}` : "Можно выбрать несколько фотографий.")}</small>
              </label>
            </div>
            <div class="account-stage-editor-actions">
              <button class="${primaryClass}" type="submit">Сохранить фото</button>
            </div>
          </form>
        </div>
      </details>`;
    ...
    if (uiState.orderStatus === "cleaning-started") {
      forms.push(renderChecklistEditor());
    } else if (uiState.orderStatus === "checklist") {
  ...
  function renderAccountAssignmentActions(item = {}, staffId = "") {
    ...
    return `<div class="admin-table-cell-stack">
      ...
      ${uiState.isStageWithCleanerFlow ? renderAccountAssignmentActionForms(item, uiState) : ""}
    </div>`;
  }
  ...
  function getAccountMobileOrderDialogId(item = {}) {
    ...
    const normalizedId =
      entryId
        ...
        .replace(/^-+|-+$/g, "")
        .slice(0, 120) || "order";
    return `account-mobile-order-dialog-${normalizedId}`;
  }

  function renderAccountMobileDialogCloseButton(dialogId) {
    return `<button
      class="admin-icon-button admin-close-button account-mobile-dialog-close"
      type="button"
      aria-label="Закрыть карточку заказа"
      title="Закрыть"
      data-admin-dialog-close="${escapeHtmlAttribute(dialogId)}"
    >
      <span aria-hidden="true">×</span>
      <span class="admin-sr-only">Закрыть</span>
    </button>`;
  }
  ...
  function renderAccountMobileFeaturedAssignment(item = null, options = {}) {
    ...
    const dialogId = getAccountMobileOrderDialogId(item);

    return `<div
      class="account-mobile-focus-card"
      data-account-mobile-card-open="${escapeHtmlAttribute(dialogId)}"
    >
  ...
  function renderAccountMobileAssignmentDialog(item = {}, options = {}) {
    ...
    const requestLabel = item.entry.requestId || item.entry.id;
    const addressLabel = item.entry.fullAddress || "Адрес уточняется";
    const notesLabel = item.assignment && item.assignment.notes ? item.assignment.notes : "";
    const checklistSummary = uiState.checklistSummary || {};
    const photoSummary = uiState.photoSummary || {};
    const checklistLabel =
      Number(checklistSummary.totalCount) > 0
        ? `${Number(checklistSummary.completedCount) || 0}/${Number(checklistSummary.totalCount) || 0}`
        : "Не открыт";
    const photoLabel =
      Number(photoSummary.beforeCount) > 0 || Number(photoSummary.afterCount) > 0
        ? `до ${Number(photoSummary.beforeCount) || 0} / после ${Number(photoSummary.afterCount) || 0}`
        : "Пока нет";

    return `<dialog class="admin-dialog admin-dialog-wide account-mobile-order-dialog" id="${escapeHtmlAttribute(dialogId)}" aria-labelledby="${escapeHtmlAttribute(titleId)}">
      <div class="admin-dialog-panel account-mobile-order-dialog-panel">
        <div class="admin-dialog-head account-mobile-dialog-head">
          <div class="admin-dialog-copy-block">
            <p class="admin-card-eyebrow">Карточка заказа</p>
            <h2 class="admin-dialog-title" id="${escapeHtmlAttribute(titleId)}">${escapeHtml(item.entry.customerName || "Клиент")}</h2>
            <p class="admin-dialog-copy">${escapeHtml(serviceLabel)} · ${escapeHtml(item.scheduleLabel || "Время уточняется")}</p>
          </div>
          ${renderAccountMobileDialogCloseButton(dialogId)}
        </div>
        <section class="account-mobile-dialog-card">
          <div class="account-mobile-badge-row">
            ${renderAccountCurrentStatusBadge(uiState)}
          </div>
          ${renderAccountMobileStageRail(uiState.orderStatus)}
          <div class="account-mobile-dialog-grid">
            <div class="account-mobile-dialog-field">
            ...
            <div class="account-mobile-dialog-field account-mobile-dialog-field-full">
              <span>Адрес</span>
              <strong>${escapeHtml(addressLabel)}</strong>
              ${renderAccountTravelEstimate(travelLeg, { compact: true })}
            </div>
            <div class="account-mobile-dialog-field">
              <span>Чеклист</span>
              <strong>${escapeHtml(checklistLabel)}</strong>
            </div>
            <div class="account-mobile-dialog-field">
              <span>Фото</span>
              <strong>${escapeHtml(photoLabel)}</strong>
            </div>
            ${
              notesLabel
                ? `<div class="account-mobile-dialog-field account-mobile-dialog-field-full">
  ...
  function renderAccountMobileAssignmentDialogs(orderItems = [], options = {}) {
    const items = Array.isArray(orderItems) ? orderItems : [];
    const seen = new Set();
    return items
      .map((item) => {
        const dialogId = getAccountMobileOrderDialogId(item);
        if (seen.has(dialogId)) return "";
        seen.add(dialogId);
        return renderAccountMobileAssignmentDialog(item, options);
      })
      .join("");
  }
  ...
  function renderAccountMobileDashboardStyles() {
    return `<style>
      ...
      .account-photo-upload-field span {
        font-weight: 700;
        color: #3c2230;
      }
      ...
      @media (max-width: 720px) {
        ...
        [data-account-mobile-card-open] {
          cursor: pointer;
          touch-action: manipulation;
        }
        [data-account-mobile-card-open]:focus-visible {
        ...
        .account-mobile-order-dialog {
          width: min(94vw, 560px);
        }
        .account-mobile-order-dialog-panel {
          border-radius: 28px;
          background:
            radial-gradient(circle at top left, rgba(165, 77, 99, 0.12), transparent 38%),
            linear-gradient(180deg, #fffdfc 0%, #fff5f7 100%);
        }
        .account-mobile-dialog-head {
          align-items: flex-start;
        }
        .account-mobile-dialog-close {
          flex-shrink: 0;
          font-size: 28px;
          line-height: 1;
        }
        .account-mobile-dialog-card {
          display: grid;
          gap: 14px;
        }
        .account-mobile-dialog-grid {
          display: grid;
          grid-template-columns: repeat(2, minmax(0, 1fr));
          gap: 12px;
        }
        .account-mobile-dialog-field {
          min-width: 0;
          display: grid;
          gap: 5px;
          padding: 13px 14px;
          border-radius: 18px;
          border: 1px solid rgba(165, 77, 99, 0.1);
          background: rgba(255, 255, 255, 0.74);
        }
        .account-mobile-dialog-field span {
          font-size: 11px;
          font-weight: 800;
        ...
        .account-mobile-dialog-field strong {
          color: #1c1421;
          word-break: break-word;
        }
        .account-mobile-dialog-field-full {
          grid-column: 1 / -1;
        }
      }
    </style>`;
  ...
  function renderAccountStageEditorsScript() {
    return `<script>
      (() => {
        ...
        function openAccountOrderDialog(dialogId) {
        ...
        document.querySelectorAll("[data-account-mobile-card-open]").forEach((card) => {
          const dialogId = card.getAttribute("data-account-mobile-card-open") || "";
          card.addEventListener("click", (event) => {
            const target = event.target instanceof Element ? event.target : null;
            if (shouldSkipCardOpen(target)) return;
            openAccountOrderDialog(dialogId);
          });
        ...
        document.querySelectorAll("[data-account-checklist-editor]").forEach((editor) => {
  ...
  function renderAccountMobileDashboard(orderItems = [], options = {}) {
    ...
    return `<section class="account-mobile-dashboard" data-account-mobile-dashboard>
      ...
      ${renderAccountMobileAssignmentDialogs(orderItems, { staffId: options.staffId || "" })}
    </section>`;
  }
  ...
  function renderDashboardPage(userContext, options = {}) {
    ...
    return renderAdminLayout(
      ...
      `<div data-account-dashboard-root>
        ...
        ${systemNotices ? `${renderAccountSystemNoticeChrome()}<div class="account-system-notice-stack">${systemNotices}</div>` : ""}
