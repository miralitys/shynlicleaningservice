...
function createAccountRenderers(deps = {}) {
  ...
  function formatPhoneFieldValue(value) {
    ...
    if (formattedPhone === "Не указан") return "";
  ...
  function renderPhoneInput(name, value = "") {
    return `<label class="admin-label">
      ...
      <span class="admin-phone-field">
        ...
        <input class="admin-input admin-phone-input" type="tel" name="${escapeHtmlAttribute(name)}" value="${escapeHtmlText(formatPhoneFieldValue(value))}" data-admin-phone-input="true" inputmode="numeric" autocomplete="tel-national" maxlength="14" placeholder="(000) 000-0000">
      </span>
    </label>`;
  }
    ...
    const secondaryCard = setupMode
      ? renderAdminCard(
          ...
          `<ul class="admin-feature-list">
            ...
            <li>Потом пароль можно будет поменять в кабинете.</li>
          </ul>`,
