...
function createAccountRenderers(deps = {}) {
  ...
  function renderAccountCalendarAvailability(staffRecord = null, dateValue = "", options = {}) {
    ...
    const availabilityMode = manualBlock && !manualBlock.allDay ? "time-range" : "all-day";
              ...
              <div class="account-calendar-availability-settings" data-account-availability-settings="true" ${manualBlock ? "" : "hidden"}>
                <label>
                  ...
                  <select name="availabilityMode" data-account-availability-mode="true">
                    ...
                    <option value="time-range"${availabilityMode === "time-range" ? " selected" : ""}>С … до …</option>
                  </select>
                </label>
                <div class="account-calendar-availability-times" data-account-availability-times="true" ${availabilityMode === "time-range" ? "" : "hidden"}>
                  <label>
                    <span>С</span>
                    <input type="time" name="availabilityStartTime" value="${escapeHtmlAttribute(startTime)}" data-account-availability-time="true" ${availabilityMode === "time-range" ? "required" : "disabled"}>
                  </label>
                  <label>
                    <span>До</span>
                    <input type="time" name="availabilityEndTime" value="${escapeHtmlAttribute(endTime)}" data-account-availability-time="true" ${availabilityMode === "time-range" ? "required" : "disabled"}>
                  </label>
                </div>
    ...
    return `<script>
      (() => {
        function syncAvailabilityForm(form) {
          ...
          const usesTimeRange = checked && mode && mode.value === "time-range";
        ...
        document.querySelectorAll('[data-account-availability-form="true"]').forEach((form) => {
          ...
          form.addEventListener("submit", (event) => {
            ...
            if (!mode || mode.value !== "time-range" || inputs.length < 2) return;
