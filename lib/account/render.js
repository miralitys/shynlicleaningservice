    ...
    const summaryBlock = w9Record
      ? `<div class="admin-alert admin-alert-info">
          ...
          ${w9Record.document && w9Record.document.relativePath
            ? ` <a href="${ACCOUNT_W9_DOWNLOAD_PATH}" download>Скачать PDF</a>.`
            : ""}
        </div>
        ${renderAdminPropertyList([
          {
    ...
    const w9FormMarkup = `<form class="admin-form" method="post" action="${ACCOUNT_ROOT_PATH}" data-account-w9-form>
        ...
        <div class="admin-inline-actions">
          ...
          ${w9Record && w9Record.document && w9Record.document.relativePath
            ? `<a class="admin-button admin-button-secondary" href="${ACCOUNT_W9_DOWNLOAD_PATH}" download>Скачать PDF</a>`
            : ""}
        </div>
      </form>`;

    const shouldCollapseCompletedW9 = Boolean(w9Record && !w9Draft);
    const w9EditorBlock = shouldCollapseCompletedW9
      ? `<details class="admin-details" data-account-w9-details>
          <summary>Обновить W-9</summary>
          <div class="admin-form-grid" style="margin-top:14px;">
            <p class="admin-field-note">Если данные изменились, раскройте форму и сохраните новую версию PDF.</p>
            ${w9FormMarkup}
          </div>
        </details>`
      : w9FormMarkup;
