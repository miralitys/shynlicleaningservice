...
function createAccountRenderers(deps = {}) {
      ...
      leg.status === "missing-destination"
        ...
        : leg.status === "missing-origin"
          ...
          : leg.status === "same-place"
            ...
            : "Дорога: считаем маршрут…";
    const fallbackAttr = !GOOGLE_PLACES_API_KEY
      ? "Дорога: карты не подключены"
      : "Дорога: маршрут недоступен";
    const travelAttrs =
      leg.status === "ready"
        ? ` data-account-travel-estimate="true"
            data-account-travel-origin="${escapeHtmlAttribute(leg.originAddress || "")}"
            data-account-travel-destination="${escapeHtmlAttribute(leg.destinationAddress || "")}"
            data-account-travel-departure="${escapeHtmlAttribute(String(Number(leg.departureTimestamp) || 0))}"
            data-account-travel-fallback="${escapeHtmlAttribute(fallbackAttr)}"`
        : "";
    const className = options.compact
      ? "account-mobile-order-travel"
      : "account-mobile-focus-travel";

    return `<p class="${className}"${travelAttrs}>${escapeHtml(fallbackText)}</p>`;
  }
  ...
  function renderAccountTravelEstimateScript() {
    ...
    return `<script>
      (() => {
        const estimateNodes = Array.from(document.querySelectorAll('[data-account-travel-estimate="true"]'));
        if (!estimateNodes.length) return;

        const cache = window.__accountTravelEstimateCache || (window.__accountTravelEstimateCache = new Map());
        const TRAVEL_TIME_BUFFER_MINUTES = 5;

        function loadMapsApi() {
          if (window.__accountMapsApiPromise) return window.__accountMapsApiPromise;
          if (window.google && window.google.maps && typeof window.google.maps.importLibrary === "function") {
            window.__accountMapsApiPromise = Promise.resolve(window.google.maps);
            return window.__accountMapsApiPromise;
          }

          window.__accountMapsApiPromise = new Promise((resolve, reject) => {
            window.__accountGoogleMapsReady = () =>
              resolve(window.google && window.google.maps ? window.google.maps : null);

            const existingScript = document.querySelector('script[data-account-google-maps="true"]');
            if (existingScript) return;

            const script = document.createElement("script");
            script.async = true;
            script.defer = true;
            script.setAttribute("data-account-google-maps", "true");
            script.src = "https://maps.googleapis.com/maps/api/js?key=" +
              encodeURIComponent(${JSON.stringify(GOOGLE_PLACES_API_KEY)}) +
              "&loading=async&v=beta&callback=__accountGoogleMapsReady";
        ...
        function getRoutesApi() {
          if (window.__accountRoutesApiPromise) return window.__accountRoutesApiPromise;
          window.__accountRoutesApiPromise = loadMapsApi()
            .then(async (mapsApi) => {
              if (!mapsApi || typeof mapsApi.importLibrary !== "function") return null;
              const routesLibrary = await mapsApi.importLibrary("routes");
              return { mapsApi, routesLibrary };
            })
            .catch((error) => {
              window.__accountRoutesApiError = error;
              return null;
            });
          return window.__accountRoutesApiPromise;
        }
        ...
        function formatUnavailable(node, reason) {
          return node.getAttribute("data-account-travel-fallback") || reason || "Дорога: маршрут недоступен";
        }

        function updateNodes(nodes, text) {
          nodes.forEach((node) => {
        ...
        function resolveRoute(nodes, apiBundle) {
          ...
          const origin = sampleNode.getAttribute("data-account-travel-origin") || "";
          const destination = sampleNode.getAttribute("data-account-travel-destination") || "";
          const departureMs = Number(sampleNode.getAttribute("data-account-travel-departure") || "0");

          if (!origin || !destination) {
            updateNodes(nodes, formatUnavailable(sampleNode, "Дорога: маршрут недоступен"));
            return;
          }

          const cacheKey = buildRouteKey(origin, destination, departureMs);
          if (cache.has(cacheKey)) {
            updateNodes(nodes, cache.get(cacheKey));
            return;
          }
          ...
          if (!RouteMatrix || typeof RouteMatrix.computeRouteMatrix !== "function") {
            const fallback = formatUnavailable(sampleNode, "Дорога: маршрут недоступен");
            cache.set(cacheKey, fallback);
            updateNodes(nodes, fallback);
            return;
          }
            ...
            .then((response) => {
              ...
              const routeExists =
                ...
                (item.condition === "ROUTE_EXISTS" ||
                  (RouteMatrixItemCondition && item.condition === RouteMatrixItemCondition.ROUTE_EXISTS));
              ...
              const distanceText =
                item && item.localizedValues && item.localizedValues.distance
                  ? item.localizedValues.distance
                  : formatDistanceFromMeters(Number(item && item.distanceMeters));
              const details = [durationText, distanceText].filter(Boolean).join(" • ");
              const successText = details ? "Дорога: " + details : "Дорога: маршрут недоступен";
              cache.set(cacheKey, successText);
              updateNodes(nodes, successText);
            })
            .catch(() => {
              const fallback = formatUnavailable(sampleNode, "Дорога: маршрут недоступен");
              cache.set(cacheKey, fallback);
        ...
        getRoutesApi().then((apiBundle) => {
          ...
          estimateNodes.forEach((node) => {
            const origin = node.getAttribute("data-account-travel-origin") || "";
            const destination = node.getAttribute("data-account-travel-destination") || "";
            const departureMs = Number(node.getAttribute("data-account-travel-departure") || "0");
