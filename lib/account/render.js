...
function createAccountRenderers(deps = {}) {
  ...
  function getAccountMobileInlineActions(item = {}, uiState = {}) {
  ...
  function renderAccountMobileStatusRow(item = {}, uiState = {}) {
    const badgeMarkup = renderAccountCurrentStatusBadge(uiState);
    const actions = getAccountMobileInlineActions(item, uiState);
    if (!actions.length) {
      return `<div class="account-mobile-badge-row">${badgeMarkup}</div>`;
    }

    return `<div class="account-mobile-badge-row account-mobile-badge-row-with-action">
      ${badgeMarkup}
      ${actions
        .map(
          (actionItem) => `<form class="account-mobile-inline-stage-form" method="post" action="${ACCOUNT_ROOT_PATH}" data-account-async-form="true">
            ...
            <button class="account-mobile-inline-stage-button${
              actionItem.tone === "secondary" ? " account-mobile-inline-stage-button-secondary" : ""
  ...
  function renderAccountAssignmentActionForms(item = {}, uiState = {}, options = {}) {
    ...
    const forms = [];
    const shouldUseMobileInlineActions = options.mobile && getAccountMobileInlineActions(item, uiState).length > 0;
          ...
          <form class="account-stage-editor-form" method="post" action="${ACCOUNT_ROOT_PATH}" enctype="multipart/form-data" data-account-async-form="true" data-account-photo-complete-form="true">
            ...
            <div class="account-photo-upload-grid">
              <label class="account-photo-upload-field">
                <span>Фото до</span>
                <input class="admin-input" type="file" name="beforePhotos" accept="image/*" multiple data-account-photo-input="before">
                <small>${escapeHtml(beforeCount > 0 ? `Сейчас загружено: ${beforeCount}` : "Можно выбрать несколько фотографий.")}</small>
              </label>
              <label class="account-photo-upload-field">
                <span>Фото после</span>
                <input class="admin-input" type="file" name="afterPhotos" accept="image/*" multiple data-account-photo-input="after">
    ...
    if (uiState.canConfirm && !shouldUseMobileInlineActions) {
    ...
    } else if (canShowNextAction && !shouldUseMobileInlineActions) {
      forms.push(`<form method="post" action="${ACCOUNT_ROOT_PATH}" data-account-async-form="true">
    ...
    if (uiState.canDecline && !shouldUseMobileInlineActions) {
      forms.push(`<form method="post" action="${ACCOUNT_ROOT_PATH}" data-account-async-form="true">
  ...
  function renderAccountMobileFeaturedAssignment(item = null, options = {}) {
    ...
    >
      ...
      ${renderAccountMobileStatusRow(item, uiState)}
  ...
  function renderAccountMobileDetailTopAction(item = {}, uiState = {}) {
    ...
    if (getAccountMobileInlineActions(item, uiState).length > 0) return "";
  ...
  function renderAccountMobileDashboardStyles() {
    return `<style>
      ...
      @media (max-width: 720px) {
        ...
        .account-mobile-badge-row-with-action {
          align-items: center;
        }
        .account-mobile-inline-stage-form {
          margin: 0;
          display: inline-flex;
        }
        .account-mobile-inline-stage-button {
          min-height: 28px;
          padding: 0 14px;
        ...
        .account-mobile-inline-stage-button-secondary {
          background: rgba(255, 255, 255, 0.96);
          color: #5d3143;
          border-color: rgba(126, 55, 77, 0.18);
        }
        .account-mobile-inline-stage-button:focus-visible {
        ...
        .account-mobile-detail-view .account-mobile-inline-stage-button {
          min-height: 38px;
          padding: 8px 16px;
          font-size: 16px;
        }
    ...
    return `<script>
      (() => {
        ...
        function updatePhotoCompletionState(editor) {
          ...
          const hasStoredBefore = Boolean(beforeInput && String(beforeInput.closest(".account-photo-upload-field")?.querySelector("small")?.textContent || "").includes("Сейчас загружено"));
          const hasStoredAfter = Boolean(afterInput && String(afterInput.closest(".account-photo-upload-field")?.querySelector("small")?.textContent || "").includes("Сейчас загружено"));
          ...
          [beforeInput, afterInput].forEach((input) => {
            if (input instanceof HTMLInputElement) input.disabled = skipped;
          });
        }
        ...
        async function submitAccountAsyncForm(form) {
          ...
          try {
            const response = await fetch(form.action || "/account", {
              method: "POST",
            ...
            await refreshAccountDashboard(payload.refreshPath || "/account", detailIdToRestore || payload.detailId || "");
          } catch (error) {
