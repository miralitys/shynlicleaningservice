  ...
  return async function handleAccountRequest(
    ...
    if (requestContext.route === ACCOUNT_LOGIN_PATH) {
      if (req.method === "GET") {
        ...
        writeHtmlWithTiming(res, 200, accountRenderers.renderLoginPage(), requestStartNs, requestContext.cacheHit);
        return;
      }
      ...
      if (!accountUser || accountUser.status !== "active" || !adminAuth.verifyPassword(password, accountUser.passwordHash)) {
        writeHtmlWithTiming(
          res,
          401,
