...
const { collectPayrollRecords, syncOrderPayrollSnapshot } = require("../payroll");
...
function createAccountRequestHandler(deps = {}) {
  ...
  function getAccountSection(value) {
    return normalizeString(value, 32).toLowerCase() === "payroll" ? "payroll" : "dashboard";
  }
  ...
  function buildAccountChecklistItems(template = {}, completion = {}) {
    const templateItems = Array.isArray(template && template.items) ? template.items : [];
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (
          ...
          const isPhotoSaveAction = action === "save-assignment-photos";
          const isPhotoCompleteAction = action === "complete-assignment-photos";
          ...
          if (
            ...
            (!isStageEditorAction && !actionConfig) ||
            (!isStageEditorAction && !actionConfig.allowedStatuses.includes(orderStatus)) ||
            (actionConfig &&
          ...
          if (isChecklistSaveAction || isChecklistCompleteAction) {
            ...
            await quoteOpsLedger.updateOrderEntry(entryId, {
              orderStatus: isChecklistCompleteAction ? "photos" : "checklist",
          ...
          if (isPhotoSaveAction || isPhotoCompleteAction) {
            const shouldSkipPhotos = Boolean(getFormValue(formBody, "photosSkipped", 20));
            const hasFileUpload =
              (Array.isArray(formFiles.beforePhotos) && formFiles.beforePhotos.length > 0) ||
              (Array.isArray(formFiles.afterPhotos) && formFiles.afterPhotos.length > 0);
            if (orderStatus !== "photos" || (!orderMediaStorage && hasFileUpload)) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }

            const existingCompletion = getEntryOrderCompletionData(entry);
            const beforeUploads = orderMediaStorage && Array.isArray(formFiles.beforePhotos)
              ? await orderMediaStorage.uploadFiles(entryId, "before", formFiles.beforePhotos)
              : [];
            const afterUploads = orderMediaStorage && Array.isArray(formFiles.afterPhotos)
              ? await orderMediaStorage.uploadFiles(entryId, "after", formFiles.afterPhotos)
              : [];

            const nextBeforePhotos =
              beforeUploads.length > 0 ? beforeUploads : existingCompletion.beforePhotos;
            const nextAfterPhotos =
              afterUploads.length > 0 ? afterUploads : existingCompletion.afterPhotos;

            if (
              isPhotoCompleteAction &&
              !shouldSkipPhotos &&
              (nextBeforePhotos.length === 0 || nextAfterPhotos.length === 0)
            ) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }
            if (
            ...
            const updatedEntry = await quoteOpsLedger.updateOrderEntry(entryId, {
              orderStatus: isPhotoCompleteAction ? "cleaning-complete" : "photos",
              completionBeforePhotos: nextBeforePhotos,
              completionAfterPhotos: nextAfterPhotos,
              completionPhotosSkipped: shouldSkipPhotos,
              completionPhotosSkippedAt: shouldSkipPhotos ? new Date().toISOString() : "",
            });
            if (isPhotoCompleteAction) {
            ...
            finishAccountMutation(
              ...
              requestStartNs,
              requestContext,
              isPhotoCompleteAction ? "assignment-photos-complete" : "assignment-photos"
            );
            return;
          }
          ...
          const updatedEntry = await quoteOpsLedger.updateOrderEntry(entryId, orderEntryUpdates);
          await syncOrderPayrollSnapshot({
            quoteOpsLedger,
            staffStore,
            entry: updatedEntry || entry,
