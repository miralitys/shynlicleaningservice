...
function createAccountRequestHandler(deps = {}) {
  ...
  return async function handleAccountRequest(
    ...
    async function loadDashboardContext() {
      ...
      const settingsSnapshot = settingsStore ? await settingsStore.getSnapshot() : { templates: [] };
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (
          ...
          const isStageEditorAction = isChecklistSaveAction || isPhotoSaveAction;
          ...
          if (isChecklistSaveAction) {
            ...
            if (
              !["cleaning-started", "checklist"].includes(orderStatus) ||
          ...
          if (isPhotoSaveAction) {
            if (!["checklist", "photos"].includes(orderStatus) || !orderMediaStorage) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }
          ...
          await quoteOpsLedger.updateOrderEntry(entryId, orderEntryUpdates);
