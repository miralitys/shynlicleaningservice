...
const ACCOUNT_DASHBOARD_LEDGER_LIMIT = 1000;
const ACCOUNT_DATA_CACHE_TTL_MS = 8 * 1000;
...
function createAccountRequestHandler(deps = {}) {
  ...
  async function loadAccountDataSnapshot({
    quoteOpsLedger,
    staffStore,
    settingsStore,
    usersStore,
    ...
    if (
      allowCached &&
      accountDataCache &&
      accountDataCache.expiresAt > now &&
      accountDataCache.quoteOpsLedger === quoteOpsLedger &&
      accountDataCache.staffStore === staffStore &&
      accountDataCache.settingsStore === settingsStore &&
      accountDataCache.usersStore === usersStore &&
      accountDataCache.staffId === staffId
    ) {
      return accountDataCache.promise;
    }
    ...
    ]).then(async ([staffSnapshot, settingsSnapshot, usersSnapshot]) => {
      const assignedEntryIds = (Array.isArray(staffSnapshot.assignments)
        ? staffSnapshot.assignments
        : [])
        .filter((assignment) =>
          Array.isArray(assignment && assignment.staffIds) &&
          assignment.staffIds.includes(staffId)
        )
        .map((assignment) => normalizeString(assignment && assignment.entryId, 180))
        .filter(Boolean);
      const allEntries = quoteOpsLedger
        ? typeof quoteOpsLedger.listEntriesByIds === "function"
          ? await quoteOpsLedger.listEntriesByIds(assignedEntryIds)
          : await quoteOpsLedger.listEntries({ limit: ACCOUNT_DASHBOARD_LEDGER_LIMIT })
        : [];
      return { allEntries, staffSnapshot, settingsSnapshot, usersSnapshot };
    });

    accountDataCache = {
      expiresAt: now + ACCOUNT_DATA_CACHE_TTL_MS,
      quoteOpsLedger,
      staffStore,
      settingsStore,
      usersStore,
    ...
    try {
      ...
      if (accountDataCache && accountDataCache.promise === promise) clearAccountDataCache();
      throw error;
    }
  }
  ...
  function isAccountTabNavigation(req) {
    const referer = normalizeString(req && req.headers ? req.headers.referer : "", 1000);
    if (!referer) return false;
    try {
      return new URL(referer).pathname === ACCOUNT_ROOT_PATH;
    } catch {
      return false;
    }
  }
  ...
  function buildAccountRedirectPath(notice = "", options = {}) {
    ...
    const section = normalizeString(options.section, 32).toLowerCase();
    if (section === "calendar" || section === "payroll") {
      reqUrl.searchParams.set("section", section);
    }
    const calendarView = normalizeString(options.calendarView, 20).toLowerCase();
    if (section === "calendar" && calendarView === "month") {
      reqUrl.searchParams.set("view", "month");
    }
    const calendarDate = normalizeStaffAvailabilityDate(options.calendarDate);
    if (section === "calendar" && calendarDate) {
      reqUrl.searchParams.set("date", calendarDate);
    }
    if (section === "calendar" && options.calendarShowDay) {
      reqUrl.searchParams.set("showDay", "1");
    }
  ...
  function normalizeAccountChecklistServiceType(value) {
    ...
    if (["moving", "movein", "moveout", "moveinout", "moveincleaning", "moveoutcleaning"].includes(compact)) {
      return "moving";
    }
    return "regular";
  }
  ...
  return async function handleAccountRequest(
    ...
    async function loadDashboardContext(options = {}) {
      const { allEntries, staffSnapshot, settingsSnapshot, usersSnapshot } =
        await loadAccountDataSnapshot({
          quoteOpsLedger,
          staffStore,
          settingsStore,
          usersStore,
      ...
      const staffAssignments = Array.isArray(staffSnapshot.assignments)
        ? staffSnapshot.assignments
        : [];
      ...
      const accountEntries = allEntries.filter((entry) =>
        assignedEntryIds.has(normalizeString(entry && entry.id, 180))
      );
      const planning = buildStaffPlanningContext(accountEntries, {
        ...staffSnapshot,
        assignments: staffAssignments.filter((assignment) =>
          assignedEntryIds.has(normalizeString(assignment && assignment.entryId, 180))
        ),
      });
      ...
      return {
        ...
        assignedOrders,
        managerContact,
        payrollSummary: options.includePayroll
          ? collectPayrollRecords({
              entries: allEntries,
              staffId: normalizeString(user && user.staffId, 120),
            })
      ...
      try {
        if (
          ...
          if (!staffStore || !user.staffId) {
            throw new Error("STAFF_AVAILABILITY_UNAVAILABLE");
          }
          ...
          const existingAvailabilityBlock = sanitizeStaffAvailabilityBlocks(
            linkedStaff.availabilityBlocks
          ).find((block) => block.date === availabilityDate) || null;
          if (
            existingAvailabilityBlock &&
            normalizeString(existingAvailabilityBlock.source, 40).toLowerCase() !== "manual"
          ) {
            throw new Error("STAFF_AVAILABILITY_EXTERNAL");
          }

          const nextAvailabilityBlocks =
            action === "clear-own-unavailable-day"
              ? removeStaffAvailabilityBlock(linkedStaff.availabilityBlocks, availabilityDate)
              : (() => {
                  const requestedAvailabilityMode = getFormValue(
                    formBody,
                    "availabilityMode",
                    32
                  ).toLowerCase();
                  const availabilityMode = requestedAvailabilityMode === "all-day"
                    ? "all-day"
                    : "time-range";
                  const startTime = normalizeStaffAvailabilityTime(
                    requestedAvailabilityMode === "morning"
                      ? "09:00"
                      : requestedAvailabilityMode === "afternoon"
                        ? "13:00"
                        : getFormValue(formBody, "availabilityStartTime", 16)
                  );
                  const endTime = normalizeStaffAvailabilityTime(
                    requestedAvailabilityMode === "morning"
                      ? "13:00"
                      : requestedAvailabilityMode === "afternoon"
                        ? "18:00"
                        : getFormValue(formBody, "availabilityEndTime", 16)
                  );
                  if (
                    ...
                    (
                      !startTime ||
                      !endTime ||
                      getStaffAvailabilityTimeMinutes(endTime) <=
                        getStaffAvailabilityTimeMinutes(startTime)
                    )
                  ) {
                    throw new Error("STAFF_AVAILABILITY_TIME_RANGE_INVALID");
                  }
                  const recurrence = getFormValue(formBody, "availabilityRecurrence", 20);
                  return buildStaffAvailabilityRecurrenceDates(availabilityDate, recurrence)
                    .reduce((blocks, date) => upsertStaffAvailabilityBlock(blocks, {
                      date,
                      availabilityMode,
                      allDay: availabilityMode !== "time-range",
                      startTime,
                      endTime,
                      summary: DEFAULT_STAFF_UNAVAILABLE_SUMMARY,
                    }), linkedStaff.availabilityBlocks);
                })();
          ...
          finishAccountMutation(
            ...
            {
              section: "calendar",
              calendarDate: availabilityDate,
              calendarView: getFormValue(formBody, "calendarView", 20),
              calendarShowDay: getFormValue(formBody, "calendarShowDay", 10) === "1",
            }
          );
          return;
        }
        ...
        finishAccountMutationError(
          ...
          action === "save-w9"
            ? { focusSection: ACCOUNT_W9_FOCUS_VALUE }
            : errorNotice === "staff-unavailable-error"
              ? {
                  section: "calendar",
                  calendarDate: getFormValue(formBody, "availabilityDate", 32),
                  calendarView: getFormValue(formBody, "calendarView", 20),
                  calendarShowDay: getFormValue(formBody, "calendarShowDay", 10) === "1",
                }
    ...
    const dashboardContext = await loadDashboardContext({
      includePayroll: activeSection === "payroll",
      allowCached:
        reqUrl.searchParams.get("tab") === "1" || isAccountTabNavigation(req),
    });
    const renderOptions = {
      ...
      calendarShowDay: normalizeString(reqUrl.searchParams.get("showDay"), 20) === "1",
      dashboardFilter: normalizeString(
        reqUrl.searchParams.get("filter") || reqUrl.searchParams.get("mobileFilter"),
        32
      ).toLowerCase(),
      ...
      showAllHistory: reqUrl.searchParams.get("history") === "all",
    };
