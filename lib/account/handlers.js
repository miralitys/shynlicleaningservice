    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (
          ...
          const isStageEditorAction = isChecklistSaveAction || isPhotoSaveAction || isNoteSaveAction;
          ...
          if (isChecklistSaveAction) {
            ...
            if (
              ...
              !updatedChecklistItems.some((item) => item.completed)
            ) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }
            ...
            redirectWithTiming(
              res,
              303,
              buildAccountRedirectPath("assignment-checklist"),
              requestStartNs,
              requestContext.cacheHit
            );
            return;
          ...
          if (isNoteSaveAction) {
            ...
            redirectWithTiming(
              res,
              303,
              buildAccountRedirectPath("assignment-note-saved"),
              requestStartNs,
              requestContext.cacheHit
            );
            return;
          ...
          if (isPhotoSaveAction) {
            if (orderStatus !== "photos" || !orderMediaStorage) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }

            const existingCompletion = getEntryOrderCompletionData(entry);
            const beforeUploads = Array.isArray(formFiles.beforePhotos)
              ? await orderMediaStorage.uploadFiles(entryId, "before", formFiles.beforePhotos)
              : [];
            const afterUploads = Array.isArray(formFiles.afterPhotos)
            ...
            if (nextBeforePhotos.length === 0 || nextAfterPhotos.length === 0) {
              throw new Error("ASSIGNMENT_CONFIRMATION_INVALID");
            }
            ...
            redirectWithTiming(
              res,
              303,
              buildAccountRedirectPath("assignment-photos"),
              requestStartNs,
              requestContext.cacheHit
            );
            return;
          ...
          redirectWithTiming(
            res,
            303,
            buildAccountRedirectPath(actionConfig.notice),
            requestStartNs,
            requestContext.cacheHit
          );
          return;
