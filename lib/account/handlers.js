...
function createAccountRequestHandler(deps = {}) {
  ...
  function normalizeAccountReturnPath(value, user = null) {
    ...
    try {
      ...
      if (shouldUseAdminWorkspace(user)) return defaultPath;
  ...
  return async function handleAccountRequest(
    ...
    const employeeDocumentsOverrideEnabled = cookies[USER_EMPLOYEE_DOCUMENTS_COOKIE] === "1";
    ...
    if (
      shouldUseAdminWorkspace(user) &&
      !(employeeDocumentsOverrideEnabled && isEmployeeLinkedUser(user))
    ) {
        ...
        if (
          ...
          const assignmentActionConfig = {
            ...
            "mark-assignment-checklist": {
              ...
              notice: "assignment-checklist",
            },
            "mark-assignment-photos": {
              allowedStatuses: ["checklist"],
              nextStatus: "photos",
              notice: "assignment-photos",
            },
          ...
          if (
            ...
            !actionConfig ||
            !actionConfig.allowedStatuses.includes(orderStatus)
          ) {
