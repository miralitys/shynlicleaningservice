...
function createAdminPagePrimitiveHelpers(deps = {}) {
  ...
  function formatAdminClientAddressPropertyTypeLabel(value) {
    ...
    if (normalized === "townhouse") return "Таунхаус";
  ...
  function renderAdminPhoneInput(name, value = "", options = {}) {
    ...
    const onInputAttr =
      " oninput=\"this.value=this.value.replace(/\\D+/g,'').replace(/^1(?=\\d{10}$)/,'').slice(0,10)\"";

    return `<label class="admin-label">
      ${escapeHtml(fieldLabel)}
      <input class="admin-input admin-phone-input" type="tel" name="${escapeHtmlAttribute(name)}" value="${escapeHtmlText(inputValue)}" inputmode="numeric" autocomplete="${escapeHtmlAttribute(autocomplete)}" placeholder="${escapeHtmlAttribute(placeholder)}"${onInputAttr}${requiredAttr}>
    </label>`;
  }
  ...
  function renderAdminDeleteIconButton(label, confirmMessage, options = {}) {
    const normalizedLabel = normalizeString(label, 120) || "Удалить";
    const normalizedConfirm = normalizeString(confirmMessage, 400);
    const confirmTitle = normalizeString(options.confirmTitle, 160) || "Точно удалить?";
    const confirmAttrs = ` data-admin-confirm="true" data-admin-confirm-title="${escapeHtmlAttribute(confirmTitle)}"${normalizedConfirm ? ` data-admin-confirm-message="${escapeHtmlAttribute(normalizedConfirm)}"` : ""}`;
  ...
  function renderClientsNotice(req) {
    ...
    if (notice === "client-photo-added") return `<div class="admin-alert admin-alert-info">Фото прикреплено к карточке клиента.</div>`;
    if (notice === "client-photo-deleted") return `<div class="admin-alert admin-alert-info">Фото удалено из карточки клиента.</div>`;
    if (notice === "client-photo-empty") {
      return `<div class="admin-alert admin-alert-error">Выберите хотя бы одно фото перед загрузкой.</div>`;
    }
    if (notice === "client-photo-missing") {
      return `<div class="admin-alert admin-alert-error">Фото уже не найдено в карточке клиента.</div>`;
    }
