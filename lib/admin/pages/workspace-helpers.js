  ...
  async function collectQuoteOpsManagerOptions(adminRuntime = {}, staffStore = null) {
    ...
    return (Array.isArray(usersSnapshot && usersSnapshot.users) ? usersSnapshot.users : [])
      .filter((user) => {
        ...
        const hasLinkedStaffRecord = Boolean(staffId && staffMeta);
        return (
          status === "active" &&
      ...
      .map((user) => {
        ...
        const staffMeta = staffMetaById.get(normalizeString(user.staffId, 120));
        const name = (staffMeta && staffMeta.name) || email || "Менеджер";
