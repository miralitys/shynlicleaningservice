...
function renderQuoteOpsDetailDialog(entry, returnTo, managerOptions = [], options = {}) {
  ...
  const errorBlock = entry.status === "error" && entry.errorMessage
    ...
    : "";
  const stageManagerCard = renderAdminCard(
    "Статус заявки",
    ...
    `<form class="admin-form admin-form-grid admin-form-grid-two admin-quote-entry-stage-form" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
      ...
      <label class="admin-label">
        ...
        <select class="admin-input" name="leadStatus">
          ...
          <option value="completed"${leadStatus === "completed" ? " selected" : ""}>Выполнено</option>
    ...
    { eyebrow: "Статус", muted: true, className: "admin-quote-entry-stage-card" }
  );
  const leadTasksCard = renderAdminCard(
    "Таски по заявке",
          ...
          <table class="admin-table">
            ...
            <tbody>
              ${openTaskRecords.map((task) => {
                const taskHref = buildAdminRedirectPath(ADMIN_QUOTE_OPS_PATH, {
                  section: "tasks",
                  task: task.id,
                });
                return `<tr
                  ...
                  tabindex="0"
                  data-admin-row-href="${escapeHtmlAttribute(taskHref)}"
                  aria-label="${escapeHtmlAttribute(`Открыть таск ${task.title}`)}"
                >
                <td>
                  <div class="admin-table-cell-stack">
                    <a class="admin-table-strong admin-table-link" href="${escapeHtmlAttribute(taskHref)}">${escapeHtml(task.title)}</a>
    ...
    <div class="admin-dialog-panel">
      ...
      <section class="admin-client-summary-panel">
        <div class="admin-client-summary-head">
          <div class="admin-client-summary-copy-block admin-quote-entry-summary-stack">
            <div class="admin-quote-entry-stage-focus">
              <div class="admin-subsection-head">
      ...
      ${renderAdminGhlSmsComposer({
        ...
        historyEntries: options.deferSmsHistory ? [] : getEntryCustomerSmsHistoryEntries(entry),
      ...
      <section>
        <div class="admin-subsection-head">
          <h3 class="admin-subsection-title">Таски по заявке</h3>
        </div>
        <div class="admin-quote-entry-detail-grid admin-quote-entry-detail-grid-single">
          ${leadTasksCard}
        </div>
      </section>
    </div>
...
function renderQuoteOpsTaskResultDialog(taskRecord, returnTo, dialogIdOverride = "", options = {}) {
  ...
  const isStandaloneTask = normalizeString(taskRecord.entry.kind, 64) === "admin_standalone_task";
  const isPostCompletionTask =
    normalizeString(taskRecord.kind, 64).toLowerCase() === "post-completion-followup";
  const taskActionCard = isManualTask
    ...
    : isPostCompletionTask
      ? renderAdminCard(
          "Следующая уборка",
          "Зафиксируйте итог разговора с клиентом после завершённой уборки.",
          `<div class="admin-quote-task-dialog-actions-row">
            <button
              class="admin-button"
              type="button"
              data-quote-task-agreed-toggle="${escapeHtmlAttribute(dialogId)}"
              aria-expanded="false"
            >Договорились</button>
            <form class="admin-inline-actions" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
              <input type="hidden" name="action" value="complete-lead-task">
              <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
              <input type="hidden" name="taskId" value="${escapeHtmlAttribute(taskRecord.id)}">
              <input type="hidden" name="taskAction" value="next-cleaning-declined">
              <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
              <button class="admin-button admin-button-secondary" type="submit">Не договорились</button>
            </form>
          </div>
          <form
            ...
            data-quote-task-agreed-panel="${escapeHtmlAttribute(dialogId)}"
            hidden
          >
            <input type="hidden" name="action" value="complete-lead-task">
            <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
            <input type="hidden" name="taskId" value="${escapeHtmlAttribute(taskRecord.id)}">
            <input type="hidden" name="taskAction" value="next-cleaning-agreed">
            <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
            <label class="admin-label admin-quote-task-dialog-field" data-visible="true">
              Дата и время следующей уборки
              <input class="admin-input" type="datetime-local" name="nextCleaningAt" required>
            </label>
            <div class="admin-inline-actions">
              <button class="admin-button" type="submit">Запланировать</button>
            </div>
          </form>`,
          { eyebrow: "После завершения", muted: true }
        )
    : renderAdminCard(
        "Следующее действие",
        ...
        <form
          ...
          data-quote-task-contacted-autosave="true"
          hidden
        >
          <input type="hidden" name="action" value="complete-lead-task">
          <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
          ...
          <div class="admin-inline-actions">
            <button class="admin-button" type="submit" data-quote-task-autosave-submit="true">Сохранить переход</button>
            <span class="admin-form-hint" data-quote-task-autosave-status="true" aria-live="polite"></span>
          </div>
        </form>`,
      ...
      <div class="admin-dialog-head admin-dialog-hero">
        <div class="admin-dialog-hero-main">
          ...
          <div class="admin-dialog-copy-block admin-dialog-hero-copy">
            ...
            <div class="admin-dialog-hero-title-block">
              <h2 class="admin-dialog-title" id="${escapeHtmlAttribute(`${dialogId}-title`)}">${escapeHtml(taskRecord.customerName)}</h2>
        ...
        <div class="admin-inline-actions admin-dialog-head-actions admin-dialog-hero-actions">
          ...
          >Открыть заявку</a>`}
          <form class="admin-inline-actions" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
            <input type="hidden" name="action" value="delete-lead-task">
            <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
            <input type="hidden" name="taskId" value="${escapeHtmlAttribute(taskRecord.id)}">
            <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
            ${renderAdminDeleteIconButton(
              "Удалить таск",
              "Удалится только этот таск. Заявка и заказы клиента останутся без изменений.",
              { confirmTitle: "Вы точно хотите удалить?" }
            )}
          </form>
          ${renderAdminDialogCloseButton(dialogId)}
        </div>
      ...
      <section class="admin-quote-task-dialog-summary">
        ...
        <div class="admin-quote-task-dialog-secondary-grid">
          <article class="admin-quote-task-dialog-secondary-card">
            ...
            <p class="admin-quote-task-dialog-secondary-value">${escapeHtml(isStandaloneTask ? "Без клиента" : requestLabel)}</p>
          </article>
          <article class="admin-quote-task-dialog-secondary-card">
...
function renderQuoteOpsTaskTableRow(taskRecord, returnTo) {
  ...
  >
    ...
    <td>
      <div class="admin-table-cell-stack">
        <span class="admin-table-strong admin-quote-task-row-id">${escapeHtml(isStandaloneTask ? "Без клиента" : taskRecord.requestId || taskRecord.entry.id)}</span>
        <span class="admin-table-muted">${escapeHtml(isStandaloneTask ? "Внутренний таск" : taskRecord.entry.customerPhone ? formatAdminPhoneNumber(taskRecord.entry.customerPhone) || taskRecord.entry.customerPhone : "Телефон не указан")}</span>
      </div>
    </td>
  </tr>`;
}
