...
function createPayrollPageRenderer(deps = {}) {
  ...
  function renderPayrollSummaryCards(totals = {}) {
    const summaryItems = [
      ...
      },
      {
        ...
        copy: "Попали в payroll по завершённым заказам.",
      },
      {
  ...
  function renderPayrollStaffSummaryTable(staffSummaries = []) {
    if (!Array.isArray(staffSummaries) || staffSummaries.length === 0) {
      return `<div class="admin-empty-state">Пока нет сотрудников с начислениями.</div>`;
    }
  ...
  async function renderPayrollPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-payroll-page">
        ...
        ${renderAdminCard(
          "Фильтр выплат",
          "Смотрите, что нужно выплатить, что уже закрыто, и как расходятся суммы по сотрудникам.",
      ...
      {
        kicker: false,
        subtitle: "Контроль начислений по завершённым заказам: что нужно выплатить и что уже закрыто.",
