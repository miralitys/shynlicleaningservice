...
function createOrderManagementDialogBuilders(deps = {}) {
  ...
  function getOrderClientKey(order = {}) {
    const phoneKey = normalizeString(order.customerPhone, 80).replace(/\D+/g, "");
    if (phoneKey) return phoneKey;

    const emailKey = normalizeString(order.customerEmail, 250).toLowerCase();
    if (emailKey) return emailKey;

    const nameKey = normalizeString(order.customerName, 250).toLowerCase();
    const addressKey = normalizeString(order.fullAddress, 500).toLowerCase();
    return normalizeString([nameKey, addressKey].filter(Boolean).join("|"), 500);
  }
  const ORDER_QUOTE_BOOLEAN_OPTIONS = Object.freeze([
    { value: "yes", label: "Да" },
    { value: "no", label: "Нет" },
  ]);
  const ORDER_QUOTE_SERVICE_TYPE_OPTIONS = Object.freeze([
    { value: "standard", label: "Standard" },
    { value: "deep", label: "Deep" },
    { value: "move-in", label: "Move-in" },
    { value: "move-out", label: "Move-out" },
    { value: "move-in/out", label: "Move-in/out (нужно уточнить)" },
    { value: "free-in-home-estimate", label: "Free in-home estimate" },
  ]);
  const ORDER_QUOTE_FREQUENCY_OPTIONS = Object.freeze([
    { value: "weekly", label: "Weekly" },
    { value: "biweekly", label: "Bi-weekly" },
    { value: "every3weeks", label: "Every 3 weeks" },
    { value: "monthly", label: "Monthly" },
  ]);
  const ORDER_QUOTE_PET_OPTIONS = Object.freeze([
    ...(Array.isArray(CLIENT_ADDRESS_PET_OPTIONS) ? CLIENT_ADDRESS_PET_OPTIONS : []),
    { value: "yes", label: "Да" },
  ]);
  ...
  function normalizeQuoteServiceTypeValue(value, fallback = "standard") {
  ...
  function normalizeQuoteFrequencyValue(value, fallback = "") {
    const normalized = normalizeString(value, 40)
      .toLowerCase()
      .replace(/[\s_-]+/g, "");
  ...
  function normalizeQuoteBooleanInputValue(value) {
  ...
  function normalizeQuotePetsInputValue(value) {
    if (value === true) return "yes";
  ...
  function getQuoteSelectedServiceKeys(quoteData = {}) {
    const labels = ORDER_QUOTE_SERVICE_LABELS && typeof ORDER_QUOTE_SERVICE_LABELS === "object"
      ? ORDER_QUOTE_SERVICE_LABELS
      : {};
    const aliases = ORDER_QUOTE_SERVICE_ALIASES && typeof ORDER_QUOTE_SERVICE_ALIASES === "object"
      ? ORDER_QUOTE_SERVICE_ALIASES
      : {};
    const labelTokenEntries = Object.entries(labels).map(([key, label]) => [
      key,
      normalizeQuoteToken(label),
    ]);
    const selected = new Set();
    const sources = [quoteData.services, quoteData.addOns, quoteData.extras];
    for (const source of sources) {
      ...
      for (const item of items) {
        const rawValue = normalizeString(item, 120);
        if (!rawValue) continue;
        const token = normalizeQuoteToken(rawValue);
        const alias = aliases[token];
        if (alias && labels[alias]) {
          selected.add(alias);
          continue;
        }
        if (labels[rawValue]) {
        ...
        const labelMatch = labelTokenEntries.find(([key, labelToken]) => {
          return token === normalizeQuoteToken(key) || token === labelToken;
        });
        if (labelMatch) selected.add(labelMatch[0]);
      }
    }
    return selected;
  }

  function getQuoteQuantityInputValue(quoteData = {}, key) {
    const quantityServices =
      quoteData && quoteData.quantityServices && typeof quoteData.quantityServices === "object"
        ...
        : {};
    if (!Object.prototype.hasOwnProperty.call(quantityServices, key)) return "0";
    const normalized = normalizeString(quantityServices[key], 32);
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const isRecurringOrder = Boolean(
      order.isRecurring ||
      order.frequency ||
      order.recurringSeriesId ||
      order.recurringAnchorDate ||
      order.recurringSourceEntryId ||
      order.recurringNextEntryId
    );
    const canScheduleNextOrder =
      canEdit && !order.recurringNextEntryId && (Boolean(order.frequency) || order.orderStatus === "completed");
    const canRestoreRecurringSeries =
      canEdit &&
      order.orderStatus === "canceled" &&
      Boolean(order.frequency || order.recurringNextEntryId);
    const dialogId = getOrderDialogId(order.id);
    const editFormId = `${dialogId}-edit-form`;
    const quoteEditPanelId = `${dialogId}-quote-fields-edit-panel`;
    const quoteEditFormId = `${dialogId}-quote-fields-edit-form`;
    const amountFormId = `${dialogId}-amount-form`;
    const amountEditPanelId = `${dialogId}-amount-edit-panel`;
    const scheduleFormId = `${dialogId}-schedule-form`;
    const scheduleEditPanelId = `${dialogId}-schedule-edit-panel`;
    ...
    const orderClientKey = getOrderClientKey(order);
    const clientProfileHref = orderClientKey
      ? buildAdminRedirectPath(ADMIN_CLIENTS_PATH, { client: orderClientKey })
      : "";
    ...
    const assignedStaffRecords =
      planningItem && Array.isArray(planningItem.assignedStaff)
        ? planningItem.assignedStaff
        : [];
    const currentOrderState = getEntryOrderState(order && order.entry ? order.entry : {});
    const payrollOverrides =
      currentOrderState.payrollOverrides && typeof currentOrderState.payrollOverrides === "object"
        ? currentOrderState.payrollOverrides
        : {};
    ...
    const orderNotificationState = getEntryOrderState(order && order.entry ? order.entry : {});
    const reminderSms =
      orderNotificationState.notifications &&
      orderNotificationState.notifications.reminderSms &&
      typeof orderNotificationState.notifications.reminderSms === "object"
        ? orderNotificationState.notifications.reminderSms
        : {};
    const hasSentVisitReminderMarker = ["sent48hAt", "sent24hAt", "sent1hAt"].some((key) =>
      Boolean(normalizeString(reminderSms[key], 80))
    );
    const hasSentVisitReminderHistory = getEntrySmsHistoryEntries(order.entry || {}).some((record) => {
      const targetType = normalizeString(record && record.targetType, 40).toLowerCase();
      const status = normalizeString(record && record.status, 20).toLowerCase();
      return ["visit-reminder", "visit-reminder-update"].includes(targetType) && status !== "failed";
    });
    const hasSentVisitReminder = hasSentVisitReminderMarker || hasSentVisitReminderHistory;
    const reminderConfirmAttributes = hasSentVisitReminder
      ? ` data-admin-reminder-confirm="true"`
      : "";
    ...
    const policyNotRequired = order.serviceType === "free-in-home-estimate";
    ...
    const normalizedOrderStatus =
      typeof normalizeOrderStatus === "function"
        ...
        : normalizeString(order.orderStatus, 40).toLowerCase();
    const canSendReviewRequest =
      options.canEdit && ["awaiting-review", "completed", "paid"].includes(normalizedOrderStatus);
    ...
    const quoteServiceTypeInputValue = normalizeQuoteServiceTypeValue(
      quoteData.serviceType || order.serviceType,
      "standard"
    );
    const quoteFrequencyInputValue = normalizeQuoteFrequencyValue(quoteData.frequency || order.frequency, "");
    const quoteSelectedDateInputValue = normalizeString(quoteData.selectedDate || order.selectedDate, 32);
    const quoteSelectedTimeInputValue = normalizeString(quoteData.selectedTime || order.selectedTime, 32);
    const quoteConsentInputValue = normalizeQuoteBooleanInputValue(quoteData.consent);
    const quotePetsInputValue = normalizeQuotePetsInputValue(quoteData.hasPets);
    const quoteBasementInputValue = normalizeQuoteBooleanInputValue(quoteData.basementCleaning);
    const quoteSelectedServiceKeys = getQuoteSelectedServiceKeys(quoteData);
    const quoteServiceCheckboxes = Object.entries(
      ORDER_QUOTE_SERVICE_LABELS && typeof ORDER_QUOTE_SERVICE_LABELS === "object"
        ? ORDER_QUOTE_SERVICE_LABELS
        : {}
    )
      .map(([serviceKey, serviceLabel]) => `<label class="admin-checkbox">
        <input
          type="checkbox"
          name="quoteServices"
          value="${escapeHtmlAttribute(serviceKey)}"
          ${quoteSelectedServiceKeys.has(serviceKey) ? "checked" : ""}
        >
        <span><strong>${escapeHtml(serviceLabel)}</strong></span>
      </label>`)
      .join("");
    const quoteEditorMarkup = `<section
      class="admin-order-quote-edit-panel"
      id="${escapeHtmlAttribute(quoteEditPanelId)}"
      data-admin-toggle-panel
      hidden
    >
      <form
        class="admin-form-grid admin-order-quote-edit-form"
        id="${escapeHtmlAttribute(quoteEditFormId)}"
        method="post"
        action="${ADMIN_ORDERS_PATH}"
        data-admin-save-confirm="true"
        ${isRecurringOrder ? `data-admin-recurring-comment-confirm="true"` : ""}
        ${reminderConfirmAttributes}
        data-admin-reminder-fields="quoteServiceType,quoteSelectedDate,quoteSelectedTime,quoteFullAddress,quoteAddress,quoteAddressLine2,quoteCity,quoteState,quoteZipCode"
      >
        <input type="hidden" name="action" value="update-order-quote-fields">
        ...
        ${isRecurringOrder
          ? `<input type="hidden" name="recurringEditScope" value="current" data-admin-recurring-edit-scope-mirror>`
          : ""}
        <div class="admin-order-quote-grid admin-order-quote-edit-grid">
          <article class="admin-order-quote-card">
            <div class="admin-subsection-head">
              <h4 class="admin-subsection-title">Что запросил клиент</h4>
            </div>
            <div class="admin-form-grid admin-form-grid-two admin-order-quote-edit-fields">
              <label class="admin-label">
                Тип уборки
                <select class="admin-input" name="quoteServiceType">
                  ${renderAdminSelectOptions(ORDER_QUOTE_SERVICE_TYPE_OPTIONS, quoteServiceTypeInputValue)}
                </select>
              </label>
              <label class="admin-label">
                Повторяемость
                <select class="admin-input" name="quoteFrequency">
                  ${renderAdminSelectOptions(ORDER_QUOTE_FREQUENCY_OPTIONS, quoteFrequencyInputValue, "Not set")}
                </select>
              </label>
              ${renderAdminPickerField({
                pickerType: "date",
                fieldId: `${quoteEditFormId}-selected-date`,
                label: "Запрошенная дата",
                name: "quoteSelectedDate",
                displayValue: formatAdminOrderDateInputValue(quoteSelectedDateInputValue),
                nativeValue: quoteSelectedDateInputValue,
                placeholder: "04/15/2026",
                pickerLabel: "Выбрать запрошенную дату",
              })}
              ${renderAdminPickerField({
                pickerType: "time",
                fieldId: `${quoteEditFormId}-selected-time`,
                label: "Запрошенное время",
                name: "quoteSelectedTime",
                displayValue: formatAdminOrderTimeInputValue(quoteSelectedTimeInputValue),
                nativeValue: quoteSelectedTimeInputValue,
                placeholder: "1:30 PM",
                pickerLabel: "Выбрать запрошенное время",
              })}
              <label class="admin-label">
                Согласие
                <select class="admin-input" name="quoteConsent">
                  ${renderAdminSelectOptions(ORDER_QUOTE_BOOLEAN_OPTIONS, quoteConsentInputValue, "Не указано")}
                </select>
              </label>
            </div>
          </article>
          <article class="admin-order-quote-card">
            ...
            <div class="admin-form-grid admin-form-grid-two admin-order-quote-edit-fields">
              <label class="admin-label">
                Спальни
                <input class="admin-input" type="text" name="quoteRooms" value="${escapeHtmlAttribute(normalizeString(quoteData.rooms, 32))}" placeholder="3">
              </label>
              <label class="admin-label">
                Санузлы
                <input class="admin-input" type="text" name="quoteBathrooms" value="${escapeHtmlAttribute(normalizeString(quoteData.bathrooms, 32))}" placeholder="2">
              </label>
              <label class="admin-label">
                Размер дома
                <input class="admin-input" type="text" name="quoteSquareMeters" value="${escapeHtmlAttribute(normalizeString(quoteData.squareMeters, 32))}" placeholder="1600">
              </label>
              <label class="admin-label">
                Питомцы
                <select class="admin-input" name="quoteHasPets">
                  ${renderAdminSelectOptions(ORDER_QUOTE_PET_OPTIONS, quotePetsInputValue, "Не указано")}
                </select>
              </label>
              <label class="admin-label">
                Basement cleaning
                <select class="admin-input" name="quoteBasementCleaning">
                  ${renderAdminSelectOptions(ORDER_QUOTE_BOOLEAN_OPTIONS, quoteBasementInputValue, "Не указано")}
                </select>
              </label>
            </div>
          </article>
          <article class="admin-order-quote-card">
            ...
            <div class="admin-form-grid admin-order-quote-edit-fields">
              <div class="admin-label">
                <span>Выбранные add-ons</span>
                <div class="admin-checkbox-grid admin-order-quote-service-checkboxes">
                  ${quoteServiceCheckboxes || `<p class="admin-order-multiselect-empty">Add-ons не настроены.</p>`}
                </div>
              </div>
              <div class="admin-form-grid admin-form-grid-three">
                <label class="admin-label">
                  ${escapeHtml(ORDER_QUOTE_QUANTITY_LABELS.interiorWindowsCleaning)}
                  <input class="admin-input" type="number" min="0" step="1" name="quoteInteriorWindowsCleaning" value="${escapeHtmlAttribute(getQuoteQuantityInputValue(quoteData, "interiorWindowsCleaning"))}" inputmode="numeric">
                </label>
                <label class="admin-label">
                  ${escapeHtml(ORDER_QUOTE_QUANTITY_LABELS.blindsCleaning)}
                  <input class="admin-input" type="number" min="0" step="1" name="quoteBlindsCleaning" value="${escapeHtmlAttribute(getQuoteQuantityInputValue(quoteData, "blindsCleaning"))}" inputmode="numeric">
                </label>
                <label class="admin-label">
                  ${escapeHtml(ORDER_QUOTE_QUANTITY_LABELS.bedLinenChange)}
                  <input class="admin-input" type="number" min="0" step="1" name="quoteBedLinenChange" value="${escapeHtmlAttribute(getQuoteQuantityInputValue(quoteData, "bedLinenChange"))}" inputmode="numeric">
                </label>
              </div>
            </div>
          </article>
          <article class="admin-order-quote-card">
            ...
            <div class="admin-form-grid admin-form-grid-two admin-order-quote-edit-fields">
              <label class="admin-label admin-order-quote-edit-wide">
                Полный адрес
                <input class="admin-input" type="text" name="quoteFullAddress" value="${escapeHtmlAttribute(normalizeString(quoteData.fullAddress || order.fullAddress, 500))}" placeholder="215 North Elm Street, Naperville, IL 60563">
              </label>
              <label class="admin-label admin-order-quote-edit-wide">
                Короткий адрес
                <input class="admin-input" type="text" name="quoteAddress" value="${escapeHtmlAttribute(normalizeString(quoteData.address, 500))}" placeholder="215 North Elm Street">
              </label>
              <label class="admin-label">
                Apt / suite
                <input class="admin-input" type="text" name="quoteAddressLine2" value="${escapeHtmlAttribute(normalizeString(quoteData.addressLine2, 200))}" placeholder="Apt 4B">
              </label>
              <label class="admin-label">
                Город
                <input class="admin-input" type="text" name="quoteCity" value="${escapeHtmlAttribute(normalizeString(quoteData.city, 120))}" placeholder="Naperville">
              </label>
              <label class="admin-label">
                Штат
                <input class="admin-input" type="text" name="quoteState" value="${escapeHtmlAttribute(normalizeString(quoteData.state, 32))}" placeholder="IL">
              </label>
              <label class="admin-label">
                ZIP
                <input class="admin-input" type="text" name="quoteZipCode" value="${escapeHtmlAttribute(normalizeString(quoteData.zipCode, 32))}" placeholder="60563">
              </label>
            </div>
          </article>
          <article class="admin-order-quote-card admin-order-quote-card-wide">
            ...
            <label class="admin-label admin-order-quote-edit-wide">
              Комментарий клиента
              <textarea class="admin-input" name="quoteAdditionalDetails" rows="5" placeholder="Комментарий клиента, пожелания или важные детали">${escapeHtml(normalizeString(quoteData.additionalDetails, 4000))}</textarea>
            </label>
          </article>
        </div>
        <div class="admin-inline-actions admin-order-quote-edit-actions">
          <button class="admin-button" type="submit">Сохранить поля формы</button>
          <button
            class="admin-button admin-button-secondary"
            type="button"
            data-admin-toggle-target="${escapeHtmlAttribute(quoteEditPanelId)}"
            data-admin-toggle-label-open="Отмена"
            data-admin-toggle-label-closed="Отмена"
            aria-controls="${escapeHtmlAttribute(quoteEditPanelId)}"
            aria-expanded="true"
          >Отмена</button>
        </div>
      </form>
    ...
    const amountHighlightCardMarkup = renderOrderSummaryCard(
      ...
      formatCurrencyAmount(order.totalPrice),
      {
        ...
        editor: `<form
          ...
          action="${ADMIN_ORDERS_PATH}"
          data-admin-save-confirm="true"
          ${isRecurringOrder ? `data-admin-recurring-edit-confirm="true"` : ""}
        ...
        >
          ...
          <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(detailReturnTo)}">
          ${isRecurringOrder
            ? `<input type="hidden" name="recurringEditScope" value="current">`
            : ""}
    ...
    const teamHighlightCardMarkup = renderOrderSummaryCard("Команда", assignedSummaryLabel, {
      ...
      >
        ...
        ${assignedStaffRecords.length > 0
          ? `<div class="admin-form-grid">
              ${assignedStaffRecords.map((record) => `<label class="admin-label">
                Фиксированная зарплата: ${escapeHtml(record.name)}
                <input type="hidden" name="payrollOverrideStaffId" value="${escapeHtmlAttribute(record.id)}">
                <input
                  class="admin-input"
                  type="text"
                  name="payrollOverrideAmount"
                  inputmode="decimal"
                  placeholder="Обычная ставка сотрудника"
                  value="${escapeHtmlAttribute(payrollOverrides[record.id] || "")}"
                >
              </label>`).join("")}
            </div>
            <span class="admin-field-help">Фикс применяется только к указанному сотруднику и только к этой уборке. Пустое поле возвращает обычную ставку.</span>`
          : ""}
      </form>`,
    });
    ...
    const scheduleHighlightCardMarkup = renderOrderSummaryCard("Дата и время", scheduleLabel, {
      className: "admin-order-summary-card-schedule",
      ...
      actions: `<div class="admin-order-summary-card-actions">
        <button
          class="admin-icon-button admin-edit-button"
          type="button"
          aria-label="Редактировать дату и время визита"
          title="Редактировать дату и время визита"
          data-admin-order-schedule-open="${escapeHtmlAttribute(scheduleEditPanelId)}"
          data-admin-order-schedule-edit-trigger="${escapeHtmlAttribute(scheduleEditPanelId)}"
        >
          ${renderAdminEditIcon("Редактировать дату и время визита")}
        </button>
        <button
          class="admin-icon-button admin-confirm-button"
          type="submit"
          form="${escapeHtmlAttribute(scheduleFormId)}"
          aria-label="Сохранить дату и время визита"
          title="Сохранить дату и время"
          data-admin-order-schedule-action-for="${escapeHtmlAttribute(scheduleEditPanelId)}"
          hidden
        >
          <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
            <path d="M9.55 17.2 4.8 12.45a.75.75 0 1 1 1.06-1.06l3.69 3.69 8.59-8.58a.75.75 0 1 1 1.06 1.06l-9.12 9.11a.75.75 0 0 1-1.06 0Z" fill="currentColor"/>
          </svg>
          <span class="admin-sr-only">Сохранить дату и время визита</span>
        </button>
        <button
          class="admin-icon-button admin-close-button"
          type="button"
          aria-label="Отменить редактирование даты и времени визита"
          title="Отменить"
          data-admin-order-schedule-cancel="${escapeHtmlAttribute(scheduleEditPanelId)}"
          data-admin-order-schedule-action-for="${escapeHtmlAttribute(scheduleEditPanelId)}"
          hidden
        >
          <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          ...
          <span class="admin-sr-only">Отменить редактирование даты и времени визита</span>
        </button>
      </div>`,
      editor: `<form
        class="admin-order-highlight-editor"
        id="${escapeHtmlAttribute(scheduleFormId)}"
        method="post"
        action="${ADMIN_ORDERS_PATH}"
        data-admin-save-confirm="true"
        ${reminderConfirmAttributes}
        data-admin-reminder-fields="selectedDate,selectedTime"
        data-admin-order-schedule-editor="${escapeHtmlAttribute(scheduleEditPanelId)}"
        hidden
      >
        <input type="hidden" name="entryId" value="${escapeHtmlAttribute(order.id)}">
        <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(detailReturnTo)}">
        <div class="admin-form-grid admin-form-grid-two admin-order-schedule-editor-fields">
          ${renderAdminPickerField({
            pickerType: "date",
            fieldId: `${scheduleFormId}-selected-date`,
            label: "Дата визита",
            className: "admin-order-highlight-editor-field",
            name: "selectedDate",
            displayValue: formatAdminOrderDateInputValue(order.selectedDate),
            nativeValue: order.selectedDate,
            placeholder: "04/15/2026",
            pickerLabel: "Выбрать дату визита",
          })}
          ${renderAdminPickerField({
            pickerType: "time",
            fieldId: `${scheduleFormId}-selected-time`,
            label: "Время визита",
            className: "admin-order-highlight-editor-field",
            name: "selectedTime",
            ...
            placeholder: "1:30 PM",
            pickerLabel: "Выбрать время визита",
          })}
        </div>
      </form>`,
    });

    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}"${autoOpenAttr}${closeHref ? ` data-admin-dialog-return-url="${escapeHtmlAttribute(closeHref)}"` : ""} data-admin-dialog-dirty-confirm="true" data-admin-dialog-dirty-message="В заказе есть несохранённые изменения. Закрыть окно и потерять их?" aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel admin-order-dialog-panel">
        <div class="admin-dialog-head admin-dialog-hero">
          ...
          <div class="admin-inline-actions admin-dialog-head-actions admin-dialog-hero-actions">
            ${clientProfileHref ? `<a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(clientProfileHref)}">Открыть клиента</a>` : ""}
        ...
        <div class="admin-order-dialog-layout">
          <div class="admin-order-dialog-main">
            ...
            ${renderAdminGhlSmsComposer({
              ...
              historyEntries: options.deferSmsHistory
                ? []
                : getEntryCustomerSmsHistoryEntries(order && order.entry ? order.entry : {}),
            ...
            <section class="admin-order-section-card admin-order-quote-section">
              <div class="admin-subsection-head admin-order-quote-section-head">
                <h3 class="admin-subsection-title">Поля из формы клиента</h3>
                <div class="admin-order-summary-card-actions">
                  ${renderAdminToggleIconButton("Редактировать поля формы клиента", quoteEditPanelId, {
                    openLabel: "Скрыть редактирование полей формы",
                    closedLabel: "Редактировать поля формы клиента",
                  })}
                </div>
              </div>
              ${quoteEditorMarkup}
          ...
          <aside class="admin-order-dialog-side">
            <section class="admin-order-section-card admin-order-side-card">
              ...
              <form class="admin-form-grid admin-order-control-form" id="${escapeHtmlAttribute(editFormId)}" method="post" action="${ADMIN_ORDERS_PATH}" data-admin-save-confirm="true" ${reminderConfirmAttributes} data-admin-reminder-fields="selectedDate,selectedTime">
                <input type="hidden" name="entryId" value="${escapeHtmlAttribute(order.id)}">
                <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(dialogReturnTo)}">
                <div class="admin-order-control-fields">
                  <label class="admin-label admin-order-control-field">
                    Статус заказа
                    <select class="admin-input" name="orderStatus" data-admin-order-status-select>
                  ...
                  ${isRecurringOrder
                    ? `<label
                        class="admin-label admin-order-control-field"
                        data-admin-order-cancel-scope
                        ${order.orderStatus === "canceled" ? "" : "hidden"}
                      >
                        Какие визиты отменить
                        <select class="admin-input" name="cancelScope">
                          <option value="current">Только этот визит</option>
                          <option value="future">Этот и все последующие</option>
                        </select>
                      </label>`
                    : ""}
                  ${isRecurringOrder
                    ? `<label class="admin-label admin-order-control-field">
                        Применить изменения
                        <select class="admin-input" name="recurringEditScope" data-admin-recurring-edit-scope>
                          <option value="current">Только к этому визиту</option>
                          <option value="future">К этому и всем следующим</option>
                        </select>
                      </label>`
                    : ""}
                  ${renderAdminPickerField({
                  ...
                  <label class="admin-label admin-order-control-field">
                    ...
                    <select class="admin-input" name="frequency">
                      ...
                      <option value="every3weeks"${order.frequency === "every3weeks" ? " selected" : ""}>Every 3 weeks</option>
              ...
              <div class="admin-order-action-row">
                <button
                  class="admin-button admin-order-primary-action"
                  type="submit"
                  form="${escapeHtmlAttribute(editFormId)}"
                  data-admin-order-primary-save
                  data-admin-order-quote-form="${escapeHtmlAttribute(quoteEditFormId)}"
                  data-admin-order-detail-return-to="${escapeHtmlAttribute(detailReturnTo)}"
                  data-admin-order-list-return-to="${escapeHtmlAttribute(dialogReturnTo)}"
                ...
                ${canSendReviewRequest
                  ? `<form class="admin-inline-actions admin-order-review-request-form" method="post" action="${ADMIN_ORDERS_PATH}">
                      <input type="hidden" name="action" value="send-review-request">
                      <input type="hidden" name="entryId" value="${escapeHtmlAttribute(order.id)}">
                      <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(dialogReturnTo)}">
                      <button class="admin-button admin-button-secondary" type="submit">Попросить отзыв</button>
                    </form>`
                  : ""}
                ${canRestoreRecurringSeries
                  ? `<form class="admin-inline-actions" method="post" action="${ADMIN_ORDERS_PATH}">
                      <input type="hidden" name="action" value="restore-recurring-series">
                      <input type="hidden" name="entryId" value="${escapeHtmlAttribute(order.id)}">
                      <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(detailReturnTo)}">
                      <button class="admin-button admin-button-secondary" type="submit">Восстановить эту и последующие уборки</button>
                    </form>`
                  : ""}
                ${renderAdminDialogCloseButton(dialogId)}
                ${canDelete
