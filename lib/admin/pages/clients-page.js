...
function createClientsPageRenderer(deps = {}) {
  ...
  async function renderClientsPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    function normalizeClientTableValue(value, maxLength = 0) {
                      ...
                      ${filteredClients
                        .map((client) => {
                          ...
                          const rowClassName = [
                            ...
                            .filter(Boolean)
                            .join(" ");
                          const phoneLabels = [client.phone, client.secondaryPhone]
                            .map((value) => normalizeClientTableValue(value, 80))
                            .filter(Boolean)
                            .filter((value, index, values) => {
                              const phoneKey = value.replace(/\D+/g, "");
                              return (
                                phoneKey &&
                                values.findIndex((candidate) => candidate.replace(/\D+/g, "") === phoneKey) === index
                              );
                            })
                          ...
                          >
                            ...
                            <td>
                              ${phoneLabels.length > 0
                                ? `<div class="admin-table-cell-stack">
                                    <span class="admin-table-strong">${escapeHtml(phoneLabels[0])}</span>
                                    ${phoneLabels[1] ? `<span class="admin-table-muted">2-й: ${escapeHtml(phoneLabels[1])}</span>` : ""}
                                  </div>`
                            ...
                            <td class="admin-table-number">
                              <div class="admin-table-cell-stack">
                                <span class="admin-table-strong">${escapeHtml(formatCurrencyAmount(client.pastVisitRevenue))}</span>
                              </div>
                            </td>
                          </tr>`;
                        })
