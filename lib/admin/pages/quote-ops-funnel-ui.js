...
function renderQuoteOpsFunnelCard(entry, returnTo) {
  ...
  const isLocked = leadStatus === "confirmed" || leadStatus === "completed";
  const hideDeadlineRow = leadStatus === "confirmed" || leadStatus === "completed" || leadStatus === "declined";
  const dueAtMs = Date.parse((openTask || {}).dueAt || "");
  const isOverdue = Boolean(openTask && Number.isFinite(dueAtMs) && dueAtMs < Date.now());
  const taskLabel = openTask
    ? openTask.title
...
function renderQuoteOpsFunnelLane(status, entries, returnTo) {
  const titles = {
    ...
    completed: {
      title: "Выполнено",
      copy: "Закрытые заявки, по которым работа успешно завершена.",
    },
    declined: {
      title: "Отказ",
