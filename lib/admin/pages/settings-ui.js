...
function createSettingsUiHelpers(deps = {}) {
  ...
  function renderSettingsNotice(req) {
    ...
    if (notice === "mail-test-sent") {
      return `<div class="admin-alert admin-alert-info">Тестовое письмо отправлено. Проверьте входящие и spam/promotions.</div>`;
    }
    if (notice === "mail-test-error") {
      return `<div class="admin-alert admin-alert-error">Не удалось отправить тестовое письмо. Проверьте подключение Gmail и адрес получателя.</div>`;
    }
    return "";
  }
