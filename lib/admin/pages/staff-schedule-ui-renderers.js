...
function renderStaffTeamCalendarCell(staffSummary, day, staffIndex = 0, colorMap = null, options = {}) {
  ...
  const timelineSlots = Array.isArray(options.timelineSlots) && options.timelineSlots.length
    ? options.timelineSlots
    : buildStaffTeamCalendarTimelineSlots([staffSummary], day);
  const unavailableByMinute = new Map();
  const assignmentsByMinute = new Map();
  for (const block of unavailableBlocks) {
    const minutes = getStaffTeamCalendarAvailabilitySlotMinutes(block);
    unavailableByMinute.set(minutes, [...(unavailableByMinute.get(minutes) || []), block]);
  }
  for (const item of assignments) {
    const minutes = getStaffTeamCalendarAssignmentSlotMinutes(item);
    assignmentsByMinute.set(minutes, [...(assignmentsByMinute.get(minutes) || []), item]);
  }
  ...
  return `<div class="admin-team-calendar-cell admin-team-calendar-cell-timeline" style="${escapeHtmlAttribute(renderStaffTeamCalendarColorVars(staffSummary, staffIndex, colorMap))}" data-admin-team-calendar-timeline="true">
    ${timelineSlots
      .map((slot) => {
        const slotUnavailableBlocks = unavailableByMinute.get(slot.minutes) || [];
        const slotAssignments = assignmentsByMinute.get(slot.minutes) || [];
        const isEmpty = slotUnavailableBlocks.length === 0 && slotAssignments.length === 0;
        return `<div
          class="admin-team-calendar-time-slot${isEmpty ? " admin-team-calendar-time-slot-empty" : ""}"
          data-admin-team-calendar-time-slot="${escapeHtmlAttribute(slot.key)}"
          style="--admin-team-calendar-slot-height:${escapeHtmlAttribute(String(slot.heightPx))}px"
          ${isEmpty ? 'aria-hidden="true"' : ""}
        >
          ${slotUnavailableBlocks.map((block) => renderStaffTeamCalendarAvailabilityEntry(block, options)).join("")}
          ${slotAssignments.map((item) => renderStaffTeamCalendarAssignmentEntry(item)).join("")}
        </div>`;
      })
      .join("")}
  </div>`;
    ...
    ${view === "month"
      ...
      <table class="admin-table admin-team-calendar-table" data-admin-team-calendar="true" style="--admin-team-calendar-staff:${escapeHtmlAttribute(String(staffSummaries.length))};">
        ...
        <tbody>
          ${days
            .map((day) => {
              const timelineSlots = buildStaffTeamCalendarTimelineSlots(staffSummaries, day);
