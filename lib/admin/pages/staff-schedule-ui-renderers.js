...
function renderStaffAssignmentDialog(orderItem, staffRecords) {
  ...
  const calculatorData =
    typeof getEntryCalculatorData === "function" ? getEntryCalculatorData(orderItem.entry || {}) : {};
  const fallbackScheduleDate = normalizeString(
    (orderItem.entry && orderItem.entry.selectedDate) || calculatorData.selectedDate,
    32
  );
  const fallbackScheduleTime = normalizeString(
    (orderItem.entry && orderItem.entry.selectedTime) || calculatorData.selectedTime,
    32
  );
  const scheduleDateValue =
    typeof normalizeAdminOrderDateInput === "function"
      ? normalizeAdminOrderDateInput(
          (orderItem.assignment && orderItem.assignment.scheduleDate) || fallbackScheduleDate
        )
      : normalizeString((orderItem.assignment && orderItem.assignment.scheduleDate) || fallbackScheduleDate, 32);
  const scheduleTimeValue =
    typeof normalizeAdminOrderTimeInput === "function"
      ? normalizeAdminOrderTimeInput(
          (orderItem.assignment && orderItem.assignment.scheduleTime) || fallbackScheduleTime
        )
      : normalizeString((orderItem.assignment && orderItem.assignment.scheduleTime) || fallbackScheduleTime, 32);
