...
function renderStaffTeamCalendarCell(staffSummary, day, staffIndex = 0, colorMap = null) {
...
function renderStaffTeamCalendarMonthGrid(staffSummaries, days, normalizedStartDate, todayDateValue, colorMap = null, lowWorkloadCount = 0) {
  const anchorDate = parseStaffCalendarDate(normalizedStartDate);
  ...
  return `<div class="admin-table-wrap admin-team-calendar-wrap admin-team-calendar-month-wrap" data-admin-team-calendar-scroll="true">
    <div class="admin-team-calendar-month-grid" data-admin-team-calendar-month="true">
...
function renderStaffTeamCalendarTable(staffSummaries, startDateValue, options = {}) {
  ...
  const metaLabel =
    view === "month"
      ? "Месяц показан сеткой: дни недели идут по колонкам, заказы лежат внутри дня."
  ...
  return `<div class="admin-team-calendar-shell" data-admin-team-calendar-shell="true" aria-live="polite">
    ...
    ${view === "month"
      ? renderStaffTeamCalendarMonthGrid(staffSummaries, days, normalizedStartDate, todayDateValue, colorMap, lowWorkloadCount)
              ...
              (day) => `<tr${day.dateValue === todayDateValue ? ' class="admin-team-calendar-today-row"' : ""}>
                ...
                ${staffSummaries.map((staffSummary, staffIndex) => {
                  ...
                  return `<td
                    ...
                    data-admin-team-calendar-cleaner-id="${escapeHtmlAttribute(staffSummary.id || "")}"
                    ${isLessLoaded ? 'data-admin-team-calendar-low-load="true"' : ""}
                  >${renderStaffTeamCalendarCell(staffSummary, day, staffIndex, colorMap)}</td>`;
                }).join("")}
              </tr>`
            )
            .join("")}
