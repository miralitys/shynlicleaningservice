...
function isStaffTeamCalendarAssignmentVisible(item = {}) {
  ...
  const payload = item && item.entry && item.entry.payloadForRetry;
  const orderState = payload && typeof payload === "object"
    ? payload.adminOrder && typeof payload.adminOrder === "object"
      ...
      : payload.orderState && typeof payload.orderState === "object"
        ...
        : null
    : null;
  const orderStatus = normalizeString(orderState && orderState.status, 32)
    .toLowerCase()
    .replace(/[\s_]+/g, "-");
  return orderStatus !== "canceled" && orderStatus !== "cancelled" && orderStatus !== "rescheduled";
}
function getStaffTeamCalendarWorkloadCount(staffSummary = {}, days = []) {
  ...
  return staffSummary.assignedOrders.filter(
    (item) => isStaffTeamCalendarAssignmentVisible(item) && dateValues.has(item && item.scheduleDate)
  ).length;
}
...
function getStaffTeamCalendarDayAssignments(staffSummary = {}, day = {}) {
  return Array.isArray(staffSummary.assignedOrders)
    ? staffSummary.assignedOrders
        .filter(
          (item) =>
            isStaffTeamCalendarAssignmentVisible(item) && item.scheduleDate === day.dateValue
        )
        .sort((left, right) => {
...
function getStaffTeamCalendarTimeSlotMinutes(value) {
  const normalized = normalizeStaffTeamCalendarTimeInput(value);
  if (!normalized) return null;
  const [hours, minutes] = normalized.split(":").map((segment) => Number(segment));
  if (!Number.isInteger(hours) || !Number.isInteger(minutes)) return null;
  return hours * 60 + minutes;
}
function getStaffTeamCalendarTimestampMinutes(value) {
...
function getStaffTeamCalendarAssignmentSlotMinutes(orderItem = {}) {
  return (
    getStaffTeamCalendarTimeSlotMinutes(orderItem.scheduleTime) ??
    getStaffTeamCalendarTimestampMinutes(orderItem.scheduleTimestamp) ??
    24 * 60
  );
}
function getStaffTeamCalendarAvailabilitySlotMinutes(block = {}) {
  if (block.allDay) return -1;
  return (
    getStaffTeamCalendarTimeSlotMinutes(block.startTime) ??
    getStaffTeamCalendarTimestampMinutes(block.startMs) ??
    24 * 60
  );
}
function getStaffTeamCalendarAvailabilityEndMinutes(block = {}) {
  if (block.allDay) return 24 * 60;
  return (
    getStaffTeamCalendarTimeSlotMinutes(block.endTime) ??
    getStaffTeamCalendarTimestampMinutes(block.endMs) ??
    getStaffTeamCalendarAvailabilitySlotMinutes(block) + 60
  );
}
function buildStaffTeamCalendarTimelineRange(staffSummaries = [], day = {}) {
  const starts = [8 * 60];
  const ends = [20 * 60];

  for (const staffSummary of Array.isArray(staffSummaries) ? staffSummaries : []) {
    getStaffTeamCalendarUnavailableBlocks(staffSummary, day).forEach((block) => {
      if (block.allDay) return;
      starts.push(getStaffTeamCalendarAvailabilitySlotMinutes(block));
      ends.push(getStaffTeamCalendarAvailabilityEndMinutes(block));
    });
    getStaffTeamCalendarDayAssignments(staffSummary, day).forEach((item) => {
      const startMinutes = getStaffTeamCalendarAssignmentSlotMinutes(item);
      const durationMinutes = Math.max(30, Number(item.serviceDurationMinutes) || 60);
      starts.push(startMinutes);
      ends.push(startMinutes + durationMinutes);
    });
  }

  const startMinutes = Math.max(0, Math.min(...starts.filter(Number.isFinite)));
  const endMinutes = Math.min(24 * 60, Math.max(...ends.filter(Number.isFinite)));
  return {
    startMinutes,
    endMinutes: Math.max(startMinutes + 60, endMinutes),
  };
}
function renderStaffTeamCalendarTimelinePosition(startMinutes, endMinutes, timelineRange = {}) {
  const timelineStart = Number(timelineRange.startMinutes) || 0;
  const timelineEnd = Math.max(timelineStart + 60, Number(timelineRange.endMinutes) || 24 * 60);
  const span = timelineEnd - timelineStart;
  const normalizedStart = Math.max(timelineStart, Math.min(timelineEnd, Number(startMinutes) || timelineStart));
  const normalizedEnd = Math.max(normalizedStart, Math.min(timelineEnd, Number(endMinutes) || normalizedStart + 60));
  const top = ((normalizedStart - timelineStart) / span) * 100;
  const height = Math.max(0, ((normalizedEnd - normalizedStart) / span) * 100);
  return `--admin-team-calendar-entry-top:${top.toFixed(3)}%;--admin-team-calendar-entry-height:${height.toFixed(3)}%`;
}
const STAFF_TEAM_CALENDAR_TIMELINE_MIN_HEIGHT_PX = 190;
const STAFF_TEAM_CALENDAR_ENTRY_GAP_PX = 6;
const STAFF_TEAM_CALENDAR_ASSIGNMENT_MIN_HEIGHT_PX = 54;
const STAFF_TEAM_CALENDAR_AVAILABILITY_MIN_HEIGHT_PX = 34;

function buildStaffTeamCalendarCellTimelineLayout(staffSummary = {}, day = {}, timelineRange = {}) {
  const timelineStart = Number(timelineRange.startMinutes) || 0;
  const timelineEnd = Math.max(timelineStart + 60, Number(timelineRange.endMinutes) || 24 * 60);
  const pixelsPerMinute = STAFF_TEAM_CALENDAR_TIMELINE_MIN_HEIGHT_PX / (timelineEnd - timelineStart);
  const items = [];

  getStaffTeamCalendarUnavailableBlocks(staffSummary, day).forEach((block) => {
    if (block.allDay) return;
    const startMinutes = getStaffTeamCalendarAvailabilitySlotMinutes(block);
    const endMinutes = Math.max(startMinutes + 1, getStaffTeamCalendarAvailabilityEndMinutes(block));
    items.push({
      item: block,
      startMinutes,
      endMinutes,
      minHeightPx: STAFF_TEAM_CALENDAR_AVAILABILITY_MIN_HEIGHT_PX,
      sortOrder: 0,
    });
  });
  getStaffTeamCalendarDayAssignments(staffSummary, day).forEach((assignment) => {
    const startMinutes = getStaffTeamCalendarAssignmentSlotMinutes(assignment);
    const durationMinutes = Math.max(30, Number(assignment.serviceDurationMinutes) || 60);
    items.push({
      item: assignment,
      startMinutes,
      endMinutes: startMinutes + durationMinutes,
      minHeightPx: STAFF_TEAM_CALENDAR_ASSIGNMENT_MIN_HEIGHT_PX,
      sortOrder: 1,
    });
  });
  ...
  items.forEach((item, index) => {
    const desiredTopPx = Math.max(0, (item.startMinutes - timelineStart) * pixelsPerMinute);
    const topPx = index === 0
      ? desiredTopPx
      : Math.max(desiredTopPx, previousBottomPx + STAFF_TEAM_CALENDAR_ENTRY_GAP_PX);
    const proportionalHeightPx = Math.max(0, (item.endMinutes - item.startMinutes) * pixelsPerMinute);
    const heightPx = Math.max(item.minHeightPx, proportionalHeightPx);
    positions.set(item.item, { topPx, heightPx });
    previousBottomPx = topPx + heightPx;
  });

  return {
    heightPx: Math.max(STAFF_TEAM_CALENDAR_TIMELINE_MIN_HEIGHT_PX, previousBottomPx),
    positions,
  };
}

function renderStaffTeamCalendarPixelPosition(position = {}) {
  const topPx = Math.max(0, Number(position.topPx) || 0);
  const heightPx = Math.max(0, Number(position.heightPx) || 0);
  return `--admin-team-calendar-entry-top:${topPx.toFixed(3)}px;--admin-team-calendar-entry-height:${heightPx.toFixed(3)}px`;
}
function buildStaffTeamCalendarTimelineSlots(staffSummaries = [], day = {}) {
  const maxItemsByMinute = new Map();

  for (const staffSummary of Array.isArray(staffSummaries) ? staffSummaries : []) {
    const itemCounts = new Map();
    const addItem = (minutes) => itemCounts.set(minutes, (itemCounts.get(minutes) || 0) + 1);
    getStaffTeamCalendarUnavailableBlocks(staffSummary, day).forEach((block) => {
      addItem(getStaffTeamCalendarAvailabilitySlotMinutes(block));
    });
    getStaffTeamCalendarDayAssignments(staffSummary, day).forEach((item) => {
      addItem(getStaffTeamCalendarAssignmentSlotMinutes(item));
    });
    for (const [minutes, itemCount] of itemCounts) {
      maxItemsByMinute.set(minutes, Math.max(maxItemsByMinute.get(minutes) || 0, itemCount));
    }
  }

  return Array.from(maxItemsByMinute.entries())
    .sort(([leftMinutes], [rightMinutes]) => leftMinutes - rightMinutes)
    .map(([minutes, itemCount]) => ({
      key: String(minutes),
      minutes,
      itemCount,
      heightPx: itemCount * 72 + Math.max(0, itemCount - 1) * 6 + 6,
    }));
}
...
function renderStaffTeamCalendarAssignmentEntry(orderItem, options = {}) {
  const customerName =
  ...
  const startMinutes = getStaffTeamCalendarAssignmentSlotMinutes(orderItem);
  const durationMinutes = Math.max(30, Number(orderItem.serviceDurationMinutes) || 60);
  const positionStyle = options.timelinePosition
    ? renderStaffTeamCalendarPixelPosition(options.timelinePosition)
    : options.timelineRange
      ? renderStaffTeamCalendarTimelinePosition(startMinutes, startMinutes + durationMinutes, options.timelineRange)
      : "";

  return `<button
    class="admin-team-calendar-entry admin-team-calendar-entry-positioned admin-team-calendar-entry-button admin-team-calendar-entry-order admin-team-calendar-entry-order-${escapeHtmlAttribute(statusTone)}"
    type="button"
    ${positionStyle ? `style="${escapeHtmlAttribute(positionStyle)}"` : ""}
...
function normalizeStaffTeamCalendarTimeInput(value) {
  if (typeof normalizeAdminOrderTimeInput === "function") {
    return normalizeAdminOrderTimeInput(value);
  }
  const normalized = normalizeString(value, 16);
  return /^([01]\d|2[0-3]):[0-5]\d$/.test(normalized) ? normalized : "";
}
function formatStaffTeamCalendarAvailabilityTimeLabel(block = {}) {
  if (block.allDay) return "All day";
  const startTime = normalizeStaffTeamCalendarTimeInput(block.startTime);
  const endTime = normalizeStaffTeamCalendarTimeInput(block.endTime);
  if (startTime && endTime) {
    return `${formatAdminClockTime(startTime)} – ${formatAdminClockTime(endTime)}`;
  }
  const startMs = Number(block.startMs);
  const endMs = Number(block.endMs);
...
function renderStaffTeamCalendarAvailabilityEntry(block, options = {}) {
  const timeLabel = formatStaffTeamCalendarAvailabilityTimeLabel(block);
  const startMinutes = block.allDay ? 0 : getStaffTeamCalendarAvailabilitySlotMinutes(block);
  const endMinutes = block.allDay ? 24 * 60 : getStaffTeamCalendarAvailabilityEndMinutes(block);
  const positionStyle = options.timelinePosition
    ? renderStaffTeamCalendarPixelPosition(options.timelinePosition)
    : options.timelineRange
      ? renderStaffTeamCalendarTimelinePosition(startMinutes, endMinutes, options.timelineRange)
      : "";

  return `<article class="admin-team-calendar-entry admin-team-calendar-entry-positioned admin-team-calendar-entry-unavailable${block.allDay ? " admin-team-calendar-entry-unavailable-all-day" : ""}"${positionStyle ? ` style="${escapeHtmlAttribute(positionStyle)}"` : ""}>
    <div class="admin-team-calendar-entry-head">
      <span class="admin-team-calendar-entry-time">${escapeHtml(timeLabel)}</span>
...
function renderStaffTeamCalendarCell(staffSummary, day, staffIndex = 0, colorMap = null, options = {}) {
  ...
  const timelineRange = options.timelineRange || buildStaffTeamCalendarTimelineRange([staffSummary], day);
  const timelineLayout = buildStaffTeamCalendarCellTimelineLayout(staffSummary, day, timelineRange);
  const timelineStyle = [
    renderStaffTeamCalendarColorVars(staffSummary, staffIndex, colorMap),
    `--admin-team-calendar-timeline-height:${timelineLayout.heightPx.toFixed(3)}px`,
  ].filter(Boolean).join(";");

  return `<div class="admin-team-calendar-cell admin-team-calendar-cell-timeline" style="${escapeHtmlAttribute(timelineStyle)}" data-admin-team-calendar-timeline="true">
    ${unavailableBlocks.map((block) => renderStaffTeamCalendarAvailabilityEntry(block, {
      ...options,
      timelineRange,
      timelinePosition: timelineLayout.positions.get(block),
    })).join("")}
    ${assignments.map((item) => renderStaffTeamCalendarAssignmentEntry(item, {
      ...options,
      timelineRange,
      timelinePosition: timelineLayout.positions.get(item),
    })).join("")}
  </div>`;
}
...
function renderStaffTeamCalendarCellMenu(staffSummary, day, options = {}) {
  ...
  const availabilityMode = manualUnavailableBlock && !manualUnavailableBlock.allDay
    ? "time-range"
    : "all-day";
  const availabilityStartTime =
    normalizeStaffTeamCalendarTimeInput(manualUnavailableBlock && manualUnavailableBlock.startTime) || "09:00";
  const availabilityEndTime =
    normalizeStaffTeamCalendarTimeInput(manualUnavailableBlock && manualUnavailableBlock.endTime) || "13:00";
  ...
  return `<details class="admin-team-calendar-cell-menu" data-admin-team-calendar-menu="true">
    ...
    <div class="admin-team-calendar-cell-menu-popover">
      ${canToggle
        ? `<form class="admin-team-calendar-busy-form" method="post" action="${escapeHtmlAttribute(ADMIN_STAFF_PATH)}" data-admin-team-calendar-busy-form="true">
          <input type="hidden" name="action" value="save-staff-unavailable-day" data-admin-team-calendar-busy-action="true">
          ...
          <div
            class="admin-team-calendar-busy-settings"
            data-admin-team-calendar-busy-settings="true"
            ${manualUnavailableBlock ? "" : "hidden"}
          >
            <label class="admin-team-calendar-busy-field">
              <span>Период</span>
              <select name="availabilityMode" data-admin-team-calendar-busy-mode="true">
            ...
            <div
              class="admin-team-calendar-busy-time-range"
              data-admin-team-calendar-busy-time-range="true"
              ${availabilityMode === "time-range" ? "" : "hidden"}
            >
              <label class="admin-team-calendar-busy-field">
                <span>С</span>
                <input
                  ...
                  value="${escapeHtmlAttribute(availabilityStartTime)}"
                  data-admin-team-calendar-busy-time="true"
                  ${availabilityMode === "time-range" ? "required" : "disabled"}
                >
              </label>
              <label class="admin-team-calendar-busy-field">
                <span>До</span>
                <input
                  ...
                  value="${escapeHtmlAttribute(availabilityEndTime)}"
                  data-admin-team-calendar-busy-time="true"
                  ${availabilityMode === "time-range" ? "required" : "disabled"}
                >
              </label>
            ...
            <button class="admin-team-calendar-busy-save" type="submit">Сохранить</button>
          </div>
        </form>`
    ...
    ${view === "month"
      ...
      <table class="admin-table admin-team-calendar-table" data-admin-team-calendar="true" style="--admin-team-calendar-staff:${escapeHtmlAttribute(String(staffSummaries.length))};">
        ...
        <tbody>
          ${days
            .map((day) => {
              const timelineRange = buildStaffTeamCalendarTimelineRange(staffSummaries, day);
              return `<tr${day.dateValue === todayDateValue ? ' class="admin-team-calendar-today-row"' : ""}>
                ...
                ${staffSummaries.map((staffSummary, staffIndex) => {
                  ...
                  return `<td
                    ...
                    ${isLessLoaded ? 'data-admin-team-calendar-low-load="true"' : ""}
                  >${renderStaffTeamCalendarCellMenu(staffSummary, day, {
...
function renderStaffTravelEstimateItem(leg, options = {}) {
  ...
  return {
    ...
    isStaffTeamCalendarAssignmentVisible,
    ...
    buildStaffTeamCalendarTimelineSlots,
