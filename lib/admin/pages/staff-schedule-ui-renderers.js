...
function renderStaffTeamCalendarAssignmentEntry(orderItem, options = {}) {
  ...
  const positionStyle = options.timelineRange
...
function renderStaffTeamCalendarCell(staffSummary, day, staffIndex = 0, colorMap = null, options = {}) {
  ...
  return `<div class="admin-team-calendar-cell admin-team-calendar-cell-timeline" style="${escapeHtmlAttribute(renderStaffTeamCalendarColorVars(staffSummary, staffIndex, colorMap))}" data-admin-team-calendar-timeline="true">
    ${unavailableBlocks.map((block) => renderStaffTeamCalendarAvailabilityEntry(block, { ...options, timelineRange })).join("")}
    ${assignments.map((item) => renderStaffTeamCalendarAssignmentEntry(item, { ...options, timelineRange })).join("")}
  </div>`;
}
