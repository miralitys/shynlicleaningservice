...
function renderStaffTeamCalendarAvailabilityEntry(block, options = {}) {
  ...
  const availabilityDate = normalizeStaffCalendarDateValue(block.date || block.startDate);
  const canClear = block.source === "manual" && options.staffId && availabilityDate;
  const clearForm = canClear
    ? `<form class="admin-team-calendar-unavailable-clear-form" method="post" action="${escapeHtmlAttribute(ADMIN_STAFF_PATH)}">
        <input type="hidden" name="action" value="clear-staff-unavailable-day">
        <input type="hidden" name="staffId" value="${escapeHtmlAttribute(options.staffId)}">
        ...
        <input type="hidden" name="calendarView" value="${escapeHtmlAttribute(options.calendarView || "week")}">
        <button class="admin-team-calendar-unavailable-clear" type="submit">Убрать</button>
      </form>`
    : "";
...
function renderStaffTeamCalendarDialogCloseButton(dialogId) {
  if (typeof renderAdminDialogCloseButton === "function") {
    return renderAdminDialogCloseButton(dialogId);
  }
  return `<button class="admin-icon-button admin-close-button" type="button" aria-label="Закрыть" data-admin-dialog-close="${escapeHtmlAttribute(dialogId)}">×</button>`;
}
function renderStaffTeamCalendarUnavailableDialog(normalizedStartDate, view) {
  const dialogId = "admin-team-calendar-unavailable-dialog";
  const normalizedView = normalizeStaffTeamCalendarView(view);
  return `<dialog
    class="admin-dialog admin-team-calendar-unavailable-dialog"
    id="${escapeHtmlAttribute(dialogId)}"
    data-admin-team-calendar-unavailable-dialog="true"
    aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}"
  >
    <div class="admin-dialog-panel admin-team-calendar-unavailable-panel">
      <div class="admin-dialog-head">
        <div class="admin-dialog-copy-block">
          <p class="admin-card-eyebrow">Календарь</p>
          <h2 class="admin-dialog-title" id="${escapeHtmlAttribute(`${dialogId}-title`)}">Клинер недоступен</h2>
          <p class="admin-dialog-copy" data-admin-team-calendar-unavailable-summary>Выберите пустую ячейку в календаре.</p>
        </div>
        ${renderStaffTeamCalendarDialogCloseButton(dialogId)}
      </div>
      <form class="admin-form-grid admin-team-calendar-unavailable-form" method="post" action="${escapeHtmlAttribute(ADMIN_STAFF_PATH)}">
        <input type="hidden" name="action" value="save-staff-unavailable-day">
        <input type="hidden" name="staffId" data-admin-team-calendar-unavailable-staff-id value="">
        <input type="hidden" name="availabilityDate" data-admin-team-calendar-unavailable-date value="">
        <input type="hidden" name="calendarStart" value="${escapeHtmlAttribute(normalizedStartDate)}">
        ...
        <label class="admin-label">
          ...
          <input
            ...
            value="Не может выйти на работу"
            maxlength="180"
          >
        </label>
        <label class="admin-label">
          Комментарий
          <textarea class="admin-textarea" name="availabilityNotes" rows="3" placeholder="Например: personal day, vacation, appointment"></textarea>
        </label>
...
function renderStaffTeamCalendarTable(staffSummaries, startDateValue, options = {}) {
  ...
  return `<div class="admin-team-calendar-shell" data-admin-team-calendar-shell="true" aria-live="polite">
              ...
              (day) => `<tr${day.dateValue === todayDateValue ? ' class="admin-team-calendar-today-row"' : ""}>
                ...
                ${staffSummaries.map((staffSummary, staffIndex) => {
                  ...
                  return `<td
                    ...
                    ${emptyCell ? 'data-admin-team-calendar-empty="true" tabindex="0" role="button"' : ""}
                    ${emptyCell ? `aria-label="${escapeHtmlAttribute(`Отметить ${staffSummary.name || "сотрудника"} недоступным на ${day.dateLabel}`)}"` : ""}
                    ${emptyCell ? 'title="Дважды нажмите, чтобы отметить клинера недоступным"' : ""}
                    ${isLessLoaded ? 'data-admin-team-calendar-low-load="true"' : ""}
    ...
    ${renderStaffTeamCalendarUnavailableDialog(normalizedStartDate, view)}
  </div>`;
}
