...
function createMessagesPageRenderer(deps = {}) {
  ...
  function renderMessageRows(records = [], options = {}) {
    const listKind = normalizeString(options.listKind, 20).toLowerCase();
    if (!Array.isArray(records) || records.length === 0) {
      return `<div class="admin-empty-state" data-admin-message-empty-state="true">Сообщений в этом списке пока нет.</div>`;
    }
          ...
          ${records
            .map((record) => {
              ...
              return `<tr
                ...
                data-admin-message-entry-id="${escapeHtmlAttribute(record.entryId || "")}"
                data-admin-message-key="${escapeHtmlAttribute(record.id || "")}"
                data-admin-message-status="${escapeHtmlAttribute(record.status || "")}"
              ...
              >
                ...
                <td data-admin-message-status-cell="true">${renderAdminBadge(record.statusLabel, record.statusTone)}</td>
    ...
    return renderAdminLayout(
      ...
      <div class="admin-messages-page">
        ...
        ${renderAdminCard(
          ...
          renderMessageRows(unreadRecords, { listKind: "unread" }),
          { eyebrow: "Сообщения", muted: true }
        )}
        ${renderAdminCard(
          "Все сообщения",
          "История входящих сообщений по заявкам и заказам.",
          renderMessageRows(records, { listKind: "all" }),
