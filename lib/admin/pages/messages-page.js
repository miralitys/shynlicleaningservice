...
function createMessagesPageRenderer(deps = {}) {
  ...
  function buildMessageTargetHref(record = {}) {
    const entryId = normalizeString(record.entryId, 120);
    if (!entryId) return ADMIN_MESSAGES_PATH;
    return record.isOrder
      ? buildAdminRedirectPath(ADMIN_ORDERS_PATH, { order: entryId })
      : buildAdminRedirectPath(ADMIN_QUOTE_OPS_PATH, { entry: entryId });
  }
    ...
    return `<div class="admin-table-wrap">
      <table class="admin-table admin-messages-table">
        ...
        <tbody>
          ${records
            .map((record) => {
              const targetHref = buildMessageTargetHref(record);
              const contacts = [record.customerPhone, record.customerEmail].filter(Boolean).join(" · ");
              const readAction =
                record.status === "new"
                  ? `<form method="post" action="${ADMIN_MESSAGES_PATH}">
                      <input type="hidden" name="action" value="mark-message-read">
                      <input type="hidden" name="entryId" value="${escapeHtmlAttribute(record.entryId)}">
                      <input type="hidden" name="messageKey" value="${escapeHtmlAttribute(record.id)}">
                      <button class="admin-link-button" type="submit">Отметить прочитанным</button>
                    </form>`
                  : "";
              return `<tr class="${record.status === "new" ? "admin-message-row-new" : ""}">
                <td>
                  <div class="admin-table-cell-stack">
                    <span class="admin-table-strong">${escapeHtml(record.customerName)}</span>
                ...
                <td>
                  <div class="admin-message-actions">
                    <a class="admin-link-button" href="${escapeHtmlAttribute(targetHref)}">Открыть карточку</a>
                    ${readAction}
                  </div>
                </td>
              </tr>`;
