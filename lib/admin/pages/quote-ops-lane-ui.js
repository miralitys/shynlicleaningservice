...
function renderQuoteOpsEntryCard(entry, returnTo) {
  ...
  const dialogUrl = buildAdminRedirectPath(ADMIN_QUOTE_OPS_PATH, {
    fragment: "entry-dialog",
    entry: entry.id,
    returnTo,
  });
  const fallbackUrl = buildAdminRedirectPath(returnTo || ADMIN_QUOTE_OPS_PATH, {
    entry: entry.id,
  });
      ...
      <div class="admin-quote-entry-main">
        ...
        <h3 class="admin-quote-entry-title">
          <button
            ...
            data-admin-dialog-url="${escapeHtmlAttribute(dialogUrl)}"
            data-admin-dialog-fallback-url="${escapeHtmlAttribute(fallbackUrl)}"
...
function renderQuoteOpsNewEntriesLane(entries, returnTo) {
  if (!entries.length) return "";

  return `<section class="admin-quote-lane admin-quote-lane-new">
    <div class="admin-quote-lane-head">
      <div>
        <p class="admin-quote-lane-kicker">Новый поток</p>
        <h2 class="admin-quote-lane-title">Новые заявки</h2>
        <p class="admin-quote-lane-copy">Свежие заявки вынесены отдельно, чтобы менеджер сразу видел входящий поток.</p>
      </div>
...
function renderQuoteOpsLeadStatusLane(meta = {}, entries, returnTo) {
  if (!entries.length) return "";

  const laneId = normalizeString(meta.id, 40) || "new";
  const badgeStatus = normalizeString(meta.badgeStatus || meta.id, 40) || "new";
  return `<section class="admin-quote-lane admin-quote-lane-lead-${escapeHtmlAttribute(laneId)}">
    <div class="admin-quote-lane-head">
      <div>
        ${meta.kicker ? `<p class="admin-quote-lane-kicker">${escapeHtml(meta.kicker)}</p>` : ""}
        <h2 class="admin-quote-lane-title">${escapeHtml(meta.title || "Заявки")}</h2>
        <p class="admin-quote-lane-copy">${escapeHtml(meta.copy || "Заявки отсортированы по дате уборки внутри статуса.")}</p>
      </div>
      <div class="admin-quote-lane-meta">
        ${renderLeadStatusBadge(badgeStatus)}
        ${renderAdminBadge(formatQuoteOpsEntryCountLabel(entries.length), "outline")}
      </div>
    </div>
    ${renderQuoteOpsSuccessTable(entries, returnTo)}
