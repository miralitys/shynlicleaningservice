...
function createOrdersUiPolicyPanel(deps = {}) {
  ...
  function renderOrderPolicyAcceptancePanel(order, options = {}) {
    const policyNotRequired = order && order.serviceType === "free-in-home-estimate";
    if (policyNotRequired) {
      return `<section class="admin-order-section-card" data-order-policy-not-required="true">
        <div class="admin-subsection-head">
          <div>
            <h3 class="admin-subsection-title">Подтверждение политик</h3>
            <p class="admin-order-detail-copy">Для бесплатной оценки дома подписание политики не требуется.</p>
          </div>
          ${renderAdminBadge("Не требуется", "muted")}
        </div>
      </section>`;
    }
