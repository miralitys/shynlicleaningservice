...
function createQuoteOpsPageRenderer(deps = {}) {
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger, adminRuntime = {}, staffStore = null) {
    ...
    const newLeadEntries = entries.filter((entry) => getLeadStatus(entry) === "new");
    ...
    const listBody = groupedEntries.length > 0
      ? `<div class="admin-quote-lanes">
          ${renderQuoteOpsNewEntriesLane(newLeadEntries, currentReturnTo)}
    ...
    const manualTaskForm = `<section class="admin-orders-filters-panel admin-quote-manual-task-card">
      <div class="admin-orders-panel-head">
        <div>
          <p class="admin-orders-panel-kicker">Ручной контроль</p>
          <h2 class="admin-orders-panel-title">Создать таск вручную</h2>
          <p class="admin-orders-panel-copy">Добавьте задачу к конкретной заявке, если менеджеру нужно сделать действие вне автоматического сценария.</p>
        </div>
        <span class="admin-action-hint">${escapeHtml(String(allEntries.length))} заявок доступно для выбора.</span>
      </div>
      <form class="admin-form admin-form-grid admin-form-grid-two admin-quote-manual-task-form" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
        ...
        <label class="admin-label">
          Заявка
          <select class="admin-input" name="entryId" required>
            <option value="">Выберите заявку</option>
            ${allEntries.map((entry) => {
              const leadStatus = getLeadStatus(entry);
              const leadLabel = leadFilterLabels[leadStatus] || leadStatus || "New";
              const optionLabel = [
                entry.customerName || "Клиент",
                entry.requestId || entry.id,
                leadLabel,
              ].filter(Boolean).join(" • ");
              return `<option value="${escapeHtmlAttribute(entry.id)}">${escapeHtml(optionLabel)}</option>`;
            }).join("")}
          </select>
        </label>
        <label class="admin-label">
          Название таска
          <input class="admin-input" type="text" name="taskTitle" maxlength="240" placeholder="Например: уточнить детали по адресу" required>
        </label>
        <label class="admin-label">
          Дедлайн
          <input class="admin-input" type="datetime-local" name="taskDueAt" required>
        </label>
        <label class="admin-label">
          Менеджер
          <select class="admin-input" name="managerId">
            <option value="">Не менять менеджера заявки</option>
            ${managerOptions.map((manager) => `
              <option value="${escapeHtmlAttribute(manager.id)}">${escapeHtml(manager.name)}</option>
            `).join("")}
          </select>
        </label>
        <div class="admin-inline-actions admin-form-grid-span-2">
          <button class="admin-button" type="submit"${allEntries.length === 0 ? " disabled" : ""}>Создать таск</button>
        </div>
      </form>
    </section>`;
    ...
    const tasksBody = `${manualTaskForm}${taskListBody}`;
