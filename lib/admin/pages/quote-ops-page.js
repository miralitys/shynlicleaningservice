...
function createQuoteOpsPageRenderer(deps = {}) {
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger, adminRuntime = {}, staffStore = null) {
    ...
    const pageLedgerLimit = filters.section === "tasks"
      ...
      : QUOTE_OPS_PAGE_LEDGER_LIMIT;
    const cachedEntries = filters.section !== "tasks" && Array.isArray(adminRuntime && adminRuntime.quoteOpsPageEntries)
      ? adminRuntime.quoteOpsPageEntries
      : null;
    const allEntriesRaw = cachedEntries
      ? cachedEntries.slice(0, pageLedgerLimit)
      : quoteOpsLedger
        ? await quoteOpsLedger.listEntries({
            limit: pageLedgerLimit,
            includeHidden: filters.section === "tasks",
          })
        : [];
    if (
      selectedEntryId &&
    ...
    const entries = filterQuoteOpsEntries(allEntries, {
      ...filters,
      includeHidden: filters.section === "tasks",
    });
    ...
    allEntries
      .filter((entry) => normalizeString(entry && entry.kind, 64) !== "admin_standalone_task")
      .forEach((entry) => {
      ...
      <div class="admin-dialog-panel">
        ...
        <div class="admin-label admin-quote-task-client-picker" data-quote-task-client-picker="true">
          ...
          <div class="admin-quote-task-client-results" id="admin-quote-task-client-results" role="listbox" hidden data-quote-task-client-results="true">
            <button
              class="admin-quote-task-client-result admin-quote-task-client-result-standalone"
              type="button"
              role="option"
              data-quote-task-clientless-result="true"
            >
              <span class="admin-quote-task-client-result-head"><strong>Без клиента</strong></span>
              <span class="admin-quote-task-client-result-meta">Обычный внутренний таск без привязки к заявке</span>
            </button>
          ...
          <p class="admin-quote-task-field-error" hidden data-quote-task-client-error="true">Выберите «Без клиента» или клиента из результатов поиска.</p>
        </div>
        <div class="admin-label admin-quote-task-title-field">
          ...
          <label class="admin-sr-only" for="admin-quote-task-title">Что нужно сделать</label>
          <input class="admin-input" id="admin-quote-task-title" type="text" name="taskTitle" maxlength="240" placeholder="Или напишите своё действие" required data-quote-task-title-input="true">
        </div>
        <div class="admin-label admin-quote-task-due-field">
          ...
          <label class="admin-sr-only" for="admin-quote-task-due-at">Дата и время напоминания</label>
          <input class="admin-input" id="admin-quote-task-due-at" type="datetime-local" name="taskDueAt" required data-quote-task-due-input="true">
        </div>
        <label class="admin-label">
          Исполнитель
