...
function createOrdersFunnelUi(deps = {}) {
  ...
  function getOrderFunnelStatus(order) {
    ...
    const policyNotRequired =
      normalizeString(order && order.serviceType, 80)
        .toLowerCase()
        ...
        ${laneOrders
          .map((order) =>
            renderOrderFunnelCard(order, returnTo, {
              ...
              dialogUrl:
                typeof options.getDialogUrl === "function" ? options.getDialogUrl(order) : "",
              dialogFallbackUrl:
                typeof options.getDialogFallbackUrl === "function"
                  ? options.getDialogFallbackUrl(order)
                  : "",
            })
          )
  ...
  function renderOrderFunnelCard(order, returnTo, options = {}) {
    ...
    return `<article
      ...
      data-order-recurring="${order.frequency || order.recurringNextEntryId ? "true" : "false"}"
      tabindex="0"
      data-admin-dialog-row="true"
      data-admin-dialog-open="${escapeHtmlAttribute(dialogId)}"
      ${options.dialogUrl ? `data-admin-dialog-url="${escapeHtmlAttribute(options.dialogUrl)}"` : ""}
      ${options.dialogFallbackUrl ? `data-admin-dialog-fallback-url="${escapeHtmlAttribute(options.dialogFallbackUrl)}"` : ""}
  ...
  function renderOrdersFunnelScript(currentReturnTo, canEdit) {
    ...
    return `<script>
      (() => {
        ...
        const stageCancelScopeInput = stageForm ? stageForm.querySelector('input[name="cancelScope"]') : null;
        const stageReturnInput = stageForm ? stageForm.querySelector('input[name="returnTo"]') : null;
        if (!stageForm || !stageEntryInput || !stageStatusInput || !stageCancelScopeInput || !stageReturnInput) return;
        ...
        document.querySelectorAll("[data-order-dropzone]").forEach((list) => {
          ...
          list.addEventListener("drop", async (event) => {
            ...
            let cancelScope = "current";
            if (nextStatus === "canceled" && draggedCard.getAttribute("data-order-recurring") === "true") {
              const cancelFuture = window.confirm(
                "Отменить этот визит и все последующие визиты серии?\\n\\nНажмите «Отмена», чтобы выбрать отмену только этого визита."
              );
              if (cancelFuture) {
                cancelScope = "future";
              } else if (!window.confirm("Отменить только этот визит?")) {
                draggedCard = null;
                return;
              }
            }
            ...
            const confirmedSave =
              typeof window.__adminConfirmSaveAction === "function"
                ? await window.__adminConfirmSaveAction()
                : window.confirm("Сохранить?");
            if (!confirmedSave) {
              draggedCard = null;
              return;
            }
            ...
            stageCancelScopeInput.value = cancelScope;
