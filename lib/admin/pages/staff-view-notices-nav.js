...
function createStaffViewNoticeNavHelpers(deps = {}) {
  ...
  function renderStaffNotice(req) {
    ...
    if (notice === "assignment-conflict") {
      return `<div class="admin-alert admin-alert-error">Назначение не сохранено: ${escapeHtml(conflictedStaff || "сотрудник")} отмечен как недоступный в календаре.</div>`;
    }
    if (notice === "staff-unavailable-saved") {
      return `<div class="admin-alert admin-alert-info">День недоступности сохранён в календаре сотрудника.</div>`;
    }
    if (notice === "staff-unavailable-cleared") {
      return `<div class="admin-alert admin-alert-info">День недоступности очищен.</div>`;
    }
