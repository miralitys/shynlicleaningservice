...
function createSettingsUsersHelpers(deps = {}) {
  ...
  function renderInviteEmailStatusCard(status = {}) {
    ...
    const canSendTest = googleConnected || legacyConfigured;
    ...
    return renderAdminCard(
      ...
      `<details class="admin-details admin-settings-disclosure">
        ...
        <div class="admin-settings-disclosure-body">
          ...
          ${canSendTest
            ? `<form class="admin-form-grid admin-form-grid-two" method="post" action="${ADMIN_SETTINGS_PATH}" style="margin-top:14px;">
                <input type="hidden" name="action" value="send_test_invite_email">
                <label class="admin-label">
                  Тестовое письмо
                  <input class="admin-input" type="email" name="testEmail" placeholder="client@example.com" required>
                </label>
                <div class="admin-inline-actions" style="align-self:end;">
                  <button class="admin-button admin-button-secondary" type="submit">Отправить тест</button>
                </div>
              </form>`
            : ""}
        </div>
