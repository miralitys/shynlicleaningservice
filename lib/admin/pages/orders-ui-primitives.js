    ...
    return (Array.isArray(clientRecords) ? clientRecords : [])
      .map((client) => {
        ...
        return {
          ...
          phone: phoneDigits,
          phoneLabel: formatAdminPhoneNumber ? formatAdminPhoneNumber(phoneDigits) : phoneDigits,
      ...
      .filter((client) => client.name || client.phone || client.email || client.address)
      .slice(0, 250);
  }
    ...
    return `<script>
      (() => {
        ...
        function renderSelectedClient(client, addressRecord) {
          ...
          meta.textContent = [client.phoneLabel || client.phone, client.email].filter(Boolean).join(" • ");
        ...
        function createSuggestionButton(client, addressRecord, index) {
          ...
          meta.textContent = [client.phoneLabel || client.phone, client.email, addressRecord && addressRecord.address]
            .filter(Boolean)
            .join(" • ");
