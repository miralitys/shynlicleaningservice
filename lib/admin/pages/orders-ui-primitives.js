...
function createOrdersUiPrimitives(deps = {}) {
        ...
        <input
          ...
          value="${escapeHtmlAttribute(
            options.serviceDurationLabel && options.serviceDurationLabel !== "Не указана"
              ...
              : normalizeString(options.emptyLabel, 120) || "Укажите часы и минуты"
          )}"
          readonly
          tabindex="-1"
          data-admin-service-duration-label="true"
        >
      </label>
    </div>`;
  }
  ...
  function renderOrderPaymentMethodBadge(method) {
    ...
    if (normalized === "venmo") return renderAdminBadge("Venmo", "outline");
    if (normalized === "check") return renderAdminBadge("Check", "outline");
    ...
    return (Array.isArray(clientRecords) ? clientRecords : [])
      .map((client) => {
        ...
        const addresses = Array.isArray(client && client.addresses)
          ? client.addresses
              .map((addressRecord) => ({
                ...
                basementCleaning: normalizeString(addressRecord && addressRecord.basementCleaning, 40),
        ...
        return {
          ...
          automaticNotificationsEnabled: client && client.automaticNotificationsEnabled !== false,
  ...
  function renderManualOrderClientLookup(clientRecords = []) {
    ...
    return `<div class="admin-manual-client-lookup admin-form-grid-span-2" data-admin-manual-client-lookup>
      ...
      <input type="hidden" name="selectedClientAutomaticNotificationsEnabled" data-admin-manual-client-automatic-notifications-enabled>
      ...
      <input type="hidden" name="selectedClientAddressBasementCleaning" data-admin-manual-client-address-basement-cleaning>
    ...
    return `<script>
      (() => {
        ...
        const hiddenFields = {
          ...
          automaticNotificationsEnabled: lookup.querySelector("[data-admin-manual-client-automatic-notifications-enabled]"),
          ...
          basementCleaning: lookup.querySelector("[data-admin-manual-client-address-basement-cleaning]"),
        ...
        function applyClient(client, addressRecord) {
          ...
          setHiddenValue("automaticNotificationsEnabled", client.automaticNotificationsEnabled === false ? "0" : "1");
          ...
          setHiddenValue("basementCleaning", selectedAddress.basementCleaning);
        ...
        function renderSuggestions() {
          ...
          for (const client of searchableClients) {
            ...
            // A saved address is a distinct booking option. Do not hide the
            // third and later addresses when the client itself matches the
            // search; the overall suggestions list is already capped below.
            const addressOptions = addressMatches.length > 0 ? addressMatches : addresses;
  ...
  function renderManualOrderEstimateDefaultsScript() {
    return `<script data-admin-manual-order-estimate-defaults>
      (() => {
        const dialog = document.getElementById(${escapeScriptJson(ADMIN_MANUAL_ORDER_DIALOG_ID)});
        const form = dialog && dialog.querySelector("form");
        if (!form) return;

        const serviceTypeInput = form.querySelector('[name="serviceType"]');
        const durationHoursInput = form.querySelector('[name="serviceDurationHours"]');
        const durationMinutesInput = form.querySelector('[name="serviceDurationMinutes"]');
        const durationLabelInput = form.querySelector('[data-admin-service-duration-label="true"]');
        ...
        function applyEstimateDefaults() {
          if (serviceTypeInput.value !== "free-in-home-estimate") return;
          if (durationHoursInput) durationHoursInput.value = "0";
          if (durationMinutesInput) durationMinutesInput.value = "30";
          if (durationLabelInput) durationLabelInput.value = "30 мин";
          if (totalPriceInput) totalPriceInput.value = "0.00";
          if (frequencyInput) frequencyInput.value = "";
        }

        serviceTypeInput.addEventListener("change", applyEstimateDefaults);
        applyEstimateDefaults();
      })();
    </script>`;
  }

  function renderManualOrderDraftScript() {
    return `<script data-admin-manual-order-draft>
      (() => {
        const dialog = document.getElementById(${escapeScriptJson(ADMIN_MANUAL_ORDER_DIALOG_ID)});
        const form = dialog && dialog.querySelector("form");
        if (!dialog || !form || !window.sessionStorage) return;

        const storageKey = "shynli:manual-order-draft:v1";
        const shouldRestore = dialog.getAttribute("data-admin-dialog-autopen") === "true";

        function getDraftFields() {
          return Array.from(form.elements)
            .filter((field) => field && field.name && !["action", "returnTo"].includes(field.name))
            .map((field) => ({
              name: field.name,
        ...
        if (!shouldRestore) {
          ...
          try {
            ...
            const isFresh =
              ...
              Date.now() - Number(draft.savedAt) < 2 * 60 * 60 * 1000;
            if (isFresh && Array.isArray(draft.fields)) {
              draft.fields.forEach((savedField) => {
                const field = Array.from(form.elements).find(
                  (candidate) => candidate && candidate.name === savedField.name
                );
                if (!field) return;
    ...
    return `<form class="admin-form-grid" method="post" action="${ADMIN_ORDERS_PATH}" data-admin-save-confirm="true">
      ...
      <div class="admin-form-grid admin-form-grid-two">
        ...
        ${renderOrderServiceDurationInputs({
          ...
          emptyLabel: "Можно указать позже",
        })}
        <label class="admin-label">
          Повторяемость
  ...
  function renderCreateManualOrderDialog(options = {}) {
    ...
    return `<dialog class="admin-dialog admin-dialog-wide" id="${ADMIN_MANUAL_ORDER_DIALOG_ID}"${autoOpenAttr} data-admin-dialog-dirty-confirm="true" data-admin-dialog-dirty-message="В форме есть несохранённые данные. Закрыть окно и сбросить их?" aria-labelledby="admin-manual-order-create-title">
      <div class="admin-dialog-panel">
        ...
        ${renderManualOrderEstimateDefaultsScript()}
        ${renderManualOrderDraftScript()}
      </div>
    </dialog>`;
  }
