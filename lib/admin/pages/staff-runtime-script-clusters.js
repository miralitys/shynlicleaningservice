      ...
      (() => {
        ...
        const isBlockedInteractiveTarget = (target) =>
          Boolean(
            target &&
            target.closest &&
            target.closest('input, select, textarea, summary, label, form, [data-admin-dialog-close], [data-admin-team-calendar-unavailable-dialog]')
          );
        ...
        const formatCalendarDateLabel = (dateValue) => {
        ...
        const openUnavailableDialog = (shell, cell) => {
          if (!shell || !cell) return;
          const dialog = shell.querySelector('[data-admin-team-calendar-unavailable-dialog="true"]');
          if (!dialog) return;

          const staffId = cell.getAttribute('data-admin-team-calendar-cleaner-id') || '';
          const staffName = cell.getAttribute('data-admin-team-calendar-cleaner-name') || 'Сотрудник';
          const availabilityDate = cell.getAttribute('data-admin-team-calendar-date') || '';
          if (!staffId || !availabilityDate) return;

          const staffIdInput = dialog.querySelector('[data-admin-team-calendar-unavailable-staff-id]');
          const dateInput = dialog.querySelector('[data-admin-team-calendar-unavailable-date]');
          const summary = dialog.querySelector('[data-admin-team-calendar-unavailable-summary]');
          const reasonInput = dialog.querySelector('input[name="availabilityReason"]');
          const notesInput = dialog.querySelector('textarea[name="availabilityNotes"]');

          if (staffIdInput) staffIdInput.value = staffId;
          if (dateInput) dateInput.value = availabilityDate;
          if (summary) {
            summary.textContent = staffName + ' • ' + formatCalendarDateLabel(availabilityDate);
          }
          if (reasonInput && !reasonInput.value.trim()) {
            reasonInput.value = 'Не может выйти на работу';
          }
          ...
          if (reasonInput && typeof reasonInput.focus === 'function') {
        ...
        const bindCalendarShell = (shell) => {
          ...
          shell.addEventListener('dblclick', (event) => {
            const target = event.target;
            const emptyCell = target && target.closest
              ? target.closest('[data-admin-team-calendar-empty="true"]')
              : null;
            if (!emptyCell || !shell.contains(emptyCell)) return;
            event.preventDefault();
            openUnavailableDialog(shell, emptyCell);
          });
          ...
          shell.addEventListener('keydown', (event) => {
            ...
            const emptyCell = target && target.closest
              ? target.closest('[data-admin-team-calendar-empty="true"]')
              : null;
            if (!emptyCell || target !== emptyCell || !shell.contains(emptyCell)) return;
            event.preventDefault();
            openUnavailableDialog(shell, emptyCell);
          });
