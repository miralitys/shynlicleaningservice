    ...
    return `<script>
      (() => {
        ...
        calendarWraps.forEach((wrap) => {
          ...
          const lowLoadToggle = shell
            ? shell.querySelector('[data-admin-team-calendar-low-load-toggle="true"]')
            : null;
          if (lowLoadToggle) {
            const syncLowLoadState = () => {
              shell.classList.toggle('admin-team-calendar-show-low-load', Boolean(lowLoadToggle.checked));
            };
            lowLoadToggle.addEventListener('change', syncLowLoadState);
            syncLowLoadState();
          }

          const fullscreenButton = shell
            ? shell.querySelector('[data-admin-team-calendar-fullscreen-button="true"]')
            : null;
          if (fullscreenButton) {
            const canUseNativeFullscreen =
              typeof shell.requestFullscreen === 'function' &&
              typeof document.exitFullscreen === 'function';
            const isCalendarFullscreen = () =>
              document.fullscreenElement === shell || shell.classList.contains('admin-team-calendar-shell-fullscreen');
            const syncFullscreenButton = () => {
              const isFullscreen = isCalendarFullscreen();
