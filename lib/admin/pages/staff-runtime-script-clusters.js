      ...
      (() => {
        ...
        const isBlockedInteractiveTarget = (target) =>
          Boolean(
            target &&
            target.closest &&
            target.closest('input, select, textarea, summary, label, form, [data-admin-dialog-close]')
          );
        ...
        const closeCalendarMenus = (shell, exceptMenu = null) => {
          if (!shell) return;
          shell.querySelectorAll('[data-admin-team-calendar-menu="true"][open]').forEach((menu) => {
            if (menu !== exceptMenu) {
        ...
        const syncBusyFormControls = (form) => {
          if (!form) return;
          const checkbox = form.querySelector('[data-admin-team-calendar-busy-checkbox="true"]');
          const settings = form.querySelector('[data-admin-team-calendar-busy-settings="true"]');
          const mode = form.querySelector('[data-admin-team-calendar-busy-mode="true"]');
          const timeRange = form.querySelector('[data-admin-team-calendar-busy-time-range="true"]');
          const timeInputs = form.querySelectorAll('[data-admin-team-calendar-busy-time="true"]');
          const isBusy = Boolean(checkbox && checkbox.checked);
          const usesTimeRange = isBusy && mode && mode.value === 'time-range';

          if (settings) settings.hidden = !isBusy;
          if (timeRange) timeRange.hidden = !usesTimeRange;
        ...
        const bindCalendarShell = (shell) => {
          ...
          shell.addEventListener('click', async (event) => {
            ...
            const cellMenu = target && target.closest
              ? target.closest('[data-admin-team-calendar-menu="true"]')
              : null;
            if (cellMenu && target.closest('[data-admin-team-calendar-menu-summary="true"]')) {
              closeCalendarMenus(shell, cellMenu);
            } else if (!cellMenu) {
              closeCalendarMenus(shell);
            }
          ...
          shell.addEventListener('change', (event) => {
            ...
            if (
              target &&
              target.matches &&
              target.matches('[data-admin-team-calendar-busy-checkbox="true"]')
            ) {
              const form = target.closest('form');
              if (form) {
                syncBusyFormControls(form);
                const actionInput = form.querySelector('[data-admin-team-calendar-busy-action="true"]');
                if (!target.checked) {
                  if (actionInput) actionInput.value = 'clear-staff-unavailable-day';
                  ...
                  actionInput.value = 'save-staff-unavailable-day';
                }
              }
              return;
            }
            if (
              target &&
              target.matches &&
              target.matches('[data-admin-team-calendar-busy-mode="true"]')
            ) {
              syncBusyFormControls(target.closest('form'));
              return;
            }
