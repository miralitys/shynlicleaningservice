...
function createClientDetailUiHelpers(deps = {}) {
  ...
  function getAdminClientPhoneLabels(client = {}) {
    const phoneValues = [client.phone, client.secondaryPhone]
      .map((value) => normalizeString(value, 80))
      .filter(Boolean);
    const seen = new Set();
    return phoneValues
      .filter((value) => {
      ...
      .map((value) => formatAdminPhoneNumber(value) || value);
  }
...
function getAdminClientHistorySortTimestamp(entry = {}) {
  const selectedDate = normalizeString(entry.selectedDate, 32);
  const selectedTime = normalizeString(entry.selectedTime, 32);
  const scheduledTimestamp = Date.parse([selectedDate, selectedTime].filter(Boolean).join(" "));
  if (Number.isFinite(scheduledTimestamp)) return scheduledTimestamp;

  const createdTimestamp = Date.parse(entry.createdAt || "");
  return Number.isFinite(createdTimestamp) ? createdTimestamp : 0;
}

function getAdminClientHistoryEntries(client = {}) {
  return (Array.isArray(client.entries) ? client.entries : [])
    .slice()
    .sort((left, right) => {
      const rightTimestamp = getAdminClientHistorySortTimestamp(right);
      const leftTimestamp = getAdminClientHistorySortTimestamp(left);
      if (rightTimestamp !== leftTimestamp) return rightTimestamp - leftTimestamp;
      return normalizeString(right.requestId, 120).localeCompare(normalizeString(left.requestId, 120), "ru");
    });
}
...
function buildAdminClientPhotoMediaPath(asset = {}) {
  const entryId = normalizeString(asset.entryId || asset.orderId, 120);
  const assetId = normalizeString(asset.id, 180);
  if (!entryId || !assetId) return "";
  return `${ADMIN_ORDERS_PATH}?media=1&entryId=${encodeURIComponent(entryId)}&asset=${encodeURIComponent(assetId)}`;
}

function formatAdminClientPhotoCountLabel(count) {
  const normalized = Math.max(0, Number(count) || 0);
  if (normalized === 0) return "Фото пока нет";
  if (normalized === 1) return "1 фото";
  if (normalized >= 2 && normalized <= 4) return `${normalized} фото`;
  return `${normalized} фото`;
}

function renderAdminClientPhotosSection(client, returnTo = ADMIN_CLIENTS_PATH) {
  if (!client || !client.key) return "";
  const photos = Array.isArray(client.photos) ? client.photos : [];

  return `<section class="admin-client-section admin-client-photo-section">
    <div class="admin-subsection-head admin-client-photo-head">
      <div>
        <h3 class="admin-subsection-title">Важные фото</h3>
        <p class="admin-card-copy admin-client-photo-copy">Фото хранятся в карточке клиента и остаются доступными при следующих уборках.</p>
      </div>
      <span class="admin-action-hint">${escapeHtml(formatAdminClientPhotoCountLabel(photos.length))}</span>
    </div>
    ${photos.length > 0
      ? `<div class="admin-client-photo-grid">
          ${photos
            .map((asset, index) => {
              const mediaPath = buildAdminClientPhotoMediaPath(asset);
              const fileName = normalizeString(asset.fileName, 180) || `Фото ${index + 1}`;
              const uploadedAt = formatAdminDateTime(asset.uploadedAt);
              const caption = uploadedAt && uploadedAt !== "—" ? uploadedAt : `Фото ${index + 1}`;
              return `<article class="admin-client-photo-card">
                ${mediaPath
                  ? `<a class="admin-client-photo-link" href="${escapeHtmlAttribute(mediaPath)}" target="_blank" rel="noreferrer">
                      <img class="admin-client-photo-thumb" src="${escapeHtmlAttribute(mediaPath)}" alt="${escapeHtmlAttribute(fileName)}" loading="lazy">
                    </a>`
                  : `<div class="admin-client-photo-thumb admin-client-photo-thumb-placeholder" aria-hidden="true">IMG</div>`}
                <div class="admin-client-photo-card-foot">
                  <div class="admin-client-photo-card-copy">
                    <strong>${escapeHtml(fileName)}</strong>
                    <span>${escapeHtml(caption)}</span>
                  </div>
                  <form class="admin-client-photo-delete-form" method="post" action="${ADMIN_CLIENTS_PATH}" data-admin-save-confirm="true">
                    <input type="hidden" name="action" value="delete-client-photo">
                    <input type="hidden" name="clientKey" value="${escapeHtmlAttribute(client.key)}">
                    <input type="hidden" name="assetId" value="${escapeHtmlAttribute(asset.id)}">
                    <input type="hidden" name="assetPath" value="${escapeHtmlAttribute(asset.path)}">
                    <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
                    <button class="admin-button admin-button-secondary admin-client-photo-delete-button" type="submit">Удалить</button>
                  </form>
                </div>
              </article>`;
            })
      ...
      : `<div class="admin-empty-state admin-client-photo-empty">Здесь можно прикрепить фото с важными деталями по клиенту: доступ, зона, которую нельзя трогать, нюансы дома.</div>`}
    <form class="admin-client-photo-upload-form" method="post" action="${ADMIN_CLIENTS_PATH}" enctype="multipart/form-data" data-admin-save-confirm="true">
      <input type="hidden" name="action" value="add-client-photo">
      <input type="hidden" name="clientKey" value="${escapeHtmlAttribute(client.key)}">
      <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
      <label class="admin-label">
        Прикрепить фото
        <input class="admin-input admin-file-input" type="file" name="clientPhotos" accept="image/*" multiple>
      </label>
      <div class="admin-inline-actions admin-client-photo-upload-actions">
        <button class="admin-button" type="submit">Прикрепить</button>
        <span class="admin-action-hint">Можно выбрать несколько фото сразу</span>
      </div>
    </form>
  </section>`;
}
...
function renderAdminClientAddressInputRow(baseId, value = "", index = 0) {
  ...
  const originalAddress = normalizeString(addressRecord.originalAddress || addressRecord.address, 500);

  return `<div class="admin-client-address-input-row" data-admin-client-address-row>
    <input type="hidden" name="addressOriginals" value="${escapeHtmlAttribute(originalAddress)}">
  ...
  return `<section class="admin-client-section admin-staff-form-section" id="${escapeHtmlAttribute(panelId)}" data-admin-toggle-panel hidden>
    ...
    >
      ...
      <div class="admin-form-grid admin-form-grid-two">
        ...
        ${renderAdminPhoneInput("secondaryPhone", client.secondaryPhone, {
          label: "Второй телефон",
          placeholder: "3125550199",
          autocomplete: "tel",
        })}
        <label class="admin-checkbox admin-client-primary-phone-toggle">
          <input type="checkbox" name="primaryPhone" value="secondary">
          <span>
            <strong>Сделать второй номер основным</strong>
            <small>После сохранения второй номер станет primary, а текущий основной останется вторым.</small>
          </span>
        </label>
        <label class="admin-label">
          Email
      ...
      <input type="hidden" name="automaticNotificationsSettingPresent" value="1">
      <label class="admin-client-notification-toggle">
        <span class="admin-client-notification-toggle-copy">
          <strong>Автоматические сообщения</strong>
          <small>SMS и email-уведомления этому клиенту. Ручная отправка останется доступна.</small>
        </span>
        <span class="admin-client-switch">
          <input type="checkbox" name="automaticNotificationsEnabled" value="1"${client.automaticNotificationsEnabled === false ? "" : " checked"}>
          <span aria-hidden="true"></span>
        </span>
      </label>
      <input type="hidden" name="paymentNotificationsSettingPresent" value="1">
      <label class="admin-client-notification-toggle">
        <span class="admin-client-notification-toggle-copy">
          <strong>Отключить уведомления об оплате</strong>
          <small>Клиент не будет получать автоматические SMS со ссылкой или просьбой об оплате. Остальные уведомления продолжат работать.</small>
        </span>
        <span class="admin-client-switch">
          <input type="checkbox" name="paymentNotificationsDisabled" value="1"${client.paymentNotificationsEnabled === false ? " checked" : ""}>
          <span aria-hidden="true"></span>
        </span>
      </label>
...
function renderAdminClientDetailPanel(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
  ...
  const clientHistoryEntries = getAdminClientHistoryEntries(client);
  ...
  const statusBadges = [
    ...
    client.automaticNotificationsEnabled === false
      ? renderAdminBadge("Автосообщения выключены", "danger")
      : renderAdminBadge("Автосообщения включены", "success"),
    client.paymentNotificationsEnabled === false
      ? renderAdminBadge("Уведомления об оплате выключены", "danger")
      : "",
  ].join("");
  ...
  return `<div class="admin-client-dialog-body">
    <section class="admin-client-summary-panel">
      ...
      <div class="admin-client-metric-grid admin-client-metric-grid-dialog">
        ...
        <article class="admin-client-metric-card">
          <span class="admin-client-metric-label">Сумма прошедших уборок</span>
          <p class="admin-client-metric-value">${escapeHtml(formatCurrencyAmount(activeAddressRecord ? activeAddressRecord.pastVisitRevenue : 0))}</p>
        </article>
        <article class="admin-client-metric-card">
    ...
    ${renderAdminClientPhotosSection(client, returnTo)}
    ${renderAdminGhlSmsComposer({
      req: options.req,
      ...
      historyEntries: getClientCustomerSmsHistoryEntries(client),
    ...
    <section class="admin-client-section">
      <div class="admin-subsection-head">
        <h3 class="admin-subsection-title">История заказов клиента</h3>
        <span class="admin-action-hint">${escapeHtml(formatClientRequestCountLabel(clientHistoryEntries.length))}</span>
      </div>
      ${clientHistoryEntries.length > 0
        ? `<div class="admin-history-list admin-client-history-list">
            ${clientHistoryEntries.map((entry) => renderAdminClientHistoryItem(entry, planningByEntryId.get(entry.id) || null)).join("")}
          </div>`
        : `<div class="admin-empty-state admin-client-history-empty">По этому клиенту пока нет заказов. Добавленный адрес уже сохранён в карточке клиента.</div>`}
    </section>
  </div>`;
}
...
function renderAdminClientDetailDialog(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
  ...
  const activeAddressRecord =
    options.activeAddressRecord && typeof options.activeAddressRecord === "object"
      ...
      : Array.isArray(client.addresses)
        ...
        : null;
  const clientMeta =
    [...getAdminClientPhoneLabels(client), client.email || ""].filter(Boolean).join(" • ") ||
  ...
  return {
    ...
    renderAdminClientPhotosSection,
