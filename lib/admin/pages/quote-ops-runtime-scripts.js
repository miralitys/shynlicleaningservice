      ...
      (() => {
        ...
        if (manualTaskForm) {
          ...
          const resultButtons = results
            ...
            : [];
          const clientlessButton = results
            ? results.querySelector('[data-quote-task-clientless-result="true"]')
            : null;
          ...
          const renderClientResults = () => {
            ...
            let visibleCount = 0;
            if (clientlessButton) clientlessButton.hidden = Boolean(query);
          ...
          const selectClientlessTask = () => {
            if (!entryInput || !searchInput || !selection) return;
            entryInput.value = "standalone";
            searchInput.value = "Без клиента";
            searchInput.setCustomValidity("");
            searchInput.setAttribute("aria-invalid", "false");
            if (clientError) clientError.hidden = true;
            if (searchShell) searchShell.hidden = true;
            selection.hidden = false;
            setSelectedText('[data-quote-task-selected-name="true"]', "Без клиента", "");
            setSelectedText(
              '[data-quote-task-selected-contacts="true"]',
              "Внутренний таск без привязки к клиенту или заявке",
              ""
            );
            setSelectedText('[data-quote-task-selected-address="true"]', "", "");
            setSelectedText('[data-quote-task-selected-stage="true"]', "", "");
            setSelectedText('[data-quote-task-selected-schedule="true"]', "", "");
            setSelectedText('[data-quote-task-selected-summary="true"]', "", "");
            resultButtons.forEach((resultButton) => resultButton.setAttribute("aria-selected", "false"));
            closeClientResults();
          };
          ...
          if (clientlessButton) {
            clientlessButton.addEventListener("click", selectClientlessTask);
          }
          ...
          manualTaskForm.addEventListener("submit", (event) => {
            ...
            if (selection) selection.hidden = true;
            if (searchInput) {
              searchInput.setCustomValidity("Выберите без клиента или клиента из результатов поиска.");
        ...
        document.querySelectorAll('[data-lead-next-status="true"]').forEach((select) => {
          ...
          const syncFieldVisibility = () => {
            ...
            contactField.setAttribute("data-visible", "true");
            const input = contactField.querySelector("input");
            if (input) {
              input.required = String(select.value || "") === "discussion";
            }
          };
          syncFieldVisibility();
