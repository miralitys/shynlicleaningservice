...
function createQuoteOpsHelpers(deps = {}) {
  ...
  function renderLeadStatusBadge(status) {
    ...
    if (normalized === "confirmed") return renderAdminBadge("Подтверждено", "warning");
    if (normalized === "completed") return renderAdminBadge("Выполнено", "success");
  ...
  function buildQuoteOpsTaskRecords(entries = []) {
    const now = Date.now();
    const futureVisitClients = new Set();

    for (const entry of entries) {
      if (!entry || !isOrderCreatedEntry(entry)) continue;
      const order = getEntryOrderState(entry);
      const orderStatus = normalizeString(order.status || order.orderStatus, 40).toLowerCase();
      if (["canceled", "cancelled", "completed", "rescheduled"].includes(orderStatus)) continue;
      const selectedDate = normalizeString(entry.selectedDate || order.selectedDate, 32);
      const dateMatch = selectedDate.match(/^(\d{4})-(\d{2})-(\d{2})$/);
      if (!dateMatch) continue;
      const visitTimestamp = Date.UTC(
        Number(dateMatch[1]),
      ...
      if (!Number.isFinite(visitTimestamp) || visitTimestamp < now) continue;
      if (!normalizeString(order.assignedStaff || order.assignee, 500)) continue;
      const identity = getTaskClientIdentity(entry);
      if (identity) futureVisitClients.add(identity);
    }

    return entries.flatMap((entry) => {
      const leadStatus = getLeadStatus(entry);
      const manager = getQuoteLeadManager(entry);
      const clientIdentity = getTaskClientIdentity(entry);
      return getEntryLeadTasks(entry).filter((task) => {
        const kind = normalizeString(task.kind, 64).toLowerCase();
        return kind !== "post-completion-followup" || !futureVisitClients.has(clientIdentity);
      }).map((task) => {
        const taskAssignee = {
          id: normalizeString(task.assigneeId, 120),
          name: normalizeString(task.assigneeName, 200),
          email: normalizeString(task.assigneeEmail, 250).toLowerCase(),
          role: normalizeString(task.assigneeRole, 40).toLowerCase(),
        };
        const assignee = taskAssignee.id || taskAssignee.name || taskAssignee.email
          ? taskAssignee
          : manager;
        return {
          ...task,
  ...
  function getTaskClientIdentity(entry = {}) {
    ...
    const name = normalizeString(entry.customerName, 200).toLowerCase();
  ...
  function renderQuoteOpsNotice(req) {
    ...
    if (notice === "task-deleted") {
      return `<div class="admin-alert admin-alert-info">Таск удалён.</div>`;
    }
    if (notice === "next-cleaning-scheduled") {
      return `<div class="admin-alert admin-alert-info">Следующая уборка запланирована.</div>`;
    }
    if (notice === "next-cleaning-required") {
      return `<div class="admin-alert admin-alert-error">Укажите дату и время следующей уборки.</div>`;
    }
    if (notice === "next-cleaning-unavailable") {
      return `<div class="admin-alert admin-alert-error">Не удалось создать следующую уборку. Проверьте повторяемость заказа.</div>`;
    }
    if (notice === "client-marked-one-time") {
      return `<div class="admin-alert admin-alert-info">Клиент отмечен как одноразовый, следующая уборка не запланирована.</div>`;
    }
