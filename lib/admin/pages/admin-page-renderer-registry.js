...
function createAdminPageRenderers(deps = {}) {
  ...
  // Client profiles must cover the same deep history as orders. Otherwise an
  // older completed order remains searchable while its client disappears from
  // the Clients workspace once newer ledger entries push it past the cutoff.
  const CLIENTS_QUOTE_OPS_PAGE_LIMIT = Math.max(Number(QUOTE_OPS_LEDGER_LIMIT) || 250, 1000);
  const STAFF_QUOTE_OPS_PAGE_LIMIT = Math.max(Number(QUOTE_OPS_LEDGER_LIMIT) || 250, 1000);
  ...
  const QUOTE_OPS_TASK_CLIENT_LEDGER_LIMIT = STAFF_QUOTE_OPS_PAGE_LIMIT;
  ...
  async function renderAdminAppPage(route, req, config, adminRuntime = {}, quoteOpsLedger = null, staffStore = null) {
    ...
    if (
      route === ADMIN_ORDERS_PATH &&
      reqUrl.searchParams.get("fragment") === "order-dialog" &&
      typeof renderOrdersPage.renderDialogFragment === "function"
    ) {
      return renderOrdersPage.renderDialogFragment(
        req,
        config,
        quoteOpsLedger,
        staffStore,
    ...
    if (
      route === ADMIN_QUOTE_OPS_PATH &&
      reqUrl.searchParams.get("fragment") === "entry-dialog" &&
      typeof renderQuoteOpsPage.renderDialogFragment === "function"
    ) {
      return renderQuoteOpsPage.renderDialogFragment(
        req,
        config,
        quoteOpsLedger,
        adminRuntime,
    ...
    const messageEntryLimit =
      route === ADMIN_ORDERS_PATH
        ? Math.max(Number(QUOTE_OPS_LEDGER_LIMIT) || 250, 1000)
        : route === ADMIN_QUOTE_OPS_PATH
          ? reqUrl.searchParams.get("section") === "tasks"
            ? QUOTE_OPS_TASK_CLIENT_LEDGER_LIMIT
            : QUOTE_OPS_PAGE_LEDGER_LIMIT
        : Math.min(QUOTE_OPS_LEDGER_LIMIT || 250, 500);
    const messageEntries = quoteOpsLedger
      ? await quoteOpsLedger.listEntries({
          limit: messageEntryLimit,
          includeHidden:
            route === ADMIN_QUOTE_OPS_PATH && reqUrl.searchParams.get("section") === "tasks",
        })
      : [];
    adminRuntime.unreadMessageCount = 0;
    ...
    adminRuntime.newLeadTaskCount = buildQuoteOpsTaskRecords(messageEntries)
      ...
      .length;
    if (route === ADMIN_ORDERS_PATH) {
      adminRuntime.orderPageEntries = messageEntries;
    }
    if (route === ADMIN_QUOTE_OPS_PATH) {
      adminRuntime.quoteOpsPageEntries = messageEntries;
    }
