...
function createOrdersPageHelpers(deps = {}) {
  ...
  function renderOrdersNotice(req) {
    ...
    if (notice === "next-order-created") {
      return `<div class="admin-alert admin-alert-info">Следующий визит создан. Его можно открыть и назначить команду.</div>`;
    }
    ...
    if (notice === "next-order-unavailable") {
      return `<div class="admin-alert admin-alert-error">Следующий визит можно создать только для regular-заказа с Weekly, Bi-weekly или Monthly.</div>`;
    }
