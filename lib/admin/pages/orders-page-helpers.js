...
function createOrdersPageHelpers(deps = {}) {
  ...
  function renderOrdersNotice(req) {
    ...
    if (notice === "recurring-series-restored") {
      return `<div class="admin-alert admin-alert-info">Эта и все последующие уборки восстановлены вместе с назначенной командой.</div>`;
    }
    if (notice === "recurring-series-restore-unavailable") {
      return `<div class="admin-alert admin-alert-error">Не удалось восстановить серию уборок. Проверьте повторяемость заказа.</div>`;
    }
    if (notice === "order-form-fields-saved") {
      return `<div class="admin-alert admin-alert-info">Поля из формы клиента обновлены.</div>`;
    }
    if (notice === "next-order-created") {
      return `<div class="admin-alert admin-alert-info">Следующий визит создан и добавлен в «Запланировано». Его можно открыть и назначить команду.</div>`;
    }
    if (notice === "next-order-updated") {
      return `<div class="admin-alert admin-alert-info">Следующий визит обновлён и остаётся в «Запланировано».</div>`;
    }
    if (notice === "next-order-already-created") {
      return `<div class="admin-alert admin-alert-info">Следующий визит для этого заказа уже был создан.</div>`;
    }
    if (notice === "next-order-unavailable") {
      return `<div class="admin-alert admin-alert-error">Не удалось создать следующий визит. Укажите дату следующей уборки или повторяемость.</div>`;
    }
    if (notice === "next-order-failed") {
      return `<div class="admin-alert admin-alert-error">Не удалось создать следующий визит. Попробуйте ещё раз.</div>`;
    }
    if (notice === "order-saved-invoice-sms-sent") {
      return `<div class="admin-alert admin-alert-info">Заказ обновлён, клиенту отправлена SMS со ссылкой на оплату через Stripe.</div>`;
    }
    if (notice === "order-saved-invoice-sms-missing-recipient") {
      return `<div class="admin-alert admin-alert-error">Заказ обновлён, но у клиента не указан телефон для SMS со ссылкой на оплату.</div>`;
    }
    if (notice === "order-saved-invoice-sms-unavailable") {
      return `<div class="admin-alert admin-alert-error">Заказ обновлён, но автоматическая отправка SMS со ссылкой на оплату сейчас недоступна.</div>`;
    }
    if (notice === "order-saved-invoice-sms-failed") {
      return `<div class="admin-alert admin-alert-error">Заказ обновлён, но SMS со ссылкой на оплату отправить не удалось.</div>`;
    }
    ...
    if (notice === "order-saved-policy-already-signed") {
      return `<div class="admin-alert admin-alert-info">Заказ обновлён. Политика уже была подписана этим клиентом, повторная отправка не нужна.</div>`;
    }
    ...
    if (notice === "order-saved-calendar-policy-already-signed") {
      return `<div class="admin-alert admin-alert-info">Заказ обновлён. Политика уже была подписана этим клиентом, повторная отправка не нужна, но синхронизация Google Calendar не обновилась.</div>`;
    }
    ...
    if (notice === "order-review-request-sent") {
      return `<div class="admin-alert admin-alert-info">Запрос отзыва отправлен клиенту повторно по email и SMS.</div>`;
    }
    if (notice === "order-review-request-email-only") {
      return `<div class="admin-alert admin-alert-info">Запрос отзыва отправлен клиенту повторно по email. SMS не удалось отправить.</div>`;
    }
    if (notice === "order-review-request-sms-only") {
      return `<div class="admin-alert admin-alert-info">Запрос отзыва отправлен клиенту повторно по SMS. Email у клиента не указан или недоступен.</div>`;
    }
    if (notice === "order-review-request-missing-recipient") {
      return `<div class="admin-alert admin-alert-error">Не удалось попросить отзыв: у клиента нет email и телефона.</div>`;
    }
    if (notice === "order-review-request-status-unavailable") {
      return `<div class="admin-alert admin-alert-error">Запрос отзыва можно отправить только для оплаченных, ожидающих отзыв или завершённых заказов.</div>`;
    }
    if (notice === "order-review-request-unavailable") {
      return `<div class="admin-alert admin-alert-error">Не удалось попросить отзыв: сервис уведомлений сейчас недоступен.</div>`;
    }
    if (notice === "order-review-request-failed") {
      return `<div class="admin-alert admin-alert-error">Не удалось отправить запрос отзыва повторно. Проверьте email/SMS-настройки и попробуйте ещё раз.</div>`;
    }
    ...
    if (notice === "manual-order-duration-invalid") {
      return `<div class="admin-alert admin-alert-error">Чтобы добавить заказ вручную, укажите длительность уборки в часах и минутах.</div>`;
    }
    if (notice === "order-duration-required") {
      return `<div class="admin-alert admin-alert-error">Укажите длительность уборки в часах и минутах перед сохранением заказа.</div>`;
    }
