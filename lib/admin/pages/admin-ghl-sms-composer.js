...
function createAdminGhlSmsComposerHelpers(deps = {}) {
  ...
  function sortSmsHistoryEntriesForThread(left, right) {
    const leftMs = Date.parse(left && left.sentAt ? left.sentAt : "");
    const rightMs = Date.parse(right && right.sentAt ? right.sentAt : "");
    if (Number.isFinite(leftMs) && Number.isFinite(rightMs)) {
      return leftMs - rightMs;
    }
    return normalizeString(left && left.sentAt, 80).localeCompare(normalizeString(right && right.sentAt, 80));
  }
  ...
  function renderAdminSmsHistoryItems(entries = []) {
    ...
    return `<div class="admin-history-list admin-ghl-sms-history-list${scrollClass}" data-admin-ghl-sms-history-list>
      ...
      ${entries
        .slice()
        .sort(sortSmsHistoryEntriesForThread)
        .map((entry) => {
          const sentAtLabel =
          ...
          const bubbleClass = isInbound ? "is-inbound" : "is-outbound";
          const metaLabels = [
            isInbound ? "Клиент" : "SHYNLI",
            formatAdminSmsHistorySourceLabel(entry.source),
            entry.status === "failed" ? "Не доставлено" : "",
          ]
            .filter(Boolean)
            .join(" · ");
          const message = normalizeString(entry.message, 1000);
          const recipientName = normalizeString(entry.recipientName, 200);
          const recipientMeta = [
            ...
            .filter(Boolean)
            .join(" · ");
          const errorMessage = normalizeString(entry.errorMessage, 300);
          const shouldShowError =
            entry.status === "failed" && errorMessage && errorMessage !== message;
          return `<article
            class="admin-history-item admin-client-history-item admin-ghl-sms-history-item ${bubbleClass}"
          ...
          >
            <div class="admin-ghl-sms-history-bubble">
              <p class="admin-client-history-address admin-ghl-sms-history-message">${escapeHtml(message)}</p>
              <div class="admin-ghl-sms-message-meta">
                <span>${escapeHtml(metaLabels)}</span>
                <span>${escapeHtml(sentAtLabel)}</span>
              </div>
            </div>
            ${recipientMeta ? `<p class="admin-ghl-sms-recipient-meta">${escapeHtml(recipientMeta)}</p>` : ""}
            ${shouldShowError ? `<p class="admin-history-copy">${escapeHtml(errorMessage)}</p>` : ""}
          </article>`;
        })
        .join("")}
    </div>`;
  ...
  function renderAdminGhlSmsComposer(options = {}) {
    ...
    return renderAdminCard(
      ...
      `<div class="admin-ghl-sms-layout"
        ...
        <section class="admin-ghl-sms-history-column admin-ghl-sms-chat-panel">
          <div class="admin-subsection-head admin-ghl-sms-chat-head">
            <div>
              <h3 class="admin-subsection-title">История SMS</h3>
              <p class="admin-action-hint">Входящие и исходящие SMS в одной переписке.</p>
              <span class="admin-sr-only">Входящие Исходящие Автоматические Ручные</span>
            </div>
          ...
          ${!hasPhone && !hasContact
            ...
            : `<form class="admin-form admin-ghl-sms-form admin-ghl-sms-chat-form" method="post" action="${escapeHtmlAttribute(actionPath)}" data-admin-ghl-sms="true">
                ...
                <label class="admin-label admin-ghl-sms-chat-input">
                  <span class="admin-sr-only">Сообщение клиенту</span>
                  <textarea class="admin-input" name="message" rows="3" maxlength="1000" placeholder="${escapeHtmlAttribute(messagePlaceholder)}">${escapeHtml(composerState.draft)}</textarea>
                </label>
                <div class="admin-ghl-sms-chat-actions">
                  <span class="admin-action-hint">${escapeHtml(hasPhone ? `Через GHL на ${phoneLabel}` : "Через контакт Go High Level")}</span>
                  <button class="admin-button" type="submit">Отправить</button>
                </div>
              </form>`}
