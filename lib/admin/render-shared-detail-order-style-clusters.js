...
[data-admin-order-schedule-editor][hidden],
[data-admin-order-schedule-action-for][hidden],
[data-admin-order-schedule-edit-trigger][hidden],
...
.admin-order-policy-link-card {
  display: grid;
  gap: 12px;
  padding: 14px;
  border: 1px solid rgba(158, 67, 90, 0.16);
  border-radius: 14px;
  background: rgba(158, 67, 90, 0.06);
}
.admin-order-policy-link-card .admin-form-actions {
  align-items: stretch;
}
.admin-order-policy-link-card .admin-input {
...
.admin-order-summary-card-schedule {
  position: relative;
}
.admin-order-summary-card-team:has([data-admin-order-team-editor]:not([hidden])),
.admin-order-summary-card-schedule:has([data-admin-order-schedule-editor]:not([hidden])) {
  grid-column: 1 / -1;
  z-index: 12;
}
...
.admin-order-schedule-editor-fields {
  gap: 8px;
}
