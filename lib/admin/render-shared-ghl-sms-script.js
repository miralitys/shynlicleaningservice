...
const ADMIN_SHARED_GHL_SMS_SCRIPT = `        const ADMIN_GHL_SMS_POLL_INTERVAL_MS = 12000;
        ...
        function renderAdminSmsHistoryListHtml(items) {
          ...
          return '<div class="admin-history-list admin-ghl-sms-history-list' + scrollClass + '" data-admin-ghl-sms-history-list>' + previewHint + sortedItems.map((item) => {
        ...
        function scrollAdminGhlSmsHistoryToBottom(target) {
          const root = resolveAdminGhlSmsRoot(target);
          if (!(root instanceof HTMLElement)) return;
          const lists = Array.from(root.querySelectorAll("[data-admin-ghl-sms-history-list]"));
        ...
        function scrollAdminGhlSmsHistoryWithin(scope) {
          ...
          targetScope.querySelectorAll("[data-admin-ghl-sms-root]").forEach((root) => {
            scrollAdminGhlSmsHistoryToBottom(root);
          });
        }
