...
function createAdminDomainFormatters(deps = {}) {
  ...
  function formatAdminServiceLabel(value) {
    ...
    if (normalized.includes("estimate")) return "Free in-home estimate";
    const compact = normalized.replace(/[\s_/-]+/g, "");
    if (compact === "movein" || compact === "moveincleaning") return "Move-in";
    if (compact === "moveout" || compact === "moveoutcleaning") return "Move-out";
    if (normalized.includes("moving") || normalized.includes("move")) return "Move-in/out";
  ...
  function renderAssignmentStatusBadge(status) {
    ...
    if (status === "issue" || status === "canceled") {
