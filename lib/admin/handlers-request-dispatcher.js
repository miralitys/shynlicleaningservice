...
function createAdminRequestHandler(deps = {}) {
  ...
  // Client mutations must search the same deep history as the Clients page.
  // Otherwise an older client can be displayed but saving its addresses fails
  // with client-missing because the POST handler only scans recent entries.
  const CLIENT_MUTATION_LEDGER_LIMIT = Math.max(Number(QUOTE_OPS_LEDGER_LIMIT) || 250, 1000);
  ...
  const ADMIN_MESSAGES_REMOTE_SYNC_LIMIT = Math.min(80, Math.max(1, Number(QUOTE_OPS_LEDGER_LIMIT || 250)));

  function getLatestSmsHistoryTime(entry = {}) {
    return getEntrySmsHistoryEntries(entry).reduce((latestTime, message) => {
      const parsed = Date.parse(normalizeString(message && (message.sentAt || message.createdAt), 80));
      return Number.isFinite(parsed) && parsed > latestTime ? parsed : latestTime;
    }, 0);
  }

  async function syncAdminMessagesRemoteHistories(quoteOpsLedger, leadConnectorClient) {
    if (
      !quoteOpsLedger ||
    ...
    let entries = [];
    try {
      entries = await quoteOpsLedger.listEntries({
        limit: Math.min(Number(QUOTE_OPS_LEDGER_LIMIT || 250) || 250, 500),
      });
    } catch {
      return;
    }
    ...
    const candidates = (Array.isArray(entries) ? entries : [])
      ...
      .filter((entry) => getEntrySmsHistoryEntries(entry).length > 0)
      .sort((left, right) => getLatestSmsHistoryTime(right) - getLatestSmsHistoryTime(left))
      .slice(0, ADMIN_MESSAGES_REMOTE_SYNC_LIMIT);
    ...
    for (const entry of candidates) {
      ...
      if (!remoteHistoryEntries.length) continue;

      const mergedHistoryEntries = mergeAdminSmsHistoryEntries(localHistoryEntries, remoteHistoryEntries);
      if (JSON.stringify(mergedHistoryEntries) === JSON.stringify(localHistoryEntries)) continue;
      ...
      try {
        ...
        if (isOrderEntry && typeof quoteOpsLedger.updateOrderEntry === "function") {
          await quoteOpsLedger.updateOrderEntry(entryId, { smsHistory: mergedHistoryEntries });
        } else if (typeof quoteOpsLedger.updateLeadEntry === "function") {
          await quoteOpsLedger.updateLeadEntry(entryId, { smsHistory: mergedHistoryEntries });
        } else if (typeof quoteOpsLedger.updateClientEntry === "function") {
          await quoteOpsLedger.updateClientEntry(entryId, { smsHistory: mergedHistoryEntries });
        }
      } catch {
        // Messages page must stay available even if a remote sync write fails.
      }
    }
  }
  ...
  return async function handleAdminRequest(
    ...
    if (ADMIN_APP_ROUTES.has(requestContext.route)) {
      ...
      if (currentUserAccess.authorized) {
        if (requestContext.route === ADMIN_MESSAGES_PATH) {
          await syncAdminMessagesRemoteHistories(quoteOpsLedger, leadConnectorClient);
        }
        writeHtmlWithTiming(
          res,
          200,
