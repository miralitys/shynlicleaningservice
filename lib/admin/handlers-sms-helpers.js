...
function createAdminSmsHelpers(deps = {}) {
  ...
  async function sendOrderPolicyAcceptanceInvite(
    ...
    if (options.respectClientAutomaticNotifications === true) {
      let automaticNotificationsEnabled = getClientAutomaticNotificationsPreference(updatedEntry);
      if (
        automaticNotificationsEnabled === null &&
        quoteOpsLedger &&
        typeof quoteOpsLedger.listEntries === "function"
      ) {
        try {
          const entries = await quoteOpsLedger.listEntries({ limit: 5000 });
          automaticNotificationsEnabled = resolveClientAutomaticNotificationsEnabled(
            updatedEntry,
            entries
          );
        } catch {
      ...
      if (automaticNotificationsEnabled === false) {
        return {
          updatedEntry,
          pendingAcceptance,
          emailState: "notifications-disabled",
          smsState,
          followUpSmsState,
        };
      }
