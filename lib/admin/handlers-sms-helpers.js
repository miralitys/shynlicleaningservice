...
function createAdminSmsHelpers(deps = {}) {
  ...
  function hasOrderPolicyFollowUpSms(entry) {
    return getEntrySmsHistoryEntries(entry).some((item) =>
      ...
      if (policySmsMessage) {
        try {
          ...
          if (smsResult && smsResult.ok) {
            ...
            if (!hasOrderPolicyFollowUpSms(updatedEntry)) {
              const followUpSmsMessage = buildOrderPolicyFollowUpSmsMessage(updatedEntry);
              try {
                const result = await leadConnectorClient.sendSmsMessage({
                ...
                if (result && result.ok) {
                  followUpSmsResult = result;
                  followUpSmsHistoryRecord = buildSmsHistoryRecord(result, {
                    message: followUpSmsMessage,
                    phone: normalizeString(updatedEntry && updatedEntry.customerPhone, 80),
                    targetType: "order",
                    targetRef: entryId,
