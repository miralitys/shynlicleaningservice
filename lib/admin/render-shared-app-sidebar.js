...
function formatNewQuoteCountLabel(count) {
  ...
  if (mod10 === 1 && mod100 !== 11) return `${normalized} новая`;
  if (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14)) return `${normalized} новые`;
  return `${normalized} новых`;
}
...
function renderAdminAppSidebarItem({
  ...
  const unreadMessageCount = Math.max(0, Number.parseInt(String(messageCount || 0), 10) || 0);
  const newQuoteCount = Math.max(0, Number.parseInt(String(quoteCount || 0), 10) || 0);
  const messageCountBadge =
    isMessagesItem && unreadMessageCount > 0
      ? `<span class="admin-nav-count" aria-label="${escapeHtml(String(unreadMessageCount))} непрочитанных сообщений">${escapeHtml(String(unreadMessageCount))}</span>`
      : "";
  ...
  const quoteOpsSubnav = isQuoteOpsItem
    ? `<div class="admin-nav-sublinks">
                    <a class="admin-nav-sublink${activeQuoteOpsSection === "funnel" ? " admin-nav-sublink-active" : ""}" href="/admin/quote-ops?section=funnel">Статус</a>
