...
const ADMIN_SHARED_DIALOG_LOGOUT_SCRIPT = `        const dialogSelector = "dialog.admin-dialog";
        ...
        function shouldConfirmRecurringComment(form, submitter) {
          ...
          return JSON.stringify(getFieldCurrentValues(quoteForm, "quoteAdditionalDetails")) !==
            JSON.stringify(getFieldInitialValues(quoteForm, "quoteAdditionalDetails"));
        }
        ...
        function setSaveConfirmMode(mode) {
          ...
          if (saveConfirmCancel instanceof HTMLElement) saveConfirmCancel.hidden = reminderMode || recurringCommentMode;
          if (reminderConfirmSkip instanceof HTMLElement) {
            reminderConfirmSkip.hidden = !reminderMode && !recurringCommentMode;
            reminderConfirmSkip.textContent = recurringCommentMode ? "Только для этой уборки" : "Don’t send";
          }
          if (saveConfirmAccept instanceof HTMLElement) {
            saveConfirmAccept.textContent = recurringCommentMode
          ...
          if (saveConfirmDialog) {
            saveConfirmDialog.setAttribute(
              ...
              recurringCommentMode ? "recurring-comment" : reminderMode ? "reminder" : "save"
            );
          }
        }
