...
const ADMIN_SHARED_DIALOG_LOGOUT_SCRIPT = `        const dialogSelector = "dialog.admin-dialog";
        ...
        function setSaveConfirmMode(mode) {
          ...
          if (saveConfirmTitle) {
            saveConfirmTitle.textContent = reminderMode
              ? "An automatic reminder for this appointment has already been sent."
              : "Сохранить?";
          }
          if (saveConfirmCopy) {
            saveConfirmCopy.textContent = reminderMode
              ? "Would you like to send an updated reminder to the client?"
              : "Изменения сохранятся только после подтверждения.";
          }
          if (saveConfirmCancel instanceof HTMLElement) saveConfirmCancel.hidden = reminderMode;
          if (reminderConfirmSkip instanceof HTMLElement) reminderConfirmSkip.hidden = !reminderMode;
          if (saveConfirmAccept instanceof HTMLElement) {
            saveConfirmAccept.textContent = reminderMode ? "Send updated reminder" : "Сохранить";
          }
          if (saveConfirmDialog) {
            saveConfirmDialog.setAttribute("data-admin-save-confirm-mode", reminderMode ? "reminder" : "save");
          }
        }
        ...
        document.addEventListener("submit", (event) => {
          ...
          if (saveConfirmDialog) {
            setSaveConfirmMode(
              shouldConfirmUpdatedReminder(form, event.submitter || null) ? "reminder" : "save"
            );
            openDialog(saveConfirmDialog);
            return;
          }
