...
const ADMIN_SHARED_DIALOG_LOGOUT_SCRIPT = `        const dialogSelector = "dialog.admin-dialog";
        ...
        const saveConfirmTitle = document.getElementById("admin-save-confirm-title");
        const saveConfirmCopy = document.getElementById("admin-save-confirm-copy");
        const saveConfirmCancel = saveConfirmDialog
          ? saveConfirmDialog.querySelector("[data-admin-save-confirm-cancel]")
          : null;
        const saveConfirmAccept = saveConfirmDialog
          ? saveConfirmDialog.querySelector("[data-admin-save-confirm-accept]")
          : null;
        const reminderConfirmSkip = saveConfirmDialog
          ? saveConfirmDialog.querySelector("[data-admin-reminder-confirm-skip]")
          : null;
        const adminDialogLoadPromises = new Map();
        ...
        function syncAdminRecurringEditScope(scopeSelect) {
          if (!(scopeSelect instanceof HTMLSelectElement)) return;
          const dialog = scopeSelect.closest(dialogSelector);
          if (!(dialog instanceof HTMLElement)) return;
          dialog.querySelectorAll("[data-admin-recurring-edit-scope-mirror]").forEach((field) => {
            if (field instanceof HTMLInputElement) field.value = scopeSelect.value;
          });
        }
        ...
        function setUpdatedReminderAction(form, action) {
          if (!(form instanceof HTMLFormElement)) return;
          let field = form.elements.namedItem("updatedReminderAction");
          if (!(field instanceof HTMLInputElement)) {
            field = document.createElement("input");
            field.type = "hidden";
            field.name = "updatedReminderAction";
            form.appendChild(field);
          }
          field.value = action === "send" ? "send" : "skip";
        }

        function setRecurringCommentScope(form, scope) {
          if (!(form instanceof HTMLFormElement)) return;
          let field = form.elements.namedItem("commentEditScope");
          if (!(field instanceof HTMLInputElement)) {
            field = document.createElement("input");
            field.type = "hidden";
            field.name = "commentEditScope";
            form.appendChild(field);
          }
          field.value = scope === "future" ? "future" : "current";
        }

        function setRecurringEditScope(form, scope) {
          if (!(form instanceof HTMLFormElement)) return;
          let field = form.elements.namedItem("recurringEditScope");
          if (!(field instanceof HTMLInputElement)) {
            field = document.createElement("input");
            field.type = "hidden";
            field.name = "recurringEditScope";
            form.appendChild(field);
          }
          field.value = scope === "future" ? "future" : "current";
        }

        function getFieldCurrentValues(form, name) {
          if (!(form instanceof HTMLFormElement)) return [];
        ...
        function getFieldInitialValues(form, name) {
          if (!(form instanceof HTMLFormElement)) return [];
          return Array.from(form.elements)
            .filter((field) => field && field.name === name)
            .map((field) => {
              ...
              if (field instanceof HTMLSelectElement) {
                ...
                return selected.length > 0 ? selected.map((option) => option.value).join("|") : field.options[0]?.value || "";
              }
              return typeof field.defaultValue === "string" ? field.defaultValue : "";
            });
        }

        function hasReminderSensitiveChanges(form) {
          if (!(form instanceof HTMLFormElement)) return false;
          const names = String(form.getAttribute("data-admin-reminder-fields") || "")
            .split(",")
            .map((name) => name.trim())
            .filter(Boolean);
          return names.some((name) =>
            JSON.stringify(getFieldCurrentValues(form, name)) !==
            JSON.stringify(getFieldInitialValues(form, name))
          );
        }

        function shouldConfirmUpdatedReminder(form, submitter) {
          if (!(form instanceof HTMLFormElement)) return false;
          if (form.getAttribute("data-admin-reminder-confirm") !== "true") return false;
          if (hasReminderSensitiveChanges(form)) return true;
          const quoteFormId = submitter instanceof HTMLElement
            ? submitter.getAttribute("data-admin-order-quote-form") || ""
            : "";
          ...
          return Boolean(
            ...
            quoteForm.getAttribute("data-admin-reminder-confirm") === "true" &&
            quotePanel &&
            !quotePanel.hidden &&
            hasReminderSensitiveChanges(quoteForm)
          );
        }

        function shouldConfirmRecurringComment(form, submitter) {
          if (!(form instanceof HTMLFormElement)) return false;
          let quoteForm = form;
          const quoteFormId = submitter instanceof HTMLElement
            ? submitter.getAttribute("data-admin-order-quote-form") || ""
            : "";
          if (quoteFormId) {
            const linkedQuoteForm = document.getElementById(quoteFormId);
            const quotePanel = linkedQuoteForm instanceof HTMLFormElement
              ? linkedQuoteForm.closest("[data-admin-toggle-panel]")
              : null;
            if (!(linkedQuoteForm instanceof HTMLFormElement) || !quotePanel || quotePanel.hidden) return false;
            quoteForm = linkedQuoteForm;
          }
          if (quoteForm.getAttribute("data-admin-recurring-comment-confirm") !== "true") return false;
          return getFieldCurrentValues(quoteForm, "quoteAdditionalDetails").length > 0;
        }

        function shouldConfirmRecurringEdit(form) {
          return Boolean(
            form instanceof HTMLFormElement &&
            form.getAttribute("data-admin-recurring-edit-confirm") === "true"
          );
        }

        function setSaveConfirmMode(mode) {
          const reminderMode = mode === "reminder";
          const recurringCommentMode = mode === "recurring-comment";
          const recurringEditMode = mode === "recurring-edit";
          if (saveConfirmTitle) {
            saveConfirmTitle.textContent = recurringCommentMode
              ? "К каким уборкам применить комментарий?"
              : recurringEditMode
                ? "К каким уборкам применить изменения?"
              : reminderMode
                ? "An automatic reminder for this appointment has already been sent."
                : "Сохранить?";
          }
          if (saveConfirmCopy) {
            saveConfirmCopy.textContent = recurringCommentMode
              ? "Выберите, сохранить его только для этой уборки или также для всех последующих уборок серии."
              : recurringEditMode
                ? "Выберите, изменить только эту уборку или также все последующие уборки серии."
              : reminderMode
                ? "Would you like to send an updated reminder to the client?"
                : "Изменения сохранятся только после подтверждения.";
          }
          if (saveConfirmCancel instanceof HTMLElement) saveConfirmCancel.hidden = reminderMode || recurringCommentMode || recurringEditMode;
          if (reminderConfirmSkip instanceof HTMLElement) {
            reminderConfirmSkip.hidden = !reminderMode && !recurringCommentMode && !recurringEditMode;
            reminderConfirmSkip.textContent = recurringCommentMode || recurringEditMode ? "Только для этой уборки" : "Don’t send";
          }
          if (saveConfirmAccept instanceof HTMLElement) {
            saveConfirmAccept.textContent = recurringCommentMode || recurringEditMode
              ? "Для этой и всех следующих"
              : reminderMode
                ? "Send updated reminder"
                : "Сохранить";
          }
          if (saveConfirmDialog) {
            saveConfirmDialog.setAttribute(
              "data-admin-save-confirm-mode",
              recurringCommentMode ? "recurring-comment" : recurringEditMode ? "recurring-edit" : reminderMode ? "reminder" : "save"
            );
          }
        }
        ...
        document.addEventListener("submit", (event) => {
          ...
          if (saveConfirmDialog) {
            ...
            setSaveConfirmMode(
              shouldConfirmRecurringComment(form, submitter)
                ? "recurring-comment"
                : shouldConfirmRecurringEdit(form)
                  ? "recurring-edit"
                : shouldConfirmUpdatedReminder(form, submitter)
                  ? "reminder"
                  : "save"
            );
            openDialog(saveConfirmDialog);
...
const ADMIN_SHARED_DIALOG_STATE_SCRIPT = `        function updateDialogState() {
        ...
        function bindAdminDialog(dialog) {
          if (!(dialog instanceof HTMLElement)) return;
          if (dialog.getAttribute("data-admin-dialog-runtime-bound") === "true") return;
          dialog.setAttribute("data-admin-dialog-runtime-bound", "true");
        ...
        function initializeAdminDialogContent(dialog) {
          if (!(dialog instanceof HTMLElement)) return;
          bindAdminDialog(dialog);
          syncAdminPhoneInputs(dialog);
          if (typeof window.__adminBindAddressAutocomplete === "function") {
            window.__adminBindAddressAutocomplete(dialog);
          }
          if (typeof window.__adminBindQuoteOpsDialogContent === "function") {
            window.__adminBindQuoteOpsDialogContent(dialog);
          }
          dialog.querySelectorAll("[data-admin-order-status-select]").forEach((statusSelect) => {
            syncAdminOrderCancelScope(statusSelect);
          });
          dialog.querySelectorAll("[data-admin-toggle-target]").forEach((trigger) => {
        ...
        async function loadAdminDialogForTrigger(trigger) {
          if (!(trigger instanceof HTMLElement)) return null;
          const dialogId = trigger.getAttribute("data-admin-dialog-open") || "";
          if (!dialogId) return null;
          const existingDialog = document.getElementById(dialogId);
          if (existingDialog) return existingDialog;

          const fragmentUrl = trigger.getAttribute("data-admin-dialog-url") || "";
          if (!fragmentUrl) return null;
          if (adminDialogLoadPromises.has(dialogId)) {
            return adminDialogLoadPromises.get(dialogId);
          }

          const loadPromise = (async () => {
            trigger.setAttribute("aria-busy", "true");
            trigger.classList.add("admin-dialog-trigger-loading");
            const response = await window.fetch(fragmentUrl, {
              method: "GET",
              headers: { Accept: "text/html" },
              credentials: "same-origin",
            });
            if (!response.ok) throw new Error("dialog-load-failed");
            const markup = await response.text();
            const template = document.createElement("template");
            template.innerHTML = markup.trim();
            const loadedDialog = Array.from(template.content.querySelectorAll("dialog")).find(
              (dialog) => dialog.id === dialogId
            );
            if (!(loadedDialog instanceof HTMLElement)) throw new Error("dialog-missing");
            const host =
              document.querySelector("[data-admin-dialog-host]") ||
              document.querySelector("[data-admin-order-dialog-host]") ||
              document.body;
            if (
              host.hasAttribute &&
              (host.hasAttribute("data-admin-dialog-host") || host.hasAttribute("data-admin-order-dialog-host"))
            ) {
              host.replaceChildren(template.content);
            } else {
              host.appendChild(template.content);
            }
            const insertedDialog = document.getElementById(dialogId);
            initializeAdminDialogContent(insertedDialog);
            return insertedDialog;
          })();
          adminDialogLoadPromises.set(dialogId, loadPromise);

          try {
            return await loadPromise;
          } finally {
            adminDialogLoadPromises.delete(dialogId);
            trigger.removeAttribute("aria-busy");
            trigger.classList.remove("admin-dialog-trigger-loading");
          }
        }

        async function openAdminDialogFromTrigger(trigger) {
          try {
            const dialog = await loadAdminDialogForTrigger(trigger);
            if (!dialog) return;
            openDialog(dialog, {
              focusSelector: trigger.getAttribute("data-admin-dialog-focus") || "",
            });
          } catch (error) {
            void error;
            const fallbackUrl = trigger.getAttribute("data-admin-dialog-fallback-url") || "";
        ...
        function mirrorAdminOrderQuoteFields(primarySaveTrigger) {
          ...
          for (const [name, value] of new FormData(quoteForm).entries()) {
            if (["action", "entryId", "returnTo", "recurringEditScope"].includes(name) || typeof value !== "string") continue;
        ...
        document.addEventListener("click", (event) => {
          ...
          if (saveConfirmAccept) {
            ...
            if (
              saveConfirmDialog &&
              saveConfirmDialog.getAttribute("data-admin-save-confirm-mode") === "reminder" &&
              pendingSaveConfirmSubmission &&
              pendingSaveConfirmSubmission.form instanceof HTMLFormElement
            ) {
              setUpdatedReminderAction(pendingSaveConfirmSubmission.form, "send");
            }
            if (
              saveConfirmDialog &&
              saveConfirmDialog.getAttribute("data-admin-save-confirm-mode") === "recurring-comment" &&
              pendingSaveConfirmSubmission &&
              pendingSaveConfirmSubmission.form instanceof HTMLFormElement
            ) {
              setRecurringCommentScope(pendingSaveConfirmSubmission.form, "future");
            }
            if (
              saveConfirmDialog &&
              saveConfirmDialog.getAttribute("data-admin-save-confirm-mode") === "recurring-edit" &&
              pendingSaveConfirmSubmission &&
              pendingSaveConfirmSubmission.form instanceof HTMLFormElement
            ) {
              setRecurringEditScope(pendingSaveConfirmSubmission.form, "future");
            }
            submitAdminSaveConfirmedForm();
            closeDialog(saveConfirmDialog);
            setSaveConfirmMode("save");
          ...
          const reminderSkip = event.target.closest("[data-admin-reminder-confirm-skip]");
          if (reminderSkip) {
            if (
              pendingSaveConfirmSubmission &&
              pendingSaveConfirmSubmission.form instanceof HTMLFormElement
              ...
              if (
                ...
                setRecurringCommentScope(pendingSaveConfirmSubmission.form, "current");
              } else if (
                saveConfirmDialog &&
                saveConfirmDialog.getAttribute("data-admin-save-confirm-mode") === "recurring-edit"
              ) {
                setRecurringEditScope(pendingSaveConfirmSubmission.form, "current");
              } else {
                setUpdatedReminderAction(pendingSaveConfirmSubmission.form, "skip");
              }
            }
            submitAdminSaveConfirmedForm();
            closeDialog(saveConfirmDialog);
          ...
          if (openTrigger) {
            ...
            void openAdminDialogFromTrigger(openTrigger);
        ...
        document.addEventListener("keydown", (event) => {
          if ((event.key === "Enter" || event.key === " ") && event.target instanceof HTMLElement) {
            ...
            if (rowOpenTrigger && event.target === rowOpenTrigger) {
              event.preventDefault();
              void openAdminDialogFromTrigger(rowOpenTrigger);
        ...
        document.addEventListener("change", (event) => {
          ...
          const recurringScopeSelect = event.target instanceof Element
            ? event.target.closest("[data-admin-recurring-edit-scope]")
            : null;
          if (recurringScopeSelect) syncAdminRecurringEditScope(recurringScopeSelect);
        });

        document.querySelectorAll(dialogSelector).forEach((dialog) => bindAdminDialog(dialog));
