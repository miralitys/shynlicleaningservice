...
function createAdminOrdersCreateHandlers(deps = {}) {
  ...
  const { ensureRecurringOrderSeries } = createAdminOrdersRecurringHelpers(deps);
  ...
  async function handleCreateManualOrder(context = {}) {
    ...
    const selectedClientAutomaticNotificationsEnabled = getFormValue(
      formBody,
      "selectedClientAutomaticNotificationsEnabled",
      8
    );
    ...
    try {
      const createdEntry = await quoteOpsLedger.recordSubmission({
        ...
        payloadForRetry: {
          ...(selectedClientAddressMeta || selectedClientAutomaticNotificationsEnabled
            ? {
                adminClient: {
                  ...(selectedClientAddressMeta ? { addressBook: [selectedClientAddressMeta] } : {}),
                  ...(selectedClientAutomaticNotificationsEnabled
                    ? {
