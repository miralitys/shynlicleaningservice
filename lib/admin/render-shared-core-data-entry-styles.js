...
const ADMIN_SHARED_CORE_DATA_ENTRY_STYLES = `
      ...
      .admin-time-picker-select option {
        color: #18181b;
        -webkit-text-fill-color: #18181b;
        background: #ffffff;
        font: inherit;
      }
      .admin-time-picker-actions {
        display: flex;
        justify-content: flex-end;
        gap: 8px;
        padding-top: 2px;
      }
      .admin-time-picker-action {
        width: 40px;
        height: 40px;
        border: 1px solid rgba(228, 228, 231, 0.96);
        border-radius: 14px;
      ...
      .admin-time-picker-action:hover {
        transform: translateY(-1px);
      }
      .admin-time-picker-action:focus-visible {
        outline: none;
        box-shadow: 0 0 0 3px rgba(158, 67, 90, 0.14);
      }
      .admin-time-picker-action-confirm {
        border-color: rgba(15, 118, 110, 0.22);
        background: rgba(15, 118, 110, 0.10);
        color: var(--success);
      }
      .admin-time-picker-action-cancel {
      ...
      .admin-manual-client-lookup {
        display: grid;
        gap: 10px;
        margin-bottom: 2px;
      }
      .admin-client-lookup-field {
        position: relative;
        min-width: 0;
      }
      .admin-client-lookup-input {
        padding-right: 42px;
      }
      .admin-client-lookup-field::after {
        content: "⌕";
        position: absolute;
        top: 50%;
        right: 15px;
      ...
      .admin-client-lookup-suggestions {
        position: absolute;
        top: calc(100% + 8px);
        left: 0;
        right: 0;
      ...
      .admin-client-lookup-suggestions[hidden] { display: none; }
      .admin-client-lookup-suggestion {
        width: 100%;
        padding: 12px 14px;
        border: 0;
        border-radius: 12px;
      ...
      .admin-client-lookup-suggestion:hover,
      .admin-client-lookup-suggestion:focus-visible {
        outline: none;
        background: rgba(158, 67, 90, 0.08);
      }
      .admin-client-lookup-suggestion-main {
        display: block;
        font-size: 14px;
        line-height: 1.45;
        font-weight: 700;
      }
      .admin-client-lookup-suggestion-copy,
      .admin-client-lookup-empty {
        display: block;
        margin-top: 4px;
        color: var(--muted);
        font-size: 12px;
      ...
      .admin-manual-client-selected[hidden] { display: none; }
      .admin-manual-client-selected-name {
        color: var(--foreground);
        font-size: 14px;
        line-height: 1.35;
      }
      .admin-manual-client-selected-meta,
      .admin-manual-client-selected-address {
        color: var(--muted);
        font-size: 12px;
        line-height: 1.45;
