      ...
      (() => {
        ...
        function renderAdminSmsHistoryListHtml(items) {
          ...
          return '<div class="admin-history-list admin-ghl-sms-history-list' + scrollClass + '">' + items.map((item) => {
            const sourceTone = item && item.source === "automatic" ? "admin-badge admin-badge-muted" : "admin-badge admin-badge-success";
        ...
        function applyAdminGhlSmsPayload(form, smsPayload) {
          const root = form instanceof HTMLFormElement ? form.closest("[data-admin-ghl-sms-root]") : null;
    ...
    return renderAdminLayout(
      ...
      `${errorBlock}
        ...
        <div class="admin-section-grid admin-form-grid-two">
          ${renderAdminCard(
            ...
            `<form class="admin-form" method="post" action="${ADMIN_LOGIN_PATH}" autocomplete="on">
              ...
              <div class="admin-inline-actions">
                ...
                <span class="admin-action-hint">Менеджеры и пользователи admin войдут сразу. Master-admin продолжит на шаг 2.</span>
              </div>
            </form>`,
            { eyebrow: "Вход" }
          )}
          ${renderAdminCard(
            "Дальше",
            "Код из приложения нужен только для master-admin входа.",
            `<ul class="admin-feature-list">
              <li>Менеджеры и admin-пользователи входят прямо по рабочей почте и паролю.</li>
              <li>Для owner/master-admin после пароля нужен текущий 6-значный код.</li>
      ...
      {
        subtitle: "Введите рабочую почту и пароль. Для master-admin затем понадобится код из приложения.",
        sidebar: renderAdminAuthSidebar("login"),
      }
    );
  }
