  ...
  <body>
    ...
    <script>
      (() => {
        ...
        function buildOrderMediaAssetUrl(form, entryId, assetId) {
        ...
        function setOrderCompletionFeedback(form, tone, message) {
          const feedback = form.querySelector("[data-admin-order-completion-feedback]");
        ...
        function setOrderCompletionPending(form, pending) {
          const submitButton = form.querySelector("[data-admin-order-completion-submit]");
          if (submitButton instanceof HTMLButtonElement) {
            const defaultLabel = submitButton.getAttribute("data-admin-default-label") || submitButton.textContent || "Сохранить отчёт клинера";
        ...
        function buildOrderCompletionRequest(form) {
        ...
        function updateOrderCompletionMedia(section, form, entryId, kind, assets) {
          ...
          mediaBody.innerHTML =
            ...
            items
              .map((asset, index) => {
                ...
                const mediaUrl = buildOrderMediaAssetUrl(form, entryId, assetId);
                return (
                  '<a class="admin-order-media-card" href="' +
                  escapeAdminHtml(mediaUrl) +
                  '" target="_blank" rel="noreferrer">' +
                  '<img class="admin-order-media-thumb" src="' +
                  escapeAdminHtml(mediaUrl) +
                  '" alt="' +
                  escapeAdminHtml(title + " " + String(index + 1)) +
                  '" loading="lazy">' +
                  '<span class="admin-order-media-caption">' +
        ...
        function applyOrderCompletionPayload(form, payload) {
          if (!(form instanceof HTMLFormElement) || !payload || !payload.completion) return;
          const section = form.closest(".admin-order-cleaner-section");
          const entryIdInput = form.elements.namedItem("entryId");
          const entryId = entryIdInput && "value" in entryIdInput ? String(entryIdInput.value || "") : "";
          const completion = payload.completion;

          const textarea = form.elements.namedItem("cleanerComment");
          if (textarea instanceof HTMLTextAreaElement) {
            textarea.value = String(completion.cleanerComment || "");
          }
          ...
          if (section instanceof HTMLElement) {
            updateOrderCompletionMedia(section, form, entryId, "before", completion.beforePhotos);
            updateOrderCompletionMedia(section, form, entryId, "after", completion.afterPhotos);
          ...
          setOrderCompletionFeedback(form, "info", payload.message || "Отчёт клинера сохранён.");
        }
        ...
        const asyncFormFeedbackTimers = new WeakMap();
        ...
        document.addEventListener("submit", async (event) => {
          const form = event.target;
          if (!(form instanceof HTMLFormElement)) return;
          if (!form.matches("[data-admin-order-completion-form]")) return;

          event.preventDefault();
          if (form.getAttribute("data-admin-order-completion-pending") === "true") return;

          form.setAttribute("data-admin-order-completion-pending", "true");
          setOrderCompletionPending(form, true);
          setOrderCompletionFeedback(form, "info", "");

          try {
            const request = buildOrderCompletionRequest(form);
            ...
            if (!response.ok || !payload || payload.ok !== true) {
              setOrderCompletionFeedback(
                ...
                payload && payload.message
                  ? payload.message
                  : "Не удалось сохранить фото или комментарий клинера. Попробуйте ещё раз."
              );
              return;
            }

            applyOrderCompletionPayload(form, payload);
          } catch (error) {
            void error;
            setOrderCompletionFeedback(
              form,
              "error",
              "Не удалось сохранить фото или комментарий клинера. Проверьте соединение и попробуйте ещё раз."
            );
          } finally {
            form.setAttribute("data-admin-order-completion-pending", "false");
            setOrderCompletionPending(form, false);
          }
        });
