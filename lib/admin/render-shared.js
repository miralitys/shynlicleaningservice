        ...
        <div class="admin-section-grid admin-form-grid-two">
          ${renderAdminCard(
            ...
            `<form class="admin-form" method="post" action="${ADMIN_LOGIN_PATH}" autocomplete="on">
              ...
              <div class="admin-inline-actions">
                ...
                <span class="admin-action-hint">После успешного входа админка откроется сразу.</span>
              </div>
            </form>`,
            { eyebrow: "Вход" }
          )}
          ${renderAdminCard(
            ...
            `<ul class="admin-feature-list">
              ...
              <li>После успешной проверки почты и пароля открывается рабочая панель.</li>
              <li>Дополнительные шаги подтверждения больше не используются.</li>
            </ul>`,
            { eyebrow: "Подсказка", muted: true }
          )}
        </div>`,
      {
        subtitle: "Введите рабочую почту и пароль, чтобы сразу открыть админку.",
        sidebar: renderAdminAuthSidebar("login"),
      }
    );
  }
