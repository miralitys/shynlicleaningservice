...
function createAdminRequestHandler(deps = {}) {
  ...
  const { handleSettingsRoute } = createAdminSettingsHandlers({
    ACCOUNT_LOGIN_PATH,
    ACCOUNT_VERIFY_EMAIL_PATH,
    ADMIN_GOOGLE_MAIL_CALLBACK_PATH,
    ADMIN_GOOGLE_MAIL_CONNECT_PATH,
  ...
  return async function handleAdminRequest(
      ...
      try {
        ...
        if (
          ...
          if (recurringSubmission) {
            ...
            if (recurringEntry) {
              const recurringAdminOrder = getPayloadOrderState(recurringSubmission.payloadForRetry);
    ...
    if (requestContext.route === ADMIN_CLIENTS_PATH && req.method === "POST") {
      ...
      try {
        const allEntries = await quoteOpsLedger.listEntries({ limit: CLIENT_MUTATION_LEDGER_LIMIT });
        ...
        if (action === "update-client") {
          ...
          const refreshedEntries = await quoteOpsLedger.listEntries({ limit: CLIENT_MUTATION_LEDGER_LIMIT });
        ...
        if (action === "load-client-sms-history") {
          ...
          const contactEntry = Array.isArray(client.entries)
            ? client.entries.find(
                (entry) =>
                  entry &&
                  (normalizeString(entry.contactId, 120) || normalizeString(entry.customerPhone, 80))
              ) || null
            : null;
          ...
          const remoteHistoryEntries = await loadRemoteSmsHistoryEntries(leadConnectorClient, {
            ...
            phone: normalizeString(contactEntry && contactEntry.customerPhone, 80),
            conversationIds: extractSmsConversationIds(localHistoryEntries),
          });
          const mergedHistoryEntries = mergeAdminSmsHistoryEntries(
            localHistoryEntries,
      ...
      try {
        ...
        if (action === "load-staff-sms-history") {
          ...
          redirectWithTiming(
            res,
            303,
            buildSmsRedirectPath(buildStaffReturnPath(getFormValue(formBody, "returnTo", 1000)), "staff-sms-history-loaded", "staff", staffId),
            requestStartNs,
            requestContext.cacheHit
          );
          return;
