    ...
    if (requestContext.route === ADMIN_QUOTE_OPS_PATH && req.method === "POST") {
      ...
      try {
        ...
        if (action === "send-quote-sms") {
          ...
          if (smsResult.ok && !normalizeString(entry.contactId, 120) && normalizeString(smsResult.contactId, 120)) {
            try {
              await quoteOpsLedger.updateLeadEntry(entryId, {
                contactId: normalizeString(smsResult.contactId, 120),
              });
            } catch {
    ...
    if (requestContext.route === ADMIN_ORDERS_PATH && req.method === "POST") {
      ...
      try {
        if (action === "send-order-sms") {
          ...
          if (
            smsResult.ok &&
            !normalizeString(currentEntry.contactId, 120) &&
            normalizeString(smsResult.contactId, 120)
          ) {
            try {
        ...
        const transitionedToScheduled =
          Boolean(updatedEntry) && previousOrderStatus !== "scheduled" && nextOrderStatus === "scheduled";
        ...
        if (transitionedToScheduled) {
          ...
          const customerName = normalizeString(updatedEntry && updatedEntry.customerName, 160) || "Client";
          ...
          if (!customerEmail) {
            ...
            await quoteOpsLedger.updateOrderEntry(entryId, {
              policyEmailLastError: "ORDER_POLICY_EMAIL_NOT_CONFIGURED",
            });
            if (notice === "order-saved") {
            ...
            const confirmationUrl =
              typeof buildOrderPolicyConfirmationUrl === "function"
                ? buildOrderPolicyConfirmationUrl(updatedEntry, {
                    env: process.env,
            ...
            if (!confirmationUrl) {
              await quoteOpsLedger.updateOrderEntry(entryId, {
                policyEmailLastError: "ORDER_POLICY_TOKEN_UNAVAILABLE",
              });
              if (notice === "order-saved") {
                notice = "order-saved-policy-email-unavailable";
              }
              ...
              try {
                const emailResult = await accountInviteEmail.sendOrderPolicyConfirmation(
                  {
                ...
                await quoteOpsLedger.updateOrderEntry(entryId, {
                  policyEmailSentAt:
                    emailResult && emailResult.sentAt
                      ? emailResult.sentAt
                ...
                await quoteOpsLedger.updateOrderEntry(entryId, {
                  policyEmailLastError: normalizeString(
                    error && error.message ? error.message : "ORDER_POLICY_EMAIL_SEND_FAILED",
                    400
                  ),
                });
                if (notice === "order-saved") {
                  notice = "order-saved-policy-email-failed";
                }
              }
            }
          }
      ...
      try {
        ...
        if (action === "send-client-sms") {
          ...
          if (
            smsResult.ok &&
            contactEntry &&
            !normalizeString(contactEntry.contactId, 120) &&
            normalizeString(smsResult.contactId, 120)
          ) {
            try {
              await quoteOpsLedger.updateClientEntry(contactEntry.id, {
                contactId: normalizeString(smsResult.contactId, 120),
              });
            } catch {
              // Best effort only: SMS already went out successfully.
