...
function createAdminRequestHandler(deps = {}) {
  ...
  return async function handleAdminRequest(
    ...
    if (requestContext.route === ADMIN_STAFF_W9_DOWNLOAD_PATH) {
      if (
      ...
      if (req.method !== "GET") {
        writeHtmlWithTiming(
          ...
          adminSharedRenderers.renderAdminLayout(
            ...
            `<div class="admin-alert admin-alert-error">W-9 доступен только через GET.</div>`
          ),
          requestStartNs,
          requestContext.cacheHit
        );
      ...
      const reqUrl = new URL(req.url || ADMIN_STAFF_W9_DOWNLOAD_PATH, "http://localhost");
      ...
      if (!staffRecord || !w9Document || !w9Document.relativePath) {
        writeHtmlWithTiming(
          res,
          404,
          adminSharedRenderers.renderAdminLayout(
            "W-9 не найден",
            `<div class="admin-alert admin-alert-error">Для этого сотрудника ещё нет прикреплённого W-9.</div>`
          ),
          requestStartNs,
          requestContext.cacheHit
        );
      ...
      try {
        ...
        writeHeadWithTiming(
          ...
          200,
          {
            ...
            "Content-Disposition": `inline; filename="${w9Document.fileName || "staff-w9.pdf"}"`,
          },
          requestStartNs,
          requestContext.cacheHit
        );
        ...
        writeHtmlWithTiming(
          ...
          adminSharedRenderers.renderAdminLayout(
            "W-9 не найден",
            `<div class="admin-alert admin-alert-error">Файл W-9 не найден на сервере. Сотруднику нужно пересоздать форму.</div>`
          ),
          requestStartNs,
          requestContext.cacheHit
        );
    ...
    if (requestContext.route === ADMIN_ORDERS_PATH && req.method === "POST") {
      ...
      const formBody = parseFormBody(await readTextBody(req, 32 * 1024));
    ...
    if (requestContext.route === ADMIN_CLIENTS_PATH && req.method === "POST") {
      ...
      try {
        ...
        if (action === "update-client") {
          ...
          const addressHomeProfiles = getFormValues(formBody, "addressHomeProfiles", 20, 250);
          ...
          const updates = {
            ...
            addressBook: addressValues.map((address, index) => ({
              address,
              propertyType: addressPropertyTypes[index] || "",
              sizeDetails: addressHomeProfiles[index] || "",
