...
function createAdminRequestHandler(deps = {}) {
  ...
  function normalizeAdminPhoneInput(value) {
    ...
    const normalizedDigits = digits.length > 10 && digits.startsWith("1")
      ? digits.slice(1, 11)
      : digits.slice(0, 10);
    ...
    if (requestContext.route === ADMIN_STAFF_PATH && req.method === "POST") {
      ...
      try {
        ...
        if (action === "create-staff") {
          const staffRole = getFormValue(formBody, "staffRole", 80) || getFormValue(formBody, "role", 80);
        ...
        if (action === "update-staff") {
          ...
          if (usersStore) {
            const nextUserRole = getFormValue(formBody, "userRole", 32);
              ...
              try {
                ...
                const linkedUser =
                  usersSnapshot && Array.isArray(usersSnapshot.users)
                    ? usersSnapshot.users.find((user) => user && user.staffId === staffId) || null
                    : null;
                userId = linkedUser ? linkedUser.id : "";
              } catch {}
            }
            if (userId && nextUserRole) {
              await usersStore.updateUser(userId, { role: nextUserRole });
            }
          }
          redirectWithTiming(
            res,
    ...
    if (requestContext.route === ADMIN_SETTINGS_PATH && req.method === "POST") {
      ...
      try {
        ...
        if (action === "create_user") {
          ...
          const staffRole = getFormValue(formBody, "staffRole", 80) || getFormValue(formBody, "role", 80);
          const userRole = getFormValue(formBody, "role", 32);
          ...
          if (
            createdUser &&
            accountInviteEmail &&
            typeof accountInviteEmail.isConfigured === "function" &&
            accountInviteEmail.isConfigured()
          ) {
            try {
