...
function createAdminRequestHandler(deps = {}) {
  ...
  function buildOrderCompletionMutationPayload(entry, notice) {
    if (!entry) {
      return {
        ok: false,
    ...
    return {
      ...
      message: "Отчёт клинера сохранён.",
      completion: {
  ...
  return async function handleAdminRequest(
    ...
    if (requestContext.route === ADMIN_ORDERS_PATH && req.method === "POST") {
      ...
      if (!quoteOpsLedger || !entryId) {
        if (ajaxRequest && action === "save-order-completion") {
          writeJsonWithTiming(
            res,
            404,
            {
          ...
          writeJsonWithTiming(
            ...
            500,
            {
              ...
              message: errorMessage
                ? `Не удалось сохранить фото или комментарий клинера: ${errorMessage}.`
