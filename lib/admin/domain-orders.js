...
} = require("../order-service-duration");
...
function createAdminOrderDomain(deps = {}) {
  ...
  function buildStaffPlanningContext(entries = [], staffSnapshot = {}) {
    ...
    const orderItems = orderEntries
      .map((entry) => {
        ...
        const serviceDurationMinutes = getOrderServiceDurationMinutes(entry);
        ...
        return {
          ...
          scheduleLabel: formatOrderScheduleLabel(scheduleDate, scheduleTime),
          serviceDurationMinutes,
          serviceDurationLabel: formatOrderServiceDurationLabel(serviceDurationMinutes),
  ...
  function normalizeOrderServiceType(value, fallback = "standard") {
    ...
    if (
      ...
      normalized === "in-home-estimate" ||
      normalized === "free-in-home-estimate" ||
      normalized === "freein-homeestimate" ||
  ...
  function formatOrderServiceTypeLabel(value) {
    ...
    if (normalized === "deep") return "Deep";
    if (normalized === "move-in") return "Move-in";
    if (normalized === "move-out") return "Move-out";
    if (normalized === "move-in/out") return "Move-in/out";
    if (normalized === "free-in-home-estimate") return "Free in-home estimate";
    return "Standard";
  }
  ...
  function formatOrderFrequencyLabel(value) {
    ...
    if (normalized === "every3weeks") return "Every 3 weeks";
    return "Not set";
  }
  ...
  function getOrderServiceDurationMinutes(entry = {}) {
    const calculatorData = getEntryCalculatorData(entry);
    const adminOrder = getEntryAdminOrderData(entry);
    return normalizeOrderServiceDurationMinutes(
      adminOrder.serviceDurationMinutes ||
        calculatorData.serviceDurationMinutes ||
  ...
  function getOrderFrequency(entry = {}) {
    const calculatorData = getEntryCalculatorData(entry);
    const adminOrder = getEntryAdminOrderData(entry);
    if (getOrderServiceType(entry) !== "standard") return "";
  ...
  function formatOrderScheduleLabel(selectedDate, selectedTime) {
    const normalizedDate = normalizeAdminOrderDateInput(selectedDate);
    const normalizedTime = normalizeAdminOrderTimeInput(selectedTime);
    if (!normalizedDate && !normalizedTime) return "Не указаны";
    const scheduleLabel = formatAdminScheduleLabel(normalizedDate, normalizedTime);
    const dateMatch = normalizedDate.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!dateMatch) return scheduleLabel;

    const [, year, month, day] = dateMatch;
    const weekdayLabels = ["Вс", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"];
    const weekday = weekdayLabels[new Date(Date.UTC(Number(year), Number(month) - 1, Number(day))).getUTCDay()];
    const dateLabel = formatAdminScheduleLabel(normalizedDate, "");
    return scheduleLabel.replace(dateLabel, `${dateLabel} (${weekday})`);
  }
  ...
  function collectAdminOrderRecords(entries = []) {
    const records = entries
      ...
      .map((entry) => {
        ...
        return {
          ...
          assignedStaff,
          recurringNextEntryId: normalizeString(adminOrder.recurringNextEntryId, 120),
          recurringGeneratedAt: normalizeString(adminOrder.recurringGeneratedAt, 80),
          recurringSeriesId: normalizeString(adminOrder.recurringSeriesId, 120),
          recurringAnchorDate: normalizeString(adminOrder.recurringAnchorDate, 32),
          recurringSourceEntryId: normalizeString(adminOrder.recurringSourceEntryId, 120),
          selectedDate,
          selectedTime,
          ...
          isRecurring: Boolean(
            frequency ||
            adminOrder.recurringSeriesId ||
            adminOrder.recurringAnchorDate ||
            adminOrder.recurringSourceEntryId ||
            adminOrder.recurringNextEntryId
          ),
          policyAcceptance,
          policyAccepted: Boolean(policyAcceptance.policyAccepted),
        };
      });
    const recurringClientKeys = new Set();
    const getClientKeys = (order) => {
      const keys = [];
      const phone = normalizeString(order.customerPhone, 80).replace(/\D+/g, "");
      const email = normalizeString(order.customerEmail, 250).toLowerCase();
    ...
    for (const order of records) {
      ...
      getClientKeys(order).forEach((key) => recurringClientKeys.add(key));
    }
    for (const order of records) {
      if (!order.isRecurring && getClientKeys(order).some((key) => recurringClientKeys.has(key))) {
