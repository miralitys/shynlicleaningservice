...
function createAdminClientAddressDomain(deps = {}) {
  ...
  function splitAdminClientAddressSizeDetails(value) {
    ...
    for (const part of parts) {
      ...
      if (!bathroomCount && /(bath|baths|bathroom|bathrooms|toilet|toilets|сануз|ванн|туалет)/i.test(lowerPart)) {
        bathroomCount = part;
        continue;
      }
      if (!roomCount && /(room|rooms|bed|beds|bedroom|bedrooms|комнат|комнаты|спальн)/i.test(lowerPart)) {
        roomCount = part;
        continue;
      }
      if (!squareFootage) {
  ...
  function buildAdminClientAddressSizeDetails(squareFootage, roomCount, bathroomCount) {
    const normalizedSquareFootage = normalizeString(squareFootage, 120);
    const normalizedRoomCount = normalizeString(roomCount, 120);
    const normalizedBathroomCount = normalizeString(bathroomCount, 120);
    return [normalizedSquareFootage, normalizedRoomCount, normalizedBathroomCount].filter(Boolean).join(" / ");
  }
  ...
  function normalizeAdminClientAddressRecordInput(value) {
    ...
    const bathroomCount = normalizeString(
      source.bathroomCount || source.bathrooms || source.bathroomLabel || legacySizeParts.bathroomCount,
      120
    );
    ...
    return {
      ...
      roomCount,
      bathroomCount,
      sizeDetails: buildAdminClientAddressSizeDetails(squareFootage, roomCount, bathroomCount) || legacySizeDetails,
      pets: normalizeAdminClientPetsValue(source.pets || source.petType || source.animals),
      basementCleaning: normalizeString(
        source.basementCleaning || source.basement || source.hasBasementCleaning,
        40
      ),
  ...
  function mergeAdminClientAddressRecord(target, source = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(source, "bathroomCount")) {
      target.bathroomCount = normalizeString(source.bathroomCount, 120);
    }
    if (source.sizeDetails && !target.squareFootage && !target.roomCount && !target.bathroomCount) {
      ...
      if (legacySizeParts.bathroomCount) target.bathroomCount = legacySizeParts.bathroomCount;
    }
    target.sizeDetails = buildAdminClientAddressSizeDetails(
    ...
    if (Object.prototype.hasOwnProperty.call(source, "basementCleaning")) {
      const sourceBasementCleaning = normalizeString(source.basementCleaning, 40);
      if (sourceBasementCleaning || !target.basementCleaning) {
        target.basementCleaning = sourceBasementCleaning;
      }
    }
