...
function createAdminClientsHandlers(deps = {}) {
  ...
  function hasFormField(formBody, key) {
    return (
      (typeof formBody?.has === "function" && formBody.has(key)) ||
      Object.prototype.hasOwnProperty.call(formBody || {}, key)
    );
  }

  function normalizeClientPhoneDigits(value) {
  ...
  function collectClientSmsLookupTargets(client = {}) {
    ...
    const addTarget = (entry = {}) => {
      ...
      const phones = [
        entry && (entry.customerPhone || entry.phone),
        entry && entry.secondaryPhone,
      ]
        .map((value) => normalizeString(value, 80))
        .filter(Boolean);
      if (!contactId && phones.length === 0) return;
      if (phones.length === 0) {
        const key = `${contactId}|`;
  ...
  async function handleAdminClientsPostRoute(context = {}) {
    ...
    try {
      ...
      if (action === "update-client") {
        ...
        const addressOriginalValues = getFormValues(formBody, "addressOriginals", 20, 500);
        ...
        const existingAddressValues = Array.isArray(client.addresses)
          ? client.addresses.map((addressRecord) =>
              normalizeString(addressRecord && addressRecord.address, 500)
            )
          : [];
        const canUseAddressIndexFallback =
          addressOriginalValues.every((value) => !normalizeString(value, 500)) &&
          addressValues.length === existingAddressValues.length;
        const addressReplacements = addressValues
          .map((address, index) => ({
            from: normalizeString(
              addressOriginalValues[index] ||
                (canUseAddressIndexFallback ? existingAddressValues[index] : ""),
              500
            ),
            to: normalizeString(address, 500),
          }))
          .filter(
            (replacement) =>
              replacement.from &&
        ...
        if (addressValues.length === 1) {
          const canonicalAddress = normalizeString(addressValues[0], 500);
          const canonicalAddressKey = canonicalAddress.toLowerCase();
          const knownReplacementKeys = new Set(
            addressReplacements.map((replacement) => replacement.from.toLowerCase())
          );
          for (const entry of client.entries) {
            const staleAddress = normalizeString(entry && entry.fullAddress, 500);
            const staleAddressKey = staleAddress.toLowerCase();
            if (
              !staleAddress ||
              !canonicalAddress ||
              staleAddressKey === canonicalAddressKey ||
              knownReplacementKeys.has(staleAddressKey)
            ) {
              continue;
            }
            knownReplacementKeys.add(staleAddressKey);
            addressReplacements.push({ from: staleAddress, to: canonicalAddress });
          }
        }
        ...
        const submittedPhone = getFormValue(formBody, "phone", 80);
        const hasSecondaryPhoneField = hasFormField(formBody, "secondaryPhone");
        let submittedSecondaryPhone = hasSecondaryPhoneField
          ? getFormValue(formBody, "secondaryPhone", 80)
          : "";
        const primaryPhoneChoice = getFormValue(formBody, "primaryPhone", 40).toLowerCase();
        let nextPhone = submittedPhone;
        let nextSecondaryPhone = submittedSecondaryPhone;
        if (hasSecondaryPhoneField && primaryPhoneChoice === "secondary" && normalizeClientPhoneDigits(submittedSecondaryPhone)) {
          nextPhone = submittedSecondaryPhone;
          nextSecondaryPhone =
            normalizeClientPhoneDigits(submittedPhone) === normalizeClientPhoneDigits(submittedSecondaryPhone)
              ? ""
              : submittedPhone;
        }
        ...
        if (hasFormField(formBody, "automaticNotificationsSettingPresent")) {
          updates.automaticNotificationsEnabled =
            getFormValue(formBody, "automaticNotificationsEnabled", 8) === "1";
        }
        if (hasFormField(formBody, "paymentNotificationsSettingPresent")) {
          updates.paymentNotificationsEnabled =
            getFormValue(formBody, "paymentNotificationsDisabled", 8) !== "1";
        }
        if (hasSecondaryPhoneField) {
          updates.secondaryPhone = nextSecondaryPhone;
        }
        if (!updates.name) {
          if (ajaxRequest) {
        ...
        const refreshedClientKey = refreshedClient ? refreshedClient.key : clientKey;

        if (ajaxRequest) {
          const ajaxPayload = {
            clientKey: refreshedClientKey,
          };
          if (refreshedClientKey && refreshedClientKey !== clientKey) {
            ajaxPayload.redirectUrl = buildAdminRedirectPath(returnTo, {
              notice: "client-saved",
              client: refreshedClientKey,
            });
          }
          writeAjaxMutationSuccess(res, requestStartNs, requestContext, "client-saved", ajaxPayload);
          return;
        }
