...
function normalizeClientRevenueScheduleDate(value) {
...
function getClientRevenueNowParts(now = new Date(), timeZone = "America/Chicago") {
...
function shouldIncludeClientVisitRevenue(input = {}, options = {}) {
  const orderStatus = String(input.orderStatus || "").trim().toLowerCase();
  if (orderStatus === "canceled" || orderStatus === "cancelled" || orderStatus === "rescheduled") {
    return false;
  }

  const selectedDate = normalizeClientRevenueScheduleDate(input.selectedDate);
  if (!selectedDate) {
    return ["cleaning-complete", "invoice-sent", "paid", "awaiting-review", "completed"].includes(orderStatus);
  }

  const current = getClientRevenueNowParts(options.now || new Date(), options.timeZone || "America/Chicago");
  if (selectedDate < current.date) return true;
  if (selectedDate > current.date) return false;

  const selectedTime = normalizeClientRevenueScheduleTime(input.selectedTime);
  if (!selectedTime) {
    return ["cleaning-complete", "invoice-sent", "paid", "awaiting-review", "completed"].includes(orderStatus);
  }
  return selectedTime <= current.time;
}
...
function createAdminClientRecordDomain(deps = {}) {
  ...
  function getEntryAdminClientSecondaryPhone(entry = {}) {
    const adminClient = getEntryAdminClientData(entry);
    return normalizeAdminClientPhoneInput(
  ...
  function addClientPhoneKey(client, phoneKey) {
    const normalizedPhoneKey = normalizePhoneFilterValue(phoneKey);
    if (!client || !normalizedPhoneKey) return;
    if (!(client.phoneKeys instanceof Set)) {
      client.phoneKeys = new Set(
        [client.phoneKey, client.secondaryPhoneKey]
          .map((value) => normalizePhoneFilterValue(value))
          .filter(Boolean)
      );
    }
    client.phoneKeys.add(normalizedPhoneKey);
    if (!client.phoneKey) client.phoneKey = normalizedPhoneKey;
  }

  function clientHasPhoneKey(client, phoneKey) {
    const normalizedPhoneKey = normalizePhoneFilterValue(phoneKey);
    if (!client || !normalizedPhoneKey) return false;
    if (client.phoneKeys instanceof Set && client.phoneKeys.has(normalizedPhoneKey)) return true;
    return (
      normalizePhoneFilterValue(client.phoneKey) === normalizedPhoneKey ||
      normalizePhoneFilterValue(client.secondaryPhoneKey) === normalizedPhoneKey ||
      normalizePhoneFilterValue(client.phone) === normalizedPhoneKey ||
      normalizePhoneFilterValue(client.secondaryPhone) === normalizedPhoneKey
    );
  }

  function getClientPhoneFilterValues(client = {}) {
    const values = [
      client.phone,
      client.secondaryPhone,
      ...(Array.isArray(client.entries)
        ? client.entries.flatMap((entry) => [entry && entry.customerPhone, entry && entry.secondaryPhone])
        : []),
    ];
  ...
  function normalizeClientServiceKind(value) {
    ...
    if (
      ...
      normalized === "estimate" ||
      normalized === "freeestimate" ||
      normalized === "inhomeestimate" ||
      normalized === "freeinhomeestimate"
    ) {
      return "nonregular";
    }
    return "";
  ...
  function applyClientEntryUpdates(entry, updates = {}) {
    ...
    const adminOrder =
      payload.adminOrder && typeof payload.adminOrder === "object" ? { ...payload.adminOrder } : null;
    ...
    const hasSecondaryPhone = Object.prototype.hasOwnProperty.call(updates, "secondaryPhone");
    ...
    const hasAutomaticNotificationsEnabled = Object.prototype.hasOwnProperty.call(
      updates,
      "automaticNotificationsEnabled"
    );
    const hasPaymentNotificationsEnabled = Object.prototype.hasOwnProperty.call(
      updates,
    ...
    const secondaryPhone = hasSecondaryPhone
      ? normalizeAdminClientPhoneInput(updates.secondaryPhone)
      : getEntryAdminClientSecondaryPhone(entry);
    ...
    const addressReplacements = (Array.isArray(updates.addressReplacements)
      ? updates.addressReplacements
      : []
    )
      .map((replacement) => ({
        from: normalizeString(replacement && replacement.from, 500),
        to: normalizeString(replacement && replacement.to, 500),
      }))
      .filter((replacement) => replacement.from && replacement.to);
    const currentAddress = normalizeString(
      entry.fullAddress || calculatorData.fullAddress || calculatorData.address,
      500
    );
    const addressReplacement = addressReplacements.find(
      (replacement) =>
        buildAdminClientAddressKey(replacement.from) === buildAdminClientAddressKey(currentAddress)
    );
    ...
    if (addressReplacement) {
      const oldAddressKey = buildAdminClientAddressKey(addressReplacement.from);
      const replaceMatchingAddress = (source, fieldNames) => {
        if (!source || typeof source !== "object" || Array.isArray(source)) return source;
        const nextSource = { ...source };
        for (const fieldName of fieldNames) {
          if (
            buildAdminClientAddressKey(nextSource[fieldName]) === oldAddressKey
          ) {
            nextSource[fieldName] = addressReplacement.to;
          }
        }
        return nextSource;
      };

      entry.fullAddress = addressReplacement.to;
      calculatorData.fullAddress = addressReplacement.to;
      if (buildAdminClientAddressKey(calculatorData.address) === oldAddressKey) {
        calculatorData.address = addressReplacement.to;
      }
      for (const payloadKey of ["quoteData", "quote", "calculation"]) {
        if (!payload[payloadKey] || typeof payload[payloadKey] !== "object") continue;
        payload[payloadKey] = replaceMatchingAddress(payload[payloadKey], ["fullAddress", "address"]);
      }
      if (adminOrder) {
        const policyAcceptance =
          adminOrder.policyAcceptance && typeof adminOrder.policyAcceptance === "object"
            ...
            : null;
        const policyIsSigned = Boolean(
          adminOrder.policyAccepted ||
            (policyAcceptance &&
              (policyAcceptance.policyAccepted ||
                policyAcceptance.signedAt ||
                normalizeString(policyAcceptance.status, 40).toLowerCase() === "accepted"))
        );
        if (
          policyAcceptance &&
          !policyIsSigned &&
          (!policyAcceptance.serviceAddress ||
            buildAdminClientAddressKey(policyAcceptance.serviceAddress) === oldAddressKey)
        ) {
          policyAcceptance.serviceAddress = addressReplacement.to;
          policyAcceptance.updatedAt = timestamp;
          adminOrder.policyAcceptance = policyAcceptance;
        }
    ...
    if (secondaryPhone) {
      adminClient.secondaryPhone = secondaryPhone;
    } else if (hasSecondaryPhone) {
      delete adminClient.secondaryPhone;
    }

    if (hasAutomaticNotificationsEnabled) {
      adminClient.automaticNotificationsEnabled = updates.automaticNotificationsEnabled !== false;
    }
    if (hasPaymentNotificationsEnabled) {
      adminClient.paymentNotificationsEnabled = updates.paymentNotificationsEnabled !== false;
    }
    ...
    if (
      ...
      normalizeString(adminClient.secondaryPhone, 80) ||
      typeof adminClient.automaticNotificationsEnabled === "boolean" ||
      typeof adminClient.paymentNotificationsEnabled === "boolean"
    ) {
  ...
  function collectAdminClientRecords(entries = []) {
    ...
    function createAddressRecord(address) {
      ...
      return {
        ...
        basementCleaning: normalizeString(normalizedInput.basementCleaning, 40),
    ...
    function pickFirstClientCalculatorValue(source = {}, keys = [], maxLength = 4000) {
    ...
    function buildClientAddressInputFromEntry(entry = {}, address = "") {
      const calculatorData = getEntryCalculatorData(entry);
      const normalizedAddress = normalizeString(address || entry.fullAddress, 500);
      if (!normalizedAddress) return "";

      return {
        address: normalizedAddress,
        propertyType: pickFirstClientCalculatorValue(
          calculatorData,
          ["propertyType", "homeType", "houseType", "objectType"],
          40
        ),
        squareFootage: pickFirstClientCalculatorValue(
          calculatorData,
          ["squareFootage", "squareFeet", "squareMeters", "homeSize", "size"],
          120
        ),
        roomCount: pickFirstClientCalculatorValue(
          calculatorData,
          ["rooms", "bedrooms", "roomCount", "bedroomCount"],
          120
        ),
        bathroomCount: pickFirstClientCalculatorValue(
          calculatorData,
          ["bathrooms", "bathroomCount", "bathroomCounts", "baths", "bathCount", "toilets", "toiletCount"],
          120
        ),
        pets: pickFirstClientCalculatorValue(
          calculatorData,
          ["hasPets", "pets", "petType", "animals"],
          40
        ),
        basementCleaning: pickFirstClientCalculatorValue(
          calculatorData,
          ["basementCleaning", "basement", "hasBasementCleaning"],
          40
        ),
        notes: pickFirstClientCalculatorValue(
          calculatorData,
          ["notes", "specialInstructions", "accessInstructions", "instructions"],
          4000
        ),
      };
    }
    ...
    function createClientRecord(key, input = {}) {
      return {
        ...
        secondaryPhone: normalizeString(input.secondaryPhone, 80),
        secondaryPhoneKey: normalizePhoneFilterValue(input.secondaryPhone),
        phoneKeys: new Set(
          [input.phoneKey, normalizePhoneFilterValue(input.secondaryPhone)]
            .map((value) => normalizePhoneFilterValue(value))
            .filter(Boolean)
        ),
        ...
        hasAutomaticNotificationsPreference: false,
        paymentNotificationsEnabled: true,
        hasPaymentNotificationsPreference: false,
      };
    }
    ...
    for (const entry of entries) {
      ...
      const adminClientData = getEntryAdminClientData(entry);
      const entryAutomaticNotificationsEnabled =
        typeof adminClientData.automaticNotificationsEnabled === "boolean"
          ? adminClientData.automaticNotificationsEnabled
          : null;
      const entryPaymentNotificationsEnabled =
        typeof adminClientData.paymentNotificationsEnabled === "boolean"
          ? adminClientData.paymentNotificationsEnabled
          : null;
      const secondaryPhone = getEntryAdminClientSecondaryPhone(entry);
      const secondaryPhoneKey = normalizePhoneFilterValue(secondaryPhone);
      ...
      const clientEntryOrderState = getClientEntryOrderState(entry);
      const includeVisitRevenue = shouldIncludeClientVisitRevenue({
        selectedDate: getOrderSelectedDate(entry),
        selectedTime: getOrderSelectedTime(entry),
        orderStatus: normalizeOrderStatus(clientEntryOrderState.status, ""),
      });
      ...
      const clientPhotos = normalizeAdminClientPhotoAssets(adminClientData.photos, entry.id);
      const nameAddressKey = normalizeString([normalizedName, normalizedAddress].filter(Boolean).join("|"), 500);
      let client = null;

      if (phoneKey) {
        client = clients.find((candidate) => clientHasPhoneKey(candidate, phoneKey)) || null;
        if (!client && email) {
          client =
            clients.find(
              (candidate) => !(candidate.phoneKeys instanceof Set && candidate.phoneKeys.size > 0) && candidate.emailKey === email
            ) || null;
        }
        if (!client && nameAddressKey) {
          client =
            clients.find(
              (candidate) =>
                !(candidate.phoneKeys instanceof Set && candidate.phoneKeys.size > 0) &&
                !candidate.emailKey &&
                candidate.fallbackKey === nameAddressKey
            ) || null;
        }
      } else if (secondaryPhoneKey) {
        client = clients.find((candidate) => clientHasPhoneKey(candidate, secondaryPhoneKey)) || null;
        if (!client && email) {
          client =
            clients.find(
              (candidate) => !(candidate.phoneKeys instanceof Set && candidate.phoneKeys.size > 0) && candidate.emailKey === email
      ...
      if (!client) {
        client = createClientRecord(phoneKey || secondaryPhoneKey || email || nameAddressKey || normalizeString(entry.id, 120), {
          phoneKey,
          emailKey: email,
        ...
        if (phoneKey) client.key = phoneKey;
        addClientPhoneKey(client, phoneKey);
        addClientPhoneKey(client, secondaryPhoneKey);
        if (!client.secondaryPhoneKey && secondaryPhoneKey) client.secondaryPhoneKey = secondaryPhoneKey;
        if (!client.emailKey && email) client.emailKey = email;
        if (!client.fallbackKey && nameAddressKey) client.fallbackKey = nameAddressKey;
      }
      addClientPhoneKey(client, phoneKey);
      addClientPhoneKey(client, secondaryPhoneKey);
      if (entryAutomaticNotificationsEnabled !== null) {
        client.hasAutomaticNotificationsPreference = true;
        if (entryAutomaticNotificationsEnabled === false) {
          client.automaticNotificationsEnabled = false;
        }
      }
      if (entryPaymentNotificationsEnabled !== null) {
        client.hasPaymentNotificationsPreference = true;
        if (entryPaymentNotificationsEnabled === false) {
          client.paymentNotificationsEnabled = false;
        }
      }
      ...
      if (includeVisitRevenue) {
        client.pastVisitRevenue += Number(entry.totalPrice || 0);
      }
      if (!client.email && email) client.email = email;
      if (!client.phone && phone) client.phone = phone;
      if (!client.secondaryPhone && secondaryPhone && normalizePhoneFilterValue(secondaryPhone) !== normalizePhoneFilterValue(phone)) {
        client.secondaryPhone = secondaryPhone;
        client.secondaryPhoneKey = secondaryPhoneKey;
      }
      ...
      if (!client.removedAddressKeys.has(currentAddressKey)) {
        const addressRecord = ensureAddressRecord(client, buildClientAddressInputFromEntry(entry, address));
        addressRecord.requestCount += 1;
        addressRecord.totalRevenue += Number(entry.totalPrice || 0);
        if (includeVisitRevenue) {
          addressRecord.pastVisitRevenue += Number(entry.totalPrice || 0);
        }
      ...
      if (isNewer || (!client.latestCreatedAt && createdAt)) {
        ...
        if (secondaryPhone && normalizePhoneFilterValue(secondaryPhone) !== normalizePhoneFilterValue(client.phone)) {
          client.secondaryPhone = secondaryPhone;
          client.secondaryPhoneKey = secondaryPhoneKey;
        }
      }
    ...
    return clients
      .map((client) => {
        ...
        return {
          ...client,
          ...segmentSummary,
          phoneKeys: Array.from(client.phoneKeys instanceof Set ? client.phoneKeys.values() : []),
          address:
    ...
    return clientRecords.filter((client) => {
      if (q) {
        const haystack = [
          ...
          normalizeString(client.secondaryPhone, 80).toLowerCase(),
          ...
          ...client.entries.flatMap((entry) => [
            ...
            normalizeString(entry.secondaryPhone, 80).toLowerCase(),
            normalizeString(entry.fullAddress, 500).toLowerCase(),
            normalizeString(entry.serviceName || entry.serviceType, 120).toLowerCase(),
          ]),
        ].join("\n");
        const matchesPhone = qPhone
          ? getClientPhoneFilterValues(client).some((value) => normalizePhoneFilterValue(value).includes(qPhone))
          : false;
      ...
      if (
        phone &&
        !getClientPhoneFilterValues(client).some((value) => normalizePhoneFilterValue(value).includes(phone))
      ) {
        return false;
      }
