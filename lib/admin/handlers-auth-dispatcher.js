...
function createAdminAuthHandlers(deps = {}) {
  ...
  function canUseAdminWorkspace(user) {
  ...
  async function authenticateWorkspaceUser(usersStore, email, password) {
    ...
    const adminWorkspaceAllowed = canUseAdminWorkspace(user);
    const accountWorkspaceAllowed = canUseAccountWorkspace(user);

    if (!user || user.status !== "active" || (!adminWorkspaceAllowed && !accountWorkspaceAllowed)) {
      return { config, user: null, error: "" };
    }
    ...
    return {
      ...
      user,
      error: "",
      redirectPath: adminWorkspaceAllowed ? ADMIN_ROOT_PATH : ACCOUNT_ROOT_PATH,
    };
  }
    ...
    if (!validEmail || !validPassword) {
      ...
      if (workspaceUserLogin.user) {
        ...
        redirectWithTiming(
          res,
          303,
          workspaceUserLogin.redirectPath || ADMIN_ROOT_PATH,
          requestStartNs,
          requestContext.cacheHit,
