...
function createAdminWorkspaceHelpers(deps = {}) {
  ...
  function buildLeadMutationPayload(entry, notice) {
    ...
    return {
      ...
      entry: {
        ...
        leadStatus,
        locked: leadStatus === "confirmed" || leadStatus === "completed",
        managerLabel:
        ...
        taskLabel: openTask
          ...
          : leadStatus === "confirmed"
            ...
            : leadStatus === "completed"
              ? "Выполнено"
            : "Нет открытой задачи",
        hideDeadline: leadStatus === "confirmed" || leadStatus === "completed" || leadStatus === "declined",
        dueLabel:
  ...
  function buildOrderStageMutationPayload(entry, notice) {
    ...
    const policyNotRequired =
      normalizeString(entry && entry.serviceType, 80)
        .toLowerCase()
        .replace(/[\s_]+/g, "-") === "free-in-home-estimate";
    const funnelStatus =
      !policyNotRequired &&
  ...
  async function resolveLeadTaskAssigneeRecord(usersStore, staffStore, assigneeId) {
    const normalizedAssigneeId = normalizeString(assigneeId, 120);
    if (!normalizedAssigneeId || !usersStore || typeof usersStore.getSnapshot !== "function") {
      return null;
    }
    ...
    const users = Array.isArray(usersSnapshot && usersSnapshot.users) ? usersSnapshot.users : [];
    const matchedUser = users.find((user) => {
      if (!user || normalizeString(user.id, 120) !== normalizedAssigneeId) return false;
      if (normalizeString(user.status, 32).toLowerCase() !== "active") return false;
      const role = normalizeString(user.role, 32).toLowerCase();
    ...
    if (staffStore && typeof staffStore.getSnapshot === "function" && matchedUser.staffId) {
      ...
      assigneeName = normalizeString(staffRecord && staffRecord.name, 200);
    }

    return {
      id: normalizeString(matchedUser.id, 120),
      name: assigneeName || normalizeString(matchedUser.email, 200),
      email: normalizeString(matchedUser.email, 250).toLowerCase(),
      role: normalizeString(matchedUser.role, 32).toLowerCase(),
    };
  }
  ...
  function normalizeManualOrderServiceType(value) {
    ...
    if (["move-in", "movein", "move-in-cleaning", "moveincleaning"].includes(normalized)) {
      return "move-in";
    }
    if (["move-out", "moveout", "move-out-cleaning", "moveoutcleaning"].includes(normalized)) {
      return "move-out";
    }
    return "standard";
  }
