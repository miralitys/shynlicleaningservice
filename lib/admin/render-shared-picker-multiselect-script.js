...
const ADMIN_SHARED_PICKER_AND_MULTISELECT_SCRIPT = `        function parseAdminDateInput(value) {
        ...
        function syncDefaultTimeMeridiemFromHour(hourSelect) {
          if (!hourSelect) return;
          const field = hourSelect.closest("[data-admin-picker-field='time']");
          const meridiemSelect = field && field.querySelector("[data-admin-time-meridiem]");
          const hour = Number(hourSelect.value || "0");
          if (!meridiemSelect || !Number.isFinite(hour) || hour < 1 || hour > 12) return;
          meridiemSelect.value = hour === 12 || hour <= 6 ? "PM" : "AM";
          enforceNoonMeridiem(field);
        }

        function enforceNoonMeridiem(field) {
          if (!field) return;
          const hourSelect = field.querySelector("[data-admin-time-hour]");
          const meridiemSelect = field.querySelector("[data-admin-time-meridiem]");
          if (!hourSelect || !meridiemSelect) return;
          const isNoonHour = Number(hourSelect.value || "0") === 12;
          const amOption = meridiemSelect.querySelector("option[value='AM']");
          if (amOption) amOption.disabled = isNoonHour;
          if (isNoonHour) meridiemSelect.value = "PM";
        }
