...
function createAdminOrdersRecurringHelpers(deps = {}) {
  ...
  async function updateRecurringOrderSeriesFutureComment({
    ...
    const metadata = await migrateSeriesMetadata(quoteOpsLedger, sourceEntry, allEntries);
    const refreshedEntries = await quoteOpsLedger.listEntries({ limit: 5000 });
    const sourceDate = getOccurrenceDate(metadata.updatedSource) || getSelectedDate(metadata.updatedSource);
    const sourceId = normalizeString(metadata.updatedSource.id, 120);
