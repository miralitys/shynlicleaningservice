...
function createAdminMessagesDomain(deps = {}) {
  ...
  const getEntryCustomerSmsHistoryEntries =
    typeof deps.getEntryCustomerSmsHistoryEntries === "function"
      ? deps.getEntryCustomerSmsHistoryEntries
  ...
  function collectAdminMessageRecords(entries = [], options = {}) {
    ...
    return (Array.isArray(entries) ? entries : [])
      .flatMap((entry) => {
        ...
        return getEntryCustomerSmsHistoryEntries(entry)
          .map((message) => {
            const direction =
              normalizeString(message && message.direction, 20).toLowerCase() === "inbound"
                ? "inbound"
                : "outbound";
            const readAt = getMessageReadAt(message);
            const isUnreadInbound = direction === "inbound" && !readAt;
            return {
              ...
              readAt,
              status: isUnreadInbound ? "new" : "read",
              statusLabel: isUnreadInbound ? "Новый" : direction === "inbound" ? "Прочитано" : "Исходящее",
              statusTone: isUnreadInbound ? "danger" : "muted",
