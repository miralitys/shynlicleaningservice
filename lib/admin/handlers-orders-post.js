...
function createAdminOrdersPostHandlers(deps = {}) {
  ...
  const { handleOrderUpdateAction, handleScheduleNextOrderAction } = createAdminOrdersUpdateHandlers(deps);
  ...
  async function handleAdminOrdersPostRoute(context = {}) {
    ...
    try {
      ...
      if (action === "schedule-next-order") {
        await handleScheduleNextOrderAction({
          ...context,
          res,
          requestStartNs,
          requestContext,
