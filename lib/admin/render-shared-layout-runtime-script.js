...
const ADMIN_SHARED_LAYOUT_RUNTIME_SCRIPT = `        if (document.readyState === "loading") {
        ...
        document.addEventListener("click", (event) => {
          ...
          if (orderAmountOpenTrigger) {
            event.preventDefault();
            resetOrderScheduleEditors(document, { reset: true });
          ...
          const orderScheduleOpenTrigger = event.target.closest("[data-admin-order-schedule-open]");
          if (orderScheduleOpenTrigger) {
            event.preventDefault();
            resetOrderAmountEditors(document, { reset: true });
            resetOrderPaymentEditors(document, { reset: true });
            resetOrderTeamEditors(document, { reset: true });
            const panelId = orderScheduleOpenTrigger.getAttribute("data-admin-order-schedule-open");
            const panel = panelId ? document.querySelector('[data-admin-order-schedule-editor="' + panelId + '"]') : null;
            setOrderScheduleEditorState(panelId, true);
            const firstField = panel ? panel.querySelector("input, textarea, select") : null;
            if (firstField && typeof firstField.focus === "function") {
              firstField.focus();
              if (typeof firstField.select === "function") {
          ...
          const orderScheduleCancelTrigger = event.target.closest("[data-admin-order-schedule-cancel]");
          if (orderScheduleCancelTrigger) {
            event.preventDefault();
            closeOrderScheduleEditor(orderScheduleCancelTrigger.getAttribute("data-admin-order-schedule-cancel"), { reset: true });
            return;
          }
          ...
          const timeConfirmTrigger = event.target.closest("[data-admin-time-confirm]");
          if (timeConfirmTrigger) {
            event.preventDefault();
            const field = timeConfirmTrigger.closest("[data-admin-picker-field='time']");
            applyTimePanelSelection(field);
            const displayInput = field ? field.querySelector("[data-admin-picker-display]") : null;
            if (displayInput) {
          ...
          const timeCancelTrigger = event.target.closest("[data-admin-time-cancel]");
          if (timeCancelTrigger) {
            event.preventDefault();
            const field = timeCancelTrigger.closest("[data-admin-picker-field='time']");
        ...
        document.addEventListener("input", (event) => {
          const timeHourInput = event.target.closest("[data-admin-time-hour]");
          if (timeHourInput) {
            syncDefaultTimeMeridiemFromHour(timeHourInput);
            return;
          }
          const timeMeridiemInput = event.target.closest("[data-admin-time-meridiem]");
          if (timeMeridiemInput) {
            enforceNoonMeridiem(timeMeridiemInput.closest("[data-admin-picker-field='time']"));
            return;
          }
        ...
        document.addEventListener("keydown", (event) => {
          ...
          if (event.key === "Escape") {
            ...
            const openOrderScheduleEditor = document.querySelector("[data-admin-order-schedule-editor]:not([hidden])");
            if (openOrderScheduleEditor) {
              event.preventDefault();
              closeOrderScheduleEditor(openOrderScheduleEditor.getAttribute("data-admin-order-schedule-editor"), { reset: true });
              return;
            }
        ...
        syncOrderScheduleEditors(document);
