...
const ADMIN_SHARED_LAYOUT_EXTRA_STYLES = [
  String.raw`
      ...
      .admin-payroll-cleaner-controls {
        display: grid;
        gap: 18px;
        min-width: 0;
      }
      .admin-payroll-cleaner-filter {
        grid-template-columns: minmax(220px, 1.1fr) minmax(180px, 0.8fr) minmax(180px, 0.8fr) auto;
        max-width: 980px;
      }
      .admin-payroll-cleaner-filter .admin-label {
        min-width: 0 !important;
      }
      .admin-payroll-cleaner-directory {
        align-items: stretch;
        gap: 8px;
        max-width: 100%;
        min-width: 0;
      }
      .admin-payroll-cleaner-directory .admin-button,
      .admin-payroll-cleaner-directory .admin-link-button {
        min-width: 0;
      ...
      .admin-ghl-sms-chat-panel {
        border: 1px solid rgba(228, 228, 231, 0.92);
        border-radius: 28px;
        background:
          radial-gradient(circle at 18% 0%, rgba(164, 64, 95, 0.08), transparent 34%),
          linear-gradient(180deg, rgba(248, 249, 252, 0.96), rgba(255, 255, 255, 0.98));
        padding: 18px;
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.82);
      }
      .admin-ghl-sms-chat-head {
        align-items: flex-start;
        gap: 16px;
      }
      ...
      .admin-ghl-sms-history-item[hidden],
      .admin-ghl-sms-history-item.is-admin-ghl-sms-filtered-out {
        display: none !important;
      }
      .admin-ghl-sms-history-item.is-inbound {
        align-items: flex-start;
      }
      .admin-ghl-sms-history-item.is-outbound {
      ...
      .admin-ghl-sms-filter-button[data-admin-ghl-sms-filter="automatic"] {
        flex-basis: 190px;
      }
      ...
      .admin-ghl-sms-history-item.is-inbound .admin-ghl-sms-history-bubble {
        border-bottom-left-radius: 6px;
        background: rgba(255,255,255,0.98);
      }
      .admin-ghl-sms-history-item.is-outbound .admin-ghl-sms-history-bubble {
        border-color: rgba(164, 64, 95, 0.28);
        border-bottom-right-radius: 6px;
        background: linear-gradient(135deg, rgba(164,64,95,0.96), rgba(112,45,70,0.96));
        color: #fff;
      }
      ...
      .admin-ghl-sms-history-item.is-outbound .admin-ghl-sms-history-message {
        color: #fff;
      }
      .admin-ghl-sms-message-meta {
        display: flex;
      ...
      .admin-ghl-sms-history-item.is-outbound .admin-ghl-sms-message-meta {
        color: rgba(255,255,255,0.78);
      }
      .admin-ghl-sms-recipient-meta {
        margin: 0 10px;
        color: var(--muted);
        font-size: 11px;
        line-height: 1.35;
      }
      .admin-ghl-sms-chat-form {
        display: grid;
      ...
      .admin-ghl-sms-chat-input {
        margin: 0;
        min-width: 0;
        width: 100%;
      }
      .admin-ghl-sms-chat-input .admin-input {
        min-height: 76px;
        resize: vertical;
        border: 0;
      ...
      .admin-ghl-sms-chat-input .admin-input:focus {
        outline: 2px solid rgba(164, 64, 95, 0.2);
        outline-offset: 2px;
      }
      .admin-ghl-sms-chat-actions {
        display: flex;
        align-items: center;
        align-self: stretch;
        gap: 10px;
      }
      .admin-ghl-sms-chat-actions .admin-button {
        min-height: 56px;
        white-space: nowrap;
      }
      .admin-ghl-sms-chat-actions .admin-action-hint {
        max-width: 220px;
        text-align: right;
      }
      .admin-history-title,
