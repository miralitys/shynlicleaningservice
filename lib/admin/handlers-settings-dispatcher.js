...
function createAdminSettingsHandlers(deps = {}) {
  ...
  async function handleSettingsRoute(context = {}) {
    ...
    try {
      if (action === "send_test_invite_email") {
        const testEmail = getFormValue(formBody, "testEmail", 200).toLowerCase();
        ...
        if (
          !testEmail ||
          !inviteEmailStatus.configured ||
          !accountInviteEmail ||
          typeof accountInviteEmail.sendTest !== "function"
        ) {
          redirectWithTiming(
            res,
            303,
            adminPageRenderers.buildSettingsUsersRedirectPath("mail-test-error"),
            requestStartNs,
            requestContext.cacheHit
          );
          return true;
        ...
        try {
          await accountInviteEmail.sendTest(
            {
              toEmail: testEmail,
          ...
          redirectWithTiming(
            res,
            303,
            adminPageRenderers.buildSettingsUsersRedirectPath("mail-test-sent"),
            requestStartNs,
            requestContext.cacheHit
          );
          return true;
