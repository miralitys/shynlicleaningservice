...
function isCustomerSmsHistoryEntry(owner = {}, entry = {}) {
  ...
  const ownerPhones = [owner.customerPhone, owner.phone, owner.secondaryPhone]
    .map(normalizeSmsPhoneDigits)
    .filter(Boolean);
  const entryPhone = normalizeSmsPhoneDigits(entry.phone);
  if (ownerPhones.length > 0 && entryPhone && ownerPhones.includes(entryPhone)) {
    return true;
  }
  ...
  return !ownerContactId && ownerPhones.length === 0;
}
