...
function createAdminOrdersCompletionHandlers(deps = {}) {
  ...
  const { restoreRecurringOrderSeries } = createAdminOrdersRecurringHelpers(deps);
  ...
  async function handleOrderCompletionAction(context = {}) {
    ...
    if (action === "restore-recurring-series") {
      if (!currentEntry) {
        redirectWithTiming(
          res,
          303,
      ...
      const restoredEntries = await restoreRecurringOrderSeries({
        quoteOpsLedger,
      ...
      redirectWithTiming(
        ...
        buildOrdersRedirect(
          returnTo,
          restoredEntries.length > 0 ? "recurring-series-restored" : "recurring-series-restore-unavailable"
        ),
        requestStartNs,
        requestContext.cacheHit
      );
