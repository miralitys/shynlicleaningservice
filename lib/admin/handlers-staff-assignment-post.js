...
const { findStaffAvailabilityConflicts } = require("../staff-availability");
...
function createAdminStaffAssignmentPostHandlers(deps = {}) {
  ...
  async function handleStaffAssignmentPostAction(context = {}) {
    ...
    if (action === "save-assignment") {
      ...
      if (staffStore && assignmentInput.staffIds.length > 0) {
        const staffSnapshot =
          typeof staffStore.getSnapshot === "function" ? await staffStore.getSnapshot() : { staff: [] };
        const calculatorData =
          typeof getEntryCalculatorData === "function" ? getEntryCalculatorData(entry) : {};
        const orderState = getEntryOrderState(entry);
        const serviceDurationMinutes = Number(
          entry.serviceDurationMinutes ||
            orderState.serviceDurationMinutes ||
            calculatorData.serviceDurationMinutes ||
            0
        );
        const manualAvailabilityConflicts = findStaffAvailabilityConflicts(
          staffSnapshot.staff,
          assignmentInput.staffIds,
          nextOrderScheduleDate,
          nextOrderScheduleTime,
          serviceDurationMinutes
        );
        if (manualAvailabilityConflicts.length > 0) {
          if (ajaxRequest) {
            writeAjaxMutationError(res, requestStartNs, requestContext, "assignment-conflict", 409, {
              staff: manualAvailabilityConflicts.map((item) => item.name).join(", "),
            });
            return true;
          }
          redirectWithTiming(
