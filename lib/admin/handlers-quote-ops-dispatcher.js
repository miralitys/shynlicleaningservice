    ...
    if (requestContext.route === ADMIN_QUOTE_OPS_PATH && req.method === "POST") {
      ...
      try {
        ...
        if (action === "load-quote-sms-history") {
          ...
          const allLocalHistoryEntries = getEntrySmsHistoryEntries(entry);
          const remoteHistoryEntries = await loadRemoteSmsHistoryEntries(leadConnectorClient, {
            ...
            conversationIds: extractSmsConversationIds(allLocalHistoryEntries),
          });
          const mergedHistoryEntries = mergeAdminSmsHistoryEntries(allLocalHistoryEntries, remoteHistoryEntries);
          let historyEntry = {
            ...entry,
            payloadForRetry: {
          ...
          if (
            mergedHistoryEntries.length > allLocalHistoryEntries.length &&
          ...
          const customerHistoryEntries = getEntryCustomerSmsHistoryEntries(historyEntry);
        ...
        if (action === "create-lead-task") {
          ...
          let taskEntryId = entryId;
          if (entryId === "standalone") {
            const requestId = `manual-task-${Date.now().toString(36)}-${Math.random()
              .toString(36)
              .slice(2, 8)}`;
            const standaloneEntry = await quoteOpsLedger.recordSubmission({
            ...
            taskEntryId = normalizeString(standaloneEntry && standaloneEntry.id, 120);
          }

          const updatedEntry = taskEntryId
            ? await quoteOpsLedger.updateLeadEntry(taskEntryId, {
        ...
        if (action === "delete-lead-task") {
          const taskId = getFormValue(formBody, "taskId", 120);
          const updatedEntry = taskId
            ? await quoteOpsLedger.updateLeadEntry(entryId, { deleteTaskId: taskId })
            : null;
          ...
          redirectWithTiming(
            ...
            303,
            buildAdminRedirectPath(returnTo, {
              notice: updatedEntry ? "task-deleted" : "lead-missing",
            }),
            requestStartNs,
            requestContext.cacheHit
          );
        ...
        if (action === "complete-lead-task") {
          ...
          if (updatedEntry && taskAction === "next-cleaning-agreed") {
            ...
            taskNotice = "client-marked-one-time";
          }
          const notice =
            !updatedEntry
              ? "lead-missing"
