...
function createAdminQuoteOpsHandlers(deps = {}) {
  ...
  const { cancelRecurringOrderSeries } = createAdminOrdersRecurringHelpers(deps);
  ...
  async function markNextCleaningDeclined(quoteOpsLedger, entry, staffStore) {
          ...
          if (updatedEntry && taskAction === "next-cleaning-agreed") {
            ...
            const latestEntry =
              typeof quoteOpsLedger.getEntry === "function"
                ? (await quoteOpsLedger.getEntry(entryId)) || updatedEntry
                : updatedEntry;
            const declineResult = await markNextCleaningDeclined(
              quoteOpsLedger,
              latestEntry,
              staffStore
            );
