...
function createAdminPageRenderers(deps = {}) {
      ...
      <div class="admin-order-media-stack">
        ...
        >
          ...
          <div class="admin-inline-actions admin-form-actions">
            <button class="admin-button admin-button-secondary" type="submit" data-admin-order-completion-submit>Сохранить фото</button>
  ...
  function renderOrdersLane(status, laneOrders, returnTo, planningByEntryId, staffRecords = [], options = {}) {
    ...
    return `<section class="admin-order-funnel-column admin-order-funnel-column-${escapeHtmlAttribute(status)}">
      <div class="admin-order-funnel-head">
        <div>
          <p class="admin-order-funnel-kicker">${escapeHtml(meta.kicker)}</p>
        ...
        <div class="admin-order-funnel-meta">
          ${renderAdminBadge(String(laneOrders.length), "outline")}
        </div>
      </div>
      <div class="admin-order-funnel-list">
        ${laneOrders
          .map((order) =>
  ...
  function renderOrderFunnelCard(order, returnTo, options = {}) {
    ...
    >
      ...
      <div class="admin-order-funnel-card-stage">${renderOrderStatusBadge(order.orderStatus)}</div>
      <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
    </article>`;
  }
  ...
  async function renderOrdersPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    const funnelStatuses = ["new", "scheduled", "in-progress", "rescheduled", "completed", "canceled"];
