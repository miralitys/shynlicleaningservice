...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderStaffTeamCalendarTable(staffSummaries, startDateValue) {
    ...
    const prevHref = buildStaffSectionPath("calendar", {
      calendarStart: addDaysToStaffCalendarDate(normalizedStartDate, -STAFF_TEAM_CALENDAR_DAYS),
    });
    const nextHref = buildStaffSectionPath("calendar", {
      calendarStart: addDaysToStaffCalendarDate(normalizedStartDate, STAFF_TEAM_CALENDAR_DAYS),
    });
    const todayHref = buildStaffSectionPath("calendar", {
            ...
            ${staffSummaries
              .map(
                (staffSummary) => `<tr>
                  ...
                  ${days.map((day) => `<td class="admin-team-calendar-day-cell">${renderStaffTeamCalendarCell(staffSummary, day)}</td>`).join("")}
                </tr>`
              )
              .join("")}
          </tbody>
  ...
  async function renderStaffPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    const sectionIntro = renderStaffSectionNav(
      activeSection,
      {
        ...
        activeStaffCount: planning.activeStaffCount,
        scheduledOrdersCount: planning.scheduledOrders.length,
      },
      calendarStartDate
    );
    const sectionContent =
      activeSection === "calendar"
        ...
        : renderAdminCard(
            ...
            renderStaffSummaryTable(staffSummaries, { canDelete: accessContext.canDelete }),
    ...
    return renderAdminLayout(
      ...
      `${renderStaffNotice(req)}
        ...
        ${renderStaffAssignmentDialogs(planning.orderItems, planning.staff)}`,
      {
        kicker: "",
        ...
        sidebar: renderAdminAppSidebar(ADMIN_STAFF_PATH, accessContext),
    ...
    const createUserForm = `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
        ...
        <div class="admin-form-grid admin-form-grid-two">
          ...
          <label class="admin-label">
            ...
            <input class="admin-input" type="text" name="staffRole" placeholder="Cleaner, Team Lead">
          </label>
          <label class="admin-label">
            Доступ в систему
            <select class="admin-input" name="status">
          ...
          .map((user) => {
            ...
            return renderAdminCard(
              ...
              `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
                ...
                <div class="admin-inline-badge-row">
                  ${renderAdminBadge(formatSettingsUserRoleLabel(user.role), user.role === "admin" ? "success" : user.role === "manager" ? "outline" : "muted")}
                ...
                <div class="admin-form-grid admin-form-grid-two">
                  ...
                  <label class="admin-label">
                    Должность сотрудника
                    <input class="admin-input" type="text" name="staffRole" value="${escapeHtmlText(linkedStaff ? linkedStaff.role : "")}" placeholder="Cleaner, Team Lead">
                  </label>
                  <label class="admin-label">
                    Доступ в систему
                    <select class="admin-input" name="status">
    ...
    const usersContent = `
      ...
      <div class="admin-section-grid">
        ${renderAdminCard(
          ...
          renderSettingsUsersSection(users, staffRecords, staffSummaryById, { canDelete: accessContext.canDelete }),
          { eyebrow: "Настройки", muted: true }
        )}
      </div>`;
