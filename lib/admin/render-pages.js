...
function createAdminPageRenderers(deps = {}) {
      ...
      <div class="admin-dialog-panel admin-client-dialog-panel">
        <div class="admin-dialog-head admin-dialog-hero">
          ...
          <div class="admin-inline-actions admin-dialog-head-actions admin-dialog-hero-actions">
            ...
            ${renderAdminDialogCloseButton("admin-client-detail-dialog")}
          </div>
        </div>
  ...
  async function renderDashboardPage(req, config, quoteOpsLedger, adminRuntime = {}) {
    ...
    return renderAdminLayout(
      ...
      ${renderAdminCard(
        "Таски на контроле",
        "Просроченные задачи и открытые таски на сегодня. Будущие дедлайны остаются в разделе «Заявки → Таски».",
  ...
  function renderDashboardLeadTasksSection({ overdueTaskRecords = [], todayTaskRecords = [] } = {}) {
    ...
    if (allTaskRecords.length === 0) {
      return `<div data-admin-dashboard-tasks="true" class="admin-alert admin-alert-info">Просроченных тасков и задач на сегодня пока нет.</div>`;
    }

    return `<div data-admin-dashboard-tasks="true" class="admin-settings-section-stack admin-dashboard-task-groups">
      ${renderDashboardLeadTaskGroup(
        "Просроченные",
        "Все открытые задачи с дедлайном до сегодняшнего дня.",
        overdueTaskRecords,
        "admin-dashboard-overdue-tasks"
      )}
      ${renderDashboardLeadTaskGroup(
        "На сегодня",
        "Открытые задачи, которые нужно закрыть сегодня.",
        todayTaskRecords,
        "admin-dashboard-today-tasks"
      )}
      ${allTaskRecords.map((taskRecord) => renderQuoteOpsTaskResultDialog(taskRecord, `${ADMIN_QUOTE_OPS_PATH}?section=tasks`)).join("")}
    </div>`;
  }

  function renderDashboardLeadTaskGroup(title, copy, taskRecords = [], dataAttr) {
    if (!Array.isArray(taskRecords) || taskRecords.length === 0) {
      return `<section data-${escapeHtmlAttribute(dataAttr)}="true" class="admin-dashboard-task-group">
        <div class="admin-card-head admin-dashboard-task-head">
          <div>
            <h3 class="admin-section-title">${escapeHtml(title)}</h3>
            <p class="admin-section-copy">${escapeHtml(copy)}</p>
          </div>
          ${renderAdminBadge("0", "muted")}
        </div>
        <div class="admin-alert admin-alert-info">Сейчас пусто.</div>
      </section>`;
    }
    ...
    return `<section data-${escapeHtmlAttribute(dataAttr)}="true" class="admin-dashboard-task-group">
      <div class="admin-card-head admin-dashboard-task-head">
        ...
        ${renderAdminBadge(String(taskRecords.length), title === "Просроченные" ? "danger" : "outline")}
      </div>
      <div class="admin-table-wrap admin-quote-table-wrap">
        <table class="admin-table admin-quote-task-table">
          ...
          <tbody>
            ${taskRecords.map((taskRecord) => renderQuoteOpsTaskTableRow(taskRecord, `${ADMIN_QUOTE_OPS_PATH}?section=tasks`)).join("")}
          </tbody>
        </table>
      </div>
    </section>`;
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}"${autoOpenAttr}${closeHref ? ` data-admin-dialog-return-url="${escapeHtmlAttribute(closeHref)}"` : ""} aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel admin-order-dialog-panel">
        <div class="admin-dialog-head admin-dialog-hero">
          <div class="admin-dialog-hero-main">
            <span class="admin-client-avatar admin-client-avatar-large ${escapeHtmlAttribute(getAdminClientAvatarToneClass(order.customerName || order.requestId || order.id))}">${escapeHtml(getAdminClientAvatarInitials(order.customerName || order.requestId || "Заказ"))}</span>
            <div class="admin-dialog-copy-block admin-dialog-hero-copy">
              ...
              <div class="admin-dialog-hero-title-block">
                ...
                <p class="admin-dialog-hero-detail">${escapeHtml(orderHeaderMeta || "Карточка заказа, график и управление в одном окне.")}</p>
                <p class="admin-dialog-hero-detail admin-order-dialog-address">${escapeHtml(orderHeaderAddress)}</p>
              </div>
            </div>
          </div>
          <div class="admin-inline-actions admin-dialog-head-actions admin-dialog-hero-actions">
  ...
  function renderQuoteOpsTaskResultDialog(taskRecord, returnTo, dialogIdOverride = "") {
    ...
    return `<dialog class="admin-dialog" id="${dialogId}" aria-labelledby="${dialogId}-title">
      <div class="admin-dialog-panel admin-quote-task-dialog-panel">
        <div class="admin-quote-task-dialog-head">
          <div class="admin-quote-task-dialog-head-main">
            ...
            <div class="admin-quote-task-dialog-head-copy">
              <p class="admin-card-eyebrow">Таск</p>
              <div class="admin-quote-task-dialog-title-block">
                <h2 class="admin-dialog-title" id="${dialogId}-title">${escapeHtml(taskRecord.customerName)}</h2>
                <p class="admin-quote-task-dialog-lead-copy">${escapeHtml(taskRecord.title)}</p>
                <p class="admin-quote-task-dialog-service">${escapeHtml(taskRecord.serviceLabel)}</p>
              </div>
            </div>
          </div>
          <div class="admin-inline-actions admin-dialog-head-actions">
