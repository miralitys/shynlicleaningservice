...
function createAdminPageRenderers(deps = {}) {
    ...
    return renderAdminLayout(
      ...
      `${renderClientsNotice(req)}
        <div class="admin-clients-layout">
          ${renderAdminCard(
            "База клиентов",
            "Поиск справа и фильтры по кнопке.",
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const quoteCards = [
      `<article class="admin-client-contact-card">
        <span class="admin-client-contact-label">Параметры уборки</span>
        ${renderAdminPropertyList([
          ...
          { label: "Дата", value: formatOrderQuoteText(quoteData.selectedDate || order.selectedDate, "Не указана", 32) },
          { label: "Время", value: formatOrderQuoteText(quoteData.selectedTime || order.selectedTime, "Не указано", 32) },
          { label: "Как на форме", value: formatOrderQuoteText(quoteData.formattedDateTime, "Не указано", 120) },
          {
      ...
      `<article class="admin-client-contact-card">
        <span class="admin-client-contact-label">Дом и условия</span>
        ${renderAdminPropertyList([
      ...
      `<article class="admin-client-contact-card">
        <span class="admin-client-contact-label">Дополнительные услуги</span>
        ${renderAdminPropertyList([
          {
            label: "Add-ons",
            value: selectedServices.length > 0 ? selectedServices.join(", ") : "Не выбраны",
          },
          {
            label: ORDER_QUOTE_QUANTITY_LABELS.interiorWindowsCleaning,
            value: getOrderQuoteQuantityValue(quoteData, "interiorWindowsCleaning"),
          },
          {
            label: ORDER_QUOTE_QUANTITY_LABELS.blindsCleaning,
            value: getOrderQuoteQuantityValue(quoteData, "blindsCleaning"),
          },
          {
            label: ORDER_QUOTE_QUANTITY_LABELS.bedLinenChange,
            value: getOrderQuoteQuantityValue(quoteData, "bedLinenChange"),
          },
        ])}
      </article>`,
      `<article class="admin-client-contact-card">
        <span class="admin-client-contact-label">Адрес из формы</span>
        ${renderAdminPropertyList([
          { label: "Адрес", value: quoteAddressLabel },
      ...
      `<article class="admin-client-contact-card admin-client-contact-card-wide">
        <span class="admin-client-contact-label">Комментарий клиента</span>
        <p class="admin-client-contact-value">${formatOrderQuoteMultiline(quoteData.additionalDetails)}</p>
      </article>`,
    ].join("");
    ...
    return `<div class="admin-table-wrap admin-quote-table-wrap">
      <table class="admin-table admin-quote-success-table">
        ...
        <tbody>
          ${entries.map((entry) => {
            const scheduleLabel = [entry.selectedDate, entry.selectedTime].filter(Boolean).join(" в ") || "Не указана";
            const clientContactLabel = [entry.customerPhone || "", entry.customerEmail || ""].filter(Boolean).join(" • ") || "Контакты не указаны";
            ...
            return `<tr>
              <td>
                <div class="admin-table-cell-stack">
                  <span class="admin-table-strong">${escapeHtml(entry.customerName || "Клиент")}</span>
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger) {
    ...
    const activeFilterCount = [filters.q, filters.status !== "all" ? filters.status : "", filters.serviceType !== "all" ? filters.serviceType : ""].filter(Boolean).length;
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-quote-ops-page">
        ...
        ${renderAdminCard(
          "Рабочая выборка",
          "Поиск и фильтры спрятаны в кнопку, а ниже остаётся только рабочая лента.",
          `<details class="admin-filter-disclosure admin-quote-ops-filter-disclosure">
            <summary class="admin-clients-toolbar-button">
              <span>Поиск и фильтры</span>
              ${activeFilterCount > 0 ? `<span class="admin-clients-toolbar-count">${escapeHtml(String(activeFilterCount))}</span>` : ""}
            </summary>
            <div class="admin-filter-disclosure-panel admin-quote-ops-filter-panel">
        ...
        ${renderAdminCard(
          ...
          `<ul class="admin-feature-list">
            ...
            <li>Успешные заявки собраны в таблицу для быстрого сканирования.</li>
            <li>Карточки с ошибками и warning оставлены развернутыми, чтобы не терялись детали.</li>
            <li>Клиент теперь связывается с заявкой по имени и адресу, поэтому разные адреса не склеиваются.</li>
          </ul>`,
