...
function createAdminPageRenderers(deps = {}) {
        ...
        .map((entry) => {
          ...
          return `<article class="admin-history-item admin-client-history-item admin-ghl-sms-history-item">
            <div class="admin-client-history-header">
              ...
              <div class="admin-client-history-side admin-ghl-sms-history-badges">${metaBadges}</div>
            </div>
        ...
        <div class="admin-ghl-sms-compose-column">
          ...
          ${!hasPhone && !hasContact
            ...
            : `<form class="admin-form admin-form-grid admin-ghl-sms-form" method="post" action="${escapeHtmlAttribute(actionPath)}" data-admin-ghl-sms="true">
                ...
                <label class="admin-label">
                  Сообщение
                  <textarea class="admin-input" name="message" rows="4" maxlength="1000" placeholder="Напишите клиенту короткое SMS-сообщение...">${escapeHtml(composerState.draft)}</textarea>
                </label>
      ...
      return `<section class="admin-order-section-card">
        ...
        ${canResend
          ? `<div class="admin-inline-actions admin-form-actions">
              ...
              <span class="admin-action-hint">Отправим новый email и SMS, а ссылка будет активна ещё 48 часов.</span>
            </div>`
          : ""}
      </section>`;
    }
      ...
      ${certificateUrl || canResend
        ? `<div class="admin-inline-actions admin-form-actions">
            ...
            ${canResend
              ...
              : ""}
            ${canResend ? `<span class="admin-action-hint">Повторная отправка продлит ссылку ещё на 48 часов и заново отправит email и SMS.</span>` : ""}
          </div>`
        : ""}
    </section>`;
  }
  ...
  function renderOrdersLane(status, laneOrders, returnTo, planningByEntryId, staffRecords = [], options = {}) {
    ...
    return `<section class="admin-order-funnel-column admin-order-funnel-column-${escapeHtmlAttribute(status)}">
      ...
      <div class="admin-order-funnel-list" data-order-dropzone="${escapeHtmlAttribute(status)}" data-order-funnel-list="true">
        ${laneOrders
          .map((order) =>
  ...
  function renderOrderFunnelCard(order, returnTo, options = {}) {
    ...
    return `<article
      class="admin-order-funnel-card admin-order-funnel-card-${escapeHtmlAttribute(order.orderStatus)}"
      data-order-funnel-card="true"
      data-order-funnel-status="${escapeHtmlAttribute(order.orderStatus)}"
    ...
    >
      ...
      <div class="admin-order-funnel-card-stage" data-order-card-stage="true">${renderOrderStatusBadge(order.orderStatus)}</div>
      <input type="hidden" name="returnTo" value="${escapeHtmlAttribute(returnTo)}">
    </article>`;
  }
  ...
  function renderOrdersFunnelScript(currentReturnTo, canEdit) {
    ...
    return `<script>
      (() => {
        ...
        const statusClasses = ["new", "scheduled", "in-progress", "rescheduled", "invoice-sent", "paid", "awaiting-review", "completed", "canceled"];
        ...
        function applyOrderPayloadToCard(card, payload) {
          ...
          const status = String(payload.order.orderStatus || "new");
  ...
  async function renderOrdersPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    const funnelStatuses = ["new", "scheduled", "in-progress", "rescheduled", "invoice-sent", "paid", "awaiting-review", "completed", "canceled"];
                    ...
                    ${funnelStatuses
                      .map((status) =>
                        renderOrdersLane(
                          ...
                          planningByEntryId,
                          staffSnapshot.staff,
                          { canDelete: accessContext.canDelete, canEdit: accessContext.canEdit }
                        )
                      )
                      .join("")}
                  </div>
