...
function createAdminPageRenderers(deps = {}) {
  ...
  function getDashboardClientDialogId(clientKey) {
    const normalized = normalizeString(clientKey, 180).toLowerCase();
    const safeKey = normalized.replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    return `admin-dashboard-client-dialog-${safeKey || "client"}`;
  }
  ...
  function renderDashboardClientsWithoutTeamTable(records = [], options = {}) {
    ...
    return `<div data-admin-dashboard-unassigned-clients="true" class="admin-table-wrap admin-clients-table-wrap">
        ...
        <tbody>
          ${records
            .map(({ client, unassignedOrders, nextOrder }) => {
              const dialogId = getDashboardClientDialogId(client.key);
                ...
                <td>
                  <div class="admin-client-table-cell">
                    ...
                    <div class="admin-table-stack">
                      <span class="admin-table-link">${escapeHtml(client.name || "Клиент")}</span>
                ...
                <td>
                  ${renderAdminClientAddressPreview(
                    nextOrder && nextOrder.fullAddress
                      ? {
                          addresses: [{ address: nextOrder.fullAddress }],
                          address: nextOrder.fullAddress,
                        }
                      : client
                  )}
                </td>
      ...
      ${records
        .map(({ client, nextOrder }) =>
          renderAdminClientDetailDialog(client, returnTo, {
            dialogId: getDashboardClientDialogId(client.key),
            autoOpen: false,
            closeHref: "",
            activeAddressRecord:
              Array.isArray(client.addresses) && nextOrder && nextOrder.fullAddress
                ? client.addresses.find((addressRecord) => normalizeString(addressRecord && addressRecord.address, 500) === normalizeString(nextOrder.fullAddress, 500)) || client.addresses[0] || null
                : null,
            planningByEntryId,
          })
        )
