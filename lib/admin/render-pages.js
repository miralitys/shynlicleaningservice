...
function createAdminPageRenderers(deps = {}) {
      ...
      (() => {
        ...
        const isInteractiveTarget = (target) =>
          Boolean(
            target &&
            target.closest &&
            target.closest('a, button, input, select, textarea, summary, label, [data-admin-dialog-open], [data-admin-dialog-close]')
          );
    ...
    return renderAdminLayout(
        ...
        </div>`,
      {
        ...
        heroActions: `<div class="admin-dialog-launcher admin-dialog-launcher-inline">
          <a class="admin-button" href="${buildSettingsSectionPath("users")}">Создать сотрудника</a>
        </div>`,
  ...
  function renderSettingsUsersSection(users, staffRecords, staffSummaryById = new Map()) {
    const createUserForm = `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
        ...
        <p class="admin-helper-copy">После создания сотрудник сразу появится в команде, назначениях и сможет входить через <strong>/account/login</strong>.</p>
      </form>`;
    ...
    const userCards = users.length > 0
      ? users
            ...
            return renderAdminCard(
              ...
              `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
                ...
                <div class="admin-form-grid admin-form-grid-two">
                  ...
                  <label class="admin-label">
                    Роль
                    <input class="admin-input" type="text" name="role" value="${escapeHtmlText(linkedStaff ? linkedStaff.role : "")}" placeholder="Cleaner, Team Lead">
                  </label>
                  <label class="admin-label">
                    Доступ в систему
                    <select class="admin-input" name="status">
          ...
          .join("")
      : `<div class="admin-empty-state">Пользователей пока нет. Создайте первого сотрудника прямо здесь, и у него сразу появится личный кабинет.</div>`;

    return `<div id="settings-users"></div>
      <div class="admin-section-grid">
        ${renderAdminCard(
          "Новый сотрудник",
          "Создайте сотрудника и его личный кабинет одним действием.",
          createUserForm,
