...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderAdminSmsHistoryItems(entries = []) {
    ...
    return `<div class="admin-history-list admin-ghl-sms-history-list" data-admin-ghl-sms-history-list>
      ${entries
        .map((entry) => {
  ...
  function renderOrderPolicyAcceptancePanel(order) {
    ...
    const statusBadge = accepted
      ...
      : policy.lastError
        ...
        : renderAdminBadge("Ожидает подтверждения", "outline");
      ...
      <div class="admin-dialog-panel admin-order-dialog-panel">
        ...
        <div class="admin-order-dialog-layout">
          ...
          <aside class="admin-order-dialog-side">
            ...
            ${renderOrderPolicyAcceptancePanel(order)}
          </aside>
        </div>
      </div>
    </dialog>`;
