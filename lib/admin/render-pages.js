...
function createAdminPageRenderers(deps = {}) {
    ...
    return `<div class="admin-client-address-input-row" data-admin-client-address-row>
      <div class="admin-address-field" data-admin-address-field>
        <input
          ...
          type="text"
          name="addresses"
          value="${escapeHtmlAttribute(normalizeString(value, 500))}"
  ...
  function renderAdminClientEditPanel(client, returnTo = ADMIN_CLIENTS_PATH, panelId = "") {
    ...
    const addressValues = Array.isArray(client.addresses)
      ? client.addresses.map((addressRecord) => addressRecord.address).filter(Boolean)
      : [];
    const initialAddresses = addressValues.length > 0 ? addressValues : [""];
    ...
    return `<section class="admin-client-section admin-staff-form-section" id="${escapeHtmlAttribute(panelId)}" data-admin-toggle-panel hidden>
      ...
      <form class="admin-form-grid" method="post" action="${ADMIN_CLIENTS_PATH}">
        ...
        <div class="admin-form-grid admin-form-grid-two">
          ...
          <label class="admin-label">
            Телефон
            <input class="admin-input" type="tel" name="phone" value="${escapeHtmlText(formatAdminPhoneNumber(client.phone) || client.phone)}"${renderAdminPhoneInputAttributes()}>
          </label>
          <label class="admin-label">
            Email
        ...
        <div class="admin-label">
          ...
          <div class="admin-client-address-editor" data-admin-client-address-editor data-admin-client-address-next-index="${escapeHtmlAttribute(String(initialAddresses.length))}">
            <div class="admin-client-address-input-list" data-admin-client-address-list>
              ${initialAddresses.map((address, index) => renderAdminClientAddressInputRow(panelId, address, index)).join("")}
            </div>
            ...
            <small class="admin-field-hint">Каждый адрес хранится отдельной строкой. По клику на адрес карточка переключается на нужный дом.</small>
            <template id="${escapeHtmlAttribute(addressTemplateId)}">${renderAdminClientAddressInputRow(panelId, "", "__INDEX__")}</template>
          </div>
        </div>
        <div class="admin-inline-actions">
        ...
        <div class="admin-dialog-head">
          ...
          <div class="admin-inline-actions admin-dialog-head-actions">
            <button
              class="admin-button"
              type="button"
              data-admin-toggle-target="${escapeHtmlAttribute(editPanelId)}"
              data-admin-toggle-label-open="Скрыть"
              data-admin-toggle-label-closed="Редактировать"
              aria-controls="${escapeHtmlAttribute(editPanelId)}"
              aria-expanded="false"
                            ...
                            <td>
                              <div class="admin-client-table-cell">
                                ...
                                <div class="admin-table-stack">
                                  ...
                                  ${client.addressCount > 1 ? `<span class="admin-table-muted">Ещё ${escapeHtml(formatClientAddressCountLabel(client.addressCount - 1))}.</span>` : ""}
                                </div>
                              </div>
                            </td>
                            <td>
  ...
  function renderSettingsTemplateEditor(template) {
  ...
  function renderChecklistTemplateDialog(template) {
    ...
    return `<dialog class="admin-dialog" id="${dialogId}" aria-labelledby="${dialogId}-title">
      <div class="admin-dialog-panel">
        ...
        ${renderSettingsTemplateEditor(template)}
      </div>
    </dialog>`;
  }
    ...
    const rows = templates
      .map((template) => {
        ...
        return `<tr>
          <td>
            <div class="admin-table-cell-stack">
              <button class="admin-table-link-button" type="button" data-admin-dialog-open="${dialogId}">${escapeHtml(template.title)}</button>
          ...
          <td>
            <div class="admin-table-cell-stack">
              <span class="admin-table-strong">${escapeHtml(String(completedCount))}</span>
              <span class="admin-table-muted">${escapeHtml(`из ${template.items.length}`)}</span>
            </div>
          </td>
          <td>
            <button class="admin-button admin-button-secondary admin-table-action-button" type="button" data-admin-dialog-open="${dialogId}">Открыть</button>
          </td>
        </tr>`;
      })
      .join("");
  ...
  function renderSettingsUserEditor(user, linkedStaff, staffSummary, options = {}) {
    ...
    return `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
      ...
      <div class="admin-form-grid admin-form-grid-two">
        ...
        <label class="admin-label">
          Телефон
          <input class="admin-input" type="tel" name="phone" value="${escapeHtmlText(formatAdminPhoneNumber(user.phone) || user.phone)}"${renderAdminPhoneInputAttributes()}>
        </label>
      </div>
      ${renderStaffAddressField({
  ...
  function renderSettingsUsersSection(users, staffRecords, staffSummaryById = new Map(), options = {}) {
    ...
    users.forEach((user) => {
        ...
        userRows.push(`<tr>
          <td>
            <div class="admin-table-cell-stack">
              <button class="admin-table-link-button" type="button" data-admin-dialog-open="${dialogId}">${escapeHtml(linkedStaff ? linkedStaff.name : user.email)}</button>
          ...
          <td>
            <div class="admin-table-cell-stack">
              ...
              <span class="admin-table-muted">${escapeHtml(formatAdminPhoneNumber(user.phone) || user.phone || "Телефон не указан")}</span>
            </div>
          </td>
          <td>
            <div class="admin-inline-badge-row">
  ...
  async function renderSettingsPage(req, config, settingsStore, usersStore, staffStore, quoteOpsLedger, adminRuntime = {}) {
    ...
    const usersContent = `
      ...
      <div class="admin-settings-section-stack">
        ${renderInviteEmailStatusCard(inviteEmailStatus)}
        ${renderAdminCard(
          "Пользователи",
