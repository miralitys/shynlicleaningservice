...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderAdminPhoneInput(name, value = "", options = {}) {
    ...
    return `<label class="admin-label">
      ...
      <input class="admin-input admin-phone-input" type="tel" name="${escapeHtmlAttribute(name)}" value="${escapeHtmlText(inputValue)}" data-admin-phone-input="true" inputmode="numeric" autocomplete="${escapeHtmlAttribute(autocomplete)}" maxlength="10" placeholder="${escapeHtmlAttribute(placeholder)}"${requiredAttr}>
    </label>`;
  }
