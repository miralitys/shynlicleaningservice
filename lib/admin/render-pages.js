...
function createAdminPageRenderers(deps = {}) {
  ...
  function formatAdminPhoneFieldValue(value) {
    const formattedPhone = formatAdminPhoneNumber(value);
    if (!formattedPhone) return "";
    return `(${formattedPhone.slice(3, 6)}) ${formattedPhone.slice(7, 10)}-${formattedPhone.slice(11, 15)}`;
  }
  ...
  function renderAdminPhoneInput(name, value = "", options = {}) {
    ...
    const placeholder = normalizeString(options.placeholder, 40) || "(000) 000-0000";
    const inputValue = formatAdminPhoneFieldValue(value);

    return `<label class="admin-label">
      ${escapeHtml(fieldLabel)}
      <span class="admin-phone-field">
        <span class="admin-phone-prefix" aria-hidden="true">+1</span>
        <input class="admin-input admin-phone-input" type="tel" name="${escapeHtmlAttribute(name)}" value="${escapeHtmlText(inputValue)}" data-admin-phone-input="true" inputmode="numeric" autocomplete="${escapeHtmlAttribute(autocomplete)}" maxlength="14" placeholder="${escapeHtmlAttribute(placeholder)}">
      </span>
    </label>`;
  }
            ...
            ${w9Record
              ? `${renderAdminClientInfoGrid(w9InfoItems)}
                <div class="admin-inline-actions" style="margin-top:14px;">
                  <a class="admin-button admin-button-secondary" href="${escapeHtmlAttribute(w9DownloadPath)}">Открыть PDF</a>
                </div>`
              : `<div class="admin-empty-state">Сотрудник ещё не заполнил W-9 в личном кабинете.</div>`}
          </section>
