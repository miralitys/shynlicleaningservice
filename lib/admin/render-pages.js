  ...
  function renderStaffTravelEstimateScript() {
    ...
    return `<script>
      (() => {
        ...
        function resolveRoute(nodes, mapsApi) {
          ...
          if (!origin || !destination) {
            updateNodes(nodes, (sourceLabel) => sourceLabel + ": маршрут недоступен");
            return;
          }
          ...
          const request = {
            ...
            unitSystem: mapsApi.UnitSystem ? mapsApi.UnitSystem.IMPERIAL : undefined,
          };
          ...
          service.getDistanceMatrix(request, (response, status) => {
            if (status !== mapsApi.DistanceMatrixStatus.OK) {
              const fallbackFormatter = (sourceLabel) => sourceLabel + ": маршрут недоступен";
            ...
            if (!element || element.status !== mapsApi.DistanceMatrixElementStatus.OK) {
              const fallbackFormatter = (sourceLabel) => sourceLabel + ": маршрут недоступен";
              cache.set(cacheKey, fallbackFormatter);
              updateNodes(nodes, fallbackFormatter);
              return;
            ...
            const durationText =
              element.duration_in_traffic && element.duration_in_traffic.text
                ? element.duration_in_traffic.text
                : element.duration && element.duration.text
                  ? element.duration.text
                  : "";
            const distanceText = element.distance && element.distance.text ? element.distance.text : "";
            const successFormatter = (sourceLabel) => [sourceLabel, durationText, distanceText].filter(Boolean).join(" • ");
            cache.set(cacheKey, successFormatter);
            updateNodes(nodes, successFormatter);
          });
        }

        getMapsApi().then((mapsApi) => {
          if (!mapsApi || typeof mapsApi.DistanceMatrixService !== "function") {
            estimateNodes.forEach((node) => {
              const sourceLabel = node.getAttribute("data-admin-travel-source-label") || "Маршрут";
              node.textContent = node.getAttribute("data-admin-travel-fallback") || (sourceLabel + ": маршрут недоступен");
            });
            return;
          }
          ...
          groupedNodes.forEach((nodes) => resolveRoute(nodes, mapsApi));
        });
      })();
    </script>`;
  }
