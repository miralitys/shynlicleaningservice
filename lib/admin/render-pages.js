...
function createAdminPageRenderers(deps = {}) {
    ...
    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}" aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel">
        ...
        <section>
          ...
          <div class="admin-settings-section-stack admin-quote-entry-detail-stack">
            ${renderAdminCard(
              "Воронка заявки",
            ...
            ${openTaskRecords.length > 0
              ? renderAdminCard(
                  ...
                  `<div class="admin-quote-entry-quick-actions-list">${openTaskRecords.map((task) => {
  ...
  function renderQuoteOpsWorkspaceStyle() {
    return `<style>
      ...
      .admin-quote-funnel-card-manager {
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 12px;
      ...
      .admin-quote-funnel-card-kicker {
        margin: 0;
        color: var(--muted);
        font-size: 11px;
        font-weight: 700;
      ...
      .admin-quote-funnel-card-manager-name {
        margin: 0;
        font-size: 15px;
        font-weight: 700;
        line-height: 1.35;
      ...
      .admin-quote-funnel-card-detail:first-child {
        padding-top: 0;
        border-top: 0;
      }
  ...
  function renderQuoteOpsFunnelCard(entry, returnTo) {
    ...
    const managerLabel = manager.name || manager.email || "Без менеджера";
    ...
    >
      <div class="admin-quote-funnel-card-head">
        ...
        <div class="admin-quote-funnel-card-stage">${renderLeadStatusBadge(leadStatus)}</div>
      </div>
      <div class="admin-quote-funnel-card-manager">
        <p class="admin-quote-funnel-card-kicker">Менеджер</p>
        <p class="admin-quote-funnel-card-manager-name">${escapeHtml(managerLabel)}</p>
      </div>
      <div class="admin-quote-funnel-card-details">
        <div class="admin-quote-funnel-card-detail">
          ...
          <p class="admin-quote-funnel-card-detail-value">${escapeHtml(taskLabel)}</p>
        </div>
        <div class="admin-quote-funnel-card-detail">
          <p class="admin-quote-funnel-card-detail-label">Дедлайн</p>
          <p class="admin-quote-funnel-card-detail-value">${escapeHtml(dueLabel)}</p>
        </div>
        <div class="admin-quote-funnel-card-detail">
      ...
      <div class="admin-dialog-panel">
        <div class="admin-dialog-head">
          <div>
            ...
            <h2 class="admin-dialog-title" id="${dialogId}-title">${escapeHtml(taskRecord.title)}</h2>
            <p class="admin-dialog-copy">${escapeHtml(taskRecord.customerName)} • ${escapeHtml(taskRecord.title)}</p>
          </div>
          <button class="admin-button admin-button-secondary" type="button" data-admin-dialog-close="${dialogId}">Закрыть</button>
        </div>
        <div class="admin-client-summary-panel">
          <div class="admin-badge-row">
            ${renderLeadStatusBadge(taskRecord.leadStatus)}
            ${renderAdminBadge(managerLabel, "outline")}
          </div>
          <div class="admin-client-info-grid admin-client-info-grid-compact">
            <article class="admin-client-info-card">
              <span class="admin-client-info-label">Дедлайн</span>
              <p class="admin-client-info-value">${escapeHtml(formatAdminDateTime(taskRecord.dueAt))}</p>
            </article>
            <article class="admin-client-info-card">
              <span class="admin-client-info-label">Заявка</span>
              <p class="admin-client-info-value">${escapeHtml(requestLabel)}</p>
            </article>
            <article class="admin-client-info-card">
              <span class="admin-client-info-label">Контакты</span>
              <p class="admin-client-info-value">${escapeHtml(phoneLabel)}</p>
            </article>
            <article class="admin-client-info-card">
              <span class="admin-client-info-label">Уборка</span>
              <p class="admin-client-info-value">${escapeHtml(scheduleLabel)}</p>
            </article>
          </div>
        </div>
        <div class="admin-settings-section-stack">
          ${renderAdminCard(
            "Быстрые действия",
            "Отметьте результат звонка или переведите заявку дальше.",
            `<div class="admin-inline-actions">
              <form method="post" action="${ADMIN_QUOTE_OPS_PATH}">
                <input type="hidden" name="action" value="complete-lead-task">
        ...
        <form class="admin-form admin-form-grid admin-form-grid-two" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
          <input type="hidden" name="action" value="complete-lead-task">
          <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
          <input type="hidden" name="taskId" value="${escapeHtmlAttribute(taskRecord.id)}">
    ...
    >
      ...
      <td>
        <div class="admin-table-cell-stack">
          ...
          <span class="admin-table-muted">${escapeHtml(taskRecord.entry.selectedDate || taskRecord.entry.selectedTime ? buildFormattedScheduleLabel(taskRecord.entry.selectedDate, taskRecord.entry.selectedTime) || "Дата не назначена" : "Дата не назначена")}</span>
        </div>
      </td>
      <td>
        <div class="admin-table-cell-stack">
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger, adminRuntime = {}, staffStore = null) {
    ...
    const interactionHint =
      activeSection === "funnel"
        ? "Перетаскивайте карточки между колонками или откройте заявку по клику."
        : activeSection === "tasks"
          ? "Откройте заявку или отметьте результат звонка прямо из строки."
          : "Откройте заявку по строке, чтобы посмотреть форму и перевести её дальше.";
      ...
      <div class="admin-clients-meta-row">
        <div class="admin-clients-meta-main">
          <p class="admin-clients-summary-copy">
            ...
            ${hasActiveFilters ? "С учётом поиска и фильтров." : "Показана общая рабочая выборка."}
          </p>
          ${filterBadges ? `<div class="admin-inline-badge-row">${filterBadges}</div>` : ""}
        </div>
        <span class="admin-action-hint">${escapeHtml(interactionHint)}</span>
      </div>
    </section>`;
