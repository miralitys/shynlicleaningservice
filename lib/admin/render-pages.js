  ...
  function renderSettingsCreateUserForm(options = {}) {
    ...
    const inviteHelperCopy = options.inviteEmailConfigured
      ? inviteStatus.googleConnected
        ? `После создания сотрудник получит письмо через Gmail API от ${escapeHtml(inviteStatus.googleAccountEmail || "подключённого ящика")}. Временный пароль нужно передать отдельно.`
