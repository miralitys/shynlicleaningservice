    ...
    return renderAdminCard(
      ...
      <div class="admin-client-action-stack">
        <div class="admin-client-action-row">
          <a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(closeHref)}">Закрыть карточку</a>
    ...
    return renderAdminLayout(
      ...
      `${renderClientsNotice(req)}
        <div class="admin-clients-layout">
          ${renderAdminCard(
            "База клиентов",
            "CRM-таблица клиентов с поиском справа, точными фильтрами под кнопкой и карточкой по клику на имя.",
            `<div class="admin-clients-workspace">
              ...
              <div class="admin-clients-summary-row">
                <p class="admin-clients-summary-copy">
                  ...
                  ${hasActiveFilters ? "Список сужен по поиску или фильтрам." : "Показан полный список клиентов."}
                </p>
                <div class="admin-clients-summary-side">
                  <div class="admin-badge-row">
                    ...
                    ${selectedClient ? renderAdminBadge(`Открыта карточка: ${selectedClient.name || "Клиент"}`, "success") : ""}
                  </div>
                  <span class="admin-action-hint">${selectedClient ? "Карточка клиента открыта ниже таблицы." : "Нажмите на имя клиента, чтобы открыть карточку."}</span>
                </div>
              </div>
              ${diagnosticsMarkup
                ? `<details class="admin-filter-disclosure admin-clients-diagnostics-disclosure"${shouldOpenDiagnostics ? " open" : ""}>
                    <summary class="admin-clients-toolbar-button admin-clients-toolbar-button-secondary">Диагностика quote ops</summary>
                    <div class="admin-filter-disclosure-panel">
                      ${diagnosticsMarkup}
                    </div>
                  </details>`
                    ...
                    <tbody>
                      ${filteredClients
                        .map((client) => {
                          ...
                          const rowHref = `${rowPath}#client-card`;
            ...
            ${selectedClient
              ? `<div id="client-card">
                  ${renderAdminClientDetailCard(selectedClient, currentReturnTo, { closeHref: closeDetailHref })}
                </div>`
              : ""}
            </div>`,
            { eyebrow: "Клиенты" }
      ...
      `${renderStaffNotice(req)}
        ...
        <div class="admin-section-grid">
          ${renderAdminCard(
            ...
            "Создание новой карточки открывается в отдельном окне, чтобы рабочий экран команды оставался чистым.",
            `<div class="admin-dialog-launcher">
              <button class="admin-button" type="button" data-admin-dialog-open="admin-staff-create-dialog">Добавить сотрудника</button>
              <span class="admin-action-hint">Откроется отдельное окно с формой. После сохранения сотрудник сразу появится в списке и станет доступен для назначений.</span>
