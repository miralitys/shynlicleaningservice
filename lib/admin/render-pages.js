...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderAdminClientDetailPanel(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
    ...
    const contactItems = [
      ...
      { label: "Телефон", value: formatAdminPhoneNumber(client.phone) || "Не указан" },
    ...
    return renderAdminLayout(
      ...
      `${renderClientsNotice(req)}
        <div class="admin-clients-layout">
          ${renderAdminCard(
            "База клиентов",
            "Полный список клиентов с быстрым поиском по имени, email и телефону.",
      ...
      {
        kicker: false,
        subtitle: "CRM-таблица клиентов с быстрым поиском и карточкой по клику на имя.",
        ...
        <div class="admin-client-table-cell">
          ...
          <div class="admin-table-stack">
            <button
              ...
              type="button"
              data-admin-dialog-open="${escapeHtmlAttribute(dialogId)}"
            >${escapeHtml(staffSummary.name || "Сотрудник")}</button>
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger) {
    ...
    const filtersSummary = hasActiveFilters
      ? "Активные фильтры сужают ленту до нужных заявок."
      : "Сейчас показан весь поток входящих заявок.";
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-quote-ops-page">
        ...
        <section class="admin-orders-filters-panel admin-quote-ops-filter-shell">
          <div class="admin-orders-panel-head">
            <div>
              <p class="admin-orders-panel-kicker">Фильтры</p>
              <h2 class="admin-orders-panel-title">Поиск и фильтры</h2>
              <p class="admin-orders-panel-copy">Поиск по имени, телефону, email, request ID и адресу. Панель открывается по кнопке, чтобы лента оставалась чистой.</p>
            </div>
              ...
              <details class="admin-filter-disclosure admin-quote-ops-filter-disclosure">
                ...
                <div class="admin-filter-disclosure-panel admin-quote-ops-filter-panel">
                  <form class="admin-form-grid admin-form-grid-three admin-quote-ops-filter-form" method="get" action="${ADMIN_QUOTE_OPS_PATH}">
                    <label class="admin-label">
                      Поиск
                      <input class="admin-input" type="search" name="q" value="${escapeHtmlText(filters.q)}" placeholder="Имя, телефон, email, request ID, адрес">
                    </label>
                    <label class="admin-label">
                      Статус
                      <select class="admin-input" name="status">
          ...
          <div class="admin-quote-ops-filter-meta">
            ...
            <div class="admin-quote-ops-filter-meta-row">
              <p class="admin-action-hint">${escapeHtml(filtersSummary)}</p>
              <span class="admin-action-hint">Показано ${escapeHtml(String(filteredCount))} заявок.</span>
            </div>
          </div>
        </section>
