...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderAdminDeleteIconButton(label, confirmMessage) {
    ...
    const confirmAttrs = normalizedConfirm
      ? ` data-admin-confirm-message="${escapeHtmlAttribute(normalizedConfirm)}" onclick="return confirm(this.getAttribute('data-admin-confirm-message'));"` : "";
  ...
  function renderAdminClientDetailCard(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
    if (!client) {
      return renderAdminCard(
        "Карточка клиента",
        "Выберите клиента в таблице, чтобы открыть быстрый обзор и историю заявок.",
      ...
      <div class="admin-dialog-panel">
        <div class="admin-dialog-head">
          <div class="admin-dialog-copy-block">
            ...
            <h2 class="admin-dialog-title" id="admin-client-detail-title">${escapeHtml(client.name || "Карточка клиента")}</h2>
            <p class="admin-dialog-copy">Контакты, сумма заказов и история обращений в отдельном окне.</p>
          </div>
        ...
        <div class="admin-client-dialog-card">
          ${renderAdminClientDetailCard(client, returnTo, { closeHref, hideCloseAction: true })}
        </div>
      </div>
    </dialog>`;
  }
    ...
    return renderAdminLayout(
      ...
      `${renderClientsNotice(req)}
        <div class="admin-clients-layout">
          ${renderAdminCard(
            "База клиентов",
            "Поиск и фильтры над таблицей клиентов.",
            `<div class="admin-clients-workspace">
              ...
              <div class="admin-clients-meta-row">
                ...
                <span class="admin-action-hint">${selectedClient ? "Карточка клиента открыта в отдельном окне." : "Нажмите на имя клиента, чтобы открыть карточку."}</span>
              </div>
  ...
  function renderOrderDetail(label, value, options = {}) {
    const className = options.wide
      ? "admin-order-detail admin-order-detail-wide"
      : "admin-order-detail";

    return `<div class="${className}">
      <span class="admin-order-detail-label">${escapeHtml(label)}</span>
      <p class="admin-order-detail-value">${escapeHtml(value)}</p>
    </div>`;
  }
  ...
  function renderOrderEntryCard(order, returnTo, options = {}) {
    ...
    const scheduleCopy = order.hasSchedule ? "Окно визита уже назначено." : "Нужно выбрать день и время визита.";
    const teamCopy = order.isAssigned ? "Состав команды уже указан." : "Команда ещё не назначена.";
    ...
    return `<article class="admin-order-card admin-order-card-${escapeHtmlAttribute(order.orderStatus)}${order.needsAttention ? " admin-order-card-attention" : ""}">
      <div class="admin-order-card-head">
        <div class="admin-order-title-block">
          <div class="admin-order-topline">
            ...
            <p class="admin-order-amount">${escapeHtml(formatCurrencyAmount(order.totalPrice))}</p>
          </div>
          <h3 class="admin-order-title">${escapeHtml(order.customerName || "Клиент")}</h3>
          <p class="admin-order-caption">${escapeHtml(order.serviceLabel)} • создан ${escapeHtml(createdLabel)}</p>
        </div>
      ...
      <div class="admin-order-alert-row">
        ${focusBadges.length > 0
          ? focusBadges.join("")
          : `<span class="admin-order-focus-copy">Дата, команда и CRM-статус уже заполнены.</span>`}
      </div>
      <div class="admin-order-primary-grid">
        <section class="admin-order-primary-card">
          <span class="admin-order-primary-label">Дата и время</span>
          <p class="admin-order-primary-value">${escapeHtml(order.scheduleLabel)}</p>
          <p class="admin-order-primary-copy">${escapeHtml(scheduleCopy)}</p>
        </section>
        <section class="admin-order-primary-card">
          <span class="admin-order-primary-label">Команда</span>
          <p class="admin-order-primary-value">${escapeHtml(assignedLabel)}</p>
          <p class="admin-order-primary-copy">${escapeHtml(teamCopy)}</p>
        </section>
      </div>
      <div class="admin-order-detail-grid">
        ${renderOrderDetail("Тип уборки", order.serviceLabel)}
        ${renderOrderDetail("Повторяемость", order.frequencyLabel)}
        ${renderOrderDetail("Контакты", contactLabel)}
        ${renderOrderDetail("Адрес", order.fullAddress || "Не указан", { wide: true })}
      </div>
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-orders-page">
        ...
        <section class="admin-orders-overview">
          <div class="admin-orders-summary-grid">
            ${renderOrdersMetric("Активные", activeCount, "Все заказы, которые ещё в работе.", { emphasis: true })}
            ${renderOrdersMetric("В графике", scheduledCount, "Есть дата или время визита.")}
            ${renderOrdersMetric("Без команды", unassignedOpenCount, "Требуется назначение сотрудников.")}
            ${renderOrdersMetric("Проверить", attentionCount, "Есть warning или error в CRM.")}
            ${renderOrdersMetric("Pipeline", formatCurrencyAmount(revenuePipeline), "Сумма текущего рабочего потока.")}
          </div>
          <section class="admin-orders-toolbar">
            <div class="admin-orders-toolbar-head">
              <div>
                <p class="admin-orders-panel-kicker">Рабочий список</p>
                <h2 class="admin-orders-panel-title">Заказы по статусам</h2>
                <p class="admin-orders-panel-copy">Фильтры применяются ко всей очереди, а ниже заказы уже разложены по текущему статусу.</p>
              </div>
              <div class="admin-orders-toolbar-meta">
                <span>Показано <strong>${escapeHtml(String(filteredCount))}</strong> из ${escapeHtml(String(totalOrders))}</span>
                <span>Recurring <strong>${escapeHtml(String(recurringCount))}</strong></span>
                <span>Scheduled <strong>${escapeHtml(String(statusCounts.scheduled))}</strong></span>
                <span>С командой <strong>${escapeHtml(String(assignedCount))}</strong></span>
              </div>
            </div>
