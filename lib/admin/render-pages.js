...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const dateLabel = formatOrderQuoteDateValue(quoteData.selectedDate || order.selectedDate);
    const timeLabel = formatOrderQuoteTimeValue(quoteData.selectedTime || order.selectedTime);
    ...
    const visitCardMarkup = renderOrderBriefCard(
      "Визит и расчёт",
      `<div class="admin-order-brief-visit">
        <div class="admin-order-brief-slot">
          <span class="admin-order-brief-slot-label">Запрошенный слот</span>
          <p class="admin-order-brief-slot-value">${escapeHtml(requestedSlotLabel)}</p>
          <p class="admin-order-brief-slot-meta">${escapeHtml(serviceTypeLabel)} • ${escapeHtml(frequencyLabel)}</p>
        </div>
        <div class="admin-order-brief-total">
          <span class="admin-order-brief-slot-label">Сумма из quote</span>
          <p class="admin-order-brief-total-value">${escapeHtml(quoteTotalLabel)}</p>
      ...
      <div class="admin-order-brief-fact-grid">
        ...
        ${renderOrderBriefFact("Дата", dateLabel)}
        ${renderOrderBriefFact("Время", timeLabel)}
        ${renderOrderBriefFact("Согласие", consentLabel)}
      </div>`,
      {
        wide: true,
        description: "Самое важное по выезду и расчёту собрано в одном месте.",
      }
    );
          ...
          <div class="admin-order-dialog-main">
            ...
            <section class="admin-order-section-card">
              <div class="admin-subsection-head">
                ...
                <span class="admin-action-hint">Собрано в короткие смысловые блоки вместо длинного списка полей.</span>
              </div>
