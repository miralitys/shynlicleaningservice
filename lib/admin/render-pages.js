...
function createAdminPageRenderers(deps = {}) {
  ...
  function buildSettingsUsersRedirectPath(notice = "") {
    return `${buildAdminRedirectPath(ADMIN_SETTINGS_PATH, { notice })}#settings-users`;
  }
  ...
  async function renderSettingsPage(req, config, settingsStore, usersStore, staffStore, quoteOpsLedger) {
    ...
    return renderAdminLayout(
        ...
        <div class="admin-section-grid">
          ...
          ${renderAdminCard(
            ...
            `<ul class="admin-feature-list">
              ...
              <li>Личные кабинеты сотрудников с доступом только к своим заявкам.</li>
            </ul>`,
            { eyebrow: "Настройки", muted: true }
          )}
        </div>`,
      {
        kicker: "Настройки",
        subtitle: "Шаблоны чек-листов, пользователи и внутренние рабочие базы.",
