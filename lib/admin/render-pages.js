...
function createAdminPageRenderers(deps = {}) {
  ...
  function renderSettingsCreateUserForm(options = {}) {
    const inviteHelperCopy = options.inviteEmailConfigured
      ? "После создания сотрудник получит письмо со ссылкой на подтверждение email. Временный пароль нужно передать отдельно."
      : "Сейчас письмо-приглашение не настроено: кабинет создастся сразу, но письмо с подтверждением автоматически не уйдёт.";
            ...
            return renderAdminCard(
              ...
              `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
                ...
                <div class="admin-form-grid admin-form-grid-two">
                  ...
                  <label class="admin-label">
                    Телефон
                    <input class="admin-input" type="tel" name="phone" value="${escapeHtmlText(user.phone || "")}"${renderAdminPhoneInputAttributes()}>
                  </label>
                  <label class="admin-label">
                    Должность
  ...
  async function renderSettingsPage(req, config, settingsStore, usersStore, staffStore, quoteOpsLedger, adminRuntime = {}) {
    ...
    const inviteEmailConfigured = Boolean(
      ...
      typeof adminRuntime.accountInviteEmail.isConfigured === "function" &&
      adminRuntime.accountInviteEmail.isConfigured()
    );
        ...
        ${activeSection === "users" ? usersContent : checklistsContent}`,
      {
        ...
        heroActions:
          activeSection === "users"
            ...
            : "",
        bodyScripts: activeSection === "users" ? renderStaffAddressAutocompleteScript() : "",
