  ...
  function renderAdminPhoneInput(name, value = "", options = {}) {
    ...
    return `<label class="admin-label">
      ...
      <span class="admin-phone-field">
        ...
        <input class="admin-input admin-phone-input" type="tel" name="${escapeHtmlAttribute(name)}" value="${escapeHtmlText(inputValue)}" data-admin-phone-input="true" inputmode="numeric" autocomplete="${escapeHtmlAttribute(autocomplete)}" maxlength="14" placeholder="${escapeHtmlAttribute(placeholder)}"${requiredAttr}>
      </span>
    </label>`;
  }
  ...
  function renderStaffEditDialog(staffSummary, options = {}) {
    ...
    const w9DownloadPath =
      w9Record && w9Record.document && w9Record.document.relativePath
        ...
        : "";
    const w9ReminderAvailable = Boolean(linkedUser && linkedUser.id && linkedUser.email);
    ...
    const w9PanelMarkup = w9Record
      ? `${renderAdminClientInfoGrid(w9InfoItems)}
          ${w9DownloadPath
            ? `<div class="admin-w9-preview-shell">
                <iframe
                  class="admin-w9-preview-frame"
                  src="${escapeHtmlAttribute(w9DownloadPath)}"
                  title="${escapeHtmlAttribute(`W-9 ${staffSummary.name || "сотрудника"}`)}"
                  loading="lazy"
                ></iframe>
              </div>
      ...
      : `<div class="admin-empty-state admin-w9-empty-state">
            <div class="admin-w9-empty-copy">Сотрудник ещё не заполнил W-9 в личном кабинете.</div>
            ${w9ReminderAvailable
