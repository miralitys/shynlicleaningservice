  ...
  function renderAdminClientEditPanel(client, returnTo = ADMIN_CLIENTS_PATH, panelId = "") {
    ...
    return `<section class="admin-client-section admin-staff-form-section" id="${escapeHtmlAttribute(panelId)}" data-admin-toggle-panel hidden>
      ...
      <form class="admin-form-grid" method="post" action="${ADMIN_CLIENTS_PATH}">
        ...
        <label class="admin-label">
          Адреса клиента
          <textarea class="admin-input" name="addresses" placeholder="58 South Grove Avenue, Elgin, IL&#10;742 Cedar Avenue, Aurora, IL 60506">${escapeHtml(
            (Array.isArray(client.addresses) ? client.addresses.map((addressRecord) => addressRecord.address).filter(Boolean) : []).join("\n")
          )}</textarea>
          <small class="admin-field-hint">По одному адресу на строку. По клику на адрес карточка переключается на нужный дом.</small>
        </label>
