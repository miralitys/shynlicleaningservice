...
function createAdminPageRenderers(deps = {}) {
  ...
  function formatAdminPhoneNumber(value) {
    ...
    if (normalizedDigits.length <= 6) {
      return `+1(${normalizedDigits.slice(0, 3)})${normalizedDigits.slice(3)}`;
    }
  ...
  function renderStaffEditDialog(staffSummary, options = {}) {
    ...
    const accessRoleValue = linkedUser && linkedUser.role ? linkedUser.role : "cleaner";
    ...
    const summaryBadges = [
      ...
      linkedUser
        ? renderAdminBadge(
            formatWorkspaceRoleLabel(linkedUser.role),
            linkedUser.role === "admin" ? "success" : linkedUser.role === "manager" ? "outline" : "muted"
          )
        : "",
      renderAdminBadge(
      ...
      <div class="admin-dialog-panel admin-staff-dialog-panel">
        ...
        <div class="admin-client-dialog-body admin-staff-dialog-body">
          <section class="admin-client-summary-panel admin-staff-summary-panel">
            ...
            <div class="admin-client-metric-grid admin-client-metric-grid-dialog">
              <article class="admin-client-metric-card">
                <span class="admin-client-metric-label">Должность</span>
                <p class="admin-client-metric-value">${escapeHtml(staffSummary.role || "Не указана")}</p>
              </article>
              ${linkedUser
                ? `<article class="admin-client-metric-card">
                    <span class="admin-client-metric-label">Роль доступа</span>
                    <p class="admin-client-metric-value">${escapeHtml(formatWorkspaceRoleLabel(linkedUser.role))}</p>
                  </article>`
                : ""}
              <article class="admin-client-metric-card">
          ...
          <section class="admin-client-section admin-staff-form-section" id="${escapeHtmlAttribute(editPanelId)}" data-admin-toggle-panel hidden>
            <form class="admin-form-grid" id="${escapeHtmlAttribute(editFormId)}" method="post" action="${ADMIN_STAFF_PATH}">
              ...
              <div class="admin-form-grid admin-form-grid-two">
                ...
                <label class="admin-label">
                  Роль
                  <select class="admin-input" name="userRole"${linkedUser ? "" : " disabled"}>
                  ...
                  <small class="admin-field-hint">${escapeHtml(
                    linkedUser
                      ? "Меняет права доступа сразу после сохранения."
                      : "Права доступа появятся после создания и привязки личного кабинета пользователя."
                  )}</small>
                </label>
                <label class="admin-label">
                  Должность
                  <input class="admin-input" type="text" name="staffRole" value="${escapeHtmlText(staffSummary.role)}" placeholder="Cleaner, Team Lead, Driver">
                </label>
                <label class="admin-label">
                  Телефон
  ...
  async function renderStaffPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    const sectionIntro = renderStaffSectionNav(
      activeSection,
      {
        staffCount: enrichedStaffSummaries.length,
    ...
    const sectionContent =
      activeSection === "calendar"
        ? renderAdminCard(
            ...
            renderStaffTeamCalendarTable(enrichedStaffSummaries, calendarStartDate),
        ...
        : renderAdminCard(
            ...
            renderStaffSummaryTable(enrichedStaffSummaries, { canDelete: accessContext.canDelete }),
