...
function createAdminPageRenderers(deps = {}) {
  ...
  const STAFF_TEAM_CALENDAR_DAYS = 7;
  ...
  function renderAdminPhoneInputAttributes() {
    ...
    return `<label class="admin-label">
      ...
      <div class="admin-picker-field" data-admin-picker-field="${escapeHtmlAttribute(pickerType)}">
        ...
        <button
          ...
          aria-controls="${escapeHtmlAttribute(nativeId)}"
        >
          ${iconMarkup}
        </button>
      </div>
  ...
  function renderAdminClientDetailPanel(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
    ...
    const activeEntry = client.entries.find((entry) => isAdminClientEntryActive(entry)) || null;
    ...
    const summaryCopy = activeEntry
      ...
      : `Последняя заявка: ${formatAdminDateTime(client.latestCreatedAt)}`;
  ...
  function renderAdminClientDetailDialog(client, returnTo = ADMIN_CLIENTS_PATH, options = {}) {
    ...
    const latestEntry = client.entries[0] || null;
    const activeEntry = client.entries.find((entry) => isAdminClientEntryActive(entry)) || latestEntry;
    const editHref = buildAdminRedirectPath(ADMIN_ORDERS_PATH, {
      q: activeEntry && activeEntry.requestId ? activeEntry.requestId : client.name || client.phone || client.email || "",
      order: activeEntry && activeEntry.id ? activeEntry.id : "",
    });
      ...
      <div class="admin-dialog-panel admin-client-dialog-panel">
        <div class="admin-dialog-head">
          ...
          <div class="admin-inline-actions admin-dialog-head-actions">
            <a class="admin-button" href="${escapeHtmlAttribute(editHref)}">Редактировать</a>
  ...
  function getOrderResponsibleOptions(order, staffRecords = [], planningItem = null) {
    const assignedValues = [
      normalizeString(order && order.assignedStaff, 120),
      normalizeString(
        planningItem && Array.isArray(planningItem.assignedStaff) && planningItem.assignedStaff.length > 0
          ? planningItem.assignedStaff.map((staffRecord) => normalizeString(staffRecord && staffRecord.name, 120)).join(", ")
          : "",
        120
      ),
    ].filter(Boolean);
  ...
  function renderOrderResponsibleSelect(order, options = {}) {
    ...
    const selectedValue = normalizeString(order && order.assignedStaff, 120);

    return `<select class="admin-input" name="assignedStaff">
      <option value=""${!selectedValue ? " selected" : ""}>Не назначен</option>
      ${selectableStaff
        .map((staffName) => `<option value="${escapeHtmlAttribute(staffName)}"${staffName === selectedValue ? " selected" : ""}>${escapeHtml(staffName)}</option>`)
        .join("")}
    </select>`;
  }
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const summaryFlags = [
      ...
      .join("");
    const visitRows = [
      { label: "Тип уборки", value: formatOrderQuoteServiceType(quoteData.serviceType, order.serviceLabel) },
      { label: "Повторяемость", value: formatOrderQuoteFrequency(quoteData.frequency, order.frequencyLabel) },
      { label: "Дата", value: formatOrderQuoteDateValue(quoteData.selectedDate || order.selectedDate) },
      { label: "Время", value: formatOrderQuoteTimeValue(quoteData.selectedTime || order.selectedTime) },
      {
        label: "Запрошенный слот",
        value: formatOrderQuoteRequestedSlot(
      ...
      { label: "Согласие", value: formatOrderQuoteBoolean(quoteData.consent) },
    ];
    const homeRows = [
      { label: "Спальни", value: formatOrderQuoteText(quoteData.rooms, "Не указано", 32) },
      { label: "Санузлы", value: formatOrderQuoteText(quoteData.bathrooms, "Не указано", 32) },
      { label: "Размер дома", value: formatOrderQuoteSquareFootage(quoteData.squareMeters) },
      { label: "Питомцы", value: formatOrderQuoteBoolean(quoteData.hasPets) },
      { label: "Basement cleaning", value: formatOrderQuoteBoolean(quoteData.basementCleaning) },
    ];
    const serviceRows = [
      { label: "Выбранные add-ons", value: selectedServices.length > 0 ? selectedServices.join(", ") : "Не выбраны" },
      { label: ORDER_QUOTE_QUANTITY_LABELS.interiorWindowsCleaning, value: getOrderQuoteQuantityValue(quoteData, "interiorWindowsCleaning") },
      { label: ORDER_QUOTE_QUANTITY_LABELS.blindsCleaning, value: getOrderQuoteQuantityValue(quoteData, "blindsCleaning") },
      { label: ORDER_QUOTE_QUANTITY_LABELS.bedLinenChange, value: getOrderQuoteQuantityValue(quoteData, "bedLinenChange") },
    ];
    const addressRows = [
      { label: "Полный адрес", value: quoteFullAddressLabel },
      { label: "Короткий адрес", value: quoteAddressLabel },
      { label: "Apt / suite", value: formatOrderQuoteText(quoteData.addressLine2, "Не указано", 200) },
      { label: "Город", value: formatOrderQuoteText(quoteData.city, "Не указано", 120) },
      { label: "Штат", value: formatOrderQuoteText(quoteData.state, "Не указано", 32) },
      { label: "ZIP", value: formatOrderQuoteText(quoteData.zipCode, "Не указано", 32) },
    ];
    const quoteCards = [
      renderOrderPropertyCard("Визит и расчёт", visitRows, {
        description: "Ключевые параметры визита из quote и текущего заказа.",
      }),
      renderOrderPropertyCard("Дом и условия", homeRows),
      renderOrderPropertyCard("Дополнительные услуги", serviceRows),
      renderOrderPropertyCard("Адрес из формы", addressRows, { wide: true }),
      renderOrderDetailCard(
        "Комментарий клиента",
        `<p class="admin-order-detail-note">${formatOrderQuoteMultiline(quoteData.additionalDetails)}</p>`,
        { wide: true }
      ),
    ].join("");
        ...
        <div class="admin-order-dialog-layout">
          <div class="admin-order-dialog-main">
            <section class="admin-order-section-card admin-order-summary-panel">
              <div class="admin-order-summary-head">
                <div class="admin-order-summary-copy-block">
                  ...
                  <p class="admin-order-summary-copy">
                    ${escapeHtml(
                      summaryFlags
                        ? "Здесь собрана короткая оперативная сводка: сразу видно визит, команду и текущие сигналы."
                        : "Карточка собрана как рабочее окно: короткая сводка сверху, все данные клиента ниже, управление справа."
                    )}
                  </p>
                </div>
              </div>
              ${summaryFlags
                ? `<div class="admin-inline-badge-row admin-order-summary-flags">${summaryFlags}</div>`
                : `<p class="admin-order-summary-ok">Заказ выглядит готовым к работе: дата, команда и CRM-статус уже на месте.</p>`}
            ...
            <section class="admin-order-section-card">
              <div class="admin-subsection-head">
                ...
                <span class="admin-action-hint">Все данные из формы quote собраны в более компактные секции.</span>
              </div>
          ...
          <aside class="admin-order-dialog-side">
            <section class="admin-order-section-card admin-order-side-card">
              ...
              <p class="admin-card-copy admin-order-detail-copy">Изменяйте статус, ответственного и окно уборки здесь, без лишней прокрутки по всей карточке.</p>
  ...
  function renderOrdersLane(status, laneOrders, returnTo, planningByEntryId, staffRecords = []) {
  ...
  function renderStaffEditDialog(staffSummary) {
    ...
    const formattedPhone = formatAdminPhoneNumber(staffSummary.phone) || staffSummary.phone || "";
        ...
        <div class="admin-dialog-head">
          <div class="admin-client-dialog-title-row">
            ...
            <div class="admin-client-dialog-title-block">
              ...
              <h2 class="admin-dialog-title" id="${escapeHtmlAttribute(`${dialogId}-title`)}">${escapeHtml(titleLabel)}${staffSummary.role ? ` <span class="admin-staff-dialog-title-role">(${escapeHtml(staffSummary.role)})</span>` : ""}</h2>
          ...
          <section class="admin-client-section admin-staff-form-section" id="${escapeHtmlAttribute(editPanelId)}" data-admin-toggle-panel hidden>
            <form class="admin-form-grid" id="${escapeHtmlAttribute(editFormId)}" method="post" action="${ADMIN_STAFF_PATH}">
              ...
              <div class="admin-form-grid admin-form-grid-two">
                ...
                <label class="admin-label">
                  Телефон
                  <input class="admin-input" type="tel" name="phone" value="${escapeHtmlText(formatAdminPhoneNumber(staffSummary.phone) || staffSummary.phone)}"${renderAdminPhoneInputAttributes()}>
                </label>
                <label class="admin-label">
                  Email
  ...
  function buildStaffTeamCalendarWindow(startDateValue, totalDays = STAFF_TEAM_CALENDAR_DAYS) {
    const startDate = normalizeStaffCalendarDateValue(startDateValue) || getStaffCalendarTodayDateValue();
    return Array.from({ length: totalDays }, (_, index) => {
      const dateValue = addDaysToStaffCalendarDate(startDate, index);
  ...
  function renderStaffTeamCalendarAssignmentEntry(orderItem) {
    ...
    return `<article class="admin-team-calendar-entry admin-team-calendar-entry-order admin-team-calendar-entry-order-${escapeHtmlAttribute(statusTone)}">
  ...
  function renderStaffTeamCalendarTable(staffSummaries, startDateValue) {
    ...
    const prevHref = buildAdminRedirectPath(ADMIN_STAFF_PATH, {
      calendarStart: addDaysToStaffCalendarDate(normalizedStartDate, -STAFF_TEAM_CALENDAR_DAYS),
    });
    const nextHref = buildAdminRedirectPath(ADMIN_STAFF_PATH, {
      calendarStart: addDaysToStaffCalendarDate(normalizedStartDate, STAFF_TEAM_CALENDAR_DAYS),
    });
    const todayHref = buildAdminRedirectPath(ADMIN_STAFF_PATH, {
      calendarStart: getStaffCalendarTodayDateValue(),
    });
    ...
    return `<div class="admin-team-calendar-shell">
      <div class="admin-team-calendar-toolbar">
        <div class="admin-team-calendar-copy">
          ...
          <span class="admin-team-calendar-meta">Назначения и day off по всей команде на ${escapeHtml(String(days.length))} дней.</span>
        </div>
        <div class="admin-team-calendar-actions">
          <a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(prevHref)}">Назад</a>
          <a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(todayHref)}">Сегодня</a>
          <a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(nextHref)}">Вперёд</a>
        </div>
      </div>
      <div class="admin-table-wrap admin-team-calendar-wrap">
        <table class="admin-table admin-team-calendar-table" data-admin-team-calendar="true">
          <thead>
            <tr>
            ...
            ${staffSummaries
              .map(
                (staffSummary) => `<tr>
                  ...
                  ${days.map((day) => `<td>${renderStaffTeamCalendarCell(staffSummary, day)}</td>`).join("")}
                </tr>`
              )
              .join("")}
          </tbody>
      ...
      <div class="admin-dialog-panel">
        ...
        <form class="admin-form-grid" method="post" action="${ADMIN_STAFF_PATH}">
          ...
          <div class="admin-form-grid admin-form-grid-two">
            ...
            <label class="admin-label">
              Телефон
              <input class="admin-input" type="tel" name="phone"${renderAdminPhoneInputAttributes()}>
            </label>
            <label class="admin-label">
              Email
  ...
  function renderStaffAssignmentsTable(orderItems, staffRecords) {
    ...
    return `<div class="admin-table-wrap admin-orders-table-wrap">
      <table class="admin-table admin-staff-schedule-table">
        ...
        <tbody>
          ${orderItems.map((item) => renderStaffAssignmentTableRow(item)).join("")}
        </tbody>
      </table>
      ${orderItems.map((item) => renderStaffAssignmentDialog(item, staffRecords)).join("")}
    </div>`;
  }
      ...
      `${renderStaffNotice(req)}
        ...
        <div class="admin-section-grid">
          ${renderAdminCard(
            ...
            renderStaffSummaryTable(staffSummaries),
        ...
        <div class="admin-section-grid">
          ${renderAdminCard(
            ...
            renderStaffTeamCalendarTable(staffSummaries, calendarStartDate),
        ...
        <div class="admin-section-grid">
          ${renderAdminCard(
            ...
            renderStaffAssignmentsTable(upcomingOrders, planning.staff),
            { eyebrow: "График" }
          )}
        </div>`,
      {
  ...
  function renderSettingsUsersSection(users, staffRecords, staffSummaryById = new Map()) {
    const linkedStaffIds = new Set(users.map((user) => user.staffId).filter(Boolean));
    const availableStaffRecords = staffRecords.filter((record) => !linkedStaffIds.has(record.id));

    const createUserForm = availableStaffRecords.length > 0
      ? `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
          <input type="hidden" name="action" value="create_user">
          <div class="admin-form-grid admin-form-grid-two">
            <label class="admin-label">
              Сотрудник
              <select class="admin-input" name="staffId" required>
                <option value="">Выберите сотрудника</option>
                ${availableStaffRecords
                  .map((record) => `<option value="${escapeHtmlAttribute(record.id)}">${escapeHtml(record.name)}${record.role ? ` • ${escapeHtml(record.role)}` : ""}</option>`)
                  .join("")}
              </select>
            </label>
            <label class="admin-label">
            ...
            <label class="admin-label">
              Телефон
              <input class="admin-input" type="tel" name="phone" placeholder="+1(312)555-0100">
            </label>
            <label class="admin-label admin-form-grid-span-2">
              Стартовый пароль
              <input class="admin-input" type="password" name="password" minlength="8" placeholder="Минимум 8 символов" required>
            </label>
          </div>
          <div class="admin-inline-actions">
            <button class="admin-button" type="submit">Создать пользователя</button>
          </div>
          <p class="admin-helper-copy">После создания сотрудник сможет входить через <strong>/account/login</strong> и видеть только свои назначенные заявки.</p>
        </form>`
      : `<div class="admin-empty-state">${
          staffRecords.length > 0
            ? "Все сотрудники уже привязаны к пользовательским кабинетам. Чтобы создать ещё один, сначала добавьте нового сотрудника."
            : "Сначала добавьте хотя бы одного сотрудника в разделе \"Сотрудники\", затем сможете создать для него личный кабинет."
        }</div>`;
    ...
    const userCards = users.length > 0
      ? users
          .map((user) => {
            ...
            const selectableStaff = staffRecords.filter((record) => !linkedStaffIds.has(record.id) || record.id === user.staffId);
            const staffSummary = linkedStaff ? staffSummaryById.get(linkedStaff.id) || null : null;
            return renderAdminCard(
              linkedStaff ? linkedStaff.name : user.email,
              linkedStaff
                ? `${linkedStaff.role || "Сотрудник"} • ${staffSummary ? formatOrderCountLabel(staffSummary.assignedCount) : "0 заказов"}`
              ...
              `<form class="admin-form-grid" method="post" action="${ADMIN_SETTINGS_PATH}">
                ...
                <div class="admin-form-grid admin-form-grid-two">
                  <label class="admin-label">
                    ...
                    <select class="admin-input" name="staffId" required>
                      ${selectableStaff
                        .map((record) => `<option value="${escapeHtmlAttribute(record.id)}"${record.id === user.staffId ? " selected" : ""}>${escapeHtml(record.name)}${record.role ? ` • ${escapeHtml(record.role)}` : ""}</option>`)
                        .join("")}
                    </select>
                  </label>
                  <label class="admin-label">
                  ...
                  <label class="admin-label">
                    Телефон
                    <input class="admin-input" type="tel" name="phone" value="${escapeHtmlText(user.phone || "")}">
                  </label>
                  <label class="admin-label admin-form-grid-span-2">
                    Новый пароль
                ...
                <div class="admin-inline-actions">
                  <button class="admin-button" type="submit">Сохранить пользователя</button>
                </div>
              </form>
              ...
              { eyebrow: "Пользователь", muted: true }
            );
          })
          .join("")
      : `<div class="admin-empty-state">Пользователей пока нет. Создайте первый аккаунт для входа сотрудников в систему.</div>`;

    return `<div id="settings-users"></div>
      <div class="admin-section-grid">
        ${renderAdminCard(
          "Новый пользователь",
          "Создайте личный кабинет для сотрудника и задайте стартовый пароль.",
          createUserForm,
  ...
  async function renderSettingsPage(req, config, settingsStore, usersStore, staffStore, quoteOpsLedger) {
  ...
  function renderQuoteOpsDetailDialog(entry, returnTo) {
    ...
    const overviewCopy = [
      `Запрос: ${serviceLabel}.`,
      scheduleLabel !== "Не указана" ? `Клиент выбрал слот ${scheduleLabel}.` : "Дата и время пока не указаны.",
      entry.status === "success"
        ? "Заявка ушла в CRM без ошибок."
        : entry.status === "warning"
          ? "Заявка дошла, но требует ручной проверки."
          : "Синхронизация не завершилась и требует внимания.",
    ].join(" ");
    const summaryItems = [
    ...
    const quoteCards = [
      ...
      `<article class="admin-order-quote-card admin-order-quote-card-wide">
        <div class="admin-subsection-head">
          <h4 class="admin-subsection-title">Адрес и комментарий</h4>
        </div>
        <div class="admin-order-quote-fields">
    ...
    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}" aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel">
        ...
        <section class="admin-client-summary-panel">
          <div class="admin-client-summary-head">
            <div class="admin-client-summary-copy-block">
              ...
              <p class="admin-client-summary-copy">${escapeHtml(overviewCopy)}</p>
            </div>
          </div>
          ${warningBlock}
          ${errorBlock}
        ...
        <section>
          <div class="admin-subsection-head">
            <h3 class="admin-subsection-title">Поля из формы клиента</h3>
            <span class="admin-action-hint">Главные детали собраны в компактные блоки</span>
          </div>
      ...
      `<div class="admin-quote-ops-page">
        ...
        <section class="admin-orders-filters-panel admin-quote-ops-filter-shell">
          ...
          <form
            ...
            data-admin-auto-submit-delay="320"
          >
  ...
  async function renderAdminAppPage(route, req, config, adminRuntime = {}, quoteOpsLedger = null, staffStore = null) {
    ...
    if (route === ADMIN_SETTINGS_PATH) {
      return renderSettingsPage(req, config, adminRuntime.settingsStore, adminRuntime.usersStore, staffStore, quoteOpsLedger);
    }
