    ...
    const quoteCards = [
      `<article class="admin-order-quote-card admin-order-quote-card-wide">
        ...
        <div class="admin-order-quote-fields admin-order-quote-fields-primary">
          ...
          ${renderOrderQuoteField("Дата", formatOrderQuoteDateValue(quoteData.selectedDate || order.selectedDate))}
          ${renderOrderQuoteField("Время", formatOrderQuoteTimeValue(quoteData.selectedTime || order.selectedTime))}
          ${renderOrderQuoteField("Запрошенный слот", formatOrderQuoteRequestedSlot(quoteData.formattedDateTime, quoteData.selectedDate || order.selectedDate, quoteData.selectedTime || order.selectedTime))}
    ...
    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}"${autoOpenAttr}${closeHref ? ` data-admin-dialog-return-url="${escapeHtmlAttribute(closeHref)}"` : ""} aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel">
        ...
        <div class="admin-client-metric-grid">
          ...
          <article class="admin-client-metric-card">
            <span class="admin-client-metric-label">Создан</span>
          ...
          <div class="admin-form-grid admin-form-grid-two">
            ...
            <label class="admin-label">
              ...
              <input
                ...
                value="${escapeHtmlAttribute(formatAdminOrderDateInputValue(order.selectedDate))}"
                autocomplete="off"
              >
            </label>
            <label class="admin-label">
              ...
              <input
                ...
                name="selectedTime"
                placeholder="1:30 PM"
                value="${escapeHtmlAttribute(formatAdminOrderTimeInputValue(order.selectedTime))}"
                autocomplete="off"
              >
            </label>
            <label class="admin-label">
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-quote-ops-page">
        ...
        <section class="admin-orders-filters-panel admin-quote-ops-filter-shell">
          ...
          <form class="admin-quote-ops-filter-form" method="get" action="${ADMIN_QUOTE_OPS_PATH}">
