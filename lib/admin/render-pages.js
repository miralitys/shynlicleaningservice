...
function createAdminPageRenderers(deps = {}) {
    ...
    >
      <div class="admin-dialog-panel admin-client-dialog-panel">
        <div class="admin-dialog-head">
          ...
          <div class="admin-client-dialog-head-actions">
            <a class="admin-link-button admin-button-secondary" href="${escapeHtmlAttribute(editHref)}">Редактировать</a>
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const sideSummaryRows = [
      { label: "Request ID", value: order.requestId || "Не указан" },
      { label: "Контакты", value: contactLabel },
      { label: "Адрес", value: order.fullAddress || quoteFullAddressLabel || "Не указан" },
      { label: "Создан", value: createdLabel },
    ];
    const quoteCards = [
      ...
      <div class="admin-dialog-panel admin-order-dialog-panel">
        ...
        <div class="admin-order-dialog-layout">
          ...
          <aside class="admin-order-dialog-side">
            ...
            ${renderOrderPropertyCard("Быстрая сводка", sideSummaryRows, {
              description: "Ключевые данные, к которым обычно возвращаются при работе с заказом.",
            })}
          </aside>
        </div>
      </div>
  ...
  function renderQuoteOpsDetailDialog(entry, returnTo) {
    ...
    const crmLabel = getQuoteOpsStatusMeta(entry.status).label;
    ...
    const crmSyncState = [
      entry.contactId ? (entry.usedExistingContact ? "Контакт найден" : "Контакт создан") : "",
      entry.noteCreated ? "Заметка создана" : "",
      entry.opportunityCreated ? "Сделка создана" : "",
    ]
      .filter(Boolean)
      .join(" • ");
    const opsItems = [
      { label: "CRM статус", value: renderQuoteOpsStatusBadge(entry.status), raw: true },
      { label: "CRM код", value: entry.code || "—" },
      { label: "HTTP", value: entry.httpStatus ? String(entry.httpStatus) : "—" },
      { label: "Контакт ID", value: entry.contactId || "—" },
      { label: "Request ID", value: entry.requestId || "Не указан" },
      { label: "Источник", value: entry.source || "site" },
      { label: "Retry", value: entry.retryCount > 0 ? `Retry ${entry.retryCount}` : "Без retry" },
      {
        label: "Что уже сделано",
        value: crmSyncState || "CRM ещё не создала связанные записи",
        hint: entry.lastRetryAt
          ? `Последняя попытка: ${formatAdminDateTime(entry.lastRetryAt)}${entry.lastRetryMessage ? ` • ${entry.lastRetryMessage}` : ""}`
          : "Повторов ещё не было.",
        wide: true,
      },
    ];
    const warningBlock = entry.warnings.length > 0
    ...
    return `<dialog class="admin-dialog admin-dialog-wide admin-dialog-orders" id="${escapeHtmlAttribute(dialogId)}" aria-labelledby="${escapeHtmlAttribute(`${dialogId}-title`)}">
      <div class="admin-dialog-panel">
        ...
        <div class="admin-client-detail-grid">
          ...
          <section class="admin-client-section admin-client-section-side">
            <div class="admin-subsection-head">
              <h3 class="admin-subsection-title">CRM и отправка</h3>
              <span class="admin-action-hint">${escapeHtml(entry.retryable ? "Можно повторить" : "Повтор обычно не нужен")}</span>
            </div>
            ${renderAdminClientInfoGrid(opsItems, { compact: true })}
          </section>
        </div>
        <section>
          <div class="admin-subsection-head">
