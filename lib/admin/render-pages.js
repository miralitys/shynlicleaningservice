...
function createAdminPageRenderers(deps = {}) {
  ...
  async function renderDashboardPage(req, config, quoteOpsLedger, adminRuntime = {}) {
    ...
    const clientsWithoutTeam = buildDashboardClientWithoutTeamRecords(clientRecords, orderRecords);
    ...
    return renderAdminLayout(
      ...
      <div class="admin-section-grid">
        ${renderAdminCard(
          "Клиенты без команды",
          "Клиенты, у которых есть активные заказы без назначенной команды.",
          renderDashboardClientsWithoutTeamTable(clientsWithoutTeam),
          { eyebrow: "Обзор", muted: true }
        )}
        ${renderAdminCard(
          "Заказы на сегодня",
  ...
  function renderDashboardNewRequestsTable(entries = []) {
    ...
    return `<div data-admin-dashboard-new-requests="true" class="admin-table-wrap admin-quote-table-wrap">
      <table class="admin-table admin-quote-success-table">
        ...
        <tbody>
          ${entries
            .map((entry) => {
              const rowHref = `${ADMIN_QUOTE_OPS_PATH}?q=${encodeURIComponent(entry.requestId || entry.customerName || entry.id)}`;
              ...
              return `<tr>
                <td>
                  <div class="admin-table-cell-stack">
                    <a class="admin-table-link" href="${escapeHtmlAttribute(rowHref)}">${escapeHtml(entry.customerName || "Клиент")}</a>
  ...
  function buildDashboardClientWithoutTeamRecords(clientRecords = [], orderRecords = []) {
    ...
    return clientRecords
      .map((client) => {
        const unassignedOrders = (Array.isArray(client.entries) ? client.entries : [])
          .map((entry) => orderByEntryId.get(entry.id))
        ...
        return {
          client,
          unassignedOrders,
          nextOrder: unassignedOrders[0] || null,
        };
      })
      .filter(Boolean)
      .sort((left, right) => getDashboardOrderSortValue(left.nextOrder).localeCompare(getDashboardOrderSortValue(right.nextOrder), "en"));
  }

  function renderDashboardClientsWithoutTeamTable(records = []) {
    if (!Array.isArray(records) || records.length === 0) {
      return `<div data-admin-dashboard-unassigned-clients="true" class="admin-alert admin-alert-info">Сейчас нет клиентов, у которых остались активные заказы без команды.</div>`;
    }

    return `<div data-admin-dashboard-unassigned-clients="true" class="admin-table-wrap admin-clients-table-wrap">
      <table class="admin-table admin-clients-table">
        ...
        <thead>
          <tr>
            ...
            <th>Адрес</th>
            <th>Без команды</th>
            <th>Ближайший заказ</th>
          </tr>
        </thead>
        <tbody>
          ${records
            .map(({ client, unassignedOrders, nextOrder }) => {
              const rowHref = `${ADMIN_CLIENTS_PATH}?client=${encodeURIComponent(client.key)}`;
              const phoneLabel = formatAdminPhoneNumber(client.phone);
              const nextOrderLabel =
                nextOrder && nextOrder.hasSchedule
                  ? nextOrder.scheduleLabel
                  : "Дата не назначена";
              return `<tr>
                <td>
                  <div class="admin-table-cell-stack">
                ...
                <td>
                  ${phoneLabel || client.email
                    ? `<div class="admin-table-cell-stack">
                        ${phoneLabel ? `<span class="admin-table-strong">${escapeHtml(phoneLabel)}</span>` : ""}
                        ${client.email ? `<span class="admin-table-muted admin-line-clamp-two">${escapeHtml(client.email)}</span>` : ""}
                      </div>`
                    : `<span class="admin-table-muted">Не указаны</span>`}
                </td>
                <td>
                  ${nextOrder && nextOrder.fullAddress
                    ? `<div class="admin-table-cell-stack">
                        <span class="admin-line-clamp-two">${escapeHtml(nextOrder.fullAddress)}</span>
                      </div>`
                    : client.address
                      ? `<div class="admin-table-cell-stack">
                ...
                <td>
                  <div class="admin-table-cell-stack">
                    <span class="admin-table-strong">${escapeHtml(formatOrderCountLabel(unassignedOrders.length))}</span>
                    <span>${renderAdminBadge("Команда не назначена", "danger")}</span>
                  </div>
                </td>
                <td>
                  <div class="admin-table-cell-stack">
                    <span class="admin-table-strong">${escapeHtml(nextOrderLabel)}</span>
                    <span class="admin-table-muted">${escapeHtml(nextOrder ? nextOrder.serviceLabel : "Нужна проверка")}</span>
                  </div>
                </td>
              </tr>`;
            })
  ...
  function renderDashboardTodayOrdersTable(orderRecords = []) {
    ...
    return `<div data-admin-dashboard-today-orders="true" class="admin-table-wrap admin-orders-table-wrap">
      <table class="admin-table admin-orders-table">
        ...
        <tbody>
          ${orderRecords
            .map((order) => {
              const rowHref = `${ADMIN_ORDERS_PATH}?q=${encodeURIComponent(order.requestId || order.customerName || order.id)}`;
              const assignedLabel = order.assignedStaff || "Команда не назначена";
              const timeLabel = order.selectedTime ? formatAdminClockTime(order.selectedTime) : "Время не указано";
              return `<tr>
                <td>
                  <div class="admin-table-cell-stack">
                    <a class="admin-table-link" href="${escapeHtmlAttribute(rowHref)}">${escapeHtml(order.customerName || "Клиент")}</a>
                    <span class="admin-table-muted">${escapeHtml(order.requestId || "Номер не указан")}</span>
                  </div>
                </td>
                <td>
                  <div class="admin-table-cell-stack">
                    <span class="admin-table-strong">${escapeHtml(timeLabel)}</span>
                ...
                <td>
                  <div class="admin-table-cell-stack">
                    ...
                    ${!order.isAssigned ? `<span>${renderAdminBadge("Без команды", "danger")}</span>` : ""}
                  </div>
                </td>
                <td>
                  <div class="admin-table-cell-stack">
                    <span class="admin-table-strong">${escapeHtml(order.serviceLabel)}</span>
                    <span class="admin-table-muted">${escapeHtml(formatCurrencyAmount(order.totalPrice))}</span>
                  </div>
                </td>
                <td>
                  ${order.fullAddress
  ...
  function renderOrderResponsibleSelect(order, options = {}) {
    ...
    return `<details class="admin-order-multiselect" data-admin-order-multiselect="true" data-admin-order-multiselect-empty-label="Не назначен">
  ...
  function renderOrderManagementDialog(order, returnTo, options = {}) {
    ...
    const amountHighlightCardMarkup = `<article class="admin-order-highlight-card">
      <div class="admin-order-highlight-head">
        <span class="admin-order-highlight-label">Сумма</span>
        <div class="admin-order-highlight-head-actions">
          <button
            class="admin-icon-button admin-edit-button"
            type="button"
      ...
      <div class="admin-order-highlight-value">${escapeHtml(formatCurrencyAmount(order.totalPrice))}</div>
      <p class="admin-order-highlight-note">Текущая сумма заказа</p>
      <form
      ...
      >
        ...
        <label class="admin-label admin-order-highlight-editor-field">
          ...
          <input
            ...
            placeholder="180.00"
          >
          <span class="admin-input-help">После сохранения сумма обновится в заказах, клиентах и заявках.</span>
        </label>
      </form>
    </article>`;
    const teamHighlightCardMarkup = `<article class="admin-order-highlight-card${order.isAssigned ? "" : " admin-order-highlight-card-danger"}">
      <div class="admin-order-highlight-head">
        <span class="admin-order-highlight-label">Команда</span>
        <div class="admin-order-highlight-head-actions">
          <button
            class="admin-icon-button admin-edit-button"
            type="button"
      ...
      <div class="admin-order-highlight-value">${escapeHtml(assignedLabel)}</div>
      <p class="admin-order-highlight-note">${escapeHtml(order.isAssigned ? "Ответственный выбран" : "Требуется назначение")}</p>
      <form
        class="admin-order-highlight-editor"
      ...
      >
        ...
        <div class="admin-label admin-order-highlight-editor-field">
          ...
          <span class="admin-input-help">Можно выбрать одного или нескольких сотрудников.</span>
        </div>
      </form>
    </article>`;
    const paymentHighlightCardTone =
      order.paymentStatus === "paid" ? "default" : order.paymentStatus === "partial" ? "default" : "danger";
    const paymentHighlightCardMarkup = `<article class="admin-order-highlight-card admin-order-highlight-card-${escapeHtmlAttribute(paymentHighlightCardTone)}">
      <div class="admin-order-highlight-head">
        <span class="admin-order-highlight-label">Оплата</span>
        <div class="admin-order-highlight-head-actions">
          <button
            class="admin-icon-button admin-edit-button"
            type="button"
      ...
      <div class="admin-order-highlight-value">${escapeHtml(order.paymentStatusLabel)}</div>
      <p class="admin-order-highlight-note">${escapeHtml(order.paymentMethod ? order.paymentMethodLabel : "Способ оплаты ещё не выбран")}</p>
      <form
        class="admin-order-highlight-editor"
  ...
  async function collectQuoteOpsManagerOptions(adminRuntime = {}, staffStore = null) {
    ...
    return (Array.isArray(usersSnapshot && usersSnapshot.users) ? usersSnapshot.users : [])
      .filter((user) => {
        ...
        return status === "active" && (role === "admin" || role === "manager");
      })
      .map((user) => {
  ...
  function renderQuoteOpsDetailDialog(entry, returnTo, managerOptions = []) {
    const dialogId = getQuoteOpsDialogId(entry.id);
        ...
        <section>
          ...
          <div class="admin-settings-section-stack">
            ${renderAdminCard(
              ...
              `<form class="admin-form admin-form-grid" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
                ...
                <label class="admin-label admin-form-grid-span-2">
                  ...
                  <input
                    ...
                    value="${escapeHtmlAttribute(formatAdminDateTimeInputValue(getEntryAdminLeadData(entry).discussionNextContactAt || ""))}"
                  >
                </label>
            ...
            ${renderAdminCard(
              ...
              leadTaskRecords.length > 0
                ? `<div class="admin-property-list">
                    ${leadTaskRecords.map((task) => `<div class="admin-property-item">
                      <span class="admin-property-label">${escapeHtml(task.title)}</span>
                      <span class="admin-property-value">
                        ${escapeHtml(formatAdminDateTime(task.dueAt))} • ${task.status === "open" ? "Открыта" : task.status === "completed" ? "Закрыта" : "Отменена"}
                      </span>
                    </div>`).join("")}
                  </div>`
                : `<div class="admin-empty-state">Для этой заявки пока нет отдельных тасков.</div>`,
              { eyebrow: "Таски", muted: true }
            )}
            ${openTaskRecords.length > 0
              ? renderAdminCard(
                  "Быстрые действия",
                  "Отмечайте результат звонка прямо из карточки заявки.",
                  openTaskRecords.map((task) => {
                    const taskDialogId = `admin-quote-entry-task-result-dialog-${normalizeString(entry.id, 120)}-${normalizeString(task.id, 120)}`;
                    return `<div class="admin-form-grid">
                    <div class="admin-property-item">
                      <span class="admin-property-label">${escapeHtml(task.title)}</span>
                      <span class="admin-property-value">${escapeHtml(formatAdminDateTime(task.dueAt))}</span>
                    </div>
                    <div class="admin-inline-actions">
                      <form method="post" action="${ADMIN_QUOTE_OPS_PATH}">
                        <input type="hidden" name="action" value="complete-lead-task">
                        <input type="hidden" name="entryId" value="${escapeHtmlAttribute(entry.id)}">
                        <input type="hidden" name="taskId" value="${escapeHtmlAttribute(task.id)}">
                      ...
                      <button class="admin-button" type="button" data-admin-dialog-open="${escapeHtmlAttribute(taskDialogId)}">Дозвонились</button>
                    </div>
                    ${renderQuoteOpsTaskResultDialog(task, returnTo, taskDialogId)}
                  </div>`;
                  }).join(""),
  ...
  function renderQuoteOpsWorkspaceStyle() {
    return `<style>
      ...
      .admin-quote-funnel-card-button {
        border: 0;
        padding: 0;
      ...
      .admin-quote-task-list {
        display: grid;
        gap: 14px;
      }
      .admin-quote-task-card {
        border: 1px solid var(--border);
        border-radius: 22px;
        padding: 18px;
      ...
      .admin-quote-task-head {
        display: flex;
        justify-content: space-between;
        gap: 16px;
        align-items: flex-start;
      }
      .admin-quote-task-title {
        margin: 0;
        font-size: 20px;
      }
      .admin-quote-task-actions {
        display: flex;
        flex-wrap: wrap;
        gap: 10px;
      }
      .admin-quote-task-actions form {
        margin: 0;
      }
  ...
  function renderQuoteOpsFunnelCard(entry, returnTo) {
    ...
    >
      <div class="admin-quote-funnel-card-top">
        <div>
          <h3 class="admin-quote-funnel-title">
            <button
              class="admin-quote-funnel-card-button"
              type="button"
              data-admin-dialog-open="${escapeHtmlAttribute(dialogId)}"
            >${escapeHtml(entry.customerName || "Клиент")}</button>
          </h3>
      ...
      <div class="admin-badge-row">
        ${manager.name || manager.email ? renderAdminBadge(manager.name || manager.email, "outline") : renderAdminBadge("Без менеджера", "muted")}
        ${renderQuoteOpsStatusBadge(entry.status)}
      </div>
      <div class="admin-quote-funnel-meta">
        <div class="admin-quote-funnel-meta-item">
          <span>Задача</span>
          <strong>${escapeHtml(openTask ? openTask.title : leadStatus === "confirmed" ? "Заказ создан" : "Нет открытой задачи")}</strong>
        </div>
        <div class="admin-quote-funnel-meta-item">
          <span>Дедлайн</span>
          <strong>${escapeHtml(openTask ? formatAdminDateTime(openTask.dueAt) : "—")}</strong>
        </div>
        <div class="admin-quote-funnel-meta-item">
          <span>Уборка</span>
  ...
  function renderQuoteOpsFunnelLane(status, entries, returnTo) {
    ...
    return `<section class="admin-quote-funnel-column" data-lead-dropzone="${escapeHtmlAttribute(status)}">
      <div class="admin-quote-funnel-head">
        ...
        ${renderAdminBadge(String(entries.length), "outline")}
      </div>
      <div class="admin-quote-funnel-list">
        ${entries.length > 0
      ...
      <div class="admin-dialog-panel">
        <div class="admin-dialog-head">
          <div>
            ...
            <h2 class="admin-dialog-title" id="${dialogId}-title">Результат звонка</h2>
            <p class="admin-dialog-copy">${escapeHtml(taskRecord.customerName)} • ${escapeHtml(taskRecord.title)}</p>
          </div>
          <button class="admin-button admin-button-secondary" type="button" data-admin-dialog-close="${dialogId}">Закрыть</button>
        </div>
        <form class="admin-form admin-form-grid" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
          ...
          <div class="admin-inline-actions">
            <button class="admin-button" type="submit">Сохранить результат</button>
          </div>
        </form>
      </div>
    </dialog>`;
  ...
  function renderQuoteOpsTaskCard(taskRecord, returnTo) {
    const dialogId = `admin-quote-task-result-dialog-${normalizeString(taskRecord.id, 120)}`;
    return `<article class="admin-quote-task-card">
      <div class="admin-quote-task-head">
        <div>
          <p class="admin-card-eyebrow">Таск</p>
          <h3 class="admin-quote-task-title">${escapeHtml(taskRecord.title)}</h3>
          <p class="admin-quote-task-card-copy">${escapeHtml(taskRecord.customerName)} • ${escapeHtml(taskRecord.serviceLabel)}</p>
        </div>
        <div class="admin-badge-row">
          ${renderLeadStatusBadge(taskRecord.leadStatus)}
          ${taskRecord.manager.name || taskRecord.manager.email ? renderAdminBadge(taskRecord.manager.name || taskRecord.manager.email, "outline") : renderAdminBadge("Без менеджера", "muted")}
        </div>
      </div>
      <div class="admin-quote-task-meta">
        <div class="admin-quote-task-meta-item">
          <span>Дедлайн</span>
          <strong>${escapeHtml(formatAdminDateTime(taskRecord.dueAt))}</strong>
        </div>
        <div class="admin-quote-task-meta-item">
          <span>Заявка</span>
          <strong>${escapeHtml(taskRecord.requestId || taskRecord.entry.id)}</strong>
        </div>
      </div>
      <div class="admin-quote-task-actions">
        <button class="admin-button" type="button" data-admin-dialog-open="${dialogId}">Дозвонились</button>
        <form method="post" action="${ADMIN_QUOTE_OPS_PATH}">
          <input type="hidden" name="action" value="complete-lead-task">
          <input type="hidden" name="entryId" value="${escapeHtmlAttribute(taskRecord.entry.id)}">
          <input type="hidden" name="taskId" value="${escapeHtmlAttribute(taskRecord.id)}">
        ...
        <button class="admin-button admin-button-secondary" type="button" data-admin-dialog-open="${escapeHtmlAttribute(getQuoteOpsDialogId(taskRecord.entry.id))}">Открыть заявку</button>
      </div>
      ${renderQuoteOpsTaskResultDialog(taskRecord, returnTo, dialogId)}
    </article>`;
  }
    ...
    return `<dialog class="admin-dialog" id="admin-quote-funnel-discussion-dialog" aria-labelledby="admin-quote-funnel-discussion-title">
      <div class="admin-dialog-panel">
        <div class="admin-dialog-head">
          ...
          <button class="admin-button admin-button-secondary" type="button" data-admin-dialog-close="admin-quote-funnel-discussion-dialog">Закрыть</button>
        </div>
        <form class="admin-form admin-form-grid" method="post" action="${ADMIN_QUOTE_OPS_PATH}">
          <input type="hidden" name="action" value="update-lead-status">
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger, adminRuntime = {}, staffStore = null) {
    ...
    const filtersSummary = hasActiveFilters
      ? "Активные фильтры помогают сузить выборку до нужного менеджера, этапа и типа уборки."
      : activeSection === "tasks"
        ? "Открыты все текущие таски по заявкам."
        : activeSection === "funnel"
          ? "Открыта вся живая воронка заявок."
    ...
    const filtersPanel = `<section class="admin-orders-filters-panel admin-quote-ops-filter-shell" id="admin-quote-ops-filters">
      ...
      <form
        ...
        data-admin-auto-submit-scroll-offset="18"
      >
        <input type="hidden" name="section" value="${escapeHtmlAttribute(activeSection)}">
        <div class="admin-quote-ops-filter-inline-row">
          ...
          <div class="admin-inline-actions admin-quote-ops-filter-inline-actions">
            <a class="admin-link-button admin-button-secondary" href="${activeSection === "list" ? ADMIN_QUOTE_OPS_PATH : `${ADMIN_QUOTE_OPS_PATH}?section=${encodeURIComponent(activeSection)}`}">Сбросить</a>
          </div>
        </div>
    ...
    const tasksBody = taskRecords.length > 0
      ? `<div class="admin-quote-task-list">
          ${taskRecords.map((taskRecord) => renderQuoteOpsTaskCard(taskRecord, currentReturnTo)).join("")}
        </div>`
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-quote-ops-page">
        ...
        ${filtersPanel}
        ${workspaceBody}
        ${entries.map((entry) => renderQuoteOpsDetailDialog(entry, currentReturnTo, managerOptions)).join("")}
      </div>`,
      {
        kicker: false,
