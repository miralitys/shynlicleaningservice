...
function createAdminPageRenderers(deps = {}) {
      ...
      `<div class="admin-orders-page">
        ...
        <div class="admin-orders-layout">
          ...
          <aside class="admin-orders-side-panel">
            ...
            <div class="admin-orders-side-block">
              <p class="admin-orders-panel-kicker">Что видно сразу</p>
              <ul class="admin-feature-list">
                <li>Дата и команда вынесены в верхнюю часть карточки.</li>
                <li>Проблемные заказы помечаются до открытия редактора.</li>
                <li>Редактирование спрятано внутрь блока «Изменить заказ».</li>
                <li>Сотрудники и график по-прежнему связаны с разделом «Сотрудники».</li>
              </ul>
            </div>
          </aside>
        </div>
  ...
  function renderStaffEditDialog(staffSummary) {
    ...
    const contactItems = [
      { label: "Телефон", value: staffSummary.phone || "Не указан" },
  ...
  function renderStaffTableRow(staffSummary) {
    ...
    if (staffSummary.phone) {
      contactBits.push(`<span class="admin-table-strong">${escapeHtml(staffSummary.phone)}</span>`);
    }
  ...
  async function renderQuoteOpsPage(req, config, quoteOpsLedger) {
    ...
    const filteredAttentionCount = entries.filter((entry) => entry.status !== "success").length;
    ...
    return renderAdminLayout(
      ...
      `<div class="admin-quote-ops-page">
        ...
        <div class="admin-orders-layout admin-quote-ops-layout">
          <section class="admin-orders-filters-panel">
            <div class="admin-orders-panel-head">
              <div>
                <p class="admin-orders-panel-kicker">Навигация</p>
                <h2 class="admin-orders-panel-title">Быстро найти нужную заявку</h2>
                <p class="admin-orders-panel-copy">Фильтры спрятаны в кнопку, а ниже остаётся только рабочая лента без лишней технической оболочки.</p>
              </div>
              <div class="admin-quote-ops-filter-actions">
          ...
          <aside class="admin-orders-side-panel admin-quote-ops-side">
            <div class="admin-orders-side-block">
              <p class="admin-orders-panel-kicker">Сводка</p>
              <h2 class="admin-orders-panel-title">Срез по потоку</h2>
              <p class="admin-orders-panel-copy">Быстрый срез по текущей операционной нагрузке.</p>
              ${renderAdminPropertyList([
                { label: "В ленте сейчас", value: String(filteredCount) },
                { label: "Требуют внимания", value: String(filteredAttentionCount) },
            ...
            <div class="admin-orders-side-block">
              <p class="admin-orders-panel-kicker">Правила</p>
              <h2 class="admin-orders-panel-title">Как читать экран</h2>
              <p class="admin-orders-panel-copy">Сначала приоритет и статус, потом детали внутри карточки.</p>
              <ul class="admin-feature-list">
