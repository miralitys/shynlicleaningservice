...
function createAdminPageRenderers(deps = {}) {
            ...
            <form class="admin-form-grid" id="${escapeHtmlAttribute(editFormId)}" method="post" action="${ADMIN_STAFF_PATH}">
              ...
              <div class="admin-form-grid admin-form-grid-two">
                ...
                ${linkedUser
                  ? `<label class="admin-label">
                      Роль
                      <select class="admin-input" name="userRole">
                        ${USER_ROLE_VALUES.map((role) => `<option value="${escapeHtmlAttribute(role)}"${linkedUser.role === role ? " selected" : ""}>${escapeHtml(formatWorkspaceRoleLabel(role))}</option>`).join("")}
                      </select>
                      <small class="admin-field-hint">Меняет права доступа сразу после сохранения.</small>
                    </label>`
                  : ""}
                <label class="admin-label">
                  Телефон
  ...
  async function renderStaffPage(req, config, quoteOpsLedger, staffStore, adminRuntime = {}) {
    ...
    const enrichedStaffSummaries = staffSummaries.map((record) => ({
      ...record,
      linkedUser: linkedUserByStaffId.get(record.id) || null,
    }));
