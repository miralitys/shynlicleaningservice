...
.admin-client-photo-section { gap: 16px; }
.admin-client-photo-head { align-items: flex-start; }
.admin-client-photo-copy {
  margin: 4px 0 0;
  color: var(--muted);
  font-size: 13px;
  line-height: 1.5;
}
.admin-client-photo-grid {
  display: grid;
  gap: 12px;
  grid-template-columns: repeat(auto-fit, minmax(180px, 1fr));
}
.admin-client-photo-card {
  display: grid;
  gap: 10px;
  min-width: 0;
  padding: 12px;
...
.admin-client-photo-link {
  display: block;
  color: inherit;
  text-decoration: none;
}
.admin-client-photo-thumb {
  width: 100%;
  aspect-ratio: 4 / 3;
  object-fit: cover;
  display: block;
...
.admin-client-photo-thumb-placeholder {
  display: grid;
  place-items: center;
  color: var(--accent);
  font-size: 12px;
  font-weight: 900;
  letter-spacing: 0.08em;
  background: rgba(158, 67, 90, 0.1);
}
.admin-client-photo-card-foot {
  display: grid;
  gap: 10px;
  min-width: 0;
}
.admin-client-photo-card-copy {
  display: grid;
  gap: 2px;
  min-width: 0;
}
.admin-client-photo-card-copy strong {
  min-width: 0;
  overflow: hidden;
...
.admin-client-photo-card-copy span {
  color: var(--muted);
  font-size: 12px;
  line-height: 1.35;
}
.admin-client-photo-delete-form { display: flex; }
.admin-client-photo-delete-button { width: 100%; min-height: 34px; }
.admin-client-photo-upload-form {
  display: grid;
  gap: 10px;
  padding: 14px;
  border: 1px dashed rgba(158, 67, 90, 0.22);
  border-radius: 16px;
  background: rgba(255,255,255,0.82);
}
.admin-client-photo-upload-actions {
  align-items: center;
}
...
.admin-client-notification-toggle {
  grid-column: 1 / -1;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.admin-client-notification-toggle-copy { display: grid; gap: 4px; min-width: 0; }
.admin-client-notification-toggle-copy strong { font-size: 14px; line-height: 1.35; }
.admin-client-notification-toggle-copy small { color: var(--muted); font-size: 12px; line-height: 1.45; }
.admin-client-switch { position: relative; flex: 0 0 auto; width: 46px; height: 26px; }
.admin-client-switch input { position: absolute; width: 1px; height: 1px; opacity: 0; }
.admin-client-switch span {
  position: absolute;
  inset: 0;
  border-radius: 999px;
  background: #d4d4d8;
...
.admin-client-switch span::after {
  content: "";
  position: absolute;
  top: 3px;
  left: 3px;
...
.admin-client-switch input:checked + span { background: var(--primary); }
.admin-client-switch input:checked + span::after { transform: translateX(20px); }
.admin-client-switch input:focus-visible + span { outline: 3px solid rgba(59,130,246,0.32); outline-offset: 2px; }
@media (max-width: 640px) {
  .admin-client-notification-toggle { align-items: flex-start; }
}
`;
