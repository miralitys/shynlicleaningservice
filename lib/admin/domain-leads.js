...
function createAdminLeadDomain(deps = {}) {
  ...
  function normalizeLeadStatus(value, fallback = "new") {
    ...
    if (compact === "completed" || compact === "complete" || compact === "done" || compact === "выполнено") return "completed";
  ...
  function normalizeLeadTaskKind(value, fallback = "contact-client") {
    ...
    if (normalized === "post-completion-followup" || normalized === "next-cleaning-followup") {
      return "post-completion-followup";
    }
  ...
  function formatLeadTaskTitle(kind) {
    ...
    if (normalized === "post-completion-followup") return "Договориться о следующей уборке";
  ...
  function parseChicagoDateTimeLocalValue(value) {
    ...
    return buildChicagoIsoDateTime(year, month, day, hour, minute);
  }

  function normalizeLeadTaskDueAt(value, fallback = "") {
    const normalized = normalizeString(value, 80);
    if (!normalized) return fallback;
    const chicagoLocalDateTime = parseChicagoDateTimeLocalValue(normalized);
    if (chicagoLocalDateTime) return chicagoLocalDateTime;
  ...
  function buildDefaultNewLeadTask(entry = {}) {
    ...
    const entryId = normalizeString(entry.id, 100);
    return createLeadTaskRecord(
      {
        id: entryId ? `default-${entryId}` : "",
        kind: "contact-client",
  ...
  function getEntryLeadTasks(entry = {}) {
    ...
    if (getLeadStatus(entry) === "new" && !adminLead.defaultTaskDismissedAt) {
  ...
  function applyLeadEntryUpdates(entry, updates = {}) {
    ...
    const hasDeleteTask = Object.prototype.hasOwnProperty.call(updates, "deleteTaskId");
    ...
    const hasPostCompletionFollowupTask = Object.prototype.hasOwnProperty.call(
      updates,
      "createPostCompletionFollowupTask"
    );
    ...
    const deleteTaskId = normalizeString(updates.deleteTaskId, 120);
    ...
    function pushTask(kind, stage, dueAt, attempt = 0, taskExtras = {}) {
    ...
    if (hasDeleteTask && deleteTaskId) {
      tasks = tasks.filter((task) => task.id !== deleteTaskId);
      adminLead.defaultTaskDismissedAt = timestamp;
      ...
      if (taskAction === "contacted") {
        ...
        if (nextContactAt) {
          pushTask("discussion-followup", targetStatus, nextContactAt);
        } else if (targetStatus === "discussion") {
          pushTask("discussion-followup", "discussion", nextContactAt || addHoursToIso(timestamp, 24));
        } else if (targetStatus === "no-response") {
          ...
          closeOpenTasks("completed", "canceled");
      ...
      } else if (taskAction === "next-cleaning-agreed") {
        touchTask(activeTask, "completed", "next-cleaning-agreed");
        const nextCleaningAt = normalizeLeadTaskDueAt(updates.nextCleaningAt, "");
        if (nextCleaningAt) {
          adminLead.nextCleaningScheduledAt = nextCleaningAt;
        }
      } else if (taskAction === "next-cleaning-declined") {
        touchTask(activeTask, "completed", "next-cleaning-declined");
        adminLead.nextCleaningDeclinedAt = timestamp;
      }
    } else if (hasStatusUpdate) {
      delete adminLead.defaultTaskDismissedAt;
      closeOpenTasks("stage-changed", "canceled");
      if (nextContactAt) {
        noResponseAttempts = nextStatus === "no-response" ? Math.max(1, noResponseAttempts || 1) : 0;
        pushTask("discussion-followup", nextStatus, nextContactAt);
    ...
    if (hasPostCompletionFollowupTask && updates.createPostCompletionFollowupTask) {
      const hasOpenPostCompletionTask = tasks.some(
        (task) =>
          task.status === "open" &&
          normalizeLeadTaskKind(task.kind, "") === "post-completion-followup"
      );
      if (!hasOpenPostCompletionTask) {
        pushTask(
          "post-completion-followup",
          nextStatus,
          updates.postCompletionFollowupDueAt || timestamp,
          0,
          {
            assigneeId: managerId,
            assigneeName: managerName,
            assigneeEmail: managerEmail,
            assigneeRole: managerId || managerName || managerEmail ? "manager" : "",
          }
        );
      }
    }

    if (!hasDeleteTask && !tasks.some((task) => task.status === "open") && nextStatus === "new") {
      pushTask("contact-client", "new", timestamp, 0);
    }
    ...
    if (nextStatus === "completed") {
      adminLead.completedAt = normalizeLeadTaskDueAt(adminLead.completedAt, timestamp);
    } else {
      delete adminLead.completedAt;
    }
    ...
    if (nextStatus === "confirmed" && !isOrderCreatedEntry(entry)) {
