...
const ADMIN_SHARED_ORDER_COMPLETION_AND_EDITOR_SCRIPT = `        function escapeAdminHtml(value) {
        ...
        function setOrderScheduleEditorState(panelId, expanded) {
          if (!panelId) return;
          document.querySelectorAll("[data-admin-order-schedule-editor]").forEach((panel) => {
            if (panel.getAttribute("data-admin-order-schedule-editor") === panelId) {
              panel.hidden = !expanded;
            }
          });
          document.querySelectorAll("[data-admin-order-schedule-edit-trigger]").forEach((button) => {
            if (button.getAttribute("data-admin-order-schedule-edit-trigger") === panelId) {
              button.hidden = expanded;
            }
          });
          document.querySelectorAll("[data-admin-order-schedule-action-for]").forEach((button) => {
            if (button.getAttribute("data-admin-order-schedule-action-for") === panelId) {
              button.hidden = !expanded;
            }
          });
        }

        function closeOrderScheduleEditor(panelId, options = {}) {
          if (!panelId) return;
          const panel = document.querySelector('[data-admin-order-schedule-editor="' + panelId + '"]');
          if (panel && options.reset !== false) {
            const form = panel instanceof HTMLFormElement ? panel : panel.closest("form");
            if (form && typeof form.reset === "function") {
              form.reset();
            }
            panel.querySelectorAll("[data-admin-picker-field]").forEach((field) => {
              if (typeof syncPickerNativeFromDisplay === "function") {
                syncPickerNativeFromDisplay(field);
              }
              if (field.getAttribute("data-admin-picker-field") === "time" && typeof syncTimePanelFromField === "function") {
                syncTimePanelFromField(field);
              }
            });
          }
          setOrderScheduleEditorState(panelId, false);
        }

        function resetOrderScheduleEditors(scope, options = {}) {
          const targetScope = scope && typeof scope.querySelectorAll === "function" ? scope : document;
          targetScope.querySelectorAll("[data-admin-order-schedule-editor]").forEach((panel) => {
            closeOrderScheduleEditor(panel.getAttribute("data-admin-order-schedule-editor"), options);
          });
        }

        function syncOrderScheduleEditors(scope) {
          const targetScope = scope && typeof scope.querySelectorAll === "function" ? scope : document;
          targetScope.querySelectorAll("[data-admin-order-schedule-editor]").forEach((panel) => {
            const panelId = panel.getAttribute("data-admin-order-schedule-editor");
            if (!panelId) return;
            setOrderScheduleEditorState(panelId, !panel.hidden);
          });
        }
