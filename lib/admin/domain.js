...
function applyClientEntryUpdates(entry, updates = {}) {
  ...
  const fullAddress = normalizeString(updates.address, 500);
  ...
  if (fullAddress) {
    ...
    calculatorData.address = fullAddress;
  } else {
    delete calculatorData.fullAddress;
    delete calculatorData.address;
  }
...
function collectAdminClientRecords(entries = []) {
  ...
  for (const entry of entries) {
    ...
    const nameAddressKey = normalizeString(
      [normalizedName, normalizedAddress].filter(Boolean).join("|"),
      500
    );
    const fallbackKey = normalizeString(
      [normalizedName, email, phoneKey, normalizedAddress, requestId.toLowerCase()].filter(Boolean).join("|"),
      500
    );
    const key = nameAddressKey || fallbackKey || normalizeString(entry.id, 120);
