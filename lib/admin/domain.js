...
function applyOrderEntryUpdates(entry, updates = {}) {
  ...
  const hasPolicyEmailSentAt = Object.prototype.hasOwnProperty.call(updates, "policyEmailSentAt");
  const hasPolicyEmailLastError = Object.prototype.hasOwnProperty.call(updates, "policyEmailLastError");
  const hasPolicyAcceptedAt = Object.prototype.hasOwnProperty.call(updates, "policyAcceptedAt");
  const hasPolicyTermsAcceptedAt = Object.prototype.hasOwnProperty.call(updates, "policyTermsAcceptedAt");
  const hasPolicyCancellationAcceptedAt = Object.prototype.hasOwnProperty.call(
    updates,
    "policyCancellationAcceptedAt"
  );
  const hasPolicyAcceptedByEmail = Object.prototype.hasOwnProperty.call(updates, "policyAcceptedByEmail");
  ...
  const currentPolicyConfirmation =
  ...
  const policyConfirmation = {
    ...currentPolicyConfirmation,
  };

  if (hasPolicyEmailSentAt) {
    const value = normalizeString(updates.policyEmailSentAt, 80);
    if (value) {
      policyConfirmation.emailSentAt = value;
    } else {
      delete policyConfirmation.emailSentAt;
    }
  }
  if (hasPolicyEmailLastError) {
    const value = normalizeString(updates.policyEmailLastError, 400);
    if (value) {
      policyConfirmation.emailLastError = value;
    } else {
      delete policyConfirmation.emailLastError;
    }
  }
  if (hasPolicyAcceptedAt) {
    const value = normalizeString(updates.policyAcceptedAt, 80);
    if (value) {
      policyConfirmation.acceptedAt = value;
    } else {
      delete policyConfirmation.acceptedAt;
    }
  }
  if (hasPolicyTermsAcceptedAt) {
    const value = normalizeString(updates.policyTermsAcceptedAt, 80);
    if (value) {
      policyConfirmation.termsAcceptedAt = value;
    } else {
      delete policyConfirmation.termsAcceptedAt;
    }
  }
  if (hasPolicyCancellationAcceptedAt) {
    const value = normalizeString(updates.policyCancellationAcceptedAt, 80);
    if (value) {
      policyConfirmation.cancellationAcceptedAt = value;
    } else {
      delete policyConfirmation.cancellationAcceptedAt;
    }
  }
  if (hasPolicyAcceptedByEmail) {
    const value = normalizeString(updates.policyAcceptedByEmail, 250).toLowerCase();
    if (value) {
      policyConfirmation.acceptedByEmail = value;
    } else {
      delete policyConfirmation.acceptedByEmail;
    }
  }

  if (Object.keys(policyConfirmation).length > 0) {
    adminOrder.policyConfirmation = policyConfirmation;
  } else {
    delete adminOrder.policyConfirmation;
  }
