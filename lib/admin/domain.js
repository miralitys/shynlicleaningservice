...
function normalizeAdminClientAddressBookInput(value) {
  ...
  for (const rawValue of rawValues) {
    const address = normalizeString(rawValue, 500);
    if (!address) continue;
    const key = buildAdminClientAddressKey(address);
...
function applyClientEntryUpdates(entry, updates = {}) {
  ...
  const addressBook = normalizeAdminClientAddressBookInput(
    updates.addresses || updates.address || adminClient.addressBook || []
  );
...
function collectAdminClientRecords(entries = []) {
  ...
  const clientKeyByIdentity = new Map();
  ...
  function ensureAddressRecord(client, address) {
    ...
    if (!addressRecord) {
      addressRecord = createAddressRecord(normalizedAddress);
      client.addressesByKey.set(addressKey, addressRecord);
    } else if (!addressRecord.address && normalizedAddress) {
      addressRecord.address = normalizedAddress;
    }
  ...
  for (const entry of entries) {
    ...
    const addressBook = getEntryAdminClientAddresses(entry);
    const identityCandidates = [
      email ? `email:${email}` : "",
      phoneKey ? `phone:${phoneKey}` : "",
      normalizedName && email ? `name-email:${normalizedName}|${email}` : "",
      normalizedName && phoneKey ? `name-phone:${normalizedName}|${phoneKey}` : "",
    ].filter(Boolean);
    const matchedClientKey = identityCandidates.map((candidate) => clientKeyByIdentity.get(candidate)).find(Boolean);
    const nameAddressKey = normalizeString([normalizedName, normalizedAddress].filter(Boolean).join("|"), 500);
    const fallbackKey = normalizeString(
      [normalizedName, email, phoneKey].filter(Boolean).join("|"),
      500
    );
    const key = matchedClientKey || fallbackKey || nameAddressKey || normalizeString(entry.id, 120);
    ...
    if (!client) {
      client = {
        ...
        latestCreatedAtMs: Number.isFinite(createdAtMs) ? createdAtMs : 0,
    ...
    for (const identityCandidate of identityCandidates) {
      clientKeyByIdentity.set(identityCandidate, key);
    }
