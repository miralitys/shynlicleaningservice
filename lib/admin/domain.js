...
function getEntryPaymentData(entry = {}) {
  const payload = getEntryPayload(entry);
  return payload.payment && typeof payload.payment === "object" ? payload.payment : {};
}
...
function applyOrderEntryUpdates(entry, updates = {}) {
  ...
  const currentNotifications =
    adminOrder.notifications && typeof adminOrder.notifications === "object"
      ? adminOrder.notifications
      : {};
  ...
  if (hasNotifications) {
    if (updates.notifications && typeof updates.notifications === "object") {
      adminOrder.notifications = JSON.parse(JSON.stringify(updates.notifications));
    } else {
      delete adminOrder.notifications;
    }
  } else if (Object.keys(currentNotifications).length > 0) {
    adminOrder.notifications = JSON.parse(JSON.stringify(currentNotifications));
  } else {
    delete adminOrder.notifications;
  }
