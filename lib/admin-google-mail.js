...
function isGoogleMailAuthErrorMessage(value) {
  ...
  return (
    ...
    normalized.includes("google_mail_refresh_token_missing") ||
    normalized.includes("google_mail_token_response_invalid")
  );
}
  ...
  return {
    ...
    async getStatus() {
      ...
      const connectionNeedsReconnect = Boolean(
        connection &&
          normalizeString(connection.status, 40).toLowerCase() === "attention" &&
          isGoogleMailAuthErrorMessage(connection.lastError)
      );
      return {
        configured: Boolean(client && typeof client.isConfigured === "function" && client.isConfigured()),
        connected: Boolean(connection && !connectionNeedsReconnect),
        needsReconnect: connectionNeedsReconnect,
    ...
    async sendQuoteRequestConfirmationEmail(payload = {}, config, transportConfig = {}) {
      return sendTemplatedEmail(payload, config, transportConfig, {
        subject: "We received your cleaning request",
        buildCopy: (messagePayload) =>
          buildQuoteRequestConfirmationEmailCopy({
            clientName: messagePayload.clientName,
            serviceName: messagePayload.serviceName,
            selectedDate: messagePayload.selectedDate,
            selectedTime: messagePayload.selectedTime,
            fullAddress: messagePayload.fullAddress,
          }),
      });
    },
    async sendReviewRequestEmail(payload = {}, config, transportConfig = {}) {
      return sendTemplatedEmail(payload, config, transportConfig, {
        ...
        buildCopy: (messagePayload) =>
          buildReviewRequestEmailCopy({
            ...
            serviceName: messagePayload.serviceName,
            fullAddress: messagePayload.fullAddress,
            reviewUrl: messagePayload.reviewUrl,
          }),
      });
    },
    async sendOrderPolicyConfirmationEmail(payload = {}, config, transportConfig = {}) {
      return sendTemplatedEmail(payload, config, transportConfig, {
        subject: "Action Required: Please Review and Accept Before Your Cleaning Appointment",
        buildCopy: (messagePayload) =>
          buildPolicyAcceptanceEmailCopy({
            clientName: messagePayload.clientName,
            confirmUrl: messagePayload.confirmUrl,
            termsUrl: messagePayload.termsUrl,
            paymentPolicyUrl: messagePayload.paymentPolicyUrl,
            termsVersion: messagePayload.termsVersion,
            termsEffectiveDate: messagePayload.termsEffectiveDate,
            paymentVersion: messagePayload.paymentVersion,
            paymentEffectiveDate: messagePayload.paymentEffectiveDate,
          }),
      });
    },
    async sendTestEmail(payload = {}, config, transportConfig = {}) {
      return sendTemplatedEmail(payload, config, transportConfig, {
        subject: "SHYNLI Cleaning email test",
        buildCopy: (messagePayload) =>
          buildTestEmailCopy({
            recipientEmail: messagePayload.toEmail,
