...
function buildQuoteRequestConfirmationEmailCopy({
  clientName,
  serviceName,
  selectedDate,
  selectedTime,
  ...
  const safeName = normalizeString(clientName, 120) || "there";
  const safeServiceName = normalizeString(serviceName, 120) || "Cleaning Service";
  const safeDate = normalizeString(selectedDate, 32);
  const safeTime = normalizeString(selectedTime, 32);
  const safeAddress = normalizeString(fullAddress, 500);
  const scheduleLabel =
    safeDate && safeTime ? `${safeDate} at ${safeTime}` : safeDate || safeTime || "";

  const detailLines = [
    `Service: ${safeServiceName}`,
    scheduleLabel ? `Requested time: ${scheduleLabel}` : "",
  ...
  const text = [
    `Hello ${safeName},`,
    "",
    "Thank you for contacting SHYNLI CLEANING.",
    "We received your cleaning request and a manager will contact you shortly.",
    "",
    ...detailLines,
    "",
    "If you need to update anything, reply to this email and our team will help you.",
    "",
    "SHYNLI CLEANING",
  ].join("\n");
  ...
  const detailHtml = detailLines.length
    ? `<div style="padding:18px 20px;border-radius:18px;background:#faf9fb;border:1px solid #ece7eb;">
        <p style="margin:0 0 10px;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#71717a;font-weight:700;">Request details</p>
        ${detailLines
          .map(
            (line) =>
              `<p style="margin:0 0 8px;font-size:15px;line-height:1.6;color:#18181b;">${escapeHtml(line)}</p>`
          )
          .join("")}
      </div>`
    : "";
    ...
    <body style="margin:0;padding:24px;background:#f6f7fb;font-family:Montserrat,'Segoe UI',sans-serif;color:#18181b;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e4e4e7;border-radius:24px;overflow:hidden;">
        <tr>
          <td style="padding:32px 32px 16px;">
            <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#9e435a;font-weight:700;">SHYNLI CLEANING</div>
            <h1 style="margin:14px 0 10px;font-size:30px;line-height:1.1;">We received your request</h1>
            <p style="margin:0 0 14px;font-size:17px;line-height:1.6;color:#52525b;">
              Hello ${escapeHtml(safeName)}, thank you for contacting SHYNLI CLEANING.
            </p>
            <p style="margin:0 0 24px;font-size:17px;line-height:1.6;color:#52525b;">
              Your request is in our system now, and a manager will contact you shortly to confirm the details.
            </p>
            ${detailHtml}
          </td>
        </tr>
...
function buildReviewRequestEmailCopy({
  clientName,
  serviceName,
  fullAddress,
  reviewUrl,
  ...
  const safeServiceName = normalizeString(serviceName, 120) || "Cleaning Service";
  const safeAddress = normalizeString(fullAddress, 500);
  const safeReviewUrl = normalizeString(reviewUrl, 4000);
  ...
  const text = [
    `Hello ${safeName},`,
    "",
    "Thank you for choosing SHYNLI CLEANING.",
    "We would really appreciate your feedback about your recent cleaning.",
    "Please leave us a quick review here:",
    safeReviewUrl,
    "",
    ...detailLines,
    "",
    "Thank you again for choosing SHYNLI CLEANING.",
    "",
    "SHYNLI CLEANING",
  ].join("\n");
  ...
  const detailHtml = detailLines.length
    ? `<div style="padding:18px 20px;border-radius:18px;background:#faf9fb;border:1px solid #ece7eb;">
        <p style="margin:0 0 10px;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#71717a;font-weight:700;">Completed visit</p>
        ${detailLines
          .map(
            (line) =>
              `<p style="margin:0 0 8px;font-size:15px;line-height:1.6;color:#18181b;">${escapeHtml(line)}</p>`
    ...
    <body style="margin:0;padding:24px;background:#f6f7fb;font-family:Montserrat,'Segoe UI',sans-serif;color:#18181b;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e4e4e7;border-radius:24px;overflow:hidden;">
        <tr>
          <td style="padding:32px 32px 16px;">
            <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#9e435a;font-weight:700;">SHYNLI CLEANING</div>
            <h1 style="margin:14px 0 10px;font-size:30px;line-height:1.1;">Leave us a quick review</h1>
            <p style="margin:0 0 14px;font-size:17px;line-height:1.6;color:#52525b;">
              Hello ${escapeHtml(safeName)}, thank you for choosing SHYNLI CLEANING.
            </p>
            <p style="margin:0 0 24px;font-size:17px;line-height:1.6;color:#52525b;">
              If you have a moment, we would really appreciate your feedback about your recent cleaning.
            </p>
            <p style="margin:0 0 28px;">
              <a href="${escapeHtml(safeReviewUrl)}" style="display:inline-block;padding:15px 24px;border-radius:999px;background:#9e435a;color:#ffffff;text-decoration:none;font-size:16px;font-weight:700;">Leave a review</a>
            </p>
            ${detailHtml}
          </td>
        </tr>
...
function buildTestEmailCopy({ recipientEmail, sentAt } = {}) {
  const safeRecipientEmail = normalizeEmail(recipientEmail) || "there";
  const safeSentAt = normalizeString(sentAt, 80) || new Date().toISOString();

  const text = [
    `Hello ${safeRecipientEmail},`,
    "",
    "This is a test email from SHYNLI Cleaning admin mail settings.",
    "If you received it, the connected Gmail API mailbox can send outgoing messages.",
    "",
    `Sent at: ${safeSentAt}`,
    "",
    "SHYNLI CLEANING",
    ...
    <body style="margin:0;padding:24px;background:#f6f7fb;font-family:Montserrat,'Segoe UI',sans-serif;color:#18181b;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e4e4e7;border-radius:24px;overflow:hidden;">
        <tr>
          <td style="padding:32px 32px 16px;">
            <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#9e435a;font-weight:700;">SHYNLI CLEANING</div>
            <h1 style="margin:14px 0 10px;font-size:30px;line-height:1.1;">Email test</h1>
            <p style="margin:0 0 14px;font-size:17px;line-height:1.6;color:#52525b;">
              This is a test email from SHYNLI Cleaning admin mail settings.
            </p>
            <p style="margin:0 0 24px;font-size:17px;line-height:1.6;color:#52525b;">
            ...
            <div style="padding:18px 20px;border-radius:18px;background:#faf9fb;border:1px solid #ece7eb;">
              <p style="margin:0 0 10px;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#71717a;font-weight:700;">Test details</p>
              <p style="margin:0 0 8px;font-size:15px;line-height:1.6;color:#18181b;">Recipient: ${escapeHtml(safeRecipientEmail)}</p>
              <p style="margin:0;font-size:15px;line-height:1.6;color:#18181b;">Sent at: ${escapeHtml(safeSentAt)}</p>
            </div>
          </td>
        </tr>
      </table>
...
async function sendQuoteRequestConfirmationEmail(options = {}) {
...
async function sendReviewRequestEmail(options = {}) {
  ...
  const { config, payload } = createEmailPayload(
    options,
    copy,
    "How was your cleaning? Leave us a quick review"
  );
...
async function sendOrderPolicyConfirmationEmail(options = {}) {
  const copy = buildPolicyAcceptanceEmailCopy({
    clientName: options.clientName,
    confirmUrl: options.confirmUrl,
    termsUrl: options.termsUrl,
    paymentPolicyUrl: options.paymentPolicyUrl,
    termsVersion: options.termsVersion,
    termsEffectiveDate: options.termsEffectiveDate,
    paymentVersion: options.paymentVersion,
    paymentEffectiveDate: options.paymentEffectiveDate,
  });
  const { config, payload } = createEmailPayload(
    options,
    copy,
