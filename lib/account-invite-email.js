...
function loadAccountInviteEmailConfig(env = process.env) {
  ...
  const smtpName = normalizeString(env.ACCOUNT_INVITE_SMTP_NAME, 240);
...
async function sendViaSmtp(config, payload, options = {}) {
  ...
  try {
    ...
    throw new Error(
      `ACCOUNT_INVITE_EMAIL_SEND_FAILED:${normalizeString(error && error.message ? error.message : "SMTP_SEND_FAILED", 240)}`
    );
  } finally {
