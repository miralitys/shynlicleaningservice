...
function isIncompatibleSupabaseMailTableError(error) {
...
function createSupabaseAdminMailClient(options = {}) {
  ...
  async function withQuoteOpsFallback(primaryOperation, fallbackOperation) {
    try {
      ...
      if (!isMissingSupabaseTableError(error) && !isIncompatibleSupabaseMailTableError(error)) {
        throw error;
      }
  ...
  async function fetchQuoteOpsSnapshot() {
    const rows = await requestQuoteOps(
      `?select=*&kind=eq.${encodeURIComponent(QUOTE_OPS_MAIL_KIND)}&request_id=eq.${encodeURIComponent(DEFAULT_INTEGRATION_ID)}&limit=1`,
      { method: "GET" }
    );
  ...
  return {
    ...
    async fetchSnapshot() {
      try {
        const fallbackSnapshot = await fetchQuoteOpsSnapshot();
        if (fallbackSnapshot.connection) {
    ...
    async upsertConnection(record = {}) {
      ...
      try {
        await requestQuoteOps(
          `?request_id=eq.${encodeURIComponent(DEFAULT_INTEGRATION_ID)}&kind=eq.${encodeURIComponent(QUOTE_OPS_MAIL_KIND)}`,
          {
            method: "DELETE",
            headers: {
      ...
      try {
        await request(config.mailTableName, "", {
          method: "POST",
          headers: {
    ...
    async deleteConnection(connectionId = DEFAULT_INTEGRATION_ID) {
      ...
      try {
        await requestQuoteOps(
          `?request_id=eq.${encodeURIComponent(normalizedId)}&kind=eq.${encodeURIComponent(QUOTE_OPS_MAIL_KIND)}`,
          {
            method: "DELETE",
            headers: {
              Prefer: "return=minimal",
