...
} = require("../stripe-checkout-options");
const { normalizeEnglishAddressLabel } = require("../address-format");
const {
  buildApplicationWebLeadRecord,
  buildQuoteWebLeadRecord,
} = require("../web-leads/records");
const { queueWebLeadDelivery } = require("../web-leads/pipeline");
const { sendTelegramTextNotification } = require("../web-leads/telegram");
...
function createApiHandlers(deps = {}) {
  ...
  function normalizeGhlInboundSmsPayload(payload) {
    ...
    const callStatus = pickWebhookString(sources, [
      "callStatus",
      "call.status",
      "status",
      "disposition",
    ...
    const normalizedEvent = `${eventType} ${messageType} ${callStatus}`.toLowerCase();
    const normalizedCallStatusKey = callStatus
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "");
    const isMissedCall =
      (normalizedEvent.includes("missed") && normalizedEvent.includes("call")) ||
      ["missed", "no-answer", "noanswer", "unanswered", "busy", "voicemail"].includes(normalizedCallStatusKey);
    const isSms =
      !isMissedCall &&
      (messageType === "SMS" || normalizedEvent.includes("sms") || Boolean(message && !messageType));
    return {
      eventType,
      messageType,
      eventKind: isMissedCall ? "missed_call" : isSms ? "sms" : "unsupported",
      callStatus,
      direction:
        directionValue === "inbound" || /inbound/i.test(eventType) || isMissedCall
          ? "inbound"
          : directionValue || "inbound",
      message,
      phone,
      phoneDigits,
      contactName: pickWebhookString(sources, [
        "contactName",
        "name",
        "fullName",
        "contact.name",
  ...
  function getEntrySecondaryPhone(entry = {}) {
    const payload =
      entry && entry.payloadForRetry && typeof entry.payloadForRetry === "object"
        ? entry.payloadForRetry
        : {};
    const adminClient = payload.adminClient && typeof payload.adminClient === "object" ? payload.adminClient : {};
  ...
  function scoreEntrySmsTarget(entry, payload) {
    ...
    if (phoneDigitsMatch(getEntrySecondaryPhone(entry), payload.phoneDigits)) {
      identityScore += 40;
    }
    if (!(identityScore > 0)) {
      return -1;
  ...
  function formatTelegramLineValue(value, fallback = "-", maxLength = 500) {
    const normalized = normalizeString(value, maxLength).replace(/\s+/g, " ").trim();
    return normalized || fallback;
  }

  function buildGhlAppUrl(path) {
    const origin = normalizeString(process.env.GHL_APP_BASE_URL || "https://app.gohighlevel.com", 300)
      .replace(/\/+$/, "");
    const normalizedPath = normalizeString(path, 600);
    return `${origin || "https://app.gohighlevel.com"}${
      normalizedPath.startsWith("/") ? normalizedPath : `/${normalizedPath}`
    }`;
  }

  function buildGhlConversationUiUrl(options = {}) {
    const locationId = normalizeString(options.locationId, 120);
    if (!locationId) return buildGhlAppUrl("/");

    const locationPath = `/v2/location/${encodeURIComponent(locationId)}`;
    const conversationId = normalizeString(options.conversationId, 120);
    if (conversationId) {
      return buildGhlAppUrl(
        `${locationPath}/conversations/conversations/${encodeURIComponent(conversationId)}`
      );
    }

    const contactId = normalizeString(options.contactId, 120);
    if (contactId) {
      return buildGhlAppUrl(`${locationPath}/contacts/detail/${encodeURIComponent(contactId)}`);
    }

    return buildGhlAppUrl(`${locationPath}/conversations`);
  }

  function getGhlLocationId(payload = {}) {
    return normalizeString((payload && payload.locationId) || process.env.GHL_LOCATION_ID, 120);
  }

  function getLatestSmsHistoryConversationId(history = []) {
    return (Array.isArray(history) ? history : [])
      .map((entry) => normalizeString(entry && entry.conversationId, 120))
      .find(Boolean) || "";
  }

  function getGhlWebhookTargetContext(bestTarget, payload) {
    const locationId = getGhlLocationId(payload);
    if (!bestTarget || !bestTarget.record) {
      const conversationId = normalizeString(payload && payload.conversationId, 120);
      const contactId = normalizeString(payload && payload.contactId, 120);
      return {
        kind: "unmatched",
        label: "Unmatched contact",
        name: normalizeString(payload && payload.contactName, 160),
        phone: normalizeString(payload && payload.phone, 80),
        locationId,
        conversationId,
        contactId,
        ghlUrl: buildGhlConversationUiUrl({ locationId, conversationId, contactId }),
      };
    }

    if (bestTarget.kind === "staff") {
      const staffRecord = bestTarget.record;
      const staffId = normalizeString(staffRecord.id, 120);
      const staffName = normalizeString(staffRecord.name, 160);
      const staffHistory = Array.isArray(staffRecord.smsHistory) ? staffRecord.smsHistory : [];
      const conversationId =
        normalizeString(payload && payload.conversationId, 120) ||
        getLatestSmsHistoryConversationId(staffHistory);
      const contactId = normalizeString(
        (payload && payload.contactId) || staffRecord.contactId || staffRecord.ghlContactId,
        120
      );
      return {
        kind: "staff",
        label: "Staff",
        name: staffName,
        phone: normalizeString(staffRecord.phone || (payload && payload.phone), 80),
        locationId,
        conversationId,
        contactId,
        ghlUrl: buildGhlConversationUiUrl({ locationId, conversationId, contactId }),
    ...
    const entryId = normalizeString(entry.id, 120);
    const isOrder = isOrderLikeEntry(entry);
    const customerName = normalizeString(entry.customerName || (payload && payload.contactName), 160);
    const conversationId =
      normalizeString(payload && payload.conversationId, 120) ||
      getLatestSmsHistoryConversationId(getEntrySmsHistory(entry));
    const contactId = normalizeString((payload && payload.contactId) || entry.contactId, 120);
    return {
      kind: isOrder ? "order" : "lead",
      label: isOrder ? "Order" : "Lead",
      name: customerName,
      phone: normalizeString(entry.customerPhone || (payload && payload.phone), 80),
  ...
  async function resolveGhlWebhookConversationContext(payload, targetContext = {}, requestLogger) {
    const locationId = normalizeString(
      targetContext.locationId || (payload && payload.locationId) || process.env.GHL_LOCATION_ID,
      120
    );
    let conversationId = normalizeString(
      targetContext.conversationId || (payload && payload.conversationId),
      120
    );
    let contactId = normalizeString(targetContext.contactId || (payload && payload.contactId), 120);

    if (!conversationId && typeof getLeadConnectorClient === "function") {
      try {
        const leadConnector = getLeadConnectorClient();
        if (leadConnector && typeof leadConnector.getSmsHistory === "function") {
          const result = await leadConnector.getSmsHistory({
            phone: payload && (payload.phoneDigits || payload.phone),
            contactId,
            conversationId,
          });
          if (result && result.ok) {
            contactId = contactId || normalizeString(result.contactId, 120);
            conversationId =
              getLatestSmsHistoryConversationId(result.history) ||
              (Array.isArray(result.conversationIds)
                ? result.conversationIds.map((entry) => normalizeString(entry, 120)).find(Boolean)
                : "") ||
              conversationId;
          } else if (requestLogger && typeof requestLogger.log === "function") {
            requestLogger.log({
              ts: new Date().toISOString(),
              type: "ghl_inbound_conversation_lookup_skipped",
              code: normalizeString(result && result.code, 80),
        ...
        if (requestLogger && typeof requestLogger.log === "function") {
          requestLogger.log({
            ts: new Date().toISOString(),
            type: "ghl_inbound_conversation_lookup_error",
            message: normalizeString(error && error.message ? error.message : "GHL lookup failed.", 300),
          });
        }
      }
    }
  ...
  function formatGhlWebhookTelegramMessage(payload, targetContext = {}) {
    const isMissedCall = payload && payload.eventKind === "missed_call";
    const title = isMissedCall ? "Missed call" : "New inbound SMS";
    const lines = [
      title,
      `Name: ${formatTelegramLineValue(targetContext.name || (payload && payload.contactName), "-")}`,
      `Phone: ${formatTelegramLineValue(targetContext.phone || (payload && payload.phone), "-")}`,
    ];

    if (isMissedCall) {
      lines.push(`Status: ${formatTelegramLineValue(payload && payload.callStatus, "missed")}`);
    } else {
      lines.push(`Message: ${formatTelegramLineValue(payload && payload.message, "-", 1200)}`);
    }

    if (targetContext.label) {
      lines.push(`Matched: ${formatTelegramLineValue(targetContext.label)}`);
    }
    if (targetContext.ghlUrl) {
      lines.push(`GHL: ${formatTelegramLineValue(targetContext.ghlUrl, "-", 300)}`);
    }
    if (payload && payload.conversationId) {
      lines.push(`Conversation: ${formatTelegramLineValue(payload.conversationId, "-", 120)}`);
    }
    return lines.join("\n");
  }

  function queueGhlWebhookTelegramNotification(payload, targetContext, requestLogger) {
    setImmediate(() => {
      resolveGhlWebhookConversationContext(payload, targetContext, requestLogger)
        .then((resolvedTargetContext) =>
          sendTelegramTextNotification(formatGhlWebhookTelegramMessage(payload, resolvedTargetContext))
        )
        .then((result) => {
          if (requestLogger && typeof requestLogger.log === "function") {
            requestLogger.log({
              ts: new Date().toISOString(),
              type: "ghl_inbound_telegram_result",
              event_kind: normalizeString(payload && payload.eventKind, 40),
        ...
        .catch((error) => {
          if (requestLogger && typeof requestLogger.log === "function") {
            requestLogger.log({
              ts: new Date().toISOString(),
              type: "ghl_inbound_telegram_error",
              event_kind: normalizeString(payload && payload.eventKind, 40),
              message: normalizeString(error && error.message ? error.message : "Telegram notification failed.", 300),
            });
          }
        });
    });
  ...
  function buildRetryPayload(contactData, calculatorData, source, requestId, submittedAt, userAgent, pagePath, metadata = {}) {
    return {
      contactData,
      calculatorData,
      source,
  ...
  async function handleGhlInboundSmsWebhookRequest(
    ...
    if (payload.eventKind === "unsupported") {
      writeJsonWithTiming(
        res,
        202,
    ...
    if (!payload.phoneDigits) {
      writeJsonWithTiming(
        ...
        {
          error:
            payload.eventKind === "missed_call"
              ? "Missed call payload is missing phone."
              : "Inbound SMS payload is missing phone.",
        },
        requestStartNs,
        requestContext.cacheHit
      );
    ...
    if (payload.eventKind === "sms" && normalizedMessageType && normalizedMessageType !== "SMS") {
      writeJsonWithTiming(
        res,
        202,
        { received: true, ignored: true, reason: "unsupported-message-type" },
    ...
    if (payload.direction !== "inbound" && payload.eventKind !== "missed_call") {
      writeJsonWithTiming(
        res,
        202,
    ...
    if (!bestTarget) {
      ...
      queueGhlWebhookTelegramNotification(
        payload,
        getGhlWebhookTargetContext(null, payload),
        requestLogger
      );
      writeJsonWithTiming(
        res,
    ...
    let persisted = null;
    const shouldPersistSmsHistory = payload.eventKind === "sms" && Boolean(payload.message);
    if (payload.eventKind === "missed_call") {
      persisted = bestTarget.record;
    } else if (shouldPersistSmsHistory && bestTarget.kind === "entry") {
      const entry = bestTarget.record;
    ...
    } else if (shouldPersistSmsHistory && bestTarget.kind === "staff") {
      const staffRecord = bestTarget.record;
      const nextHistory = prependSmsHistoryEntry(
    ...
    requestLogger.log({
      ts: new Date().toISOString(),
      type: "ghl_inbound_event_received",
      endpoint: GHL_INBOUND_SMS_WEBHOOK_ENDPOINT,
    ...
    queueGhlWebhookTelegramNotification(
      payload,
      getGhlWebhookTargetContext(bestTarget, payload),
      requestLogger
    );
  ...
  async function handleQuoteSubmissionRequest(
    ...
    calculatorData.fullAddress = normalizeEnglishAddressLabel(calculatorData.fullAddress, 500);
    calculatorData.address = normalizeEnglishAddressLabel(calculatorData.address, 500);
    ...
    const requestType = normalizeString(
      body?.requestType || calculatorData?.requestType || body?.intent,
      40
    ).toLowerCase();
    const isCallMeRequest =
      requestType === "call_me" ||
      requestType === "call-me" ||
      requestType === "callback" ||
      requestType === "phone_call";

    const calculatedPricing = calculateQuotePricing(calculatorData);
    const pricing = isCallMeRequest
      ? {
          ...calculatedPricing,
    ...
    const canonicalCalculatorData = {
      ...
      requestType: isCallMeRequest ? "call_me" : requestType,
    };
    const serviceAreaZip = resolveQuoteServiceAreaZip(canonicalCalculatorData);
    if (!isCallMeRequest && (!serviceAreaZip || !isSupportedServiceAreaZip(serviceAreaZip))) {
    ...
    const leadType = normalizeString(body?.leadType || body?.type, 120);
    const sourceWebsite = normalizeString(body?.sourceWebsite || body?.origin, 256);
    const sourceDomain = normalizeString(body?.sourceDomain, 120);
    const landingPageUrl = normalizeString(body?.landingPageUrl || body?.landing_page_url || body?.landingPage, 256);
    const sourcePage = normalizeString(body?.sourcePage || body?.source_page || body?.sourcePagePath || body?.pagePath || body?.returnPath, 120);
    const siteLeadMetadata = {
      ...(leadType ? { leadType } : {}),
    ...
    const webLeadRecord = buildQuoteWebLeadRecord({
      body,
      req,
      contactData,
      calculatorData: canonicalCalculatorData,
    ...
    if (quoteOpsLedger) {
      recordedEntry = await recordQuoteSubmissionWithAutoManager({
        ok: true,
        requestId,
        sourceRoute: publicQuotePath,
        source,
      ...
      if (recordedEntry && recordedEntry.id && webLeadRecord && webLeadRecord.summary) {
        webLeadRecord.summary.internalCrmUrl = `${String(SITE_ORIGIN).replace(/\/+$/, "")}/admin/quote-ops?entry=${encodeURIComponent(recordedEntry.id)}`;
      }
    ...
    queueWebLeadDelivery(webLeadRecord, { requestLogger });
