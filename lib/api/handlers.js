...
function createApiHandlers(deps = {}) {
  ...
  function buildAdminUrl(path) {
    const origin = normalizeString(SITE_ORIGIN, 300).replace(/\/+$/, "");
    return `${origin || "https://shynlicleaningservice.com"}${path}`;
  }
  ...
  function getGhlWebhookTargetContext(bestTarget, payload) {
    if (!bestTarget || !bestTarget.record) {
      return {
        ...
        adminUrl: buildAdminUrl("/admin/messages"),
      };
    }
    ...
    if (bestTarget.kind === "staff") {
      ...
      return {
        ...
        adminUrl: buildAdminUrl(`/admin/staff?staff=${encodeURIComponent(staffName || staffId)}`),
      };
    }
    ...
    return {
      ...
      adminUrl: buildAdminUrl(
        isOrder
          ? `/admin/orders?order=${encodeURIComponent(entryId)}`
          : `/admin/quote-ops?entry=${encodeURIComponent(entryId)}`
      ),
    };
  }
  ...
  function formatGhlWebhookTelegramMessage(payload, targetContext = {}) {
    ...
    if (targetContext.adminUrl) {
      lines.push(`CRM: ${formatTelegramLineValue(targetContext.adminUrl, "-", 300)}`);
    }
  ...
  function queueGhlWebhookTelegramNotification(payload, targetContext, requestLogger) {
    const text = formatGhlWebhookTelegramMessage(payload, targetContext);
    setImmediate(() => {
      sendTelegramTextNotification(text)
        .then((result) => {
