...
} = require("./client-automatic-notifications");

const DEFAULT_TIME_ZONE = "America/Chicago";
const DEFAULT_REMINDER_SCAN_LIMIT = 1000;
const LEGACY_CUSTOMER_REVIEW_URL = "https://maps.app.goo.gl/4u9s7onykNrJEEn99";
const DEFAULT_CUSTOMER_REVIEW_URL = "https://g.page/r/CUxwaxYdaymBEAI/review";
...
function normalizeCustomerReviewUrl(value) {
  const candidate = normalizeString(value, 4000);
  if (!candidate) return DEFAULT_CUSTOMER_REVIEW_URL;

  const lower = candidate.toLowerCase();
  if (
    lower === LEGACY_CUSTOMER_REVIEW_URL.toLowerCase() ||
    lower.includes("maps.app.goo.gl/4u9s7onyknrjeen99") ||
    (lower.includes("google.com/search") &&
      lower.includes("ludocid=9307087877612204108")) ||
    (lower.includes("google.com/maps/place/shynli+cleaning+service") &&
      lower.includes("0x81296b1d166b704c"))
  ) {
    return DEFAULT_CUSTOMER_REVIEW_URL;
  }
  ...
  return {
    ...
    cleaningCompleteSmsByManagerId:
      raw.cleaningCompleteSmsByManagerId && typeof raw.cleaningCompleteSmsByManagerId === "object"
        ...
        : {},
    invoiceSms:
      raw.invoiceSms && typeof raw.invoiceSms === "object"
        ? cloneSerializable(raw.invoiceSms, {})
        : {},
  };
}
...
function formatUsScheduleDate(dateValue) {
  const date = normalizeString(dateValue, 32);
  if (!date) return "";

  const isoMatch = date.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if (isoMatch) {
    return `${padSchedulePart(isoMatch[2])}/${padSchedulePart(isoMatch[3])}/${isoMatch[1]}`;
  }

  const usMatch = date.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (usMatch) {
    return `${padSchedulePart(usMatch[1])}/${padSchedulePart(usMatch[2])}/${usMatch[3]}`;
  }
...
function formatUsScheduleTime(timeValue) {
  ...
  const existingPeriod = normalizeString(match[3], 2).toUpperCase();
  if (!Number.isFinite(hour) || hour < 0 || hour > 23) return time;

  if (existingPeriod) {
    if (hour === 0) hour = 12;
    if (hour > 12) return time;
    return `${padSchedulePart(hour)}:${minute} ${existingPeriod}`;
  }

  const period = hour >= 12 ? "PM" : "AM";
  const displayHour = hour % 12 || 12;
  return `${padSchedulePart(displayHour)}:${minute} ${period}`;
}

function buildScheduleLabel(dateValue, timeValue) {
  const date = formatUsScheduleDate(dateValue);
  const time = formatUsScheduleTime(timeValue);
  if (!date && !time) return "";
...
function buildRussianScheduleLabel(dateValue, timeValue) {
  return buildScheduleLabel(dateValue, timeValue);
}
...
function buildQuoteRequestSummary(entry = {}) {
  ...
  const address = normalizeEnglishAddressLabel(entry.fullAddress, 500);
  return {
    serviceName,
    scheduleLabel,
    address,
...
function buildQuoteConfirmationSmsMessage(entry = {}) {
  ...
  const lines = [
    `${greeting} we received your request from Shynli Cleaning Service.`,
...
function buildStaffAssignmentSmsMessage(entry = {}, staffRecord = {}, assignment = {}, siteOrigin = "") {
  ...
  const origin = normalizeString(siteOrigin, 500).replace(/\/+$/g, "");
  const entryId = normalizeString(entry.id || entry.requestId, 180);
  const accountUrl = origin
    ? `${origin}/account${entryId ? `?order=${encodeURIComponent(entryId)}` : ""}`
    : "";
...
function buildClientVisitReminderSmsMessage(entry = {}, hoursBefore = 24) {
  ...
  const greeting = firstName ? `Hi ${firstName},` : "Hi,";
  const summary = buildQuoteRequestSummary(entry);
  const visitLabel = summary.serviceName.toLowerCase().includes("estimate")
    ? "free in-home estimate"
    : "cleaning";
  const timingLabel =
    hoursBefore === 1 ? "in 1 hour" : hoursBefore === 24 ? "in 24 hours" : `in ${hoursBefore} hours`;
  return `${greeting} this is a reminder from Shynli Cleaning Service. Your ${visitLabel} is ${timingLabel}. ${
    summary.serviceName ? `Service: ${summary.serviceName}. ` : ""
...
function buildUpdatedClientVisitReminderSmsMessage(entry = {}) {
  const firstName = getCustomerFirstName(entry.customerName);
  const greeting = firstName ? `Hi ${firstName},` : "Hi,";
  const summary = buildQuoteRequestSummary(entry);
  const visitLabel = summary.serviceName.toLowerCase().includes("estimate")
  ...
  return `${greeting} your ${visitLabel} with Shynli Cleaning Service has been updated. ${
    summary.serviceName ? `Service: ${summary.serviceName}. ` : ""
  }${summary.scheduleLabel ? `Time: ${summary.scheduleLabel}. ` : ""}${
    summary.address ? `Address: ${summary.address}.` : ""
  }`
...
function buildReviewRequestSmsMessage(entry = {}, reviewUrl = DEFAULT_CUSTOMER_REVIEW_URL) {
  ...
  const greeting = firstName ? `Hi ${firstName},` : "Hi,";
  const summary = buildQuoteRequestSummary(entry);
  const safeReviewUrl = normalizeCustomerReviewUrl(reviewUrl);
...
function buildReviewRequestFollowUpSmsMessage(entry = {}) {
  const firstName = getCustomerFirstName(entry.customerName);
  const greeting = firstName || "Hi";
  return `${greeting}, If you have a moment, we'd truly appreciate it if you could leave us a review using the link we just sent.\n\nIf possible, it would also mean a lot to us if you could mention the city or area where we cleaned your home (for example, Naperville, Oswego, Plainfield, etc.). It really helps future customers find us.\n\nThank you so much for supporting our small business! We truly appreciate it. 😊`;
}
...
function buildInvoicePaymentLinkSmsMessage(entry = {}, paymentUrl = "") {
  const firstName = getCustomerFirstName(entry.customerName);
  const greeting = firstName ? `Hi ${firstName},` : "Hi,";
  const summary = buildQuoteRequestSummary(entry);
  const safePaymentUrl = normalizeString(paymentUrl, 4000);
  if (!safePaymentUrl) return "";
  return `${greeting} your invoice from Shynli Cleaning Service is ready. Pay securely here: ${safePaymentUrl} ${
    summary.serviceName ? `Service: ${summary.serviceName}. ` : ""
  }${summary.scheduleLabel ? `Scheduled: ${summary.scheduleLabel}. ` : ""}${
    summary.address ? `Address: ${summary.address}.` : ""
  }`
    .replace(/\s+/g, " ")
    .trim();
...
function getReminderSweepDateRange(now, timeZone = DEFAULT_TIME_ZONE) {
  ...
  return {
    ...
    endDate: formatDate(startTimestamp + 30 * 24 * 60 * 60 * 1000),
  };
}

function shouldRepairAcceptedOrderStatus(entry = {}) {
  const orderState = getEntryOrderState(entry);
  const policyAcceptance = orderState.policyAcceptance && typeof orderState.policyAcceptance === "object"
    ? orderState.policyAcceptance
    : {};
  return (
    getEntryOrderStatus(entry) === "new" &&
    policyAcceptance.policyAccepted === true &&
    Boolean(normalizeString(entry.selectedDate || orderState.selectedDate, 32)) &&
    Boolean(normalizeString(entry.selectedTime || orderState.selectedTime, 32)) &&
    Boolean(normalizeString(entry.assignedStaff || orderState.assignedStaff, 800))
  );
}
...
function createReminderSmsState(signature = "") {
  return {
...
function buildReminderHistoryTargetRef(entry = {}, signature = "", reminderKey = "") {
  return [
    normalizeString(entry.id, 80),
    encodeURIComponent(normalizeString(signature, 80)),
    normalizeString(reminderKey, 40),
  ].join(":");
}

function restoreReminderSmsStateFromHistory(entry = {}, signature = "") {
  const reminderSms = createReminderSmsState(signature);
  ...
  for (const record of history) {
    ...
    for (const window of REMINDER_WINDOWS) {
      const signedTargetRef = buildReminderHistoryTargetRef(entry, signature, window.key);
      const legacyTargetRef = `${normalizeString(entry.id, 80)}:${window.key}`;
      if (
        !reminderSms[window.key] &&
        (targetRef === signedTargetRef || targetRef === legacyTargetRef)
      ) {
...
function createAutoNotificationService(options = {}) {
  const {
    ...
    createOrderInvoicePaymentLink = null,
  ...
  const reviewSmsRetryDelays = Array.isArray(options.reviewSmsRetryDelays)
    ? options.reviewSmsRetryDelays
  ...
  async function areClientAutomaticNotificationsEnabled(entry, knownEntries = null) {
    const directPreference = getClientAutomaticNotificationsPreference(entry);
    if (directPreference !== null) return directPreference;
    if (!quoteOpsLedger || typeof quoteOpsLedger.listEntries !== "function") return true;
    try {
      const entries = Array.isArray(knownEntries)
        ? knownEntries
        : await quoteOpsLedger.listEntries({ limit: reminderScanLimit });
      return resolveClientAutomaticNotificationsEnabled(entry, entries);
    } catch (error) {
      logEvent("client_notification_preference_lookup_failed", {
  ...
  async function areClientPaymentNotificationsEnabled(entry, knownEntries = null) {
    const directPreference = getClientPaymentNotificationsPreference(entry);
    if (directPreference !== null) return directPreference;
    if (!quoteOpsLedger || typeof quoteOpsLedger.listEntries !== "function") return true;
    try {
      const entries = Array.isArray(knownEntries)
        ? knownEntries
        : await quoteOpsLedger.listEntries({ limit: reminderScanLimit });
      return resolveClientPaymentNotificationsEnabled(entry, entries);
    } catch (error) {
      logEvent("client_payment_notification_preference_lookup_failed", {
        entryId: normalizeString(entry && entry.id, 120),
        message: normalizeString(error && error.message, 300),
      });
      return true;
  ...
  function isRetryableReviewSmsResult(result) {
  ...
  async function waitForReviewSmsRetry(delayMs) {
  ...
  async function sendReviewSmsWithRetry(leadConnector, input) {
    let lastResult = null;
    for (let attempt = 0; attempt <= reviewSmsRetryDelays.length; attempt += 1) {
      try {
        lastResult = await leadConnector.sendSmsMessage(input);
      } catch (error) {
        lastResult = {
          ok: false,
      ...
      if (
        ...
        attempt >= reviewSmsRetryDelays.length ||
        !isRetryableReviewSmsResult(lastResult)
      ) {
        return lastResult;
      }
      await waitForReviewSmsRetry(reviewSmsRetryDelays[attempt]);
    }
    return lastResult;
  }
  ...
  async function handleUpdatedClientVisit({
    previousEntry,
    entry,
    ...
    if (!previousEntry || !entry) {
      return { sent: false, entry, reason: "missing-entry" };
    }

    const previousNotifications = getOrderNotificationState(previousEntry);
    const storedPreviousReminderSms = previousNotifications.reminderSms || {};
    const previousReminderSms = hasReminderSmsSentMarker(storedPreviousReminderSms)
      ? storedPreviousReminderSms
      : restoreReminderSmsStateFromHistory(
          previousEntry,
          buildReminderSignature(previousEntry)
        );
    if (!hasReminderSmsSentMarker(previousReminderSms)) {
      return { sent: false, entry, reason: "no-reminder-sent" };
    }
    ...
    const reminderSms = {
      ...previousReminderSms,
      signature,
      previousSignature: normalizeString(previousReminderSms.signature, 500),
      appointmentUpdatedAt: new Date().toISOString(),
    };
    let updatedEntry = await appendOrderSmsHistory(entry, null, {
      ...getOrderNotificationState(entry),
      reminderSms,
    });

    if (!sendUpdatedReminder) {
      return { sent: false, entry: updatedEntry, reason: "not-approved" };
    }
    ...
    if (
      ...
      return { sent: false, entry: updatedEntry, reason: "sms-unavailable" };
    }

    const phone = normalizePhone(updatedEntry.customerPhone);
    const message = buildUpdatedClientVisitReminderSmsMessage(updatedEntry);
    if (!phone || !message) {
      return { sent: false, entry: updatedEntry, reason: "missing-recipient" };
    }
    ...
    try {
      const smsResult = await leadConnector.sendSmsMessage({
        ...
        customerEmail: normalizeString(updatedEntry.customerEmail, 250).toLowerCase(),
        message,
      });
      if (!smsResult || !smsResult.ok) {
        return { sent: false, entry: updatedEntry, reason: "sms-send-failed" };
      }

      const record = buildSmsHistoryRecord(smsResult, {
        message,
        phone,
        targetType: "visit-reminder-update",
        targetRef: `${normalizeString(updatedEntry.id, 80)}:${encodeURIComponent(signature)}`,
        source: "manual",
      });
      reminderSms.lastUpdatedReminderSentAt = record.sentAt;
      updatedEntry = await appendOrderSmsHistory(updatedEntry, record, {
        ...getOrderNotificationState(updatedEntry),
        reminderSms,
      });
      return { sent: true, entry: updatedEntry, reason: "sent" };
    } catch (error) {
      logEvent("updated_visit_reminder_sms_failed", {
  ...
  async function notifyQuoteSubmissionSuccess({ entry, pricing = null, leadConnectorClient = null } = {}) {
    ...
    const leadConnector = getLeadConnectorClientSafe(leadConnectorClient);
    const customerAutomaticNotificationsEnabled =
      await areClientAutomaticNotificationsEnabled(updatedEntry);

    const customerEmail = normalizeString(updatedEntry.customerEmail, 250).toLowerCase();
    if (
      customerAutomaticNotificationsEnabled &&
      customerEmail &&
      accountInviteEmail &&
    ...
    if (canSendSms) {
      ...
      if (
        customerAutomaticNotificationsEnabled &&
        customerSmsMessage &&
        normalizePhone(updatedEntry.customerPhone)
      ) {
        try {
          const smsResult = await leadConnector.sendSmsMessage({
            contactId: normalizeString(updatedEntry.contactId, 120),
  ...
  async function notifyClientEnRoute({ entry, leadConnectorClient = null } = {}) {
    ...
    if (!(await areClientAutomaticNotificationsEnabled(entry))) {
      return {
        sent: false,
        entry,
        reason: "client-notifications-disabled",
      };
    }
  ...
  async function notifyAwaitingReviewRequest({
    entry,
    leadConnectorClient = null,
    force = false,
    source = "automatic",
    ...
    const manualEligibleStatuses = new Set(["awaiting-review", "completed", "paid"]);
    if (
      !entry ||
      (!force && orderStatus !== "awaiting-review") ||
      (force && !manualEligibleStatuses.has(orderStatus))
    ) {
      return {
        entry,
        customerEmailSent: false,
        customerSmsSent: false,
        reason: entry ? "status-unavailable" : "order-missing",
      };
    }

    if (!force && !(await areClientAutomaticNotificationsEnabled(entry))) {
      return {
        entry,
        customerEmailSent: false,
        customerSmsSent: false,
        customerFollowUpSmsSent: false,
        reason: "client-notifications-disabled",
      };
    }
    ...
    let customerEmailSent = false;
    let customerSmsSent = false;
    let customerFollowUpSmsSent = false;
    const notifications = getOrderNotificationState(entry);
    const existingReviewRequest =
      notifications.reviewRequest && typeof notifications.reviewRequest === "object"
        ...
        : {};
    const normalizedReviewUrl = normalizeCustomerReviewUrl(reviewUrl);
    const reviewRequestState = {
      signature: `awaiting-review|${normalizedReviewUrl}`,
      ...
      followUpSmsSentAt: normalizeString(existingReviewRequest.followUpSmsSentAt, 80),
    };

    const customerEmail = normalizeString(updatedEntry.customerEmail, 250).toLowerCase();
    if (
      customerEmail &&
      (force || !reviewRequestState.emailSentAt) &&
      accountInviteEmail &&
      typeof accountInviteEmail.getStatus === "function" &&
    ...
    const canSendSms =
      ...
      typeof leadConnector.isConfigured === "function" &&
      leadConnector.isConfigured();
    const customerPhone = normalizePhone(updatedEntry.customerPhone);
    let shouldSendFollowUpSms = false;
    if (canSendSms && customerPhone && (force || !reviewRequestState.smsSentAt)) {
      const message = buildReviewRequestSmsMessage(updatedEntry, reviewRequestState.reviewUrl);
      const smsResult = await sendReviewSmsWithRetry(leadConnector, {
        contactId: normalizeString(updatedEntry.contactId, 120),
        phone: customerPhone,
        customerName: normalizeString(updatedEntry.customerName, 160),
        customerEmail,
      ...
      const record = buildSmsHistoryRecord(smsResult, {
        ...
        targetRef: normalizeString(updatedEntry.id, 120),
        source,
        status: smsResult && smsResult.ok ? "sent" : "failed",
        errorMessage:
          smsResult && smsResult.ok
            ? ""
            : extractSmsFailureMessage(smsResult, "Не удалось отправить SMS со ссылкой на отзыв."),
      });
      if (smsResult && smsResult.ok) {
        reviewRequestState.smsSentAt = record.sentAt;
        customerSmsSent = true;
        shouldSendFollowUpSms = true;
      } else {
        logEvent("review_request_sms_failed", {
          entryId: normalizeString(updatedEntry.id, 120),
          code: normalizeString(smsResult && smsResult.code, 80).toUpperCase(),
          message: extractSmsFailureMessage(smsResult, "Failed to send review request SMS."),
        });
      }
      updatedEntry = await appendOrderSmsHistory(updatedEntry, record, {
        ...notifications,
      ...
      customerPhone &&
      !force &&
      reviewRequestState.smsSentAt &&
      !reviewRequestState.followUpSmsSentAt
    ) {
      shouldSendFollowUpSms = true;
    }

    if (canSendSms && customerPhone && shouldSendFollowUpSms) {
      const followUpMessage = buildReviewRequestFollowUpSmsMessage(updatedEntry);
      const followUpResult = await sendReviewSmsWithRetry(leadConnector, {
        contactId: normalizeString(updatedEntry.contactId, 120),
        phone: customerPhone,
        customerName: normalizeString(updatedEntry.customerName, 160),
        customerEmail,
      ...
      const followUpRecord = buildSmsHistoryRecord(followUpResult, {
        message: followUpMessage,
        phone: customerPhone,
        targetType: "review-request",
        targetRef: `${normalizeString(updatedEntry.id, 120)}:follow-up`,
        source,
        status: followUpResult && followUpResult.ok ? "sent" : "failed",
        errorMessage:
          followUpResult && followUpResult.ok
            ? ""
            : extractSmsFailureMessage(
                followUpResult,
                "Не удалось отправить дополнительную просьбу об отзыве."
              ),
      });
      if (followUpResult && followUpResult.ok) {
        reviewRequestState.followUpSmsSentAt = followUpRecord.sentAt;
        customerFollowUpSmsSent = true;
      } else {
        logEvent("review_request_follow_up_sms_failed", {
          entryId: normalizeString(updatedEntry.id, 120),
          code: normalizeString(followUpResult && followUpResult.code, 80).toUpperCase(),
          message: extractSmsFailureMessage(
            followUpResult,
            "Failed to send review request follow-up SMS."
          ),
        });
      }
      updatedEntry = await appendOrderSmsHistory(updatedEntry, followUpRecord, {
        ...notifications,
        reviewRequest: reviewRequestState,
      });
    }
    ...
    return {
      ...
      customerEmailSent,
      customerSmsSent,
      customerFollowUpSmsSent,
      reason: customerEmailSent || customerSmsSent || customerFollowUpSmsSent ? "sent" : "not-sent",
    };
  }

  async function notifyInvoicePaymentLink({ entry, leadConnectorClient = null } = {}) {
    if (!entry || getEntryOrderStatus(entry) !== "invoice-sent") {
      return {
        sent: false,
        entry,
        reason: "status-not-invoice-sent",
      };
    }
    ...
    if (!(await areClientPaymentNotificationsEnabled(entry))) {
      return {
        sent: false,
        entry,
        reason: "client-payment-notifications-disabled",
      };
    }
    ...
    if (typeof createOrderInvoicePaymentLink !== "function") {
      return {
        sent: false,
        entry,
    ...
    try {
      paymentLinkResult = await createOrderInvoicePaymentLink(entry);
    } catch (error) {
      logEvent("invoice_payment_link_create_failed", {
        entryId: normalizeString(entry.id, 120),
      ...
      return {
        sent: false,
        entry,
        reason: "payment-link-failed",
      };
    }

    if (!paymentLinkResult || paymentLinkResult.ok === false || !normalizeString(paymentLinkResult.url, 4000)) {
      logEvent("invoice_payment_link_create_failed", {
        entryId: normalizeString(entry.id, 120),
        requestId: normalizeString(entry.requestId, 120),
        code: normalizeString(paymentLinkResult && paymentLinkResult.code, 80).toUpperCase(),
        message: normalizeString(paymentLinkResult && paymentLinkResult.message, 300),
      });
      return {
        sent: false,
        entry,
    ...
    const customerPaymentUrl =
      normalizeString(paymentLinkResult.shortUrl, 500) ||
      normalizeString(paymentLinkResult.url, 4000);
    const message = buildInvoicePaymentLinkSmsMessage(entry, customerPaymentUrl);
    if (!message) {
      return {
        sent: false,
        entry,
        reason: "sms-message-required",
      };
    }
    ...
    try {
      ...
      if (!smsResult || !smsResult.ok) {
        updatedEntry = await appendOrderSmsHistory(
          ...
          buildSmsHistoryRecord(smsResult, {
            message,
            phone: customerPhone,
            targetType: "invoice-payment-link",
            targetRef: normalizeString(entry.id, 120),
            source: "automatic",
            status: "failed",
            errorMessage: extractSmsFailureMessage(
              smsResult,
              "Не удалось отправить клиенту SMS со ссылкой на оплату."
            ),
            recipientName: normalizeString(entry.customerName, 160),
          })
        );
        logEvent("invoice_payment_link_sms_failed", {
          entryId: normalizeString(entry.id, 120),
          requestId: normalizeString(entry.requestId, 120),
          code: normalizeString(smsResult && smsResult.code, 80).toUpperCase(),
          message: extractSmsFailureMessage(smsResult, "Failed to send invoice payment link SMS."),
        });
        return {
          sent: false,
          entry: updatedEntry,
      ...
      updatedEntry = await appendOrderSmsHistory(updatedEntry, record, {
        ...
        invoiceSms: {
          sentAt: record.sentAt,
          phone: customerPhone,
          checkoutUrl: normalizeString(paymentLinkResult.url, 4000),
          checkoutSessionId: normalizeString(paymentLinkResult.id, 120),
          shortCode: normalizeString(paymentLinkResult.shortCode, 80),
          shortUrl: normalizeString(paymentLinkResult.shortUrl, 500),
        },
      });
      return {
        sent: true,
      ...
      updatedEntry = await appendOrderSmsHistory(
        ...
        buildSmsHistoryRecord(null, {
          ...
          status: "failed",
          errorMessage:
            normalizeString(error && error.message, 300) ||
            "Не удалось отправить клиенту SMS со ссылкой на оплату.",
          recipientName: normalizeString(entry.customerName, 160),
        })
      );
  ...
  async function runClientReminderSweep({ now = new Date(), leadConnectorClient = null } = {}) {
    ...
    const reminderRange = getReminderSweepDateRange(new Date(nowMs), timeZone);
    const entries = typeof quoteOpsLedger.listReminderEntries === "function"
      ? await quoteOpsLedger.listReminderEntries({
          ...reminderRange,
    ...
    for (const candidate of Array.isArray(entries) ? entries : []) {
      let entry = candidate;
      if (
        shouldRepairAcceptedOrderStatus(entry) &&
        typeof quoteOpsLedger.updateOrderEntry === "function"
      ) {
        entry =
          (await quoteOpsLedger.updateOrderEntry(entry.id, { orderStatus: "scheduled" })) || entry;
        logEvent("accepted_policy_order_auto_scheduled", {
          entryId: normalizeString(entry.id, 120),
        });
      }
      if (!entry || getEntryOrderStatus(entry) !== "scheduled") continue;
      if (!resolveClientAutomaticNotificationsEnabled(entry, entries)) continue;
      ...
      const notifications = getOrderNotificationState(entry);
      const signature = buildReminderSignature(entry);
      const storedReminderSignature = normalizeString(
        notifications.reminderSms.signature,
        500
      );
      const hasMatchingReminderState =
        storedReminderSignature === signature;
      const reminderSms = hasMatchingReminderState
        ? { ...notifications.reminderSms }
        : storedReminderSignature && hasReminderSmsSentMarker(notifications.reminderSms)
          ? {
              ...notifications.reminderSms,
              signature,
              previousSignature: storedReminderSignature,
              appointmentUpdatedAt:
                normalizeString(notifications.reminderSms.appointmentUpdatedAt, 80) ||
                new Date(nowMs).toISOString(),
            }
          : storedReminderSignature
            ? createReminderSmsState(signature)
            : restoreReminderSmsStateFromHistory(entry, signature);
      if (!hasMatchingReminderState) {
      ...
      for (const window of REMINDER_WINDOWS) {
        ...
        try {
          ...
          const record = buildSmsHistoryRecord(smsResult, {
            ...
            targetRef: buildReminderHistoryTargetRef(entry, signature, window.key),
            source: "automatic",
          });
...
module.exports = {
  ...
  buildInvoicePaymentLinkSmsMessage,
  buildManagerCleaningCompleteSmsMessage,
  buildManagerLeadAlertSmsMessage,
  buildQuoteConfirmationSmsMessage,
  buildReviewRequestFollowUpSmsMessage,
