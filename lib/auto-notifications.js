...
function buildManagerLeadAlertSmsMessage(entry = {}, manager = {}, siteOrigin = "") {
  ...
  const details = [
    ...
    .filter(Boolean)
    .join(" ");
  return `${greeting} a new Shynli lead was assigned to you. ${details}`.replace(/\s+/g, " ").trim();
}

function buildStaffAssignmentSmsMessage(entry = {}, staffRecord = {}, assignment = {}) {
...
function createAutoNotificationService(options = {}) {
  ...
  async function appendOrderSmsHistory(entry, record, notifications) {
    if (!entry || !record || !quoteOpsLedger || typeof quoteOpsLedger.updateOrderEntry !== "function") {
      return entry;
    }
    return (
  ...
  async function notifyQuoteSubmissionSuccess({ entry, pricing = null, leadConnectorClient = null } = {}) {
    ...
    if (canSendSms) {
      ...
      if (assignedManagerId && typeof listLeadManagers === "function") {
        try {
          const managers = await listLeadManagers();
          const assignedManager = (Array.isArray(managers) ? managers : []).find(
            (manager) => normalizeString(manager && manager.id, 120) === assignedManagerId
          );
          const managerPhone = normalizePhone(assignedManager && assignedManager.phone);
          const managerSmsMessage = buildManagerLeadAlertSmsMessage(updatedEntry, assignedManager, siteOrigin);
          if (assignedManager && managerPhone && managerSmsMessage) {
            const smsResult = await leadConnector.sendSmsMessage({
              phone: managerPhone,
              customerName: normalizeString(assignedManager.name, 160),
              customerEmail: normalizeString(assignedManager.email, 250).toLowerCase(),
  ...
  async function notifyScheduledAssignment({ entry, assignment, leadConnectorClient = null } = {}) {
    ...
    for (const staffId of assignment.staffIds) {
      ...
      const message = buildStaffAssignmentSmsMessage(updatedEntry, staffRecord, assignment);
      if (!message) continue;
