...
function createAdminGoogleCalendarClient(options = {}) {
  ...
  return {
    ...
    buildConnectUrl({ staffId, secret, loginHint = "", source = "" } = {}) {
      ...
      const normalizedSource = normalizeString(source, 32).toLowerCase();
      const state = signState(
        {
          staffId: normalizedStaffId,
          source: normalizedSource === "account" ? "account" : "admin",
          iat: Date.now(),
        },
        secret
      );
    ...
    readStateSource({ state, secret } = {}) {
      const parsedState = verifyState(state, secret);
      return normalizeString(parsedState && parsedState.source, 32).toLowerCase() === "account"
        ? "account"
        : "admin";
    },
    async exchangeCode({ code, state, secret }) {
      ...
      return {
        ...
        source: normalizeString(parsedState.source, 32).toLowerCase() === "account" ? "account" : "admin",
...
function createAdminGoogleCalendarIntegration(options = {}) {
  ...
  async function handleOAuthCallback({ code, state, config }) {
    ...
    return {
      staffId: exchangeResult.staffId,
      source: exchangeResult.source,
    };
  }

  async function readOAuthStateSource({ state, config }) {
    if (!client || !client.isConfigured || !client.isConfigured()) {
      throw new Error("GOOGLE_CALENDAR_NOT_CONFIGURED");
    }
    if (!client.readStateSource) return "admin";
    return client.readStateSource({
      state,
      secret: getSecret(config),
    });
  }

  async function buildConnectUrl(staffId, config, loginHint = "", options = {}) {
    if (!client || !client.isConfigured || !client.isConfigured()) {
      throw new Error("GOOGLE_CALENDAR_NOT_CONFIGURED");
    }
    const source = normalizeString(options && options.source, 32).toLowerCase();
