...
const MAX_EMBEDDED_DOCUMENT_BASE64_LENGTH = 8 * 1024 * 1024;
...
function normalizeBase64Binary(value, maxLength = MAX_EMBEDDED_DOCUMENT_BASE64_LENGTH) {
...
function sanitizeStaffW9Record(input = {}) {
  ...
  const dataBase64 = normalizeBase64Binary(document.dataBase64);
...
async function readStoredStaffW9Document(document = {}, config = loadStaffW9Config()) {
  const relativePath = normalizeString(document.relativePath, 240).replace(/^\/+/, "");
  const dataBase64 = normalizeBase64Binary(document.dataBase64);
  let absolutePath = "";
  let lastError = null;
  ...
  if (relativePath) {
    absolutePath = resolveStaffW9DocumentAbsolutePath(relativePath, config);
    try {
  ...
  throw lastError || new Error("W9_DOCUMENT_NOT_FOUND");
}
