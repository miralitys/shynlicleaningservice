...
function renderPolicyAcceptancePage(options = {}) {
  ...
  const summaryItems = [
    ...
    { label: "Booking ID", value: normalizeString(booking.requestId || booking.id, 180) || "Not set" },
  ];
  ...
  const documentMarkup = documents
    .map((doc) => {
      const documentType = normalizeString(doc.documentType, 80);
      const buttonLabel =
        documentType === TERMS_DOCUMENT_TYPE
          ? "Review Terms of Service"
          : "Review Payment & Cancellation Policy";
      ...
      return `<article class="doc-card">
        <span class="doc-label">${escapeHtml(documentType.replaceAll("_", " "))}</span>
        <p class="doc-title">${escapeHtml(title)}</p>
        <p class="doc-meta">Version ${escapeHtml(version)} • Effective ${escapeHtml(effectiveDate)}</p>
        <a class="doc-button" href="${escapeHtmlAttribute(publicUrl)}" target="_blank" rel="noreferrer">${escapeHtml(
          buttonLabel
        )}</a>
        <span class="doc-url">${escapeHtml(publicUrl)}</span>
      </article>`;
    })
    .join("");
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing:border-box; }
        ...
        .grid { display:grid; grid-template-columns:minmax(0, 1.15fr) minmax(300px, .85fr); gap:18px; padding:24px 36px 36px; }
        ...
        .summary-item, .doc-card { border:1px solid var(--line); border-radius:18px; padding:14px 16px; background:#fafafa; }
        .summary-item { min-height:102px; }
        .summary-item-inline { grid-column:1 / -1; display:flex; align-items:center; gap:12px; min-height:auto; padding:12px 16px; }
        .summary-item-amount { background:var(--accent-soft); border-color:rgba(158, 67, 90, 0.18); }
        .summary-label, .doc-label { display:block; margin:0 0 8px; font-size:12px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:#7a7d87; }
        .summary-item-amount .summary-label { color:var(--accent); }
        .summary-value { font-size:16px; font-weight:700; line-height:1.28; }
        .summary-item-amount .summary-value { font-size:22px; line-height:1.1; }
        .summary-item-inline .summary-label { margin:0; flex:0 0 auto; }
        .summary-item-inline .summary-value { min-width:0; flex:1 1 auto; font-size:15px; line-height:1.3; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .summary-meta, .doc-meta { margin-top:6px; font-size:14px; line-height:1.6; color:var(--muted); }
        .doc-list { display:grid; gap:14px; }
        .doc-title { margin:0; font-size:20px; font-weight:700; line-height:1.25; }
        .doc-link { color:var(--accent); text-decoration:none; font-weight:700; }
        .doc-button {
          display:inline-flex;
          align-items:center;
        ...
        .doc-button:hover { filter:brightness(0.98); }
        .doc-url {
          display:block;
          margin-top:10px;
          font-size:13px;
        ...
        <div class="shell">
          ...
          <div class="grid">
            <section class="section">
              ...
              <p class="copy">This link is tied to a single booking and stores a formal audit trail once you confirm.</p>
        ...
        (function () {
          ...
          function renderSummary(payload) {
            ...
            const items = [
              ...
              { label: "Booking ID", value: booking.requestId || booking.id || "Not set" },
            ];
          ...
          function renderDocuments(payload) {
            ...
            docList.innerHTML = docs.map(function(doc) {
              const buttonLabel = doc.documentType === "TERMS_OF_SERVICE"
                ? "Review Terms of Service"
                : "Review Payment & Cancellation Policy";
              return '<article class="doc-card">' +
                '<span class="doc-label">' + doc.documentType.replaceAll("_", " ") + '</span>' +
                '<p class="doc-title">' + doc.title + '</p>' +
                '<p class="doc-meta">Version ' + (doc.version || 'Not set') + ' • Effective ' + (doc.effectiveDate || 'Not set') + '</p>' +
                '<a class="doc-button" href="' + doc.publicUrl + '" target="_blank" rel="noreferrer">' + buttonLabel + '</a>' +
                '<span class="doc-url">' + doc.publicUrl + '</span>' +
              '</article>';
            }).join("");
          }

          function updateButtonState() {
            button.disabled = !(acceptTerms.checked && acceptPayment.checked && typedSignature.value.trim());
          }
