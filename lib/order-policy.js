...
function buildPolicyAcceptancePageUrl(token = "") {
  const safeToken = normalizeString(token, 6000);
  if (!safeToken) return POLICY_ACCEPTANCE_PAGE_PATH;
  return `${POLICY_ACCEPTANCE_PAGE_PATH}?token=${encodeURIComponent(safeToken)}`;
}
  ...
  const reviewSectionMarkup = isAccepted
    ? `<section class="section section-success">
          <div class="success-state">
            <p class="eyebrow success-eyebrow">Booking confirmed</p>
            <h2>Thank you, everything is signed.</h2>
            <p class="copy">
            ...
            <div class="success-actions">
              ...
              <span class="hint">You are all set for your upcoming cleaning appointment.</span>
            </div>
          </div>
        </section>`
        ...
        (function () {
          ...
          form.addEventListener("submit", async function (event) {
            ...
            try {
              ...
              window.location.assign(${JSON.stringify(buildPolicyAcceptancePageUrl(token))});
            } catch (error) {
  ...
  async function handlePublicFormSubmit(req, res, requestStartNs, requestContext, quoteOpsLedger) {
    ...
    try {
      ...
      writeHeadWithTiming(
        ...
        303,
        {
          Location: buildPolicyAcceptancePageUrl(token),
          "Cache-Control": "no-store",
        },
        requestStartNs,
        requestContext.cacheHit
      ...
      if (segments.length === 2 && segments[1] === "submit" && req.method === "POST") {
        ...
        writeJsonWithTiming(
          ...
          200,
          {
            ...
            redirectUrl: buildPolicyAcceptancePageUrl(token),
            acceptance: {
