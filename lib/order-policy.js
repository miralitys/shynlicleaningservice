...
const POLICY_ACCEPTANCE_CERTIFICATE_GENERATOR_VERSION = "2026-04-14-template-v1";
...
async function generatePolicyAcceptanceCertificate(record) {
  ...
  if (templateBytes) {
    ...
    function clearTextBand(x, y, width, height, opacity = 0.97) {
      page.drawRectangle({
        x,
        y,
        width,
    ...
    function drawTemplateValue(value, x, y, width, options = {}) {
      const font = options.font || fontRegular;
      const color = options.color || dark;
      const align = options.align || "left";
      const preferredSize = options.size || 9.5;
      const minSize = options.minSize || 6;
      const safeValue = normalizeString(value, options.maxLength || 500) || options.fallback || "Not set";
      const fitted = fitPdfText(font, safeValue, width, preferredSize, minSize);
      const clearPaddingX = Number.isFinite(options.clearPaddingX) ? options.clearPaddingX : 2;
      const clearPaddingY = Number.isFinite(options.clearPaddingY) ? options.clearPaddingY : 2;
      const clearHeight =
        Number.isFinite(options.clearHeight) ? options.clearHeight : fitted.size + clearPaddingY * 2 + 1;
      const clearOpacity = Number.isFinite(options.clearOpacity) ? options.clearOpacity : 1;

      clearTextBand(
        x - clearPaddingX,
        y - clearPaddingY,
        width + clearPaddingX * 2,
        clearHeight,
        clearOpacity
      );
    ...
    function drawTemplateRule(x1, x2, y, thickness = 0.9, color = dark) {
      page.drawLine({
        start: { x: x1, y },
        end: { x: x2, y },
        color,
    ...
    function drawTemplateInlineField(label, value, options = {}) {
      const labelText = normalizeString(label, 80).toUpperCase();
      const valueFont = options.valueFont || fontRegular;
      const valueX = options.valueX;
      const valueY = options.valueY;
      const lineEndX = options.lineEndX || 306;
      const fitted = fitPdfText(
        valueFont,
        normalizeString(value, options.maxLength || 500) || "Not set",
        Math.max(40, lineEndX - valueX - 6),
        options.size || 8.8,
        options.minSize || 6.2
      );
      ...
      drawTemplateRule(
        Math.min(lineEndX - 8, valueX + valueFont.widthOfTextAtSize(fitted.text, fitted.size) + 6),
        lineEndX,
        options.lineY || valueY - 1
      );
    }

    function drawTemplateStackedField(label, value, options = {}) {
      const labelText = normalizeString(label, 80).toUpperCase();
      const valueFont = options.valueFont || fontRegular;
      const valueX = options.valueX || 68;
      const lineEndX = options.lineEndX || 306;
      const fitted = fitPdfText(
        valueFont,
        normalizeString(value, options.maxLength || 500) || "Not set",
      ...
      drawTemplateRule(
        Math.min(lineEndX - 8, valueX + valueFont.widthOfTextAtSize(fitted.text, fitted.size) + 6),
        lineEndX,
        options.lineY || options.valueY - 1
      );
    }

    drawTemplateValue(certificateDisplayId, 174, 703, 112, {
      size: 8.9,
      minSize: 6.5,
      fallback: "NOT SET",
      clearPaddingX: 1,
    ...
    drawTemplateValue(bookingDisplayId, 133, 686, 152, {
      size: 8.5,
      minSize: 6.2,
      clearPaddingX: 1,
      clearPaddingY: 1,
    ...
    drawTemplateValue(formatChicagoDate(completedAt), 456, 702, 92, {
      size: 8.4,
      minSize: 6.4,
      align: "right",
      clearPaddingX: 1,
    ...
    drawTemplateValue(formatChicagoTime(completedAt), 462, 686, 86, {
      size: 7.4,
      minSize: 6,
      align: "right",
      color: muted,
    ...
    clearTextBand(66, 414, 248, 142, 1);
    drawTemplateInlineField("NAME", sanitized.customerFullName || "Not set", {
      labelX: 68,
      labelY: 528,
      valueX: 143,
      valueY: 526,
    ...
    drawTemplateStackedField("EMAIL", sanitized.customerEmail || "Not set", {
      labelX: 68,
      labelY: 491,
      valueX: 68,
      valueY: 479,
    ...
    drawTemplateStackedField("PHONE", sanitized.customerPhone || "Not set", {
      labelX: 68,
      labelY: 459,
      valueX: 68,
      valueY: 447,
    ...
    drawTemplateStackedField("SERVICE ADDRESS", sanitized.serviceAddress || "Not set", {
      labelX: 68,
      labelY: 427,
      valueX: 68,
      valueY: 415,
    ...
    drawTemplateValue(formatChicagoDateTime(sanitized.sentAt), 376, 520, 161, {
      size: 8.6,
      minSize: 6.4,
      clearHeight: 15,
      clearPaddingY: 3,
    });
    drawTemplateValue(formatChicagoDateTime(sanitized.firstViewedAt || sanitized.lastViewedAt), 376, 486, 161, {
      size: 8.6,
      minSize: 6.4,
      clearHeight: 15,
      clearPaddingY: 3,
    });
    drawTemplateValue(formatChicagoDateTime(sanitized.signedAt), 376, 452, 161, {
      size: 8.6,
      minSize: 6.4,
      clearHeight: 15,
      clearPaddingY: 3,
    ...
    drawTemplateValue(sanitized.termsDocument.publicUrl || "Not set", 109, 334, 410, {
      size: 7.8,
      minSize: 5.9,
      maxLength: 2000,
      clearHeight: 12,
    ...
    drawTemplateValue(
      `${sanitized.termsDocument.version || "Not set"} / ${sanitized.termsDocument.effectiveDate || "Not set"}`,
      242,
      316,
      300,
      {
    ...
    drawTemplateValue(sanitized.paymentPolicyDocument.publicUrl || "Not set", 109, 271, 410, {
      size: 7.8,
      minSize: 5.9,
      maxLength: 2000,
      clearHeight: 12,
    ...
    drawTemplateValue(
      `${sanitized.paymentPolicyDocument.version || "Not set"} / ${sanitized.paymentPolicyDocument.effectiveDate || "Not set"}`,
      242,
      253,
      300,
      {
    ...
    clearTextBand(65, 41, 182, 28, 0.92);
    drawTemplateValue(`Typed electronic signature: ${sanitized.typedSignature || "Not set"}`, 68, 59, 292, {
      size: 7.3,
      minSize: 6,
      color: muted,
      clearOpacity: 0,
  ...
  function drawHeaderMeta(label, value, x, y, width, align = "left") {
    ...
    if (align === "right") {
      ...
      const valueWidth = fontRegular.widthOfTextAtSize(safeValue, 9);
  ...
  drawHeaderMeta("CERTIFICATE ID", certificateId, 56, 730, 188);
  drawHeaderMeta("BOOKING ID", bookingDisplayId, 56, 712, 188);
  drawHeaderMeta("DOCUMENT COMPLETED ON", formatChicagoDateTime(completedAt), 360, 730, 156, "right");
          ...
          <div class="grid">
            ...
            <section class="section">
              ...
              <form class="form-stack" id="policy-acceptance-form" method="post" action="${escapeHtmlAttribute(
                token ? `${POLICY_ACCEPTANCE_PAGE_PATH}?token=${encodeURIComponent(token)}` : POLICY_ACCEPTANCE_PAGE_PATH
              )}">
...
function createOrderPolicyAcceptanceService(options = {}) {
  ...
  async function handlePublicFormSubmit(req, res, requestStartNs, requestContext, quoteOpsLedger) {
    ...
    try {
      const result = await submitAcceptance(quoteOpsLedger, token, formBody, req);
      writeHtmlWithTiming(
        res,
        200,
        renderPolicyAcceptancePage({
          token,
          pageTitle: "Policy Acceptance",
          payload: buildPublicPolicyPayload(result.entry, result.record, { token }),
        }),
        requestStartNs,
        requestContext.cacheHit
      );
