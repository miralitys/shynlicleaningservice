...
(() => {
  ...
  const EXCLUDED_PATHS = new Set(["/quote", "/quote-no-calculator", "/quote2"]);
...
function customizeQuoteNoCalculatorForm(html, routePath) {
  if (normalizeRoute(routePath) !== "/quote-no-calculator") return html;
  return String(html || "")
    .replace(
      /<p class="quote2-field-note">Enter your full name and phone number to calculate the service cost\.<\/p>/,
      '<p class="quote2-field-note">Enter your full name and phone number, and we will call you with a free quote.</p>'
    )
    .replace(
...
function sanitizeHtml(html, routePath = "/") {
  ...
  cleaned = customizeQuoteNoCalculatorForm(cleaned, normalizedRoute);
  ...
  if (
    ["/quote", "/quote-no-calculator", "/quote2"].includes(normalizedRoute) &&
