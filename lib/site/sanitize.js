...
CITY_ROUTE_LABEL_BY_PATH.set("/sugargrove2", "Sugar Grove");
...
function isSugarGroveRoute(routePath) {
  const normalizedRoute = normalizeRoute(routePath);
  return normalizedRoute === "/sugargrove" || normalizedRoute === "/sugargrove2";
}
...
function localizeCleanSugarGroveHeadAssets(html, normalizedRoute) {
  if (!isSugarGroveRoute(normalizedRoute)) return html;
  return String(html || "").replaceAll(SHARED_ICON_PATH, SUGAR_GROVE_CLEAN_ICON_PATH);
}
...
function sanitizeHtml(html, routePath = "/") {
  ...
  cleaned = localizeCleanSugarGroveHeadAssets(cleaned, normalizedRoute);
