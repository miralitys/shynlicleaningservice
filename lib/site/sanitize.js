...
function buildBlogRuntimeScript(routePath) {
  const categoryDetail = getBlogCategoryDetail(routePath);
  const currentCategoryLiteral = categoryDetail
    ? JSON.stringify({
        label: categoryDetail.label,
        aliases: categoryDetail.filterAliases,
      })
    : "null";
...
(function(){
  ...
  var CURRENT_CATEGORY = ${currentCategoryLiteral};
...
function buildZeroPopupFormMarkup(recordId, formElemOpenTag, fields) {
  ...
  const inputGroupMarkup = fields
    .map((field) => {
      ...
      if (fieldType === "nm") {
        return `<div class="t-input-group t-input-group_nm" data-input-lid="${inputLid}" data-field-type="nm" data-field-name="${escapeHtml(fieldName)}" style="${marginBottomStyle}"><div class="t-input-block"><input aria-label="name" type="text" name="${escapeHtml(field.li_name || "name")}" ${commonInputClass} data-tilda-rule="name" placeholder="${escapeHtml(placeholder)}"${requiredAttr} style="${inputInlineStyle};"><div class="t-input-error"></div></div></div>`;
      }

      if (fieldType === "ph") {
        return `<div class="t-input-group t-input-group_ph" data-input-lid="${inputLid}" data-field-type="ph" data-field-name="${escapeHtml(fieldName)}" style="${marginBottomStyle}"><div class="t-input-block" style="overflow: visible;"><div class="t-input shynli-zero-phone-shell" style="${inputInlineStyle}; display: flex; align-items: center; overflow: hidden; padding: 0 12px 0 0;"><span class="shynli-zero-phone-code" aria-hidden="true" style="display: inline-flex; align-items: center; flex: none; padding: 0 0 0 16px; color: ${inputColor}; font-size: ${inputFontSize}; font-weight: ${inputFontWeight}; white-space: nowrap;">+1</span><input type="hidden" name="tildaspec-phone-part[]-iso" value="+1" tabindex="-1"><input aria-label="phone" type="tel" class="t-input shynli-zero-phone-display" name="tildaspec-phone-part[]" value="" placeholder="${escapeHtml(field.li_masktype === "a" ? "(000) 000-0000" : placeholder || "(000) 000-0000")}" inputmode="tel" autocomplete="tel-national" maxlength="14" style="flex: 1 1 auto; min-width: 0; padding: 0 16px 0 10px; color: ${inputColor}; font-size: ${inputFontSize}; font-weight: ${inputFontWeight}; height: ${inputHeight}; border: 0; background: transparent; box-shadow: none;"><input type="hidden" class="js-tilda-rule shynli-zero-phone-hidden" data-tilda-rule="phone" name="${escapeHtml(fieldName)}" value=""${requiredAttr} tabindex="-1" data-tilda-rule-minlength="17"></div><div class="t-input-error"></div></div></div>`;
      }

      if (fieldType === "em") {
        return `<div class="t-input-group t-input-group_em" data-input-lid="${inputLid}" data-field-type="em" data-field-name="${escapeHtml(fieldName)}" style="${marginBottomStyle}"><div class="t-input-block"><input aria-label="email" type="email" name="${escapeHtml(fieldName)}" ${commonInputClass} data-tilda-rule="email" placeholder="${escapeHtml(placeholder)}"${requiredAttr} style="${inputInlineStyle};"><div class="t-input-error"></div></div></div>`;
      }
      ...
      if (fieldType === "sb") {
        const variants = String(field.li_variants || "")
          ...
          .join("");
        return `<div class="t-input-group t-input-group_sb" data-input-lid="${inputLid}" data-field-type="sb" data-field-name="${escapeHtml(fieldName)}" style="${marginBottomStyle}"><div class="t-input-block"><div class="t-select__wrapper"><select id="sb-${inputLid}" name="${escapeHtml(fieldName)}" class="t-select js-tilda-rule t-input-inline-styles"${requiredAttr} style="${inputInlineStyle};"><option value="" style="color: ${escapeHtml(inputColor)};"${
