...
(() => {
  ...
  function initStickyCta() {
    ...
    const usesNoPriceQuote = noPriceQuotePaths.has(currentPath);
    ...
    wrap.innerHTML =
      ...
      '<a class="cta-book" href="' + quoteHref + '">Get My Free Quote</a>';
...
function pointAdsCtasToNoCalculatorQuote(html, routePath) {
  ...
  const destination =
    ...
    normalizedRoute === MOVE_IN_MOVE_OUT_ADS_V2_ROUTE
      ? "/quote-no-price"
      : "/quote-no-calculator";
  let output = String(html || "").replace(/\bhref=(["'])\/quote(?=([?#])?\1)/g, `href=$1${destination}`);
  if (destination === "/quote-no-price") {
    output = output.replace(/\bhref=(["'])\/quote-no-calculator(?=([?#])?\1)/g, "href=$1/quote-no-price");
  }
  return output;
}
...
@media (max-width:760px){body.home-copy-page.shynli-ads-v2-page .hero__copy{transform:none;}body.home-copy-page.shynli-ads-v2-page .hero__team{margin-top:60px;}}
...
function customizeAdsHomepageHero(html, routePath) {
  ...
  if (normalizeRoute(routePath) === "/ads-v2") {
