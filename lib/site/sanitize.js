...
const SERVICE_AREA_ZIP_MAP = Object.freeze({
  ...
  "60186": { city: "West Chicago", url: "/westchicago" },
  ...
  "60197": { city: "Carol Stream", url: "/carolstream" },
  "60199": { city: "Carol Stream", url: "/carolstream" },
  "60439": { city: "Lemont", url: "/lemont" },
  "60440": { city: "Bolingbrook", url: "/bolingbrook" },
  "60441": { city: "Lockport", url: "/lockport" },
  "60446": { city: "Romeoville", url: "/romeoville" },
  "60490": { city: "Bolingbrook", url: "/bolingbrook" },
  "60491": { city: "Homer Glen", url: "/homerglen" },
  ...
  "60521": { city: "Hinsdale", url: "/hinsdale" },
  "60522": { city: "Hinsdale", url: "/hinsdale" },
  "60523": { city: "Oak Brook", url: "/oakbrook" },
...
const EXTRA_SERVICE_AREA_CITIES = Object.freeze([
  { path: "/northaurora", label: "North Aurora" },
  { path: "/sugargrove", label: "Sugar Grove" },
  { path: "/yorkville", label: "Yorkville" },
]);
...
function replaceServiceAreaZipMap(html) {
  return String(html || "").replace(
    /const serviceZips=\{[\s\S]*?\};function checkZipArea\(\)/g,
    `const serviceZips=${JSON.stringify(SERVICE_AREA_ZIP_MAP)};function checkZipArea()`
  );
}

function injectExtraServiceAreaCities(html, routePath) {
  if (routePath !== "/service-areas") return html;
  const output = String(html || "");
  if (output.includes('id="shynli-extra-service-areas"')) return output;

  const extraSection = `<section id="shynli-extra-service-areas" style="margin:0 0 28px;"><style>#shynli-extra-service-areas{padding:12px 0 6px;}#shynli-extra-service-areas .shynli-extra-service-areas__eyebrow{margin:0 0 8px;font:600 12px/1.2 Montserrat,sans-serif;letter-spacing:.14em;text-transform:uppercase;color:#9e435a;}#shynli-extra-service-areas .shynli-extra-service-areas__title{margin:0 0 18px;font:400 30px/1.15 'Playfair Display',serif;color:#313131;}#shynli-extra-service-areas .shynli-extra-service-areas__grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;}#shynli-extra-service-areas .shynli-extra-service-areas__link{display:flex;align-items:center;justify-content:center;min-height:52px;padding:0 18px;border:1px solid #9e435a;border-radius:999px;background:#faf9f6;color:#313131;text-decoration:none;font:500 15px/1.2 Montserrat,sans-serif;transition:background-color .2s ease,color .2s ease,border-color .2s ease;}#shynli-extra-service-areas .shynli-extra-service-areas__link:hover{background:#9e435a;color:#faf9f6;}@media (max-width:767px){#shynli-extra-service-areas .shynli-extra-service-areas__title{font-size:24px;}#shynli-extra-service-areas .shynli-extra-service-areas__grid{grid-template-columns:1fr;gap:12px;}}</style><p class="shynli-extra-service-areas__eyebrow">New service areas</p><h2 class="shynli-extra-service-areas__title">Now serving these cities too</h2><div class="shynli-extra-service-areas__grid">${EXTRA_SERVICE_AREA_CITIES.map(
    (city) =>
      `<a class="shynli-extra-service-areas__link" href="${city.path}">${city.label}</a>`
  ).join("")}</div></section>`;

  return output.replace(/<div class="faq-single-wrapper">/i, `${extraSection}<div class="faq-single-wrapper">`);
}
...
function sanitizeHtml(html, routePath = "/") {
  ...
  if (normalizedRoute === "/oauth/callback") {
    return injectGoogleTagManager(buildOauthCallbackShell(), GOOGLE_TAG_MANAGER_CONTAINER_ID);
  }
  if (isManagedBlogRoute(normalizedRoute)) {
  ...
  cleaned = injectExtraServiceAreaCities(cleaned, normalizedRoute);
