...
function buildFeaturedArticleCards(categoryPath = "") {
  ...
  return `<section class="shynli-blog-featured"><div class="shynli-blog-featured__intro"><div><p class="shynli-blog-hub__eyebrow">Featured article</p><h3 class="shynli-blog-featured__title">Start with the strongest guide in this topic.</h3></div><p class="shynli-blog-featured__text">Use this article as the core resource for the category, then browse the feed below for related angles and supporting topics.</p></div><div class="shynli-blog-featured__grid">${featuredArticles
    .map(
      (article) =>
...
#blog-topics-hub .shynli-blog-article__toc{position:sticky;top:110px;display:grid;gap:16px;padding:22px;border-radius:24px;background:#fff;border:1px solid rgba(158,68,90,.12);}
...
#blog-topics-hub .shynli-blog-article__callout{display:grid;gap:10px;padding:22px 24px;border-radius:22px;background:#313131;color:#faf9f6;}
#blog-topics-hub .shynli-blog-article__callout-eyebrow{margin:0;color:#d8cfc4;font-family:'Montserrat',Arial,sans-serif;font-size:12px;line-height:1.2;font-weight:700;letter-spacing:.16em;text-transform:uppercase;}
#blog-topics-hub .shynli-blog-article__callout-title{margin:0;color:#faf9f6;font-family:'Playfair Display',serif;font-size:30px;line-height:1.08;font-weight:400;}
#blog-topics-hub .shynli-blog-article__callout-text{margin:0;color:rgba(250,249,246,.8);font-family:'Montserrat',Arial,sans-serif;font-size:15px;line-height:1.72;font-weight:400;}
...
function buildBlogArticleTableOfContents(article) {
  return article.toc
    .map(
      (item) =>
        `<li><a class="shynli-blog-article__toc-link" href="#${escapeBlogAttribute(item.id)}">${escapeBlogHtml(
          item.label
        )}</a></li>`
    )
    .join("");
...
function buildBlogArticleSection(article) {
  return `<section id="blog-topics-hub" class="shynli-blog-hub shynli-blog-hub--article">${buildBlogHubStyles()}<div class="shynli-blog-hub__shell"><header class="shynli-blog-article__header"><a class="shynli-blog-article__crumb" href="${escapeBlogAttribute(
  ...
  )}">Back to ${escapeBlogHtml(article.categoryLabel)}</a><div class="shynli-blog-article__meta"><span class="shynli-blog-article__meta-item">${escapeBlogHtml(
    article.categoryLabel
    ...
    article
  )}</div><div class="shynli-blog-article__footer-note"><h3 class="shynli-blog-article__footer-note-title">More from ${escapeBlogHtml(
    article.categoryLabel
  )}</h3><p class="shynli-blog-article__footer-note-text">The article feed below stays filtered to this category so readers can keep exploring closely related checklist content after finishing this guide.</p></div></div></section>`;
}
...
function buildBlogHeroHeading(routePath) {
  ...
  if (article) {
    return `<h1 class='tn-atom'field='tn_text_1768231764218'>${escapeBlogHtml(
      article.heroLead
    )} <span style="color: rgb(158, 68, 90);">${escapeBlogHtml(article.heroAccent)}</span></h1>`;
  }
  const detail = getBlogCategoryDetail(routePath);
  if (!detail) return "";
  return `<h1 class='tn-atom'field='tn_text_1768231764218'>${escapeBlogHtml(
