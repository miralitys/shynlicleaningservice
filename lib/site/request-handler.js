...
function createSiteRequestHandler(deps = {}) {
  const {
    ...
    ALERT_P95_MS,
    ALERT_P99_MS,
    BLOG_ARTICLE_LEGACY_REDIRECTS = new Map(),
  ...
  return async function handleSiteRequest(
    ...
    try {
      ...
      const lowerCaseNormalizedPath = normalizeRoute(pathname.toLowerCase());
      requestContext.route = normalizedPath;

      const redirectTarget =
        appendSearchToLocation(REDIRECT_ROUTES.get(normalizedPath), reqUrl.search) ||
        appendSearchToLocation(BLOG_ARTICLE_LEGACY_REDIRECTS.get(normalizedPath), reqUrl.search) ||
        (lowerCaseNormalizedPath !== normalizedPath
          ? appendSearchToLocation(REDIRECT_ROUTES.get(lowerCaseNormalizedPath), reqUrl.search) ||
            appendSearchToLocation(BLOG_ARTICLE_LEGACY_REDIRECTS.get(lowerCaseNormalizedPath), reqUrl.search) ||
            (runtimeIndex.routeFileByPath.has(lowerCaseNormalizedPath)
              ? appendSearchToLocation(lowerCaseNormalizedPath, reqUrl.search)
              : "")
          : "") ||
      ...
      if (normalizedPath === "/__ready") {
        const readyPayload = JSON.stringify(
          {
            ok: Boolean(STARTUP_ENV_INTEGRITY.readinessOk),
        ...
        writeHeadWithTiming(
          res,
          STARTUP_ENV_INTEGRITY.readinessOk ? 200 : 503,
          {
            "Content-Type": "application/json; charset=utf-8",
            "Cache-Control": "no-store",
          },
      ...
      if (
        PAYMENT_SHORT_LINK_PATH_PREFIX &&
        normalizedPath.startsWith(PAYMENT_SHORT_LINK_PATH_PREFIX) &&
        typeof handlePaymentShortLinkRequest === "function"
      ) {
        await handlePaymentShortLinkRequest(req, res, requestStartNs, requestContext, requestLogger, quoteOpsLedger);
        return;
      }
      ...
      if (normalizedPath === CLEANER_APPLICATION_SUBMIT_ENDPOINT) {
        await handleCleanerApplicationSubmissionRequest(
          req,
          res,
          requestStartNs,
          requestContext,
