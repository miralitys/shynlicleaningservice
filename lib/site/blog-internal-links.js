...
function splitSections(bodyHtml) {
  const sectionRegex = /<section class="shynli-blog-article__section"[\s\S]*?<\/section>/g;
  const matches = [...String(bodyHtml).matchAll(sectionRegex)];
  if (!matches.length) return null;

  const prefix = bodyHtml.slice(0, matches[0].index);
  const suffix = bodyHtml.slice(matches[matches.length - 1].index + matches[matches.length - 1][0].length);
  const sections = matches.map((match) => match[0]);
