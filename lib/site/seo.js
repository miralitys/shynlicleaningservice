...
function createSiteSeoHelpers(deps = {}) {
  ...
  function buildPricingPageSchema() {
    const pricingLocalBusiness = {
      "@type": "HouseCleaningBusiness",
      "@id": `${SITE_ORIGIN}/#localbusiness`,
      name: "Shynli Cleaning",
      url: `${SITE_ORIGIN}/`,
      image: `${SITE_ORIGIN}/images/tild3531-6535-4863-b761-323963303436__logo_2.png`,
      telephone: "+1-630-812-7077",
    ...
    const provider = { "@id": `${SITE_ORIGIN}/#localbusiness` };
    const fixedRecurringServices = [
      ["weekly-home-cleaning", "Weekly Home Cleaning", "135", "week"],
      ["bi-weekly-home-cleaning", "Bi-Weekly Home Cleaning", "145", "two weeks"],
      ["monthly-home-cleaning", "Monthly Home Cleaning", "155", "month"],
    ].map(([slug, serviceType, price, unitText]) => ({
      "@type": "Service",
      "@id": `${SITE_ORIGIN}/pricing-v2#${slug}`,
      serviceType,
      provider,
    ...
    const oneTimeServices = [
      ["regular-cleaning", "Regular Cleaning", "120", "200"],
      ["deep-cleaning", "Deep Cleaning", "180", "350"],
      ["move-in-out-cleaning", "Move-In/Out Cleaning", "250", "500"],
    ].map(([slug, serviceType, minPrice, maxPrice]) => ({
      "@type": "Service",
      "@id": `${SITE_ORIGIN}/pricing-v2#${slug}`,
      serviceType,
      provider,
    ...
    const addOnCatalog = {
      "@type": "OfferCatalog",
      "@id": `${SITE_ORIGIN}/pricing-v2#add-on-cleaning-services`,
      name: "Add-On Cleaning Services",
      itemListElement: [
        ["Wet Baseboards", "22"],
    ...
    return {
      "@context": "https://schema.org",
      "@graph": [
        pricingLocalBusiness,
        ...fixedRecurringServices,
        ...oneTimeServices,
        addOnCatalog,
      ],
    };
  ...
  function buildServiceSchema(routePath, routeSeo = {}) {
    const normalizedRoute = normalizeRoute(routePath);
    if (normalizedRoute === "/pricing-v2") return buildPricingPageSchema();
