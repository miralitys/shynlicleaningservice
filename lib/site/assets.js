...
function createSiteStaticHelpers(deps = {}) {
  ...
  const DEFAULT_HTML_CACHE_MAX_ENTRIES = 8;
  const DEFAULT_HTML_CACHE_MAX_TOTAL_BYTES = 16 * 1024 * 1024;
  ...
  function createHtmlCacheStore() {
    return {
      entries: new Map(),
      totalBytes: 0,
      maxEntries: parsePositiveInteger(HTML_CACHE_MAX_ENTRIES, DEFAULT_HTML_CACHE_MAX_ENTRIES),
      maxTotalBytes: parsePositiveInteger(HTML_CACHE_MAX_TOTAL_BYTES, DEFAULT_HTML_CACHE_MAX_TOTAL_BYTES),
    };
  }
  ...
  function estimateHtmlCacheEntryBytes(body, linkHeader = "") {
    const bodyBytes = Buffer.byteLength(String(body || ""), "utf8");
    const linkHeaderBytes = Buffer.byteLength(String(linkHeader || ""), "utf8");
    return bodyBytes * 2 + linkHeaderBytes + 512;
  }

  function buildHtmlCacheEntry(sanitizedHtml, fileMeta, options = {}) {
    const mtimeMs = fileMeta?.mtimeMs ?? Date.now();
    const linkHeader = String(options.linkHeader || "");
    return {
      body: sanitizedHtml,
      ...
      mtimeMs,
      estimatedBytes: estimateHtmlCacheEntryBytes(sanitizedHtml, linkHeader),
    };
  }
  ...
  function getCachedHtmlEntry(htmlCache, cacheKey) {
    if (!htmlCache || !htmlCache.entries || !htmlCache.entries.has(cacheKey)) return null;
    const entry = htmlCache.entries.get(cacheKey);
    htmlCache.entries.delete(cacheKey);
    htmlCache.entries.set(cacheKey, entry);
    return entry;
  }

  function evictOldestHtmlCacheEntry(htmlCache) {
    if (!htmlCache || !htmlCache.entries || htmlCache.entries.size === 0) return false;
    const oldestKey = htmlCache.entries.keys().next().value;
    const oldestEntry = htmlCache.entries.get(oldestKey);
    htmlCache.entries.delete(oldestKey);
    htmlCache.totalBytes = Math.max(0, htmlCache.totalBytes - (oldestEntry?.estimatedBytes || 0));
    return true;
  }

  function setCachedHtmlEntry(htmlCache, cacheKey, entry) {
    if (!htmlCache || !htmlCache.entries || !entry) return false;
    const estimatedBytes = Math.max(0, Number(entry.estimatedBytes) || 0);
    if (
      estimatedBytes > 0 &&
      (estimatedBytes > htmlCache.maxTotalBytes || htmlCache.maxEntries <= 0 || htmlCache.maxTotalBytes <= 0)
    ) {
      return false;
    }

    if (htmlCache.entries.has(cacheKey)) {
      const previousEntry = htmlCache.entries.get(cacheKey);
      htmlCache.entries.delete(cacheKey);
      htmlCache.totalBytes = Math.max(0, htmlCache.totalBytes - (previousEntry?.estimatedBytes || 0));
    }

    while (
      htmlCache.entries.size > 0 &&
      (htmlCache.entries.size >= htmlCache.maxEntries || htmlCache.totalBytes + estimatedBytes > htmlCache.maxTotalBytes)
    ) {
      evictOldestHtmlCacheEntry(htmlCache);
    }

    htmlCache.entries.set(cacheKey, entry);
    htmlCache.totalBytes += estimatedBytes;
    return true;
  }
  ...
  async function warmHtmlCache(runtimeIndex) {
    const htmlCache = createHtmlCacheStore();
      ...
      try {
        ...
        setCachedHtmlEntry(
          ...
          buildHtmlCacheEntry(sanitizedHtml, runtimeIndex.fileMetaByPath.get(absolutePath), {
            linkHeader: buildHtmlLinkHeader(absolutePath, sanitizedHtml),
          })
        );
        warmedCount += 1;
      } catch {}
  ...
  async function getHtmlFromCache(absolutePath, routePath, htmlCache, fileMeta) {
    ...
    const cachedEntry = getCachedHtmlEntry(htmlCache, cacheKey);
    if (cachedEntry) {
      return {
        entry: cachedEntry,
        cacheHit: true,
    ...
    const rawHtml = await fsp.readFile(absolutePath, "utf8");
    const sanitizedHtml = sanitizeHtml(rawHtml, routePath);
    const htmlEntry = buildHtmlCacheEntry(sanitizedHtml, fileMeta, {
      linkHeader: buildHtmlLinkHeader(absolutePath, sanitizedHtml),
    });
    setCachedHtmlEntry(htmlCache, cacheKey, htmlEntry);
    return {
      entry: htmlEntry,
      cacheHit: false,
    };
  ...
  function buildHtmlLinkHeader(absolutePath, htmlBody) {
    ...
    const isCommercialCleanPage = /class=["'][^"']*\bcommercial-clean-page\b/i.test(body);
    ...
    if (isCommercialCleanPage) {
      ...
      pushFirstMatch(
        /<link[^>]+rel=["']stylesheet["'][^>]+href=["']([^"']*css\/commercial-cleaning-copy\.css[^"']*)["']/i,
        "style"
      );
      pushFirstMatch(
        /<script[^>]+src=["']([^"']*js\/commercial-cleaning-copy\.js[^"']*)["'][^>]*><\/script>/i,
        "script"
      );
    }
    ...
    if (isTildaPage) {
      ...
      pushFirstMatch(
        /<link[^>]+rel=["']stylesheet["'][^>]+href=["']([^"']*css\/tilda-feed-1\.1\.min\.css[^"']*)["']/i,
        "style"
      );
      pushFirstMatch(
        /<link[^>]+rel=["']stylesheet["'][^>]+href=["']([^"']*css\/tilda-slds-1\.4\.min\.css[^"']*)["']/i,
        "style"
      );
      pushFirstMatch(
        /<script[^>]+src=["']([^"']*js\/tilda-feed-1\.1\.min\.js[^"']*)["'][^>]*><\/script>/i,
        "script"
      );
      pushFirstMatch(
        /<script[^>]+src=["']([^"']*js\/tilda-slds-1\.4\.min\.js[^"']*)["'][^>]*><\/script>/i,
        "script"
      );
      pushFirstMatch(
        /<script[^>]+src=["']([^"']*js\/hammer\.min\.js[^"']*)["'][^>]*><\/script>/i,
        "script"
      );
    }
  ...
  async function sendFile(
    ...
    try {
      ...
      if (ext === ".html") {
        ...
        const linkHeader = htmlEntry.linkHeader || buildHtmlLinkHeader(absolutePath, htmlEntry.body);
        if (linkHeader) {
