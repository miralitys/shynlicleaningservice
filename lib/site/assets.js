...
function createSiteStaticHelpers(deps = {}) {
  ...
  function buildHtmlPreloadHeader(absolutePath, htmlBody) {
    if (path.basename(absolutePath) !== "quote2.html") return "";
    ...
    function pushPreload(href, asType) {
      ...
      preloadEntries.push(`</${normalizedHref.replace(/^\/+/, "")}>; rel=preload; as=${asType}`);
    }

    const stylesheetMatch = body.match(
      /<link[^>]+rel=["']stylesheet["'][^>]+href=["']([^"']*css\/quote2\.css[^"']*)["']/i
    );
    if (stylesheetMatch && stylesheetMatch[1]) {
      pushPreload(stylesheetMatch[1], "style");
    }
    ...
    if (scriptMatch && scriptMatch[1]) {
      pushPreload(scriptMatch[1], "script");
    }

    return preloadEntries.join(", ");
  }
  ...
  async function sendFile(
    ...
    try {
      ...
      if (ext === ".html") {
        ...
        const preloadHeader = buildHtmlPreloadHeader(absolutePath, htmlEntry.body);
        if (preloadHeader) {
          headers.Link = preloadHeader;
        }
