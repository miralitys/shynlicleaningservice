...
const PRICING = Object.freeze({
  regular: Object.freeze({
    ...
    defaultSelectedServices: Object.freeze([]),
  }),
  deep: Object.freeze({
    basePrice: 84.5,
    roomPrice: 22.5,
...
const ALLOWED_SQUARE_FEET_BUCKETS = Object.freeze([0, 1, 2, 3, 4, 5, 6, 7]);
const SQUARE_FEET_PRICING_BUCKETS = Object.freeze([
...
function normalizeSquareFeetBucket(value) {
  const parsed = normalizeInteger(value, 0, 0, 999999);
  if (parsed <= 0) return 0;

  // The pricing page stores square footage as step indexes. Keep that as the
  // canonical representation, and map old raw-square-foot submissions into it.
  if (ALLOWED_SQUARE_FEET_BUCKETS.includes(parsed)) return parsed;

  for (const bucket of SQUARE_FEET_PRICING_BUCKETS) {
    if (parsed <= bucket.max) return bucket.step;
  }
...
function normalizeFrequency(value, serviceType = "regular") {
  if (serviceType !== "regular") return "";
  const normalized = normalizeString(value, 32).toLowerCase();
...
function normalizeQuotePricingInput(rawInput = {}) {
  ...
  return {
    serviceType,
    frequency: normalizeFrequency(quote.frequency, serviceType),
...
function calculateQuotePricing(rawInput = {}) {
  ...
  const selectedServices = Array.from(new Set([...(quote.services || []), ...includedServices]));
  ...
  return {
    ...
    defaultSelectedServices: [...(typePricing.defaultSelectedServices || [])],
    totalPrice,
