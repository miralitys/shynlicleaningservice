...
function normalizeSquareFeetBucket(value) {
  ...
  // New UI stores square footage as step indexes: 0 for the first range,
  // then 1, 2, 3... where each next step adds the configured step price.
  if (Number.isInteger(parsed) && parsed <= 20) return parsed;
...
function calculateSquareFeetPrice(typePricing, squareFeet) {
  ...
  if (Number.isInteger(numericValue) && numericValue <= 20) {
    return numericValue * stepPrice;
  }
...
function calculateQuotePricing(rawInput = {}) {
  ...
  const billableServices = selectedServices.filter((service) => !includedServices.includes(service));
