...
function createOrderPolicyAcceptancePublicService(options = {}) {
  ...
  function shouldScheduleAcceptedOrder(entry = {}) {
    const payload = entry.payloadForRetry && typeof entry.payloadForRetry === "object"
      ? entry.payloadForRetry
      : {};
    const orderState = payload.orderState && typeof payload.orderState === "object"
      ? payload.orderState
      : payload.adminOrder && typeof payload.adminOrder === "object"
        ? payload.adminOrder
        : {};
    const status = normalizeString(orderState.status, 40).toLowerCase() || "new";
    const serviceType = normalizeString(entry.serviceType || orderState.serviceType, 80)
      .toLowerCase()
      .replace(/[\s_]+/g, "-");
    const selectedDate = normalizeString(entry.selectedDate || orderState.selectedDate, 32);
    const selectedTime = normalizeString(entry.selectedTime || orderState.selectedTime, 32);
    const assignedStaff = normalizeString(entry.assignedStaff || orderState.assignedStaff, 800);
    return (
      status === "new" &&
      serviceType !== "free-in-home-estimate" &&
      Boolean(selectedDate && selectedTime && assignedStaff)
    );
  }
  ...
  async function submitAcceptance(quoteOpsLedger, token, payload, req) {
    ...
    let updatedEntry =
      (await updateEntryPolicyAcceptance(
        quoteOpsLedger,
        normalizeString(entry.id, 120),
        pendingRecord
      )) || entry;
    if (
      shouldScheduleAcceptedOrder(updatedEntry) &&
      quoteOpsLedger &&
      typeof quoteOpsLedger.updateOrderEntry === "function"
    ) {
      updatedEntry =
        (await quoteOpsLedger.updateOrderEntry(normalizeString(entry.id, 120), {
