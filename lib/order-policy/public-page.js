...
function renderPolicyAcceptancePage(options = {}) {
  ...
  const summaryItems = [
    ...
    {
      label: "Address",
      value: normalizeEnglishAddressLabel(customer.serviceAddress || "", 240) || "Not set",
      className: "summary-item-inline",
      title: normalizeEnglishAddressLabel(customer.serviceAddress || "", 240) || "Not set",
    },
    {
      label: "Email",
  ...
  const reviewSectionMarkup = isAccepted
    ...
    : `<section class="section">
          ...
          <form class="form-stack" id="policy-acceptance-form" method="post" action="${escapeHtmlAttribute(
            ...
            <div class="loading-box hidden" id="policy-loading" role="status" aria-live="polite">
              <span class="loading-spinner" aria-hidden="true"></span>
              <span>Submitting your signature. Please wait a few seconds.</span>
            </div>
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing:border-box; }
        ...
        .form-stack[data-submitting="true"] { pointer-events:none; }
        .loading-box { display:flex; align-items:center; gap:12px; border:1px solid rgba(158, 67, 90, 0.16); border-radius:18px; padding:14px 16px; background:rgba(158, 67, 90, 0.08); color:var(--accent); font-size:14px; font-weight:700; line-height:1.45; }
        .loading-spinner { width:20px; height:20px; flex:0 0 auto; border:3px solid rgba(158, 67, 90, 0.22); border-top-color:var(--accent); border-radius:50%; animation:policy-spin 0.8s linear infinite; }
    ...
    <body>
      ...
      <script>
        (function () {
          ...
          const loadingBox = document.getElementById("policy-loading");
          ...
          function updateButtonState() {
            if (!button) return;
            button.setAttribute("aria-disabled", isSubmitting || !isFormReady() ? "true" : "false");
            button.setAttribute("aria-busy", isSubmitting ? "true" : "false");
          }

          function setFormSubmitting(nextSubmitting) {
            ...
            if (button) {
              ...
              button.textContent = isSubmitting ? "Signing..." : initialButtonText;
            }
            if (loadingBox) {
              loadingBox.classList.toggle("hidden", !isSubmitting);
            }
            updateButtonState();
          }
