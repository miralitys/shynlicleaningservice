    ...
    <body>
      ...
      <script>
        (function () {
          ...
          function showPolicyDocumentFallback(message, href) {
            if (!policyDocBody) return;
            policyDocBody.innerHTML = '<div class="policy-doc-error">' +
              '<p>' + escapeClientHtml(message || "We could not load this policy document inside the signing page.") + '</p>' +
              '<a class="button policy-doc-fallback-link" href="' + escapeClientAttribute(href || "") + '">Open in this tab</a>' +
            '</div>';
          }
          ...
          async function loadPolicyDocument(href, title) {
            ...
            if (!requestUrl) {
              showPolicyDocumentFallback("This document can be opened in the same tab, then the Back button will return you to signing.", href);
              return;
            }
            try {
              ...
              showPolicyDocumentFallback("We could not load this document inside the signing page. Open it in this tab, then use Back to return to signing.", href);
            }
          }
