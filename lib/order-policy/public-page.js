  ...
  const documentMarkup = documents
    .map((doc) => {
      ...
      return `<a class="doc-card doc-card-link" href="${escapeHtmlAttribute(
        publicUrl
      )}" target="_blank" rel="noopener noreferrer" data-policy-doc-link>
        ...
        (function () {
          ...
          function renderDocuments(payload) {
            ...
            docList.innerHTML = docs.map(function(doc) {
              return '<a class="doc-card doc-card-link" href="' + doc.publicUrl + '" target="_blank" rel="noopener noreferrer" data-policy-doc-link>' +
                '<div class="doc-topline">' +
                  '<p class="doc-title">' + doc.title + '</p>' +
                '</div>' +
                '<p class="doc-url">' + doc.publicUrl + '</p>' +
                '<p class="doc-meta">Version ' + (doc.version || 'Not set') + ' • Effective ' + (doc.effectiveDate || 'Not set') + '</p>' +
              '</a>';
            }).join("");
          }

          function openPolicyDocumentInNewTab(event) {
            ...
            if (openedWindow && typeof openedWindow.opener !== "undefined") {
              try {
                ...
                // Ignore cross-window hardening failures; the navigation already happened.
              }
            }
          }
          ...
          if (docList) {
            docList.addEventListener("click", openPolicyDocumentInNewTab);
          }
          if (!form) {
            return;
          }
