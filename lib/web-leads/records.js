...
function resolveLeadSourceOverride(body = {}) {
  const rawValue = cleanCell(
    body.leadSource || body.lead_source || body.leadType || body.lead_type || body.type || "",
    120
  );
  if (!rawValue) return "";
...
function getHostname(value) {
  ...
  if (!text) return "";
  try {
    return new URL(text, "https://shynlicleaningservice.com").hostname.replace(/^www\./i, "").toLowerCase();
  } catch {
    return "";
  }
}

function resolveQuoteLeadSource({ body = {}, landingPage = "", source = "" } = {}) {
  const explicitType = cleanCell(body.leadType || body.type, 80);
  if (explicitType) return explicitType;

  const isPopupOrContact = /popup|callback|call/i.test(source || body.source || "");
  const fallback = isPopupOrContact ? "website_contact" : "website_quiz";
  const hostname = getHostname(body.sourceWebsite || body.origin || landingPage);
  if (hostname === "shynli.com") {
    return isPopupOrContact ? "shynli.com_website_contact" : "shynli.com_website_quiz";
  }
  return fallback;
}
...
function buildQuoteWebLeadRecord({ body = {}, req = null, contactData = {}, calculatorData = {}, pricing = {}, source = "", publicQuotePath = "", siteOrigin = "" } = {}) {
  ...
  const leadSource = resolveLeadSourceOverride(body) || resolveQuoteLeadSource({ body, landingPage, source });
  ...
  const price = formatPrice(pricing.totalPrice || calculatorData.totalPrice);
