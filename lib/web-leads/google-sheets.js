...
function buildRowIdempotencyKey(row) {
...
function loadGoogleSheetsConfig(options = {}) {
  ...
  const envTimeout = Number.parseInt(env.WEB_LEADS_TIMEOUT_MS || "", 10);
  return {
    configured,
    ...
    timeoutMs: envTimeout || (appsScriptUrl ? APPS_SCRIPT_TIMEOUT_MS : DEFAULT_TIMEOUT_MS),
  };
}
...
async function appendViaAppsScript(row, config) {
  ...
  try {
    const body = {
      ...
      idempotencyKey: buildRowIdempotencyKey(row),
      values: [row],
