...
function formatTelegramLeadMessage(record) {
  ...
  if (summary.price) lines.push(`Price: ${clean(summary.price, 80)}`);
  if (summary.zipCode) lines.push(`ZIP: ${clean(summary.zipCode)}`);
  if (summary.internalCrmUrl) lines.push(`CRM: ${clean(summary.internalCrmUrl, 220)}`);
...
async function sendTelegramTextNotification(text, options = {}) {
  ...
  const messageText = cleanMessageText(text, 3900);
  if (!messageText) {
    return {
      ok: true,
      skipped: true,
      reason: "telegram_empty_message",
    };
  }
  ...
  try {
    ...
    if (!response.ok || payload.ok === false) {
      const error = new Error(`Telegram notification failed with status ${response.status || 0}`);
...
async function sendTelegramLeadNotification(record, options = {}) {
  return sendTelegramTextNotification(formatTelegramLeadMessage(record), options);
}
