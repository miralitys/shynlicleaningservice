...
function formatTravelEstimateUnavailableText(record = {}, options = {}) {
  const status = normalizeTravelStatus(record.status);
  const technical = Boolean(options.technical);
  if (status === "not-configured") return "карты не подключены";
  if (status !== "unavailable") return "";

  const rawError = normalizeString(record.error, 240);
  const error = rawError.toLowerCase();
  if (error.includes("маршрут не найден") || error.includes("route_not_found") || error.includes("zero_results")) {
    return "маршрут не найден";
  }
  if (
    error.includes("routes api") ||
    error.includes("google maps") ||
    error.includes("api key") ||
    error.includes("request_denied") ||
    error.includes("billing")
  ) {
    return technical && rawError ? rawError : "карты временно недоступны";
  }
  return technical && rawError ? rawError : "маршрут недоступен";
}
