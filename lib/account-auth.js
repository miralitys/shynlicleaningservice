...
const USER_PASSWORD_SETUP_TTL_SECONDS = 24 * 60 * 60;
...
function createUserPasswordSetupToken(config, payload, options = {}) {
  return createSignedToken(
    payload,
    config,
    "password-setup",
    USER_PASSWORD_SETUP_TTL_SECONDS,
    options
  );
}

function verifyUserPasswordSetupToken(token, config, options = {}) {
  return verifySignedToken(token, config, "password-setup", options);
}
...
module.exports = {
  ...
  createUserPasswordSetupToken,
  createUserSessionToken,
  loadUserAuthConfig,
  verifyUserEmailVerificationToken,
  verifyUserPasswordSetupToken,
