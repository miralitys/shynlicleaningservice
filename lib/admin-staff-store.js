...
const STAFF_COMPENSATION_TYPE_VALUES = ["fixed", "percent"];
const STAFF_COMPENSATION_TYPE_SET = new Set(STAFF_COMPENSATION_TYPE_VALUES);
const ASSIGNMENT_STATUS_VALUES = ["planned", "confirmed", "completed", "issue", "canceled"];
...
function getSafeStaffCompensationType(value) {
  const normalized = normalizeString(value, 32).toLowerCase();
  return STAFF_COMPENSATION_TYPE_SET.has(normalized) ? normalized : "fixed";
}
...
function sanitizeStaffRecord(input = {}) {
  ...
  return {
    ...
    compensationValue: normalizeCompensationValue(
      input.compensationValue || input.salaryValue || input.payRateValue
    ),
    compensationType: getSafeStaffCompensationType(
      input.compensationType || input.salaryType || input.payRateType
    ),
    ...
    availabilityBlocks: sanitizeStaffAvailabilityBlocks(
      input.availabilityBlocks ||
        input.manualAvailabilityBlocks ||
        (input.calendar && input.calendar.manualAvailabilityBlocks)
    ),
    contract: sanitizeStaffContractRecord(input.contract),
    w9: sanitizeStaffW9Record(input.w9),
    smsHistory: normalizeStaffSmsHistoryEntries(input.smsHistory, createdAt),
    createdAt,
...
function sanitizeAssignmentRecord(input = {}) {
  ...
  return {
    ...
    travelEstimates: sanitizeTravelEstimateRecords(input.travelEstimates),
    createdAt,
    updatedAt: normalizeString(input.updatedAt, 80) || createdAt,
  };
}
  ...
  return Boolean(
    record.entryId &&
      (
        ...
        record.scheduleTime ||
        record.notes ||
        record.travelEstimates.length > 0 ||
...
function cloneStaffRecord(record) {
  return {
    ...
    availabilityBlocks: Array.isArray(record.availabilityBlocks)
      ? record.availabilityBlocks.map((entry) => ({ ...entry }))
      : [],
    contract: record.contract ? JSON.parse(JSON.stringify(record.contract)) : null,
    w9: record.w9 ? JSON.parse(JSON.stringify(record.w9)) : null,
    smsHistory: Array.isArray(record.smsHistory)
      ? record.smsHistory.map((entry) => ({ ...entry }))
      : [],
  };
}
...
function cloneAssignmentRecord(record) {
  return {
    ...
    travelEstimates: Array.isArray(record.travelEstimates)
      ? record.travelEstimates.map((item) => ({ ...item }))
      : [],
  };
}
