...
const ASSIGNMENT_STATUS_VALUES = new Set(["planned", "confirmed", "completed", "issue", "canceled"]);
...
function buildStaffCalendarConnectionPayload(record = {}) {
  const calendarConnection = sanitizeGoogleCalendarConnection(record.calendar);
  const availabilityBlocks = sanitizeStaffAvailabilityBlocks(
    record.availabilityBlocks ||
      record.manualAvailabilityBlocks ||
      (record.calendar && record.calendar.manualAvailabilityBlocks)
  );
  const payload = calendarConnection ? { ...calendarConnection } : {};
  if (availabilityBlocks.length > 0) {
    payload.manualAvailabilityBlocks = availabilityBlocks;
  }
...
function mapStaffRecordToRow(record = {}) {
  return {
    ...
    calendar_connection: buildStaffCalendarConnectionPayload(record),
...
function mapRowToStaffRecord(row = {}) {
  const calendarConnectionSource = row.calendar_connection || row.calendarConnection || {};
  return {
    ...
    calendar: sanitizeGoogleCalendarConnection(calendarConnectionSource),
    availabilityBlocks: sanitizeStaffAvailabilityBlocks(
      row.availability_blocks ||
        row.availabilityBlocks ||
        calendarConnectionSource.manualAvailabilityBlocks ||
        calendarConnectionSource.availabilityBlocks
    ),
...
function mapStaffRecordToQuoteOpsRow(record = {}) {
  const normalized = {
    ...
    availabilityBlocks: sanitizeStaffAvailabilityBlocks(record.availabilityBlocks),
  };
  return {
    id: normalized.id,
...
function mapQuoteOpsRowToStaffRecord(row = {}) {
  ...
  return {
    ...
    availabilityBlocks: sanitizeStaffAvailabilityBlocks(
      payloadStaff.availabilityBlocks ||
        payloadStaff.manualAvailabilityBlocks ||
        (payloadStaff.calendar && payloadStaff.calendar.manualAvailabilityBlocks)
    ),
