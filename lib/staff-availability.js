...
function sanitizeStaffAvailabilityBlock(input = {}) {
  ...
  const endMs = getStaffAvailabilityStartMs(endDate) || startMs + 24 * 60 * 60 * 1000;
...
function findStaffAvailabilityConflicts(staffRecords = [], staffIds = [], scheduleDate = "") {
  ...
  return (Array.isArray(staffRecords) ? staffRecords : [])
    ...
    .map((record) => {
      ...
      const block = blocks.find((candidate) => doesStaffAvailabilityBlockCoverDate(candidate, date));
