...
function normalizeStaffAvailabilityRecurrence(value) {
...
function buildStaffAvailabilityRecurrenceDates(startDateValue, recurrenceValue, monthsAhead = 12) {
  const startDate = parseStaffAvailabilityDate(startDateValue);
  if (!startDate) return [];
  const recurrence = normalizeStaffAvailabilityRecurrence(recurrenceValue);
  const firstDate = normalizeStaffAvailabilityDate(startDateValue);
  if (recurrence === "once") return [firstDate];

  const horizon = new Date(startDate.getTime());
  horizon.setUTCMonth(horizon.getUTCMonth() + Math.max(1, Number(monthsAhead) || 12));
  const dates = [];
  if (recurrence === "weekly") {
    for (let cursor = new Date(startDate.getTime()); cursor <= horizon; cursor.setUTCDate(cursor.getUTCDate() + 7)) {
  ...
  for (let monthOffset = 0; monthOffset <= Math.max(1, Number(monthsAhead) || 12); monthOffset += 1) {
    const year = startDate.getUTCFullYear();
    const month = startDate.getUTCMonth() + monthOffset;
    const lastDay = new Date(Date.UTC(year, month + 1, 0, 12)).getUTCDate();
    const cursor = new Date(Date.UTC(year, month, Math.min(anchorDay, lastDay), 12));
    if (cursor <= horizon) dates.push(cursor.toISOString().slice(0, 10));
  }
  return dates;
}
...
module.exports = {
  ...
  normalizeStaffAvailabilityRecurrence,
