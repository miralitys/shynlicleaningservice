...
async function findContactByPhone(fetchImpl, config, phoneDigits) {
  ...
  const extractContactList = (payload) => {
    const list =
  ...
  const phoneMatches = (candidatePhone) => {
    const candidateDigits = cleanInlineText(candidatePhone, 32).replace(/\D/g, "");
    if (!candidateDigits) return false;
    return (
      candidateDigits === normalizedDigits ||
      candidateDigits.endsWith(normalizedDigits) ||
      normalizedDigits.endsWith(candidateDigits)
    );
  };

  const matchContactFromPayload = (payload) => {
    const contact = extractContactList(payload).find((entry) =>
      phoneMatches(entry && (entry.phone || entry.phoneNumber || entry.mobilePhone || ""))
    );
    return cleanInlineText(extractContactId(contact || {}), 120);
  };

  const requestContacts = async (mode) => {
    const url = new URL("/contacts/", config.apiBaseUrl);
    url.searchParams.set("locationId", config.locationId);
    url.searchParams.set("limit", "100");
    if (mode === "query") {
      url.searchParams.set("query", normalizedDigits);
    }
    return requestJson(
      fetchImpl,
      url.toString(),
  ...
  const queriedResponse = await requestContacts("query");
  const queriedMatch = queriedResponse.ok ? matchContactFromPayload(queriedResponse.body) : "";
  if (queriedMatch) {
    return {
      ...queriedResponse,
      contactId: queriedMatch,
    };
  }

  const listResponse = await requestContacts("list");
  const listMatch = listResponse.ok ? matchContactFromPayload(listResponse.body) : "";
  if (listMatch) {
    return {
      ...listResponse,
      contactId: listMatch,
    ...
    async getSmsHistory(input = {}) {
      ...
      try {
        ...
        if (conversationIds.length === 0) {
          resolvedContactId = await resolveSmsHistoryContactId(
            fetchImpl,
            config,
            resolvedContactId,
            normalizedPhone
          ...
          if (resolvedContactId) {
            conversationIds = await findConversationsByContactId(fetchImpl, config, resolvedContactId);
          }
        }

        if (conversationIds.length === 0 && normalizedPhone && directContactId) {
          const phoneResolvedContactId = await resolveSmsHistoryContactId(
            fetchImpl,
            config,
            "",
            normalizedPhone
          );
          if (phoneResolvedContactId && phoneResolvedContactId !== resolvedContactId) {
            resolvedContactId = phoneResolvedContactId;
            conversationIds = await findConversationsByContactId(fetchImpl, config, resolvedContactId);
          }
        }
