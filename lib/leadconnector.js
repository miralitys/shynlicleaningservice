...
async function findContactByEmail(fetchImpl, config, email) {
...
async function findExistingSmsContact(fetchImpl, config, normalizedPhone, email) {
  const attempts = [];
  ...
  const normalizedEmail = normalizeEmail(email);
  if (normalizedEmail) {
    const emailResponse = await findContactByEmail(fetchImpl, config, normalizedEmail);
    attempts.push(emailResponse);
    if (emailResponse.ok && emailResponse.contactId) {
      return {
        contactId: cleanInlineText(emailResponse.contactId, 120),
        attempts,
      };
    }
  }
...
async function ensureSmsContactId(fetchImpl, config, input = {}, normalizedPhone) {
  ...
  const normalizedEmail = normalizeEmail(input.email || input.customerEmail || "");
  const lookupResult = await findExistingSmsContact(fetchImpl, config, normalizedPhone, normalizedEmail);
  ...
  if (
    ...
    const retryLookup = await findExistingSmsContact(fetchImpl, config, normalizedPhone, normalizedEmail);
