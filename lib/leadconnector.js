...
const CUSTOM_FIELD_ALIASES = Object.freeze({
  ...
  leadType: ["type", "lead_type", "website_contact_type", "contact.type", "contact.lead_type"],
  sourceWebsite: ["source_website", "website", "origin", "contact.source_website"],
  sourceDomain: ["source_domain", "domain", "contact.source_domain"],
  landingPageUrl: ["landing_page_url", "landing_page", "landing", "contact.landing_page_url"],
  sourcePage: ["source_page", "source_path", "page_path", "contact.source_page"],
});
...
function normalizeZipCode(rawValue) {
...
function normalizeQuoteSubmission(input = {}, options = {}) {
  ...
  const frequency = serviceType === "regular" ? normalizeFrequency(quote.frequency) : "";
  ...
  const leadType = cleanInlineText(input.leadType || input.type || quote.leadType || quote.type || options.leadType || "", 120);
  const sourceWebsite = cleanInlineText(input.sourceWebsite || input.origin || quote.sourceWebsite || quote.origin || "", 256);
  const sourceDomain = cleanInlineText(input.sourceDomain || quote.sourceDomain || "", 120);
  const landingPageUrl = cleanInlineText(
    input.landingPageUrl || input.landing_page_url || input.landingPage || quote.landingPageUrl || quote.landing_page_url || "",
    256
  );
  const sourcePage = cleanInlineText(
    input.sourcePage || input.source_page || input.sourcePagePath || input.returnPath || quote.sourcePage || quote.source_page || "",
    120
  );
...
function normalizeCleanerApplicationSubmission(input = {}, options = {}) {
  const application = input.application || input.cleanerApplication || input.contactData || input.contact || {};
  const fullName = cleanInlineText(
    application.fullName || application.name || [application.firstName, application.lastName].filter(Boolean).join(" "),
    120
  );
  const phone = normalizePhoneNumber(application.phone || application.phoneNumber || input.phone || "");
  const email = normalizeEmail(application.email || input.email || "");
  const zipCode = normalizeZipCode(application.zipCode || application.zip || input.zipCode || input.zip || "");
  const experience = cleanInlineText(application.experience || application.cleaningExperience || input.experience, 160);
  const details = cleanMultilineText(
    application.details || application.about || application.notes || input.details || "",
    DEFAULT_DETAILS_MAX_LENGTH
  );
  ...
  if (!email) {
    throw new LeadConnectorError("A valid email is required", {
      status: 400,
      code: "INVALID_SUBMISSION",
    });
  }
  ...
  if (!zipCode) {
    throw new LeadConnectorError("A valid ZIP code is required", {
      status: 400,
      code: "INVALID_SUBMISSION",
    });
  }
  ...
  if (!experience) {
    throw new LeadConnectorError("Cleaning experience is required", {
      status: 400,
      code: "INVALID_SUBMISSION",
    });
  }

  const firstName = cleanInlineText(application.firstName || fullName.split(" ")[0] || fullName, 80);
  const lastName = cleanInlineText(application.lastName || fullName.split(" ").slice(1).join(" "), 80);
  const source =
    cleanInlineText(input.source || application.source || options.source || "Website Cleaner Application", 120) ||
    "Website Cleaner Application";
  const pageUrl = cleanInlineText(input.pageUrl || input.requestOrigin || "", 256);
  const requestId = cleanInlineText(input.requestId || input.id || "", 120);
  const userAgent = cleanInlineText(input.userAgent || "", 180);
  const submittedAt = cleanInlineText(input.submittedAt || new Date().toISOString(), 64);
  ...
  return {
    ...
    meta: {
      source,
      tags: parseTagList(
        Array.isArray(input.tags) ? input.tags : ["Cleaner Application", "Careers", "Website"],
        ["Cleaner Application", "Careers", "Website"]
      ),
      pageUrl,
      requestId,
      userAgent,
...
function buildQuoteNote(submission, maxLength = DEFAULT_NOTE_MAX_LENGTH) {
  ...
  if (submission.meta.leadType) {
    lines.push(`Type: ${submission.meta.leadType}`);
  }
  if (submission.meta.sourceWebsite) {
    lines.push(`Website: ${submission.meta.sourceWebsite}`);
  }
  if (submission.meta.sourceDomain) {
    lines.push(`Domain: ${submission.meta.sourceDomain}`);
  }
  if (submission.meta.landingPageUrl) {
    lines.push(`Landing: ${submission.meta.landingPageUrl}`);
  }
  if (submission.meta.sourcePage) {
    lines.push(`Source page: ${submission.meta.sourcePage}`);
  }
...
function buildCleanerApplicationNote(submission, maxLength = DEFAULT_NOTE_MAX_LENGTH) {
  const lines = [
    "WEBSITE CLEANER APPLICATION",
    `Name: ${submission.contact.fullName}`,
    `Phone: ${submission.contact.phone}`,
    `Email: ${submission.contact.email}`,
    `ZIP: ${submission.application.zipCode}`,
    `Experience: ${submission.application.experience}`,
  ...
  if (submission.application.details) {
    lines.push(`Details: ${submission.application.details}`);
  }
...
function buildContactPayload(submission, config) {
  const quote = submission && submission.quote && typeof submission.quote === "object" ? submission.quote : {};
  const application =
    submission && submission.application && typeof submission.application === "object"
      ? submission.application
      : {};
  const source = cleanInlineText(
    submission && submission.meta && submission.meta.source ? submission.meta.source : config.source,
    120
  ) || config.source;
  const tags = parseTagList(
    submission && submission.meta && Array.isArray(submission.meta.tags) ? submission.meta.tags : config.tags,
    config.tags
  );
  const payload = {
  ...
  if (quote.fullAddress || quote.address) {
    payload.address1 = quote.fullAddress || quote.address;
  }
  if (quote.city) {
    payload.city = quote.city;
  }
  if (quote.state) {
    payload.state = quote.state;
  }
  if (quote.zipCode || application.zipCode) {
    payload.postalCode = quote.zipCode || application.zipCode;
  }
...
function buildOpportunityFailureReason(response) {
  const upstreamMessage = extractResponseMessage(response && response.body);
  if (upstreamMessage) {
  ...
  if (status === 401 || status === 403) {
    return "Go High Level rejected opportunity creation. Check opportunities API permissions.";
  }
  if (status === 404) {
    return "Go High Level could not find the configured opportunity pipeline or stage.";
  }
  if (status === 400 || status === 422) {
    return "Go High Level rejected the opportunity payload.";
  }
  if (status >= 500) {
    return "Go High Level could not create the opportunity right now.";
  }

  return "Go High Level did not create the opportunity.";
}

function buildQuoteSubmissionWarningMessage(result) {
  const warnings = Array.isArray(result && result.warnings) ? result.warnings : [];
  if (warnings.length === 0) return "";

  const details = [];
  if (warnings.includes("opportunity_failed")) {
    details.push(
      result && result.opportunitySyncReason
        ? `Сделка в CRM: ${result.opportunitySyncReason}`
        : "Сделка в CRM не создалась."
    );
  }
  if (warnings.includes("custom_fields_update_failed")) {
    details.push(
      result && result.customFieldSyncReason
        ? `Кастомные поля CRM: ${result.customFieldSyncReason}`
        : "CRM не обновила кастомные поля."
    );
  }
  if (warnings.includes("note_failed")) {
    details.push("CRM не создала заметку по заявке.");
  }

  return cleanInlineText(details.join(" • "), 300);
}
...
function extractConversationId(payload) {
  ...
  return cleanInlineText(
    payload.conversationId ||
      ...
      payload.data?.message?.conversation?.id ||
      "",
    120
  );
}
...
function extractConversationList(payload) {
  const candidates = [
    ...
    payload && payload.conversations && payload.conversations.conversations,
    payload && payload.conversations && payload.conversations.items,
    payload && payload.conversations && payload.conversations.results,
    payload && payload.items,
    payload && payload.results,
    payload && payload.data,
    payload && payload.data && payload.data.conversations,
    payload && payload.data && payload.data.items,
    payload && payload.data && payload.data.results,
    payload && payload.data && payload.data.conversations && payload.data.conversations.conversations,
    payload && payload.data && payload.data.conversations && payload.data.conversations.items,
    Array.isArray(payload) ? payload : null,
  ];
...
function extractConversationMessages(payload) {
  const candidates = [
    ...
    payload && payload.messages && payload.messages.messages,
    payload && payload.messages && payload.messages.items,
    payload && payload.messages && payload.messages.results,
    payload && payload.items,
    payload && payload.results,
    payload && payload.data,
    ...
    payload && payload.data && payload.data.items,
    payload && payload.data && payload.data.results,
    payload && payload.data && payload.data.messages && payload.data.messages.messages,
    payload && payload.data && payload.data.messages && payload.data.messages.items,
    payload && payload.conversationMessages,
    payload && payload.conversationMessages && payload.conversationMessages.messages,
    payload && payload.conversation && payload.conversation.messages,
    Array.isArray(payload) ? payload : null,
  ];
...
function extractTextLikeValue(value, depth = 0) {
  if (value == null) return "";
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
    return cleanMultilineText(value, 1000);
  }
  ...
  for (const candidate of candidates) {
    const normalized = extractTextLikeValue(candidate, depth + 1);
    if (normalized) return normalized;
  }
...
function extractMessageText(payload) {
  ...
  for (const candidate of candidates) {
    const normalized = extractTextLikeValue(candidate);
    if (normalized) return normalized;
  }
...
function extractPhoneLikeValue(...values) {
  for (const value of values) {
    if (value == null) continue;
    if (typeof value === "string" || typeof value === "number") {
      const normalized = cleanInlineText(value, 80);
      if (normalized) return normalized;
      continue;
    }
    if (typeof value !== "object") continue;

    const nested = extractPhoneLikeValue(
      value.phone,
      value.phoneNumber,
      value.number,
      value.rawPhone,
...
function normalizeConversationMessageRecord(payload, fallbackConversationId = "") {
  ...
  const direction = normalizeConversationMessageDirection(payload);
  const phone =
    direction === "inbound"
      ? extractPhoneLikeValue(
          ...
          payload.to,
          payload.recipient,
          payload.meta && payload.meta.phone,
  ...
  return {
    id:
      cleanInlineText(
        payload.id ||
          ...
          payload.data && payload.data.message && payload.data.message.id ||
          "",
        120
      ) || "",
    sentAt: cleanInlineText(
      payload.dateAdded ||
        ...
        payload.date ||
        payload.timestamp ||
        payload.message && (payload.message.dateAdded || payload.message.createdAt || payload.message.sentAt) ||
        payload.data && (payload.data.dateAdded || payload.data.createdAt || payload.data.sentAt) ||
        "",
      80
    ),
    message,
    phone: cleanInlineText(phone, 80),
    contactId: cleanInlineText(
      payload.contactId ||
        payload.contact && (payload.contact.id || payload.contact.contactId) ||
        payload.data && payload.data.contactId ||
        payload.data && payload.data.contact && (payload.data.contact.id || payload.data.contact.contactId) ||
        "",
      120
    ),
    channel: "ghl",
    direction,
    source: direction === "inbound" ? "client" : "manual",
    targetType: "",
    targetRef: "",
    conversationId: cleanInlineText(
      payload.conversationId ||
        ...
        payload.message && payload.message.conversation && payload.message.conversation.id ||
        payload.data && payload.data.conversationId ||
        payload.data && payload.data.conversation && payload.data.conversation.id ||
        payload.data && payload.data.message && payload.data.message.conversationId ||
    ...
    messageId: cleanInlineText(
      payload.messageId ||
        payload.id ||
        payload.message && payload.message.id ||
        payload.data && payload.data.messageId ||
        payload.data && payload.data.id ||
        payload.data && payload.data.message && payload.data.message.id ||
        "",
      120
...
async function updateCustomFields(fetchImpl, config, contactId, submission) {
  ...
  const entries = {
    ...
    source: submission.meta.source,
    leadType: submission.meta.leadType,
    sourceWebsite: submission.meta.sourceWebsite,
    sourceDomain: submission.meta.sourceDomain,
    landingPageUrl: submission.meta.landingPageUrl,
    sourcePage: submission.meta.sourcePage,
  };

  for (const [logicalKey, value] of Object.entries(entries)) {
    if (value === undefined || value === null) continue;
    if (value === "" && logicalKey !== "frequency") continue;
...
async function createContactNote(fetchImpl, config, contactId, body) {
  if (!config.enableNotes) {
    return { ok: true, skipped: true, reason: "notes_disabled" };
  }

  const noteBody = cleanMultilineText(body, config.noteMaxLength);
  if (!noteBody) {
    return { ok: true, skipped: true, reason: "empty_note" };
  }
...
async function createNote(fetchImpl, config, contactId, submission) {
  return createContactNote(fetchImpl, config, contactId, buildQuoteNote(submission, config.noteMaxLength));
}

async function createCleanerApplicationNote(fetchImpl, config, contactId, submission) {
  return createContactNote(
    fetchImpl,
    config,
    contactId,
    buildCleanerApplicationNote(submission, config.noteMaxLength)
  );
}

async function upsertSubmissionContact(fetchImpl, config, submission, submitOptions = {}) {
  ...
  if (
    ...
    if (contactId) {
      ...
      if (!updateResponse.ok) {
        throw new LeadConnectorError("Failed to update existing contact", {
          status: updateResponse.status || 502,
          code: "CONTACT_UPDATE_FAILED",
          retryable: true,
          details: getResponseBodyFromResult(updateResponse),
  ...
  if (!contactId) {
    throw new LeadConnectorError("Failed to create contact", {
...
async function createOpportunity(fetchImpl, config, contactId, submission) {
  ...
  if (!response.ok) {
    return {
      ...response,
      reason: buildOpportunityFailureReason(response),
    };
  }
...
function createLeadConnectorClient(options = {}) {
  ...
  return {
    ...
    normalizeCleanerApplication(input, normalizeOptions) {
      return normalizeCleanerApplicationSubmission(input, normalizeOptions);
    },
    buildNote(submission) {
      return buildQuoteNote(submission);
    },
    buildCleanerApplicationNote(submission) {
      return buildCleanerApplicationNote(submission);
    },
    ...
    async sendSmsMessage(input = {}) {
      ...
      try {
        ...
        const allowDirectToNumber = Boolean(input.allowDirectToNumber);
        ...
        if (contactId) {
          ...
          contactResolution = await ensureSmsContactId(fetchImpl, config, input, normalizedPhone);
          contactId = cleanInlineText(contactResolution.contactId, 120);
        }
          ...
          return toResultError(
            new LeadConnectorError(
              extractResponseMessage(response.body) || "Failed to send SMS via Go High Level",
              {
                ...
                retryable:
                  Number(response.status || 0) === 408 ||
                  Number(response.status || 0) === 425 ||
    ...
    async submitQuoteSubmission(input, submitOptions = {}) {
      ...
      try {
        const contactResolution = await upsertSubmissionContact(fetchImpl, config, submission, submitOptions);
        const contactId = contactResolution.contactId;
        const usedExistingContact = Boolean(contactResolution.usedExistingContact);

        const syncQuoteCustomFields = Boolean(submitOptions.syncQuoteCustomFields);
        const syncQuoteNotes = Boolean(submitOptions.syncQuoteNotes);
        const syncQuoteOpportunity = Boolean(submitOptions.syncQuoteOpportunity);

        let customFieldResponse = {
          ok: true,
          skipped: true,
          reason: "quote_custom_fields_local_only",
        };
        if (syncQuoteCustomFields) {
          customFieldResponse = await updateCustomFields(fetchImpl, config, contactId, submission);
        }
        ...
        let noteResponse = {
          ok: true,
          skipped: true,
          reason: "quote_notes_local_only",
        };
        if (syncQuoteNotes) {
          noteResponse = await createNote(fetchImpl, config, contactId, submission);
        }
        if (!noteResponse.ok) {
        ...
        let opportunityResponse = {
          ok: true,
          skipped: true,
          reason: "quote_opportunity_local_only",
        };
        if (syncQuoteOpportunity) {
          opportunityResponse = await createOpportunity(fetchImpl, config, contactId, submission);
        }
        ...
        result.warningMessage = buildQuoteSubmissionWarningMessage(result);
        return result;
      } catch (error) {
        return toResultError(error);
      }
    },
    async submitCleanerApplication(input, submitOptions = {}) {
      let submission;
      try {
        submission = normalizeCleanerApplicationSubmission(input, submitOptions);
      } catch (error) {
        return toResultError(error);
      }
      ...
      try {
        ...
        const noteResponse = await createCleanerApplicationNote(fetchImpl, config, contactId, submission);
        if (!noteResponse.ok) {
          result.warnings.push("note_failed");
        }
