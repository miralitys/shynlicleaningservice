...
async function findContactIdsByPhone(fetchImpl, config, phoneDigits, options = {}) {
  ...
  const queriedMatches = queriedResponse.ok
    ? getMatchingContactIdsFromPayload(queriedResponse.body, normalizedDigits)
    : [];

  if (queriedMatches.length > 0 && options.includeListFallback !== true) {
    return {
      ...queriedResponse,
      contactIds: queriedMatches,
    };
  }

  const listResponse = await requestContacts("list");
  const listMatches = listResponse.ok
    ? getMatchingContactIdsFromPayload(listResponse.body, normalizedDigits)
    : [];
  const contactIds = Array.from(new Set([...queriedMatches, ...listMatches]));
