...
function buildOpportunityPayload(submission, config, contactId) {
  ...
  if (!config.pipelineId || !config.pipelineStageId) return null;
...
async function updateCustomFields(fetchImpl, config, contactId, submission) {
  ...
  for (const [logicalKey, value] of Object.entries(entries)) {
    ...
    const customFieldId = config.customFieldMap[logicalKey] || config.customFieldMap[logicalKey.toLowerCase()];
...
async function createOpportunity(fetchImpl, config, contactId, submission) {
  const payload = buildOpportunityPayload(submission, config, contactId);
  if (!payload) {
    return {
      ok: true,
      skipped: true,
...
function loadLeadConnectorConfig(env = process.env) {
  ...
  return {
    ...
    pipelineName: cleanInlineText(env.GHL_PIPELINE_NAME, 120) || "Pipelines Main",
...
function createLeadConnectorClient(options = {}) {
  ...
  return {
    ...
    buildOpportunityPayload(submission, contactId) {
      return buildOpportunityPayload(submission, config, contactId);
    },
    async submitQuoteSubmission(input, submitOptions = {}) {
      ...
      try {
        ...
        if (!contactId && createResponse && !createResponse.ok && looksDuplicateLike(createResponse.status, createResponse.body)) {
