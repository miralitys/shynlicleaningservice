...
async function findContactIdsByPhone(fetchImpl, config, phoneDigits, options = {}) {
  ...
  const pageSize = 100;
  const maxPages = Number.isFinite(options.maxPages) ? Math.max(1, options.maxPages) : 10;
  ...
  const queriedResponse = await requestContacts({ query: normalizedDigits });
  const uniqueQueriedMatches = queriedResponse.ok
    ? Array.from(new Set(getMatchingContactIdsFromPayload(queriedResponse.body, normalizedDigits)))
    : [];
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    listMatches.push(...getMatchingContactIdsFromPayload(response.body, normalizedDigits));
