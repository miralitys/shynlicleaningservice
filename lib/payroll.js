...
function formatCompensationRateValue(value) {
  const numericValue = Number(value);
...
function normalizePayrollItem(input = {}, index = 0) {
  ...
  return {
    ...
    appliedCompensationValue: normalizeCompensationValue(input.appliedCompensationValue),
...
function calculateStaffPayrollItem(entry = {}, staffRecord = {}, options = {}) {
  const fixedOverrideValue = normalizeCompensationValue(options.fixedOverrideValue);
  const compensationType = fixedOverrideValue
    ? "fixed"
    : normalizeCompensationType(staffRecord && staffRecord.compensationType);
  const compensationValue = fixedOverrideValue || normalizeCompensationValue(staffRecord && staffRecord.compensationValue);
  ...
  const appliedCompensationNumeric =
    compensationType === "percent" && Number.isFinite(compensationNumeric) && compensationNumeric > 0
      ? compensationNumeric / teamSize
      : compensationNumeric;
  const appliedCompensationValue =
    compensationType === "percent"
      ? formatCompensationRateValue(appliedCompensationNumeric)
      : compensationValue;
  let amountCents = 0;

  if (Number.isFinite(appliedCompensationNumeric) && appliedCompensationNumeric > 0) {
    if (compensationType === "percent") {
      amountCents = Math.max(0, Math.round(orderTotalCents * (appliedCompensationNumeric / 100)));
    } else {
      amountCents = Math.max(0, Math.round(appliedCompensationNumeric * 100));
    }
  }
...
function buildOrderPayrollSnapshot(options = {}) {
  ...
  const assignmentTeamSize = normalizeTeamSize(Array.isArray(assignment.staffIds) ? assignment.staffIds.length : 0);
  const orderState = getEntryOrderState(entry);
  const payrollOverrides =
    orderState && orderState.payrollOverrides && typeof orderState.payrollOverrides === "object"
      ? orderState.payrollOverrides
      : {};
  const generatedItems = assignment.staffIds
    .map((staffId) => {
      ...
      const nextItem = calculateStaffPayrollItem(entry, staffRecord, {
        teamSize: assignmentTeamSize,
        fixedOverrideValue: payrollOverrides[normalizedStaffId],
      });
      ...
      if (existingItem.status === "paid") {
        return {
          ...
          appliedCompensationValue:
            existingItem.appliedCompensationValue || nextItem.appliedCompensationValue,
          teamSize: normalizeTeamSize(existingItem.teamSize || nextItem.teamSize),
  ...
  for (const entry of entries) {
    ...
    for (const item of payroll.items) {
      ...
      rows.push({
        ...
        appliedCompensationValue: normalizeCompensationValue(item.appliedCompensationValue),
        teamSize: normalizeTeamSize(item.teamSize),
